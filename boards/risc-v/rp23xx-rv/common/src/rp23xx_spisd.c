/****************************************************************************
 * boards/risc-v/rp23xx-rv/common/src/rp23xx_spisd.c
 *
...
int board_spisd_initialize(int minor, int bus)
{
  ...
  if (bus == 0)
    {
      rp23xx_gpio_set_pulls(CONFIG_RP23XX_RV_SPI0_RX_GPIO, true, false);
    }
#endif
  ...
  if (bus == 1)
    {
      rp23xx_gpio_set_pulls(CONFIG_RP23XX_RV_SPI1_RX_GPIO, true, false);
    }
#endif
  ...
  ret = mmcsd_spislotinitialize(minor, CONFIG_RP23XX_RV_SPISD_SLOT_NO, spi);
  if (ret < 0)
    {
      ferr("ERROR: Failed to bind SPI device to MMC/SD slot %d: %d\n",
           CONFIG_RP23XX_RV_SPISD_SLOT_NO, ret);
      return ret;
    }
