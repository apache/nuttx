/****************************************************************************
 * boards/arm/n32h7/n32h762iil7/src/n32_userleds.c
 *
...
uint32_t board_userled_initialize(void)
{
  ...
  for (i = 0; i < nitems(g_ledcfg); i++)
    {
      n32_configgpio(GPIO_OUTPUT | GPIO_PUSHPULL | GPIO_SLEW_RATE_SLOW |
                     g_ledcfg[i]);
    }
...
void board_userled(int led, bool ledon)
{
  if ((unsigned)led < nitems(g_ledcfg))
    {
      n32_gpiowrite(g_ledcfg[led], ledon);
    }
}
...
void board_userled_all(uint32_t ledset)
{
  ...
  for (i = 0; i < nitems(g_ledcfg); i++)
    {
      n32_gpiowrite(g_ledcfg[i], (ledset & (1 << i)) != 0);
    }
}
...
#if defined(CONFIG_USERLED_LOWER_READSTATE)
void board_userled_getall(uint32_t *ledset)
{
  ...
  *ledset = 0;
  for (i = 0; i < nitems(g_ledcfg); i++)
    {
      *ledset |= (n32_gpioread(g_ledcfg[i]) << i);
    }
}
#endif
