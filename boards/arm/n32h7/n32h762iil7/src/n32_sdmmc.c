/****************************************************************************
 * boards/arm/n32h7/n32h762iil7/src/n32_sdmmc.c
 *
...
#ifdef HAVE_NCD
static int n32_ncd_interrupt(int irq, void *context, void *param)
{
  bool present;

  present = !n32_gpioread(GPIO_SDIO_NCD);
...
int n32_sdmmc_initialize(int minor)
{
  ...
  n32_configgpio(GPIO_SDIO_NCD);

  /* Register an interrupt handler for the card detect pin */

  n32_gpiosetevent(GPIO_SDIO_NCD, true, true, true,
                     n32_ncd_interrupt, NULL);
#endif
  ...
  ret = mmcsd_slotinitialize(minor, g_sdio_dev);
  if (ret != OK)
    {
  ...
  g_sd_inserted = !n32_gpioread(GPIO_SDIO_NCD);
  finfo("Card detect : %d\n", g_sd_inserted);
