/****************************************************************************
 * boards/arm/n32h7/n32h762iil7/src/n32_usb.c
 *
...
#  define HAVE_USB 1
#else
#  warning "CONFIG_N32H7_USBHS is enabled but neither CONFIG_USBDEV nor CONFIG_USBHOST"
#  undef HAVE_USB
#endif

#ifndef CONFIG_N32H762IIL7_USBHOST_PRIO
#  define CONFIG_N32H762IIL7_USBHOST_PRIO 100
#endif

#ifndef CONFIG_N32H762IIL7_USBHOST_STACKSIZE
#  define CONFIG_N32H762IIL7_USBHOST_STACKSIZE 2048
#endif
...
extern struct usbhost_connection_s *n32_usbhshost_initialize(int controller);
#endif
...
void n32_usbinitialize(void)
{
  /* The OTG FS has an internal soft pull-up.
   * No GPIO configuration is required
   */

  /* Configure the OTG FS Power On GPIOs
   */

#ifdef CONFIG_N32H7_USBHS
#  ifdef CONFIG_USBHOST
#  if defined(CONFIG_N32H7_USBHS1_HOST)
#  define GPIO_USBHS_PWRON GPIO_USBHS1_PWRON
  n32_configgpio(GPIO_USBHS1_PWRON);
#  endif
#  if defined(CONFIG_N32H7_USBHS2_HOST)
#  define GPIO_USBHS_PWRON GPIO_USBHS2_PWRON
  n32_configgpio(GPIO_USBHS2_PWRON);
#  endif
#  endif
#endif
}
...
int n32_usbhost_initialize(void)
{
  ...
  g_usbconn = n32_usbhshost_initialize(0);
  if (g_usbconn)
    {
      ...
      ret = kthread_create("usbhost", CONFIG_N32H762IIL7_USBHOST_PRIO,
                           CONFIG_N32H762IIL7_USBHOST_STACKSIZE,
                           usbhost_waiter, NULL);
 ...
 *   provided be each platform that implements the N32H7 USBHS host
 *   interface
 *
...
#ifdef CONFIG_USBHOST
void n32_usbhs_vbusdrive(int iface, bool enable)
{
  DEBUGASSERT(iface == 0);

  /* Set the Power Switch by driving the active high enable pin */

  n32_gpiowrite(GPIO_USBHS_PWRON, enable);
}
#endif
...
#ifdef CONFIG_USBDEV
void n32_usbsuspend(struct usbdev_s *dev, bool resume)
{
  uinfo("resume: %d\n", resume);
}
#endif
