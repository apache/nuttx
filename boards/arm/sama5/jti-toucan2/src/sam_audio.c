/****************************************************************************
 * boards/arm/sama5/sama5/jti-toucan2/src/sam_audio.c
 *
...
{
  FAR struct audio_lowerhalf_s *classd_audio;
  FAR struct audio_lowerhalf_s *pcm;
  FAR struct audio_lowerhalf_s *tone;
  ...
  if (!initialized)
    {
      /* Get a ClassD audio interface */

      classd_audio = sama5_classd_initialize();
      if (classd_audio == NULL)
      {
        auderr("ERROR:  Failed to get classd interface\n");
        ret= -ENODEV;
        goto errout;
      }
        /* Now we can embed the audio interface into a PCM decoder
       * instance so that we will have a PCM front end for the CLASSD
       * audio driver.
       */

      pcm = pcm_decode_initialize(classd_audio);
      if (!pcm)
        {
          auderr("ERROR: Failed create the PCM decoder\n");
          ret = -ENODEV;
          goto errout_with_classd_audio;
        }
...
errout_with_classd_audio:
errout_with_classd_pcm:
errout:
  return ret;  
}
#endif
  ...
  if (!initialized)
    {
      ...
      /* Now we can embed the null audio interface into a PCM decoder
...
errout_with_null_audio:
errout_with_null_pcm:
errout:
  return ret;
}
