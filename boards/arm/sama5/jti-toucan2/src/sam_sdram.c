/****************************************************************************
 * boards/arm/sama5/jti-toucan2/src/sam_sdram.c
 *
...
#if defined(CONFIG_JTI_TOUCAN2_W971GG6SB) || defined(CONFIG_JTI_TOUCAN2_W9751G6KB)
  /* Used for SDRAM command handshaking */

#  define DDR2_BA0    (1 << 26)
#  define DDR2_BA1    (1 << 27)

// is it the same for both types??
...
#if defined(CONFIG_JTI_TOUCAN2_492MHZ)
 ...
 *
 * Description:
 *   Configures DDR2 (W975G6KB, 512Mbit) (W971GG6SB, 1Gbit as a possible future option so included for completeness)
 *
 *   TODO: test (so far only booting into SDRAM has been tested using U-Boot)
 *
 *   Per the SAMA5D27 SIP Data sheet:
 
 *   "The SAMA5D2 System-In-Package (SIP) integrates the Arm® Cortex®-A5 processor-based SAMA5D2
 *	 MPU with up to 1 Gbit DDR2-SDRAM or up to 2 Gbit LPDDR2-SDRAM in a single package.
 *   By combining the high-performance, ultra-low power SAMA5D2 with LPDDR2/DDR2-SDRAM in a single
 *   package, PCB routing complexity, area and number of layers is reduced in the majority of cases. This
 *   makes board design easier and more robust by facilitating design for EMI, ESD and signal integrity.
 ...
 *   While the smallest option targets applications with a small OS or bare metal, the larger options are
 *   suitable for applications using Linux®."
 *
 *   The JTi Toucan2 board started as "bare metal" before migrating to NuttX, hence the choice of 512Mbit memory
 *
 *   Per the SAMA5D27 SIP Data sheet and Windbond data sheet
 *
 *     W975G6KB: 8 Mwords x 16 x 4 banks DDR2 SDRAM (512 Mbit) 
 *     Refresh count: 8K
 *     Row address: A[12:0] (8K)
 *     Column address A[9:0] (1K)
 *     Bank address BA[2:0] a(24,25) (8)
 *
  *    W971GG6SB: 8 Mwords x 16 x 8 banks DDR2 SDRAM (1 Gbit) 
 *     Refresh count: 8K
 *     Row address: A[12:0] (8K)
 *     Column address A[9:0] (1K)
 *     Bank address BA[2:0] a(24,25) (8)
 *
 *  Input Parameters:
 *     devtype - Either DDRAM_W975G6KB or DDRAM_W971GG6SB
 *
 *  Assumptions:
...
void sam_sdram_config(void)
{
...
#if defined(CONFIG_JTI_TOUCAN2_W971GG6SB) //1Gbit
...
#elif defined (CONFIG_JTI_TOUCAN2_W9751G6KBB) //512Mbit
  /* For W9751G6KBB
   *
   *   NC      = 13 DDR column bits
   *   NR      = 16 DDR row bits
  ...
  regval = MPDDRC_CR_NC_10 |    /* Number of Column Bits */
           ...
           MPDDRC_CR_CAS_4 |    /* CAS Latency */
           MPDDRC_CR_OCD_EXIT | /* Off-chip Driver */
           MPDDRC_CR_4BANKS |   /* Number of Banks */
           MPDDRC_CR_NDQS |     /* Not DQS */
           MPDDRC_CR_UNAL;      /* upport Unaligned Access */
#else
#  error Unknown SDRAM type
