/****************************************************************************
 * boards/arm/rtl8721dx/pke8721daf/src/rtl8721dx_bringup.c
 *
 ...
 * os_wrapper (ameba_os_wrap.c); IPC_* are SDK ROM symbols.  RTL8721Dx's
 * IPC_patch_function takes no lock-id argument.
 */
...
extern void IPC_patch_function(void (*enter)(uint32_t),
                               void (*leave)(uint32_t));
extern void IPC_SEMDelay(void (*delay)(uint32_t));
...
int rtl8721dx_bringup(void)
{
  ...
  ret = rtl8721dx_wifi_initialize();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: rtl8721dx_wifi_initialize failed: %d\n", ret);
    }
#endif
