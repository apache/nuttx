/****************************************************************************
 * boards/arm/stm32n6/stm32n6570-dk/src/stm32_userleds.c
 *
...
uint32_t board_userled_initialize(void)
{
  ...
  /* Configure LD1 and LD2 GPIOs for output */
...
void board_userled(int led, bool ledon)
{
  if ((unsigned)led < nitems(g_ledcfg))
    {
      /* LD1 is active high; LD2 is active low. */

      stm32_gpiowrite(g_ledcfg[led], led == BOARD_LED_RED ? !ledon : ledon);
    }
}
...
void board_userled_all(uint32_t ledset)
{
  ...
  /* Apply the polarity of each LED. */
