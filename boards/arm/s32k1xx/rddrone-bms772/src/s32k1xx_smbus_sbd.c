...
struct smbus_sbd_dev_s
{
  ...
  FAR struct smbus_sbd_data_s *data;     /* Most recent battery data */
...
static int smbus_sbd_open(FAR struct file *filep);
static int smbus_sbd_close(FAR struct file *filep);
static ssize_t smbus_sbd_read(FAR struct file *filep, FAR char *buffer,
                              size_t buflen);
static ssize_t smbus_sbd_write(FAR struct file *filep,
...
static int smbus_sbd_callback(FAR void *arg)
{
  FAR struct smbus_sbd_dev_s *dev;
  int buffer_length;
  int i;
  ...
  dev = (FAR struct smbus_sbd_dev_s *)arg;
  ...
  switch (dev->read_buffer[0])
    {
      ...
      case SBD_MANUFACTURER_NAME:
        {
          DEBUGASSERT(dev->data->manufacturer_name);
          ...
          dev->write_buffer[0] = strnlen(dev->data->manufacturer_name, 15);
      ...
      case SBD_DEVICE_NAME:
        {
          DEBUGASSERT(dev->data->device_name);
          ...
          for (i = 0; i < dev->write_buffer[0]; i++)
            {
              dev->write_buffer[i + 1] = (uint8_t)dev->data->device_name[i];
            }
      ...
      case SBD_DEVICE_CHEMISTRY:
        {
          DEBUGASSERT(dev->data->device_chemistry);
          ...
          dev->write_buffer[0] = strnlen(dev->data->device_chemistry, 15);
      ...
      case SBD_MANUFACTURER_DATA:
        {
          DEBUGASSERT(dev->data->manufacturer_data);
...
int smbus_sbd_initialize(int minor, FAR struct i2c_slave_s *i2c_slave_dev)
{
  irqstate_t flags;
  ...
  if (smbus_sbd_dev == NULL)
  ...
  else
    {
      ...
      ret = register_driver(dev_name, &smbus_sbd_fops, 0,
                            smbus_sbd_dev);
      if (ret < 0)
        {
  ...
  smbus_sbd_dev->data =
    (struct smbus_sbd_data_s *)kmm_zalloc(
      sizeof(struct smbus_sbd_data_s));
