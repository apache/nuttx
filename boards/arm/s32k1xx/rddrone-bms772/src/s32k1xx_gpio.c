...
static int gpio_irqattach(FAR struct gpio_dev_s *dev,
                          pin_interrupt_t callback);
static int gpio_irqenable(FAR struct gpio_dev_s *dev, bool enable);
...
static int s32k1xx_gpio_interrupt(int irq, void *context, void *arg)
{
  FAR struct s32k1xx_gpio_dev_s *s32k1xx_gpio =
    (FAR struct s32k1xx_gpio_dev_s *)arg;
                           ...
                           enum gpio_pintype_e pintype)
{
  int ret = OK;
  uint32_t pinconfig;
  FAR const struct gpio_operations_s *gpio_ops;
  FAR struct s32k1xx_gpio_dev_s *s32k1xx_gpio =
    (FAR struct s32k1xx_gpio_dev_s *)dev;
