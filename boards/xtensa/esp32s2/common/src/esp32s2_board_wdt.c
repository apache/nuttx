...
int board_wdt_init(void)
{
  ...
  ret = esp32s2_wdt_initialize("/dev/watchdog0", ESP32S2_WDT_MWDT0);
  if (ret < 0)
    {
      syslog(LOG_ERR, "Failed to initialize MWDT0: %d\n", ret);
      return ret;
    }
#endif /* CONFIG_ESP32S2_MWDT0 */

#ifdef CONFIG_ESP32S2_MWDT1
  ret = esp32s2_wdt_initialize("/dev/watchdog1", ESP32S2_WDT_MWDT1);
  if (ret < 0)
    {
      syslog(LOG_ERR, "Failed to initialize MWDT1: %d\n", ret);
      return ret;
    }
#endif /* CONFIG_ESP32S2_MWDT1 */

#ifdef CONFIG_ESP32S2_RWDT
  ret = esp32s2_wdt_initialize("/dev/watchdog2", ESP32S2_WDT_RWDT);
  if (ret < 0)
    {
