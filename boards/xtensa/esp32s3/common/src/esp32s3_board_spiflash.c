...
static const struct ota_partition_s g_ota_partition_table[] =
{
  ...
  {
    .offset  = CONFIG_ESP32S3_OTA_SCRATCH_OFFSET,
    .size    = CONFIG_ESP32S3_OTA_SCRATCH_SIZE,
    .devpath = CONFIG_ESP32S3_OTA_SCRATCH_DEVPATH
  }
};
#endif
  ...
  for (int i = 0; i < nitems(g_ota_partition_table); ++i)
    {
      ...
      mtd = esp32s3_spiflash_alloc_mtdpart(part->offset, part->size,
                                           OTA_ENCRYPT);
