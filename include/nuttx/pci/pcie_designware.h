...
int dw_pcie_get_resources(FAR struct dw_pcie_s *pci);
void dw_pcie_version_detect(FAR struct dw_pcie_s *pci);
uint8_t dw_pcie_find_capability(FAR struct dw_pcie_s *pci, uint8_t cap);
uint16_t dw_pcie_find_ext_capability(FAR struct dw_pcie_s *pci, uint8_t cap);
...
int dw_pcie_suspend_noirq(FAR struct dw_pcie_s *pci);
int dw_pcie_resume_noirq(FAR struct dw_pcie_s *pci);
...
int dw_pcie_setup_rc(FAR struct dw_pcie_rp_s *pp);
int dw_pcie_host_init(FAR struct dw_pcie_rp_s *pp);
void dw_pcie_host_deinit(FAR struct dw_pcie_rp_s *pp);
int dw_pcie_allocate_domains(FAR struct dw_pcie_rp_s *pp);
FAR void *
dw_pcie_own_conf_map_bus(FAR struct pci_bus *bus, unsigned int devfn,
                         int where);
...
void dw_pcie_ep_cleanup(FAR struct dw_pcie_ep_s *ep);
...
FAR struct dw_pcie_ep_func_s *
dw_pcie_ep_get_func_from_ep(FAR struct dw_pcie_ep_s *ep, uint8_t funcno);
