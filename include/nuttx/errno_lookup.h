...
ERRNO_ITEM(ELIBMAX,         82,  "Attempting to link in too many shared libraries")
ERRNO_ITEM(ELIBEXEC,        83,  "Cannot exec a shared library directly")
ERRNO_ITEM(EILSEQ,          84,  "Illegal byte sequence")
ERRNO_ITEM(ERESTART,        85,  "Interrupted system call should be restarted")
...
ERRNO_ITEM(EOPNOTSUPP,      95,  "Operation not supported on transport endpoint")
...
ERRNO_ITEM(ENETRESET,       102, "Network dropped connection because of reset")
...
ERRNO_ITEM(ESHUTDOWN,       108, "Cannot send after transport endpoint shutdown")
