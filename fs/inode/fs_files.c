      ...
      for (j = 0; j < CONFIG_NFILE_DESCRIPTORS_PER_BLOCK; j++)
        {
          ...
          /* Assign filep to the child's descriptor list. Omit the close
           * on exec flag.
           */

          fdflags.f_cloexec = 0;
          fdlist_finstall(clist, fd, filep, &fdflags);
        }
    }
      ...
      if (i >= list->fl_rows)
        {
          ...
          /* We know local interrupts are still disabled, but we must
           * re-acquire the lock.
           */
...
int nx_dup(int fd, int minfd, int oflags)
{
  ...
  filep = fget_by_fd(list, fd, &fdflags);
  if (filep == NULL)
    {
      return -EBADF;
    }
  ...
  if (fd2 >= 0)
    {
      ...
      fdp->f_cloexec     = oflags & O_CLOEXEC ? true : false;
#ifdef CONFIG_FDSAN
      fdp->f_tag_fdsan   = fdflags.f_tag_fdsan;
#endif
#ifdef CONFIG_FDCHECK
      fdp->f_tag_fdcheck = fdflags.f_tag_fdcheck;
#endif
    }
  else
    {
 ...
 *
 * Description:
 *   nx_fcntl() will perform the operation specified by 'cmd' on an open
 *   file.
 *
 * Input Parameters:
...
int nx_fcntl(int fd, int cmd, unsigned long arg)
{
  int i = fd / CONFIG_NFILE_DESCRIPTORS_PER_BLOCK;
  int j = fd % CONFIG_NFILE_DESCRIPTORS_PER_BLOCK;
  FAR struct fdlist *list;
  ...
  switch (cmd)
    ...
    case F_GETFD:
      ...
      {
        ret = fdp->f_cloexec ? FD_CLOEXEC : 0;
      }
      break;
    ...
    case F_SETFD:
...

      {
        ...
        fdp->f_cloexec = arg & FD_CLOEXEC ? true : false;
        ret = OK;
      }
      break;
#ifdef CONFIG_FDSAN
...
int nx_dup3_from_tcb(FAR struct tcb_s *tcb, int fd1, int fd2, int flags)
{
  ...
  filep1 = fget_by_fd(list, fd1, &fdflags);
  if (filep1 == NULL)
    {
      return -EBADF;
    }
  ...
  fdflags.f_cloexec = flags == FD_CLOEXEC ? true : false;
  filep2 = fdlist_finstall(list, fd2, filep1, &fdflags);
