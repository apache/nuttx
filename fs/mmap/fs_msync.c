  ...
  for (curr = g_rammaps.head; curr; curr = curr->flink)
    {
      ...
      /* Get the offset from the beginning of the region and the actual
       * number of bytes to "msync".
       */
      ...
      if (fpos < 0)
        {
          /* Seek failed... errno has already been set, but EINVAL
           * is probably the correct response.
           */
