...
static int i2c_interrupt(int irq, void *context, void *arg)
{
  ...
  state = getreg32(RP23XX_I2C_IC_INTR_STAT(priv->controller));

  /* -- We need to transmit data (Read Request) -- */

  if (state & RP23XX_I2C_IC_INTR_STAT_R_RD_REQ)
    {
      length = priv->tx_buf_end - priv->tx_buf_ptr;
      if (length > 0)
        {
          while (priv->tx_buf_ptr < priv->tx_buf_end
                 &&   getreg32(RP23XX_I2C_IC_TXFLR(priv->controller))
                    < FIFO_LENGTH)
            {
              putreg32(*priv->tx_buf_ptr++,
                       RP23XX_I2C_IC_DATA_CMD(priv->controller));
            }
        ...
        {
          putreg32(0, RP23XX_I2C_IC_DATA_CMD(priv->controller));
        }

      getreg32(RP23XX_I2C_IC_CLR_RD_REQ(priv->controller));
    }
  ...
  if (state & RP23XX_I2C_IC_INTR_STAT_R_RX_FULL)
    {
      while (getreg32(RP23XX_I2C_IC_RXFLR(priv->controller)) > 0)
        {
          data_cmd = getreg32(RP23XX_I2C_IC_DATA_CMD(priv->controller));

          if (data_cmd & RP23XX_I2C_IC_DATA_CMD_FIRST_DATA_BYTE)
            {
  ...
  if (state & RP23XX_I2C_IC_INTR_STAT_R_RESTART_DET)
    {
      ...
      getreg32(RP23XX_I2C_IC_CLR_RESTART_DET(priv->controller));
    }
  ...
  if (state & RP23XX_I2C_IC_INTR_STAT_R_STOP_DET)
    {
      ...
      getreg32(RP23XX_I2C_IC_CLR_STOP_DET(priv->controller));
    }
  ...
  if (state & RP23XX_I2C_IC_INTR_STAT_R_TX_ABRT)
    {
      getreg32(RP23XX_I2C_IC_CLR_TX_ABRT(priv->controller));
      priv->error = -ENODEV;
    }

  if (state & RP23XX_I2C_IC_INTR_STAT_R_TX_OVER)
    {
      getreg32(RP23XX_I2C_IC_CLR_TX_OVER(priv->controller));
      priv->error = -EIO;
    }

  if (state & RP23XX_I2C_IC_INTR_STAT_R_RX_OVER)
    {
      getreg32(RP23XX_I2C_IC_CLR_RX_OVER(priv->controller));
      priv->error = -EIO;
    }

  if (state & RP23XX_I2C_IC_INTR_STAT_R_RX_UNDER)
    {
      getreg32(RP23XX_I2C_IC_CLR_RX_UNDER(priv->controller));
      priv->error = -EIO;
    }
  ...
  if (state & RP23XX_I2C_IC_INTR_STAT_R_TX_EMPTY)
    {
      ...
      modbits_reg32(RP23XX_I2C_IC_INTR_MASK(priv->controller),
                    0,
                    RP23XX_I2C_IC_INTR_MASK_M_TX_EMPTY);
    }
  ...
  if (state & RP23XX_I2C_IC_INTR_STAT_R_RX_FULL)
    {
      ...
      modbits_reg32(RP23XX_I2C_IC_INTR_MASK(priv->controller),
                    0,
                    RP23XX_I2C_IC_INTR_MASK_M_RX_FULL);
...
static void enable_i2c_slave(struct i2c_slave_s *dev)
{
  ...
  uint32_t intr_mask =   RP23XX_I2C_IC_INTR_STAT_R_RD_REQ
                       ...
                       | RP23XX_I2C_IC_INTR_STAT_R_STOP_DET
                       | RP23XX_I2C_IC_INTR_STAT_R_RESTART_DET
                       | RP23XX_I2C_IC_INTR_STAT_R_TX_ABRT;

  putreg32(0, RP23XX_I2C_IC_ENABLE(priv->controller));

  putreg32(intr_mask, RP23XX_I2C_IC_INTR_MASK(priv->controller));

  putreg32(0, RP23XX_I2C_IC_ENABLE(priv->controller));

  if (priv->controller == 0)
    {
      irq_attach(RP23XX_I2C0_IRQ, i2c_interrupt, dev);
      up_enable_irq(RP23XX_I2C0_IRQ);
    }
  else
    {
      irq_attach(RP23XX_I2C1_IRQ, i2c_interrupt, dev);
      up_enable_irq(RP23XX_I2C1_IRQ);
    }
                              ...
                              int                  nbits)
{
  ...
  uint32_t con =   RP23XX_I2C_IC_CON_RX_FIFO_FULL_HLD_CTRL
  ...
  if (nbits == 10)
    {
      con |= RP23XX_I2C_IC_CON_IC_10BITADDR_SLAVE;
    }
                            ...
                            i2c_slave_callback_t *callback)
{
  ...
  rp23xx_gpio_set_pulls(CONFIG_RP23XX_I2C0_SDA_GPIO, true, false);
...
#ifdef CONFIG_RP23XX_I2C0_SLAVE_10BIT
  my_set_own_address(&(priv->dev),
                     CONFIG_RP23XX_I2C0_SLAVE_ADDRESS,
                     10);
#else
  my_set_own_address(&(priv->dev),
                     CONFIG_RP23XX_I2C0_SLAVE_ADDRESS,
...
#endif /* CONFIG_RP23XX_I2C0_SLAVE */
                            ...
                            i2c_slave_callback_t *callback)
{
  ...
  rp23xx_gpio_set_pulls(CONFIG_RP23XX_I2C1_SDA_GPIO, true, false);
...
#ifdef CONFIG_RP23XX_I2C1_SLAVE_10BIT
  my_set_own_address(&(priv->dev),
                     CONFIG_RP23XX_I2C1_SLAVE_ADDRESS,
                     10);
#else
  my_set_own_address(&(priv->dev),
                     CONFIG_RP23XX_I2C1_SLAVE_ADDRESS,
...
#endif /* CONFIG_RP23XX_I2C1_SLAVE */

#endif /* CONFIG_RP23XX_I2C_SLAVE */
