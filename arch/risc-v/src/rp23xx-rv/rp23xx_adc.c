/****************************************************************************
 * arch/risc-v/src/rp23xx-rv/rp23xx_adc.c
 *
...
static int interrupt_handler(int irq, void *context, void *arg)
{
  ...
  if (g_active_count == 0)
    {
      ...
      putreg32(0, RP23XX_RV_ADC_CS);

      putreg32(0, RP23XX_RV_ADC_INTE);

      up_disable_irq(RP23XX_RV_ADC_IRQ_FIFO);

      irq_detach(RP23XX_RV_ADC_IRQ_FIFO);

      /* Flush the FIFO */

      while (getreg32(RP23XX_RV_ADC_FCS) & RP23XX_RV_ADC_FCS_LEVEL_MASK)
        {
          getreg32(RP23XX_RV_ADC_FIFO);
        }
  ...
  value         = getreg32(RP23XX_RV_ADC_FIFO);
  error_bit_set = (value & RP23XX_RV_ADC_FIFO_ERR) != 0;
          ...
          if (PRIV(a_device)->has_channel[next])
            {
              ...
              while (getreg32(RP23XX_RV_ADC_FCS)
                     & RP23XX_RV_ADC_FCS_LEVEL_MASK)
                {
                  getreg32(RP23XX_RV_ADC_FIFO);
                }
              ...
              putreg32(RP23XX_RV_ADC_INTE_FIFO, RP23XX_RV_ADC_INTE);

              /* Configure CS to read one value from current channel */

              value =   (g_current_channel << RP23XX_RV_ADC_CS_AINSEL_SHIFT)
                      | RP23XX_RV_ADC_CS_EN;
              ...
              putreg32(value, RP23XX_RV_ADC_CS);

              while ((getreg32(RP23XX_RV_ADC_CS)
                      & RP23XX_RV_ADC_CS_READY) == 0)
                {
              ...
              value += RP23XX_RV_ADC_CS_START_ONCE;
  ...
  if (PRIV(g_first_device)->next_device == NULL)
    {
      ...
      value =   (1 << RP23XX_RV_ADC_FCS_THRESH_SHIFT)
              | RP23XX_RV_ADC_FCS_OVER
              | RP23XX_RV_ADC_FCS_UNDER
              | RP23XX_RV_ADC_FCS_ERR
              | RP23XX_RV_ADC_FCS_EN;

      putreg32(value, RP23XX_RV_ADC_FCS);

      /* Set up for interrupts */

      irq_attach(RP23XX_RV_ADC_IRQ_FIFO, interrupt_handler, NULL);

      up_enable_irq(RP23XX_RV_ADC_IRQ_FIFO);
