/****************************************************************************
 * arch/risc-v/src/rp23xx-rv/rp23xx_uart.c
 *
...
/* Select UART parameters for the selected console */
 ...
 #define CONSOLE_BASE     RP23XX_RV_UART0_BASE
 ...
 #define CONSOLE_BASE     RP23XX_RV_UART1_BASE
 #define CONSOLE_BASEFREQ BOARD_UART_BASEFREQ
...
/* Get word length setting for the console */
  ...
  #define CONSOLE_LCR_WLS RP23XX_RV_UART_LCR_H_WLEN(CONSOLE_BITS)
 #else
...
/* Get parity setting for the console */
  ...
  #define CONSOLE_LCR_PAR (RP23XX_RV_UART_UARTLCR_H_PEN)
 #elif CONSOLE_PARITY == 2
  #define CONSOLE_LCR_PAR (RP23XX_RV_UART_UARTLCR_H_PEN | RP23XX_RV_UART_UARTLCR_H_EPS)
 #else
...
#if defined(HAVE_CONSOLE)
 #if CONSOLE_2STOP != 0
  #define CONSOLE_LCR_STOP RP23XX_RV_UART_UARTLCR_H_STP2
 #else
...
void riscv_lowputc(char ch)
{
  ...
  while ((getreg32(CONSOLE_BASE + RP23XX_RV_UART_UARTFR_OFFSET) &
          RP23XX_RV_UART_UARTFR_TXFF))
    ;

  /* Send the character */

  putreg32((uint32_t)ch, CONSOLE_BASE + RP23XX_RV_UART_UARTDR_OFFSET);
#endif
}
...
void rp23xx_lowsetup(void)
{
  ...
  cr = getreg32(CONSOLE_BASE + RP23XX_RV_UART_UARTCR_OFFSET);
  putreg32(cr & ~RP23XX_RV_UART_UARTCR_UARTEN,
           CONSOLE_BASE + RP23XX_RV_UART_UARTCR_OFFSET);

  putreg32(CONSOLE_LCR_VALUE,
           CONSOLE_BASE + RP23XX_RV_UART_UARTLCR_H_OFFSET);
  rp23xx_setbaud(CONSOLE_BASE, CONSOLE_BASEFREQ, CONSOLE_BAUD);
  putreg32(0, CONSOLE_BASE + RP23XX_RV_UART_UARTIFLS_OFFSET);
  putreg32(RP23XX_RV_UART_INTR_ALL,
           CONSOLE_BASE + RP23XX_RV_UART_UARTICR_OFFSET);

  cr |= RP23XX_RV_UART_UARTCR_RXE | RP23XX_RV_UART_UARTCR_TXE |
        RP23XX_RV_UART_UARTCR_UARTEN;
  putreg32(cr, CONSOLE_BASE + RP23XX_RV_UART_UARTCR_OFFSET);
#endif
}
...
void rp23xx_setbaud(uintptr_t uartbase, uint32_t basefreq, uint32_t baud)
{
  ...
  putreg32(ibrd, uartbase + RP23XX_RV_UART_UARTIBRD_OFFSET);
  putreg32(fbrd, uartbase + RP23XX_RV_UART_UARTFBRD_OFFSET);

  /* Baud rate is updated by writing to LCR_H */

  lcr_h = getreg32(uartbase + RP23XX_RV_UART_UARTLCR_H_OFFSET);
