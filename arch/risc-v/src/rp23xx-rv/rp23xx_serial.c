                                     ...
                                     uint32_t *ier)
{
  ...
  if (ier)
    {
      *ier = priv->ier & RP23XX_UART_INTR_ALL;
    }
...
static void up_set_format(struct uart_dev_s *dev)
{
  ...
  if ((5 <= priv->bits) && (priv->bits < 8))
    {
      lcr |= RP23XX_UART_LCR_H_WLEN(priv->bits);
    }
  else
    {
      lcr |= RP23XX_UART_LCR_H_WLEN(8);
    }
