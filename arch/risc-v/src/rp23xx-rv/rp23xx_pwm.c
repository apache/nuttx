/****************************************************************************
 * arch/risc-v/src/rp23xx-rv/rp23xx_pwm.c
 *
  ...
  if (priv->pin[0] >= 0)
    {
      pwminfo("PWM%d setting pin_a %d\n",
              priv->num,
              (priv->flags & RP23XX_RV_PWM_CSR_A_INV) ? 1 : 0);

      rp23xx_gpio_setdir(priv->pin[0], true);
      rp23xx_gpio_put(priv->pin[0],
                      ((priv->flags & RP23XX_RV_PWM_CSR_A_INV) != 0));
  ...
  if (priv->pin[1] >= 0)
    {
      pwminfo("PWM%d setting pin_b %d\n",
              priv->num,
              (priv->flags & RP23XX_RV_PWM_CSR_B_INV) ? 1 : 0);

      rp23xx_gpio_setdir(priv->pin[1], true);
      rp23xx_gpio_put(priv->pin[1],
                      ((priv->flags & RP23XX_RV_PWM_CSR_B_INV) != 0));
               ...
               unsigned long             arg)
{
  ...
  switch (cmd)
    ...
    case PWMIOC_RP23XX_SETINVERTPULSE:
      priv->flags &= ~(RP23XX_RV_PWM_CSR_B_INV | RP23XX_RV_PWM_CSR_A_INV);
    ...
    case PWMIOC_RP23XX_GETINVERTPULSE:
      return (priv->flags &  (RP23XX_RV_PWM_CSR_B_INV
                            | RP23XX_RV_PWM_CSR_A_INV)) >> 2;

    case PWMIOC_RP23XX_SETPHASECORRECT:
      priv->flags &= ~(RP23XX_RV_PWM_CSR_PH_CORRECT);
      priv->flags |= (arg != 0) ? RP23XX_RV_PWM_CSR_PH_CORRECT : 0x00;
    ...
    case PWMIOC_RP23XX_GETPHASECORRECT:
      return (priv->flags & RP23XX_RV_PWM_CSR_PH_CORRECT) ? 1 : 0;
  }
...
void setup_period(struct rp23xx_pwm_lowerhalf_s  * priv)
{
  ...
  if (priv->flags & RP23XX_RV_PWM_CSR_PH_CORRECT)
    {
      frequency *= 2;
    }
  ...
  putreg32(priv->top,     RP23XX_RV_PWM_TOP(priv->num));
  putreg32(priv->divisor, RP23XX_RV_PWM_DIV(priv->num));
...
void setup_pulse(struct rp23xx_pwm_lowerhalf_s  * priv)
{
  ...
  putreg32(compare, RP23XX_RV_PWM_CC(priv->num));

  modreg32(priv->flags,
            RP23XX_RV_PWM_CSR_DIVMODE_MASK
          | RP23XX_RV_PWM_CSR_B_INV
          | RP23XX_RV_PWM_CSR_A_INV
          | RP23XX_RV_PWM_CSR_PH_CORRECT,
          RP23XX_RV_PWM_CSR(priv->num));
