/****************************************************************************
 * arch/risc-v/src/rp23xx-rv/rp23xx_i2s_pio.c
 *
...
/* Configuration ************************************************************/
  ...
  #define CONFIG_RP23XX_RV_I2S_PIO     0
#endif

#ifndef  CONFIG_RP23XX_RV_I2S_PIO_SM
  #define CONFIG_RP23XX_RV_I2S_PIO_SM  0
#endif
...
static const struct rp23xx_i2s_pio_config g_pio_i2s_configs[] =
  {
    [RP23XX_RV_I2S_PIO_16BIT_STEREO] =
      {
        {
          pio_program_i2s_16s,
...
static const uint32_t g_i2s_pio = CONFIG_RP23XX_RV_I2S_PIO;
static const uint32_t g_i2s_pio_sm = CONFIG_RP23XX_RV_I2S_PIO_SM;
...
{
  const struct rp23xx_i2s_pio_config *conf;
  rp23xx_pio_sm_config sm_config;

  uint32_t data_pin = CONFIG_RP23XX_RV_I2S_DATA;
  uint32_t clock_pin_base = CONFIG_RP23XX_RV_I2S_CLOCK;
  uint32_t pin_mask = (1u << data_pin) | (3u << clock_pin_base);

  /* Check parameters */

  if (mode < 0 || mode >= RP23XX_RV_I2S_PIO_MAX_MODE ||
      samplerate == 0)
    {
      return -1;
    }
  ...
  if (mode == g_pio_current_mode)
    {
      if (samplerate == g_pio_current_samplerate)
      ...
      else
        {
          ...
          rp23xx_pio_sm_set_clkdiv(g_i2s_pio, g_i2s_pio_sm,
                                   get_clkdiv(mode, samplerate));
          rp23xx_pio_sm_clkdiv_restart(g_i2s_pio, g_i2s_pio_sm);
          return 0;
        }
    }
  ...
  if (g_pio_current_mode < 0)
    {
      ...
      rp23xx_pio_sm_claim(g_i2s_pio, g_i2s_pio_sm);
    }
  else
    {
      /* Remove existing PIO program to change the I2S mode */

      rp23xx_pio_remove_program(CONFIG_RP23XX_RV_I2S_PIO,
  ...
  g_pio_current_offset = rp23xx_pio_add_program(CONFIG_RP23XX_RV_I2S_PIO,
                          &conf->program);
  ...
  rp23xx_sm_config_set_sideset(&sm_config, 2, false, false);

  rp23xx_sm_config_set_out_pins(&sm_config, data_pin, 1);
  rp23xx_sm_config_set_sideset_pins(&sm_config, clock_pin_base);
  rp23xx_sm_config_set_out_shift(&sm_config, false, conf->autopull, 32);
  rp23xx_sm_config_set_set_pins(&sm_config, data_pin, 1);
  rp23xx_sm_config_set_clkdiv(&sm_config, get_clkdiv(mode, samplerate));
  rp23xx_pio_sm_init(g_i2s_pio, g_i2s_pio_sm,
                     g_pio_current_offset, &sm_config);

  rp23xx_pio_sm_set_pindirs_with_mask(g_i2s_pio, g_i2s_pio_sm,
                                      pin_mask, pin_mask);
  rp23xx_pio_sm_set_pins(g_i2s_pio, g_i2s_pio_sm, 1); /* clear pins */
  rp23xx_pio_sm_exec(g_i2s_pio, g_i2s_pio_sm,
...
{
  rp23xx_pio_sm_set_enabled(g_i2s_pio, g_i2s_pio_sm, enable);
}
...
{
  return RP23XX_PIO_TXF(g_i2s_pio, g_i2s_pio_sm);
}
...
{
  return RP23XX_DMA_DREQ_PIO0_TX0 + g_i2s_pio_sm + g_i2s_pio * 8;
}
