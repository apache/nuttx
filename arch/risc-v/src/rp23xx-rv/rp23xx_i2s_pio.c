...
#ifndef CONFIG_RP23XX_I2S_PIO
  #define CONFIG_RP23XX_I2S_PIO     0
#endif

#ifndef  CONFIG_RP23XX_I2S_PIO_SM
  #define CONFIG_RP23XX_I2S_PIO_SM  0
#endif
...
static const uint32_t g_i2s_pio = CONFIG_RP23XX_I2S_PIO;
static const uint32_t g_i2s_pio_sm = CONFIG_RP23XX_I2S_PIO_SM;
...
int rp23xx_i2s_pio_configure(int mode, uint32_t samplerate)
{
  ...
  uint32_t data_pin = CONFIG_RP23XX_I2S_DATA;
  uint32_t clock_pin_base = CONFIG_RP23XX_I2S_CLOCK;
  uint32_t pin_mask = (1u << data_pin) | (3u << clock_pin_base);

  /* Check parameters */

  if (mode < 0 || mode >= RP23XX_I2S_PIO_MAX_MODE ||
      samplerate == 0)
    {
      return -1;
    }
  ...
  if (g_pio_current_mode < 0)
  ...
  else
    {
      ...
      rp23xx_pio_remove_program(CONFIG_RP23XX_I2S_PIO,
  ...
  g_pio_current_offset = rp23xx_pio_add_program(CONFIG_RP23XX_I2S_PIO,
                          &conf->program);
