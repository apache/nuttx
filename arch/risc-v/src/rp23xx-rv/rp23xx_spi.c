/****************************************************************************
 * arch/risc-v/src/rp23xx-rv/rp23xx_spi.c
 *
...
#ifdef CONFIG_RP23XX_RV_SPI_DMA
#include "rp23xx_dmac.h"
#endif

#ifdef CONFIG_RP23XX_RV_SPI
...
struct rp23xx_spidev_s
{
  struct spi_dev_s spidev;      /* Externally visible part of the SPI interface */
  uint32_t         spibase;     /* SPIn base address */
  uint32_t         spibasefreq;
#ifdef CONFIG_RP23XX_RV_SPI_INTERRUPTS
...
static inline uint32_t spi_getreg(struct rp23xx_spidev_s *priv,
                                  uint8_t offset);
static inline void spi_putreg(struct rp23xx_spidev_s *priv,
...
static void spi_dmatxsetup(struct rp23xx_spidev_s *priv,
...
#ifdef CONFIG_RP23XX_RV_SPI0
static const struct spi_ops_s g_spi0ops =
{
  .lock              = spi_lock,
  .select            = rp23xx_spi0select,   /* Provided externally */
...
#endif
  .status            = rp23xx_spi0status,   /* Provided externally */
#ifdef CONFIG_SPI_CMDDATA
  .cmddata           = rp23xx_spi0cmddata,  /* Provided externally */
#endif
  .send              = spi_send,
  ...
  .registercallback  = rp23xx_spi0register, /* Provided externally */
#else
  .registercallback  = 0,                   /* Not implemented */
#endif
};

static struct rp23xx_spidev_s g_spi0dev =
{
  .spidev            =
  {
    .ops             = &g_spi0ops,
  },
  .spibase           = RP23XX_RV_SPI0_BASE,
  .spibasefreq       = 0,
  .port              = 0,
  .initialized       = 0,
#ifdef CONFIG_RP23XX_RV_SPI_INTERRUPTS
  .spiirq            = RP23XX_RV_SPI0_IRQ,
#endif
...
#ifdef CONFIG_RP23XX_RV_SPI1
static const struct spi_ops_s g_spi1ops =
{
  .lock              = spi_lock,
  .select            = rp23xx_spi1select,   /* Provided externally */
  .setfrequency      = spi_setfrequency,
  .setmode           = spi_setmode,
  .setbits           = spi_setbits,
#ifdef CONFIG_SPI_HWFEATURES
  .hwfeatures        = 0,                   /* Not supported */
#endif
  .status            = rp23xx_spi1status,   /* Provided externally */
#ifdef CONFIG_SPI_CMDDATA
  .cmddata           = rp23xx_spi1cmddata,  /* Provided externally */
#endif
  .send              = spi_send,
#ifdef CONFIG_SPI_EXCHANGE
  .exchange          = spi_exchange,
...
#endif
#ifdef CONFIG_SPI_CALLBACK
  .registercallback  = rp23xx_spi1register, /* Provided externally */
#else
  .registercallback  = 0,                   /* Not implemented */
#endif
};

static struct rp23xx_spidev_s g_spi1dev =
{
  .spidev            =
  {
    .ops             = &g_spi1ops,
  },
  .spibase           = RP23XX_RV_SPI1_BASE,
  .spibasefreq       = 0,
  .port              = 1,
  .initialized       = 0,
#ifdef CONFIG_RP23XX_RV_SPI_INTERRUPTS
...
{
  struct rp23xx_spidev_s *priv = (struct rp23xx_spidev_s *)dev;
  uint32_t regval = spi_getreg(priv, RP23XX_RV_SPI_SSPCR1_OFFSET);
  regval |= RP23XX_RV_SPI_SSPCR1_SSE;
  spi_putreg(priv, RP23XX_RV_SPI_SSPCR1_OFFSET, regval);
}
                                 ...
                                 uint32_t frequency)
{
  ...
  spi_putreg(priv, RP23XX_RV_SPI_SSPCPSR_OFFSET, divisor);
  ...
  if (mode != priv->mode)
    {
      ...
      regval = spi_getreg(priv, RP23XX_RV_SPI_SSPCR0_OFFSET);
      regval &= ~(RP23XX_RV_SPI_SSPCR0_SPO | RP23XX_RV_SPI_SSPCR0_SPH);
      ...
      switch (mode)
        {
          ...
          case SPIDEV_MODE1: /* CPOL=0; CPHA=1 */
            regval |= RP23XX_RV_SPI_SSPCR0_SPH;
            break;

          case SPIDEV_MODE2: /* CPOL=1; CPHA=0 */
            regval |= RP23XX_RV_SPI_SSPCR0_SPO;
            break;
      ...
      spi_putreg(priv, RP23XX_RV_SPI_SSPCR0_OFFSET, regval);
  ...
  if (nbits != priv->nbits)
    {
      ...
      regval = spi_getreg(priv, RP23XX_RV_SPI_SSPCR0_OFFSET);
      regval &= ~RP23XX_RV_SPI_SSPCR0_DSS_MASK;
      regval |= ((nbits - 1) << RP23XX_RV_SPI_SSPCR0_DSS_SHIFT);
      spi_putreg(priv, RP23XX_RV_SPI_SSPCR0_OFFSET, regval);
...
static uint32_t spi_send(struct spi_dev_s *dev, uint32_t wd)
{
  ...
  while (!(spi_getreg(priv, RP23XX_RV_SPI_SSPSR_OFFSET) &
           RP23XX_RV_SPI_SSPSR_TNF))
    ;

  /* Write the byte to the TX FIFO */

  spi_putreg(priv, RP23XX_RV_SPI_SSPDR_OFFSET, wd);
  ...
  regval = spi_getreg(priv, RP23XX_RV_SPI_SSPDR_OFFSET);
  ...
  while (nwords || rxpending)
    {
      ...
      while ((spi_getreg(priv, RP23XX_RV_SPI_SSPSR_OFFSET) &
              RP23XX_RV_SPI_SSPSR_TNF) &&
             (rxpending < RP23XX_RV_SPI_FIFOSZ) && nwords)
        {
          if (txbuffer)
            {
              if (priv->nbits > 8)
          ...
          spi_putreg(priv, RP23XX_RV_SPI_SSPDR_OFFSET,
...
#ifdef CONFIG_RP23XX_RV_SPI_DMATHRESHOLD
  size_t dmath = CONFIG_RP23XX_RV_SPI_DMATHRESHOLD;
#else
  size_t dmath = 0;
#endif
...
struct spi_dev_s *rp23xx_spibus_initialize(int port)
{
  ...
  switch (port)
    {
      ...
      case 0:
        priv = &g_spi0dev;
#ifdef CONFIG_RP23XX_RV_SPI_DMA
        txconf.dreq = RP23XX_DMA_DREQ_SPI0_TX;
        rxconf.dreq = RP23XX_DMA_DREQ_SPI0_RX;
#endif
        break;
#endif
      ...
      case 1:
        priv = &g_spi1dev;
#ifdef CONFIG_RP23XX_RV_SPI_DMA
        txconf.dreq = RP23XX_DMA_DREQ_SPI1_TX;
        rxconf.dreq = RP23XX_DMA_DREQ_SPI1_RX;
#endif
        break;
#endif
...
#ifdef CONFIG_RP23XX_RV_SPI_DMA
  priv->txdmach = rp23xx_dmachannel();
  txconf.size = RP23XX_DMA_SIZE_BYTE;
  txconf.noincr = false;
  priv->txconfig = txconf;

  priv->rxdmach = rp23xx_dmachannel();
  rxconf.size = RP23XX_DMA_SIZE_BYTE;
  rxconf.noincr = false;
  ...
  spi_putreg(priv, RP23XX_RV_SPI_SSPCR0_OFFSET,
             ((8 - 1) << RP23XX_RV_SPI_SSPCR0_DSS_SHIFT) |
             (0 << RP23XX_RV_SPI_SSPCR0_FRF_SHIFT));

  /* Disable SPI and all interrupts (we'll poll for all data) */

  spi_putreg(priv, RP23XX_RV_SPI_SSPCR1_OFFSET, 0);
  spi_putreg(priv, RP23XX_RV_SPI_SSPIMSC_OFFSET, 0);

  /* Clear interrupts */

  spi_putreg(priv, RP23XX_RV_SPI_SSPICR_OFFSET, 0x3);
  ...
  regval = spi_getreg(priv, RP23XX_RV_SPI_SSPCR1_OFFSET);
  spi_putreg(priv, RP23XX_RV_SPI_SSPCR1_OFFSET, regval |
                   RP23XX_RV_SPI_SSPCR1_SSE);

  for (i = 0; i < RP23XX_RV_SPI_FIFOSZ; i++)
    {
...
void spi_flush(struct spi_dev_s *dev)
{
  ...
  while (!(spi_getreg(priv, RP23XX_RV_SPI_SSPSR_OFFSET) &
                            RP23XX_RV_SPI_SSPSR_TNF))
    ;
  spi_putreg(priv, RP23XX_RV_SPI_SSPDR_OFFSET, 0xff);

  /* Wait until TX FIFO and TX shift buffer are empty */

  while (spi_getreg(priv, RP23XX_RV_SPI_SSPSR_OFFSET) &
                          RP23XX_RV_SPI_SSPSR_BSY);
                            ...
                            void *rxbuffer, size_t nwords)
{
  ...
  rp23xx_dmastart(priv->rxdmach, spi_dmarxcallback, priv);
  rp23xx_dmastart(priv->txdmach, spi_dmatxcallback, priv);
...
static void spi_dmatxcallback(DMA_HANDLE handle, uint8_t status, void *data)
{
  struct rp23xx_spidev_s *priv = (struct rp23xx_spidev_s *)data;
                           ...
                           const void *txbuffer, size_t nwords)
{
  ...
  val = spi_getreg(priv, RP23XX_RV_SPI_SSPDMACR_OFFSET);
  val |= RP23XX_RV_SPI_SSPDMACR_TXDMAE;
  spi_putreg(priv, RP23XX_RV_SPI_SSPDMACR_OFFSET, val);

  dst = priv->spibase + RP23XX_RV_SPI_SSPDR_OFFSET;
  ...
  rp23xx_txdmasetup(priv->txdmach, (uintptr_t)dst, (uintptr_t)txbuffer,
                           ...
                           const void *rxbuffer, size_t nwords)
{
  ...
  val = spi_getreg(priv, RP23XX_RV_SPI_SSPDMACR_OFFSET);
  val |= RP23XX_RV_SPI_SSPDMACR_RXDMAE;
  spi_putreg(priv, RP23XX_RV_SPI_SSPDMACR_OFFSET, val);

  src = priv->spibase + RP23XX_RV_SPI_SSPDR_OFFSET;
  ...
  rp23xx_rxdmasetup(priv->rxdmach, (uintptr_t)src, (uintptr_t)rxbuffer,
...
static void spi_dmatrxwait(struct rp23xx_spidev_s *priv)
{
  ...
  val = spi_getreg(priv, RP23XX_RV_SPI_SSPDMACR_OFFSET);
  val &= ~(RP23XX_RV_SPI_SSPDMACR_RXDMAE | RP23XX_RV_SPI_SSPDMACR_TXDMAE);
  spi_putreg(priv, RP23XX_RV_SPI_SSPDMACR_OFFSET, val);
}
