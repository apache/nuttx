...
#ifdef CONFIG_RP23XX_SPI_DMA
#include "rp23xx_dmac.h"
#endif

#ifdef CONFIG_RP23XX_SPI
...
#ifdef CONFIG_RP23XX_SPI_INTERRUPTS
...
static struct rp23xx_spidev_s g_spi0dev =
{
  ...
  .spibase           = RP23XX_SPI0_BASE,
  .spibasefreq       = 0,
  .port              = 0,
  .initialized       = 0,
#ifdef CONFIG_RP23XX_SPI_INTERRUPTS
  .spiirq            = RP23XX_SPI0_IRQ,
#endif
...
static struct rp23xx_spidev_s g_spi1dev =
{
  ...
  .spibase           = RP23XX_SPI1_BASE,
  .spibasefreq       = 0,
  .port              = 1,
  .initialized       = 0,
#ifdef CONFIG_RP23XX_SPI_INTERRUPTS
  .spiirq            = RP23XX_SPI1_IRQ,
#endif
  .lock              = NXMUTEX_INITIALIZER,
#ifdef CONFIG_RP23XX_SPI_DMA
  .dmasem            = SEM_INITIALIZER(0),
      ...
      switch (mode)
        {
          ...
          case SPIDEV_MODE1: /* CPOL=0; CPHA=1 */
            regval |= RP23XX_SPI_SSPCR0_SPH;
            break;

          case SPIDEV_MODE2: /* CPOL=1; CPHA=0 */
            regval |= RP23XX_SPI_SSPCR0_SPO;
            break;
...
static uint32_t spi_send(struct spi_dev_s *dev, uint32_t wd)
{
  ...
  while (!(spi_getreg(priv, RP23XX_SPI_SSPSR_OFFSET) & RP23XX_SPI_SSPSR_TNF))
    ;

  /* Write the byte to the TX FIFO */

  spi_putreg(priv, RP23XX_SPI_SSPDR_OFFSET, wd);

  /* Wait for the RX FIFO not empty */

  while (!(spi_getreg(priv, RP23XX_SPI_SSPSR_OFFSET) & RP23XX_SPI_SSPSR_RNE))
    ;

  /* Get the value from the RX FIFO and return it */

  regval = spi_getreg(priv, RP23XX_SPI_SSPDR_OFFSET);
  ...
  while (nwords || rxpending)
    {
      ...
      while ((spi_getreg(priv, RP23XX_SPI_SSPSR_OFFSET) &
              RP23XX_SPI_SSPSR_TNF) &&
             (rxpending < RP23XX_SPI_FIFOSZ) && nwords)
        {
          if (txbuffer)
            {
              if (priv->nbits > 8)
          ...
          spi_putreg(priv, RP23XX_SPI_SSPDR_OFFSET,
      ...
      while (spi_getreg(priv, RP23XX_SPI_SSPSR_OFFSET) &
             RP23XX_SPI_SSPSR_RNE)
        {
          data = spi_getreg(priv, RP23XX_SPI_SSPDR_OFFSET);
          if (rxbuffer)
            {
              if (priv->nbits > 8)
                {
...
#ifdef CONFIG_RP23XX_SPI_DMATHRESHOLD
  size_t dmath = CONFIG_RP23XX_SPI_DMATHRESHOLD;
#else
  size_t dmath = 0;
#endif
...
struct spi_dev_s *rp23xx_spibus_initialize(int port)
{
  ...
  spi_putreg(priv, RP23XX_SPI_SSPCR0_OFFSET,
             ((8 - 1) << RP23XX_SPI_SSPCR0_DSS_SHIFT) |
             (0 << RP23XX_SPI_SSPCR0_FRF_SHIFT));

  /* Disable SPI and all interrupts (we'll poll for all data) */

  spi_putreg(priv, RP23XX_SPI_SSPCR1_OFFSET, 0);
  spi_putreg(priv, RP23XX_SPI_SSPIMSC_OFFSET, 0);

  /* Clear interrupts */

  spi_putreg(priv, RP23XX_SPI_SSPICR_OFFSET, 0x3);
  ...
  regval = spi_getreg(priv, RP23XX_SPI_SSPCR1_OFFSET);
  spi_putreg(priv, RP23XX_SPI_SSPCR1_OFFSET, regval | RP23XX_SPI_SSPCR1_SSE);

  for (i = 0; i < RP23XX_SPI_FIFOSZ; i++)
    {
      spi_getreg(priv, RP23XX_SPI_SSPDR_OFFSET);
    }
...
void spi_flush(struct spi_dev_s *dev)
{
  ...
  while (!(spi_getreg(priv, RP23XX_SPI_SSPSR_OFFSET) & RP23XX_SPI_SSPSR_TNF))
    ;
  spi_putreg(priv, RP23XX_SPI_SSPDR_OFFSET, 0xff);

  /* Wait until TX FIFO and TX shift buffer are empty */

  while (spi_getreg(priv, RP23XX_SPI_SSPSR_OFFSET) & RP23XX_SPI_SSPSR_BSY);
                           ...
                           const void *txbuffer, size_t nwords)
{
  ...
  val |= RP23XX_SPI_SSPDMACR_TXDMAE;
  spi_putreg(priv, RP23XX_SPI_SSPDMACR_OFFSET, val);

  dst = priv->spibase + RP23XX_SPI_SSPDR_OFFSET;
                           ...
                           const void *rxbuffer, size_t nwords)
{
  ...
  val = spi_getreg(priv, RP23XX_SPI_SSPDMACR_OFFSET);
  val |= RP23XX_SPI_SSPDMACR_RXDMAE;
  spi_putreg(priv, RP23XX_SPI_SSPDMACR_OFFSET, val);

  src = priv->spibase + RP23XX_SPI_SSPDR_OFFSET;
