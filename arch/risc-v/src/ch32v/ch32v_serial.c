/****************************************************************************
 * arch/risc-v/src/ch32v/ch32v_serial.c
 *
...
#define CH32V_CONSOLE_TX     GPIO_UART0_TX
#define CH32V_CONSOLE_RX     GPIO_UART0_RX
#define HAVE_UART
...
#define CH32V_CONSOLE_TX     GPIO_UART1_TX
#define CH32V_CONSOLE_RX     GPIO_UART1_RX
#define HAVE_UART
#endif
#endif /* HAVE_CONSOLE */
...
static int  ch32v_setup(struct uart_dev_s *dev);
static void ch32v_shutdown(struct uart_dev_s *dev);
static int  ch32v_attach(struct uart_dev_s *dev);
static void ch32v_detach(struct uart_dev_s *dev);
static int  ch32v_ioctl(struct file *filep, int cmd, unsigned long arg);
static int  ch32v_receive(struct uart_dev_s *dev, unsigned int *status);
static void ch32v_rxint(struct uart_dev_s *dev, bool enable);
static bool ch32v_rxavailable(struct uart_dev_s *dev);
static void ch32v_send(struct uart_dev_s *dev, int ch);
static void ch32v_txint(struct uart_dev_s *dev, bool enable);
static bool ch32v_txready(struct uart_dev_s *dev);
static bool ch32v_txempty(struct uart_dev_s *dev);
...
{
  .setup       = ch32v_setup,
  .shutdown    = ch32v_shutdown,
  .attach      = ch32v_attach,
  .detach      = ch32v_detach,
  .ioctl       = ch32v_ioctl,
  .receive     = ch32v_receive,
  .rxint       = ch32v_rxint,
  .rxavailable = ch32v_rxavailable,
#ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol = NULL,
#endif
  .send    = ch32v_send,
  .txint   = ch32v_txint,
  .txready = ch32v_txready,
  .txempty = ch32v_txempty,
};

/* I/O buffers */

#ifdef CONFIG_CH32V_UART0
static char g_uart0rxbuffer[CONFIG_UART0_RXBUFSIZE];
static char g_uart0txbuffer[CONFIG_UART0_TXBUFSIZE];

static struct ch32v_uart_s g_uart0priv =
{
  .irq      = CH32V_IRQ_UART0,
  .config =
    {
...
#ifdef CONFIG_CH32V_UART1
static char g_uart1rxbuffer[CONFIG_UART1_RXBUFSIZE];
static char g_uart1txbuffer[CONFIG_UART1_TXBUFSIZE];

static struct ch32v_uart_s g_uart1priv =
{
  .irq      = CH32V_IRQ_UART1,
  .config =
    {
...
static int __uart_interrupt(int irq, void *context, void *arg)
{
  ...
  struct ch32v_uart_s *priv = dev->priv;
  uint8_t uart_idx          = priv->config.idx;
  uint32_t int_status;
  uint32_t int_mask;

  int_status = getreg32(CH32V_UART_INT_STS(uart_idx));
  int_mask = getreg32(CH32V_UART_INT_MASK(uart_idx));
...
static int ch32v_setup(struct uart_dev_s *dev)
{
  struct ch32v_uart_s *priv = (struct ch32v_uart_s *)dev->priv;

  ch32v_uart_configure(&priv->config);
  return OK;
}
...
static void ch32v_shutdown(struct uart_dev_s *dev)
{
  ...
  modifyreg32(ch32v_UART_UTX_CONFIG(uart_idx), UART_UTX_CONFIG_CR_EN, 0);
  modifyreg32(ch32v_UART_URX_CONFIG(uart_idx), UART_URX_CONFIG_CR_EN, 0);
}
          ...
          if (ret == OK)
            {
              ...
              tmp_val = getreg32(CH32V_UART_INT_MASK(config.idx));
              ch32v_uart_configure(&config);
              putreg32(tmp_val, CH32V_UART_INT_MASK(config.idx));
            }
        }
      while (0);
      break;
...
static int ch32v_receive(struct uart_dev_s *dev, unsigned int *status)
{
  ...
  if (getreg32(CH32V_UART_FIFO_CONFIG_1(uart_idx)) & \
      UART_FIFO_CONFIG_1_RX_CNT_MASK)
    {
      rxdata = getreg32(CH32V_UART_FIFO_RDATA(uart_idx)) & \
  ...
  if (enable)
    {
      ...
      int_mask &= ~(UART_INT_MASK_CR_URX_FIFO_MASK);
      int_mask &= ~(UART_INT_MASK_CR_URX_END_MASK);
      putreg32(int_mask, CH32V_UART_INT_MASK(uart_idx));
#endif
    }
  else
    {
...
static bool ch32v_rxavailable(struct uart_dev_s *dev)
{
  ...
  return (getreg32(CH32V_UART_FIFO_CONFIG_1(uart_idx)) & \
...
static void ch32v_send(struct uart_dev_s *dev, int ch)
{
  ...
  while ((getreg32(CH32V_UART_FIFO_CONFIG_1(uart_idx)) & \
         UART_FIFO_CONFIG_1_TX_CNT_MASK) == 0);

  putreg32(ch, CH32V_UART_FIFO_WDATA(uart_idx));
}
