...
static int esp_pcnt_open(struct pcnt_dev_s *dev);
static int esp_pcnt_close(struct pcnt_dev_s *dev);
static ssize_t esp_pcnt_read(struct pcnt_dev_s *dev,
                             char *buffer,
                             size_t buflen);
...
static int esp_pcnt_ioctl(struct pcnt_dev_s *dev, int cmd,
                          unsigned long arg);
static int esp_pcnt_unit_enable(struct pcnt_dev_s *dev);
static int esp_pcnt_unit_disable(struct pcnt_dev_s *dev);
static int esp_pcnt_unit_start(struct pcnt_dev_s *dev);
static int esp_pcnt_unit_stop(struct pcnt_dev_s *dev);
static int esp_pcnt_unit_get_count(struct pcnt_dev_s *dev, int *ret);
static int esp_pcnt_unit_clear_count(struct pcnt_dev_s *dev);
static int esp_pcnt_unit_set_glitch_filter(struct pcnt_dev_s *dev,
    const struct esp_pcnt_glitch_filter_config_s *config);
static int esp_pcnt_unit_register_event_callback(struct pcnt_dev_s *dev,
...
static const struct pcnt_ops_s g_esp_pcnt_fops =
{
  esp_pcnt_open,    /* open */
  esp_pcnt_close,   /* close */
  esp_pcnt_read,    /* read */
  esp_pcnt_ioctl,   /* ioctl */
};
...
static int esp_pcnt_close(struct pcnt_dev_s *dev)
{
  ...
  if (ret != OK)
    {
      pcnterr("Failed to stop pcnt-%d", priv->unit_id);
      return ret;
    }
  ...
  if (priv->state != PCNT_UNIT_INIT)
    {
      ...
      if (ret != OK)
        {
          pcnterr("Failed to disable pcnt-%d", priv->unit_id);
        }
    }
...
static int esp_pcnt_open(struct pcnt_dev_s *dev)
{
  ...
  if (priv->state == PCNT_UNIT_INIT)
    {
      ...
      if (ret != OK)
        {
          pcnterr("Failed to enable pcnt-%d", priv->unit_id);
          return ret;
        }
    }
  ...
  if (ret != OK)
    {
       pcnterr("Failed to start pcnt-%d", priv->unit_id);
    }
 ...
 *
 * Description:
 *   Read PCNT unit count value.
 *
 * Input Parameters:
 *   filep  - The pointer of file, represents each user using the sensor
 *   buffer - Buffer to save pulse counter (PCNT) value
...
static ssize_t esp_pcnt_read(struct pcnt_dev_s *dev, char *buffer,
                             size_t buflen)
{
  struct esp_pcnt_priv_s *priv = (struct esp_pcnt_priv_s *)dev;
  int count = 0;
  ...
  if (buflen != sizeof(count))
    {
      return -EINVAL;
    }

  ret = esp_pcnt_unit_get_count(dev, &count);
  if (ret != OK)
    {
      pcnterr("Could not get pcnt-%d count!\n", priv->unit_id);
      return ret;
    }
...
static int esp_pcnt_ioctl(struct pcnt_dev_s *dev, int cmd, unsigned long arg)
{
  struct esp_pcnt_priv_s *priv = (struct esp_pcnt_priv_s *)dev;
  ...
  switch (cmd)
    {
      case SNIOC_RESET:
        ...
        if (ret != OK)
          {
            pcnterr("Could not clear pcnt-%d!\n", priv->unit_id);
          }
      ...
      case SNIOC_THRESHOLD:
        ...
        if (ret != OK)
          {
            pcnterr("Could not to set glitch filter to pcnt-%d!\n",
                    priv->unit_id);
          }
      ...
      case SNIOC_CONFIGURE:
        ...
        if (ret != OK)
          {
            pcnterr("Could not register callback-%lx to pcnt-%d!\n",
      ...
      case SNIOC_CFGR:
        ...
        if (ret != OK)
          {
            pcnterr("Could not to add watch point-%d to pcnt-%d!\n",
      ...
      case SNIOC_ENABLE:
        ...
        if (ret != OK)
          {
            pcnterr("Failed to enable pcnt-%d!\n",
                    priv->unit_id);
          }
      ...
      case SNIOC_DISABLE:
        ...
        if (ret != OK)
          {
            pcnterr("Failed to disable pcnt-%d!\n",
                    priv->unit_id);
          }
      ...
      default:
        pcnterr("Unrecognized IOCTL command: %d\n", cmd);
        ret = -ENOTTY;
        break;
    }
                                 ...
                                 int intr_alloc_flags)
{
  ...
  if (cpuint < 0)
    {
      pcnterr("Failed to allocate a CPU interrupt.\n");
      return ERROR;
    }
  ...
  if (ret < 0)
    {
      pcnterr("Couldn't attach IRQ to handler.\n");
...
static int esp_pcnt_unit_enable(struct pcnt_dev_s *dev)
{
  ...
  if (!priv->unit_used)
    {
      pcnterr("Invalid unit ID!\n");
      return ERROR;
    }

  if (priv->state != PCNT_UNIT_INIT)
    {
      pcnterr("Unit is already enabled!\n");
      return ERROR;
    }
...
static int esp_pcnt_unit_disable(struct pcnt_dev_s *dev)
{
  ...
  if (priv->state != PCNT_UNIT_ENABLE)
    {
      pcnterr("Unit is already disabled!\n");
      return ERROR;
    }
  ...
  if (config != NULL)
    {
      ...
      if (glitch_filter_thres > PCNT_LL_MAX_GLITCH_WIDTH)
        {
          pcnterr("Glitch width %" PRId32 " out of range!\n",
...
struct pcnt_dev_s *esp_pcnt_new_unit(struct esp_pcnt_unit_config_s *config)
{
  ...
  if (config == NULL)
    {
      pcnterr("Configuration struct is NULL!\n");
      return NULL;
    }

  if (config->low_limit >= 0 && config->low_limit < PCNT_LL_MIN_LIN)
    {
      pcnterr("Configuration low limit is out of range!\n");
      return NULL;
    }

  if (config->high_limit < 0 && config->high_limit >= PCNT_LL_MAX_LIM)
    {
      pcnterr("Configuration high limit is out of range!\n");
      return NULL;
    }
  ...
  if (unit_id == PCNT_UNIT_COUNT)
    {
      pcnterr("No available PCNT unit for allocation\n");
  ...
  spin_unlock_irqrestore(&pcnt_units[unit_id].lock, flags);
  pcntinfo("Allocated pcnt unit: %" PRId16 "\n", unit_id);
  ...
  pcntinfo("Allocated pcnt unit: %" PRId16 " count range:[%d %d]\n", unit_id,
           pcnt_units[unit_id].config.low_limit,
           pcnt_units[unit_id].config.high_limit);

  return (struct pcnt_dev_s *)&pcnt_units[unit_id];
}
  ...
  for (i = 0; i < SOC_PCNT_CHANNELS_PER_UNIT; i++)
    {
      if (!priv->channels[i])
        {
          pcnterr("Channel %" PRId8
                  " still in working!\n", i);
          return ERROR;
        }
    }

  pcntinfo("Delete pcnt unit: %" PRId8 "\n", priv->unit_id);
...
int esp_pcnt_unit_add_watch_point(struct pcnt_dev_s *dev, int watch_point)
{
  ...
  if (watch_point > unit->config.high_limit ||
    ...
    {
      pcnterr("Watch point is out of limit!\n");
      return ERROR;
    }
  ...
  if (watch_point == 0)
    {
      if (unit->watchers[PCNT_LL_WATCH_EVENT_ZERO_CROSS].event_id !=
        ...
        {
          pcnterr("Zero cross event watcher has been installed already!\n");
          ret = ERROR;
        }
      else
        {
  ...
  /* high limit watch point */
  ...
  else if (watch_point == unit->config.high_limit)
    {
      if (unit->watchers[PCNT_LL_WATCH_EVENT_HIGH_LIMIT].event_id !=
          PCNT_LL_WATCH_EVENT_INVALID)
        {
          pcnterr("High limit event watcher has been installed already!\n");
          ret = ERROR;
        }
      else
        {
  ...
  /* low limit watch point */
  ...
  else if (watch_point == unit->config.low_limit)
    {
      if (unit->watchers[PCNT_LL_WATCH_EVENT_LOW_LIMIT].event_id !=
          PCNT_LL_WATCH_EVENT_INVALID)
        {
          pcnterr("Low limit event watcher has been installed already!\n");
          ret = ERROR;
        }
      else
        {
  ...
  /* other threshold watch point */
  ...
  else
    {
      ...
      switch (thres_num)
        {
          case 1:
            ...
            {
              pcnterr("Watcher has been installed already!\n");
              ret = ERROR;
              break;
            }
      ...
      default:
          pcnterr("No free threshold watch point available!\n");
          ret = ERROR;
          break;
      }
    }
  ...
  if (ret != ERROR)
    {
      pcntinfo("Watchpoint %d for pcnt unit-%d added successfully\n",
               watch_point,
               unit->unit_id);
    }
...
int esp_pcnt_unit_remove_watch_point(struct pcnt_dev_s *dev, int watch_point)
{
  struct esp_pcnt_priv_s *unit = (struct esp_pcnt_priv_s *)dev;
  int ret = OK;
  int i;
                         ...
                         struct esp_pcnt_chan_config_s *config)
{
  ...
  if (!config)
    {
      pcnterr("Channel config is NULL!\n");
      return ERROR;
    }

  if (!priv->unit_used)
    {
      pcnterr("Unit did not initialize yet!\n");
      return ERROR;
    }

  if (priv->state != PCNT_UNIT_INIT)
    {
      pcnterr("Unit already running!\n");
      return ERROR;
    }
  ...
  if (channel_id == -1)
    {
      pcnterr("no free channel in unit %" PRId8 "!\n", priv->unit_id);
      return ERROR;
    }
  ...
  pcntinfo("Added pcnt unit: %" PRId8 " channel: %" PRId8 "\n",
           unit_id, channel_id);
...
int esp_pcnt_del_channel(int channel)
{
  ...
  pcntinfo("Deleted pcnt unit: %" PRId8 " channel: %" PRId8 "\n",
           unit_id, chan_id);
  return OK;
}
