...
struct qe_dev_lowerhalf_s
{
  ...
  struct pcnt_dev_s *pcnt;             /* Device handle */
...
static int esp_setup(struct qe_lowerhalf_s *lower)
{
  ...
  priv->pcnt->ops->ioctl(priv->pcnt, SNIOC_RESET, 0);
  priv->pcnt->ops->open(priv->pcnt);
...
static int esp_position(struct qe_lowerhalf_s *lower, int32_t *pos)
{
  ...
  ret = (priv->pcnt->ops->read(priv->pcnt, (char *)&count, sizeof(count))
         & 0xffff);
  if (ret < 0)
    {
