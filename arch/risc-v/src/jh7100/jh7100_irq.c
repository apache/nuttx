/****************************************************************************
 * arch/risc-v/src/jh7100/jh7100_irq.c
 *
...
void up_irqinitialize(void)
{
  ...
  putreg32(0x0, JH7100_PLIC_MIE0);
  putreg32(0x0, JH7100_PLIC_MIE1);

  /* Clear pendings in PLIC */

  uint32_t val = getreg32(JH7100_PLIC_MCLAIM);
  putreg32(val, JH7100_PLIC_MCLAIM);

  /* Colorize the interrupt stack for debug purposes */
#undef CONFIG_STACK_COLORATION
  ...
  for (id = 1; id <= NR_IRQS; id++)
    {
      putreg32(1, (uintptr_t)(JH7100_PLIC_PRIORITY + (4 * id)));
    }

  /* Set irq threshold to 0 (permits all global interrupts) */

  putreg32(0, JH7100_PLIC_MTHRESHOLD);
  ...
  irq_attach(JH7100_IRQ_ECALLM, riscv_swint, NULL);

#ifdef CONFIG_BUILD_PROTECTED
  irq_attach(JH7100_IRQ_ECALLU, riscv_swint, NULL);
#endif
