...
static const struct esp32c3_spi_config_s esp32c3_spi2_config =
{
...
static const struct spi_ops_s esp32c3_spi2_ops =
{
  .lock              = esp32c3_spi_lock,
#ifdef CONFIG_ESP32C3_SPI_UDCS
  .select            = esp32c3_spi2_select,
#else
  .select            = esp32c3_spi_select,
#endif
  .setfrequency      = esp32c3_spi_setfrequency,
  .setmode           = esp32c3_spi_setmode,
  .setbits           = esp32c3_spi_setbits,
#ifdef CONFIG_SPI_HWFEATURES
  .hwfeatures        = esp32c3_spi_hwfeatures,
#endif
  .status            = esp32c3_spi2_status,
#ifdef CONFIG_SPI_CMDDATA
  .cmddata           = esp32c3_spi2_cmddata,
#endif
  .send              = esp32c3_spi_send,
#ifdef CONFIG_SPI_EXCHANGE
  .exchange          = esp32c3_spi_exchange,
#else
  .sndblock          = esp32c3_spi_sndblock,
  .recvblock         = esp32c3_spi_recvblock,
#endif
#ifdef CONFIG_SPI_TRIGGER
  .trigger           = esp32c3_spi_trigger,
#endif
...
static inline void esp32c3_spi_set_regbits(uint32_t addr, uint32_t bits)
{
...
static inline void esp32c3_spi_clr_regbits(uint32_t addr, uint32_t bits)
{
  uint32_t tmp = getreg32(addr);
 ...
 *
 *   If ESP32C3_SPI_SWCS is disabled, the driver will use hardware CS so that
                                         ...
                                         uint32_t frequency)
{
  ...
  putreg32(reg_val, SPI_CLOCK_REG);
  ...
  if (mode != priv->mode)
    {
      ...
      esp32c3_spi_clr_regbits(SPI_MISC_REG, SPI_CK_IDLE_EDGE_M);
      esp32c3_spi_set_regbits(SPI_MISC_REG,
                              VALUE_MASK(ck_idle_edge, SPI_CK_IDLE_EDGE));

      esp32c3_spi_clr_regbits(SPI_USER_REG, SPI_CK_OUT_EDGE_M);
      esp32c3_spi_set_regbits(SPI_USER_REG,
...
/****************************************************************************
 * Name: esp32c3_spi_poll_exchange
 *
 * Description:
  ...
  while (bytes_remaining != 0)
    {
      ...
      putreg32((transfer_size * 8) - 1, SPI_MS_DLEN_REG);
                                 ...
                                 size_t nwords)
{
  ...
  esp32c3_spi_exchange(dev, txbuffer, NULL, nwords);
}

/****************************************************************************
 * Name: esp32c3_spi_recvblock
 *
 * Description:
                                  ...
                                  size_t nwords)
{
  ...
  esp32c3_spi_exchange(dev, NULL, rxbuffer, nwords);
}
#endif

/****************************************************************************
 * Name: esp32c3_spi_trigger
 *
 * Description:
 ...
 *
 * Description:
 *   Initialize ESP32-C3 SPI hardware interface.
 *
 * Input Parameters:
 *   dev      - Device-specific state data
 *
...
static void esp32c3_spi_init(struct spi_dev_s *dev)
{
  ...
  esp32c3_gpiowrite(config->cs_pin, true);
  esp32c3_gpiowrite(config->mosi_pin, true);
  esp32c3_gpiowrite(config->miso_pin, true);
  esp32c3_gpiowrite(config->clk_pin, true);

#if SPI_HAVE_SWCS
  esp32c3_configgpio(config->cs_pin, OUTPUT_FUNCTION_1);
  esp32c3_gpio_matrix_out(config->cs_pin, SIG_GPIO_OUT_IDX, 0, 0);
#endif

#if SPI_VIA_IOMUX
#if !SPI_HAVE_SWCS
  esp32c3_configgpio(config->cs_pin, OUTPUT_FUNCTION_2);
  esp32c3_gpio_matrix_out(config->cs_pin, SIG_GPIO_OUT_IDX, 0, 0);
#endif
  esp32c3_configgpio(config->mosi_pin, OUTPUT_FUNCTION_2);
  esp32c3_gpio_matrix_out(config->mosi_pin, SIG_GPIO_OUT_IDX, 0, 0);

  esp32c3_configgpio(config->miso_pin, INPUT_FUNCTION_2 | PULLUP);
  esp32c3_gpio_matrix_out(config->miso_pin, SIG_GPIO_OUT_IDX, 0, 0);

  esp32c3_configgpio(config->clk_pin, OUTPUT_FUNCTION_2);
  esp32c3_gpio_matrix_out(config->clk_pin, SIG_GPIO_OUT_IDX, 0, 0);
#else
#if !SPI_HAVE_SWCS
  esp32c3_configgpio(config->cs_pin, OUTPUT_FUNCTION_1);
  esp32c3_gpio_matrix_out(config->cs_pin, config->cs_outsig, 0, 0);
#endif
  esp32c3_configgpio(config->mosi_pin, OUTPUT_FUNCTION_1);
  esp32c3_gpio_matrix_out(config->mosi_pin, config->mosi_outsig, 0, 0);

  esp32c3_configgpio(config->miso_pin, INPUT_FUNCTION_1 | PULLUP);
  esp32c3_gpio_matrix_in(config->miso_pin, config->miso_insig, 0);

  esp32c3_configgpio(config->clk_pin, OUTPUT_FUNCTION_1);
  esp32c3_gpio_matrix_out(config->clk_pin, config->clk_outsig, 0, 0);
#endif
  ...
  putreg32(SPI_CS1_DIS_M | SPI_CS2_DIS_M, SPI_MISC_REG);

  regval = SPI_CLK_EN_M | SPI_MST_CLK_ACTIVE_M | SPI_MST_CLK_SEL_M;
  putreg32(regval, SPI_CLK_GATE_REG);

#if SPI_HAVE_SWCS
  esp32c3_spi_set_regbits(SPI_MISC_REG, SPI_CS0_DIS_M);
#endif
...
/****************************************************************************
 * Name: esp32c3_spi_deinit
 *
 * Description:
 *   Deinitialize ESP32-C3 SPI hardware interface.
 *
 * Input Parameters:
 *   dev      - Device-specific state data
 *
