/****************************************************************************
 * arch/arm/src/rtl8720f/ameba_timer_chip.h
 *
...
#ifndef __ARCH_ARM_SRC_RTL8720F_AMEBA_TIMER_CHIP_H
#define __ARCH_ARM_SRC_RTL8720F_AMEBA_TIMER_CHIP_H
...
/* Per-chip general-purpose timer wiring for RTL8720F.  The shared driver
 * (arch/arm/src/common/ameba/ameba_timer.c) includes this header to learn
 * which timer instances it may expose and, for each one, its register base,
 * input clock, RCC clock masks and interrupt line.  A port to another Ameba
 * chip supplies a same-named header on the chip include path; the shared
 * driver is never edited -- it reads only the macros below and the instance
 * table, computing the period from clkfreq with one formula.
 *
 ...
 *   --------  ------   ----------   ---------   --------------   ----------
 *   0         TIM1     0x40808200   32.768 kHz  APBPeriph_LTIM1  IRQ_TIMER1
 *   1         TIM2     0x40808400   32.768 kHz  APBPeriph_LTIM2  IRQ_TIMER2
 *
 * TIM1 and TIM2 are two of the four "basic" (LTIM) timers clocked from the
...
{                                                                           \
  {                                                                         \
    0x40808200ul, AMEBA_TIMER_LTIM1_MASK, AMEBA_TIMER_LTIM1_MASK,           \
    32768ul, RTL8720F_IRQ_TIMER1, 1                                         \
  },                                                                        \
  {                                                                         \
    0x40808400ul, AMEBA_TIMER_LTIM2_MASK, AMEBA_TIMER_LTIM2_MASK,           \
    32768ul, RTL8720F_IRQ_TIMER2, 2                                         \
  },                                                                        \
}

#endif /* __ARCH_ARM_SRC_RTL8720F_AMEBA_TIMER_CHIP_H */
