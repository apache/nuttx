############################################################################
# arch/arm/src/rtl8720f/ameba_board.mk
#
# Shared board-build definitions for the RTL8720F (Ameba WHC) -- the SDK
...
# just `include`s this file, so every RTL8720F board shares one definition
...
# (linked via arch/.../rtl8720f/Make.defs EXTRA_LIBS) instead of vendoring a
...
# RTL8720F: ameba_pmu.c lives in fwlib/ram_common (not misc/), and DiagPrintf
...
# NuttX owns the image2 entry: arch/arm/src/rtl8720f/ameba_app_start.c is a
...
AMEBA_FWLIB_SRCS += $(TOPDIR)/arch/arm/src/rtl8720f/ameba_app_start.c \
...
# --- WiFi (WHC host) glue lib (CONFIG_RTL8720F_WIFI) ----------------------
#
...
define PREBUILD
	...
	$(Q) if [ "$(CONFIG_RTL8720F_WIFI)" = "y" ]; then \
...
define POSTBUILD
	...
	$(Q) TARGET_SOC=RTL8720F PATH=$(AMEBA_VENV_BIN):$$PATH $(CMAKE) \
	...
	$(Q) echo "PACK: wrote $(TOPDIR)$(DELIM){app.bin,boot.bin,km4ns_image2_all.bin}"
endef
