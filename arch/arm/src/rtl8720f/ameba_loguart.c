/***************************************************************************
 * arch/arm/src/rtl8720f/ameba_loguart.c
 *
...
/* This driver targets the RTL8720F LOG-UART, which NuttX (on
...
/* LOG-UART device base (UARTLOG_REG_BASE, from SDK RTL8720F
 * hal_platform.h).  NOTE: this is the RTL8720F address (0x401C6000) -- it
...
/* These live in the RTL8720F ROM and are resolved at link time from the
...
static char g_loguart_rxbuffer[CONFIG_RTL8720F_LOGUART_RXBUFSIZE];
static char g_loguart_txbuffer[CONFIG_RTL8720F_LOGUART_TXBUFSIZE];
...
static struct uart_dev_s g_loguart_port =
{
  ...
  .recv      =
  {
    .size    = CONFIG_RTL8720F_LOGUART_RXBUFSIZE,
    .buffer  = g_loguart_rxbuffer,
  },
  .xmit      =
  {
    .size    = CONFIG_RTL8720F_LOGUART_TXBUFSIZE,
...
static int loguart_attach(struct uart_dev_s *dev)
{
  ...
  ret = irq_attach(RTL8720F_IRQ_UART_LOG, loguart_interrupt, dev);
  if (ret == OK)
    {
      ...
      LOGUART_INTCoreConfig(LOGUART_DEV, LOGUART_BIT_INTR_MASK_AP,
                            AMEBA_ENABLE);
      up_enable_irq(RTL8720F_IRQ_UART_LOG);
    }
...
{
  UNUSED(dev);
  up_disable_irq(RTL8720F_IRQ_UART_LOG);
  irq_detach(RTL8720F_IRQ_UART_LOG);
}
