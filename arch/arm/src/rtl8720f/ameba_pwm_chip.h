/****************************************************************************
 * arch/arm/src/rtl8720f/ameba_pwm_chip.h
 *
...
#ifndef __ARCH_ARM_SRC_RTL8720F_AMEBA_PWM_CHIP_H
#define __ARCH_ARM_SRC_RTL8720F_AMEBA_PWM_CHIP_H
...
/* Per-chip PWM wiring for RTL8720F.  The shared driver
 ...
 *
 * Unlike amebadplus (one 8-channel TIM8), RTL8720F spreads PWM across
 * several timers of four channels each (TIM4/TIM5, see IS_TIM_PWM_TIM in the
 ...
 *     hal_platform.h); the secure alias (0x501C7000) must not be used from
 ...
 *     PINMUX_FUNCTION_TIM4_PWM0..3 (45..48).
 *   - AMEBA_PWM_APBPERIPH / _CLK: the "function" and "clock" args to
 *     RCC_PeriphClockCmd().  Unlike amebadplus, RTL8720F uses distinct
 ...
 *     driver registers no callback (NULL); TIM4 is IRQ 9 (TIMER4_IRQ).
 */

#define AMEBA_PWM_TIMER_IDX       4            /* TIM4 (IS_TIM_PWM_TIM)     */
#define AMEBA_PWM_NCHAN           4            /* CCR0..CCR3 (PWM_CHAN_MAX) */
#define AMEBA_PWM_BASE            0x401c7000ul /* NON-secure TIM4 base      */
 ...
 * minus one (channel n uses AMEBA_PWM_PINMUX_FIDS[n - 1]).  On RTL8720F
 * these are PINMUX_FUNCTION_TIM4_PWM0..3 (45..48).
 */

#define AMEBA_PWM_PINMUX_FIDS     { 45, 46, 47, 48 }

/* APBPeriph_PWM0 (function) and APBPeriph_PWM0_CLOCK masks.  The group
 * selector (bit30) is 0; RTL8720F splits function-enable (bit23) and
...
#endif /* __ARCH_ARM_SRC_RTL8720F_AMEBA_PWM_CHIP_H */
