...
#      error STM32L4 USART2/3 receive DMA requires CONFIG_STM32_DMA1
#    endif
#  endif

#  if defined(CONFIG_UART4_RXDMA) || defined(CONFIG_UART5_RXDMA)
#    if !defined(CONFIG_STM32_DMA2) && !defined(CONFIG_STM32_DMAMUX)
#      error STM32L4 UART4/5 receive DMA requires CONFIG_STM32_DMA2
#    endif
#  endif
...
static int  stm32l4serial_dmareceive(struct uart_dev_s *dev,
                                     unsigned int *status);
static void stm32l4serial_dmareenable(struct stm32_serial_s *priv);
#ifdef CONFIG_SERIAL_IFLOWCONTROL
static bool stm32l4serial_dmaiflowrestart(struct stm32_serial_s *priv);
#endif
...
const g_uart_devs[STM32_NLPUART + STM32_NUSART + STM32_NUART] =
{
#ifdef CONFIG_STM32_LPUART1_SERIALDRIVER
  [0] = &g_lpuart1priv,
#endif
...
static inline
uint32_t stm32l4serial_getreg(struct stm32_serial_s *priv, int offset)
{
...
static inline void stm32l4serial_putreg(struct stm32_serial_s *priv,
...
static inline
void stm32l4serial_setusartint(struct stm32_serial_s *priv,
                               uint16_t ie)
{
  uint32_t cr;
  ...
  cr = stm32l4serial_getreg(priv, STM32_USART_CR1_OFFSET);
  cr &= ~(USART_CR1_USED_INTS);
  cr |= (ie & (USART_CR1_USED_INTS));
  stm32l4serial_putreg(priv, STM32_USART_CR1_OFFSET, cr);

  cr = stm32l4serial_getreg(priv, STM32_USART_CR3_OFFSET);
  cr &= ~USART_CR3_EIE;
  cr |= (ie & USART_CR3_EIE);
  stm32l4serial_putreg(priv, STM32_USART_CR3_OFFSET, cr);
}
...
static void stm32l4serial_restoreusartint(struct stm32_serial_s *priv,
                                          uint16_t ie)
{
  irqstate_t flags;
...
static void stm32l4serial_disableusartint(struct stm32_serial_s *priv,
                                          uint16_t *ie)
{
  irqstate_t flags;
  ...
  if (ie)
    {
      ...
      cr3 = stm32l4serial_getreg(priv, STM32_USART_CR3_OFFSET);
...
#ifdef SERIAL_HAVE_RXDMA
static int stm32l4serial_dmanextrx(struct stm32_serial_s *priv)
{
  size_t dmaresidual;
...
#ifndef CONFIG_SUPPRESS_UART_CONFIG
static void stm32l4serial_setbaud_usart(struct stm32_serial_s *priv)
{
  ...
  cr1 = stm32l4serial_getreg(priv, STM32_USART_CR1_OFFSET);
  brr = stm32l4serial_getreg(priv, STM32_USART_BRR_OFFSET);
  ...
  stm32l4serial_putreg(priv, STM32_USART_CR1_OFFSET, cr1);
  stm32l4serial_putreg(priv, STM32_USART_BRR_OFFSET, brr);
}
#endif
...
#ifndef CONFIG_SUPPRESS_UART_CONFIG
#ifdef CONFIG_STM32_LPUART1_SERIALDRIVER
static void stm32l4serial_setbaud_lpuart(struct stm32_serial_s *priv)
{
  uint32_t brr;
...
static void stm32l4serial_setformat(struct uart_dev_s *dev)
{
  ...
  regval  = stm32l4serial_getreg(priv, STM32_USART_CR1_OFFSET);
  ...
  stm32l4serial_putreg(priv, STM32_USART_CR1_OFFSET, regval);

  /* Configure STOP bits */

  regval = stm32l4serial_getreg(priv, STM32_USART_CR2_OFFSET);
  ...
  stm32l4serial_putreg(priv, STM32_USART_CR2_OFFSET, regval);

  /* Configure hardware flow control */

  regval  = stm32l4serial_getreg(priv, STM32_USART_CR3_OFFSET);
  ...
  stm32l4serial_putreg(priv, STM32_USART_CR3_OFFSET, regval);
}
  ...
  if (suspend)
    {
      ...
      while ((stm32l4serial_getreg(priv, STM32_USART_ISR_OFFSET) &
              USART_ISR_TC) == 0);
  ...
  if (priv->iflow)
    {
      ...
      stm32_dmastart(priv->rxdma, stm32l4serial_dmarxcallback,
                       (void *)priv, false);
    }
  else
#endif
  ...
  for (passes = 0; passes < 256 && handled; passes++)
    {
      ...
      priv->sr = stm32l4serial_getreg(priv, STM32_USART_ISR_OFFSET);
       ...
       */
                ...
                (USART_ISR_ORE | USART_ISR_NF | USART_ISR_FE)) != 0)
        {
          ...
          stm32l4serial_putreg(priv, STM32_USART_ICR_OFFSET,
    ...
    case TIOCSSINGLEWIRE:
      {
        ...
        uint32_t cr = stm32l4serial_getreg(priv, STM32_USART_CR3_OFFSET);
        ...
        stm32l4serial_putreg(priv, STM32_USART_CR1_OFFSET, cr1 | cr1_ue);
        leave_critical_section(flags);
      }
     break;
#endif
    ...
    case TIOCSINVERT:
      {
        ...
        uint32_t cr = stm32l4serial_getreg(priv, STM32_USART_CR2_OFFSET);
        ...
        stm32l4serial_putreg(priv, STM32_USART_CR2_OFFSET, cr);
    ...
    case TIOCSBRK:  /* No BSD compatibility: Turn break on for M bit times */
      {
        ...
        flags = enter_critical_section();
        cr1   = stm32l4serial_getreg(priv, STM32_USART_CR1_OFFSET);
        stm32l4serial_putreg(priv, STM32_USART_CR1_OFFSET,
                             cr1 | USART_CR1_SBK);
        leave_critical_section(flags);
      }
      break;
                                 ...
                                 unsigned int *status)
{
  ...
  rdr      = stm32l4serial_getreg(priv, STM32_USART_RDR_OFFSET);
...
static bool stm32l4serial_rxavailable(struct uart_dev_s *dev)
{
  ...
  return ((stm32l4serial_getreg(priv, STM32_USART_ISR_OFFSET) &
           USART_ISR_RXNE) != 0);
}
#endif
...
static void stm32l4serial_send(struct uart_dev_s *dev, int ch)
{
  ...
  stm32l4serial_putreg(priv, STM32_USART_TDR_OFFSET, (uint32_t)ch);
}
