...
#if defined(CONFIG_STM32_STM32L4X3) || defined(CONFIG_STM32_STM32L4XR)
#  if defined(CONFIG_STM32_ADC2) || defined(CONFIG_STM32_ADC3)
...
#    error "STM32L4 ADC DMA support requires CONFIG_STM32_DMA1 or CONFIG_STM32_DMA2"
#  endif
#endif
...
#ifndef CONFIG_STM32_ADC_SMPR
#  define ADC_SMPR_DEFAULT    ADC_SMPR_640p5
#else
#  define ADC_SMPR_DEFAULT CONFIG_STM32_ADC_SMPR
#endif
...
#  endif
#endif
#if defined(ADC_HAVE_DFSDM) || defined(CONFIG_STM32_ADC_LL_OPS)
...
static int adc_setup(struct adc_dev_s *dev)
{
#if !defined(CONFIG_STM32_ADC_NOIRQ) ||  defined(ADC_HAVE_TIMER) || \
  !defined(CONFIG_STM32_ADC_NO_STARTUP_CONV) || defined(HAVE_ADC_RESOLUTION)
  struct stm32_dev_s *priv = (struct stm32_dev_s *)dev->ad_priv;
#endif
  int ret = OK;
  irqstate_t flags;
              ...
              switch (priv->r_chanlist[i])
                {
                  ...
                  case 17:
#if !(defined(CONFIG_STM32_STM32L4X3) && defined(CONFIG_STM32_DAC1_OUTPUT_ADC))
                    *adc_ccr |= ADC_CCR_TSEN;
#endif
                    break;
                  case 18:
#if !(defined(CONFIG_STM32_STM32L4X3) && defined(CONFIG_STM32_DAC2_OUTPUT_ADC))
  ...
  switch (intf)
    {
      ...
      case 1:
        {
          ainfo("ADC1 selected\n");
          cjchannels = CONFIG_STM32_ADC1_INJ_CHAN;
...
#  endif
      ...
      case 2:
        {
          ainfo("ADC2 selected\n");
          cjchannels = CONFIG_STM32_ADC2_INJ_CHAN;
          crchannels = cchannels - cjchannels;
          ainfo("  Reg. chan: %d Inj chan: %d\n", crchannels, cjchannels);
#  ifdef ADC_HAVE_INJECTED
          if (cjchannels > 0)
...
#  endif
      ...
      case 3:
        {
          ainfo("ADC3 selected\n");
          cjchannels = CONFIG_STM32_ADC3_INJ_CHAN;
          crchannels = cchannels - cjchannels;
          ainfo("  Reg. chan: %d Inj chan: %d\n", crchannels, cjchannels);
#  ifdef ADC_HAVE_INJECTED
          if (cjchannels > 0)
...
#  endif
  ...
  DEBUGASSERT(crchannels <= CONFIG_STM32_ADC_MAX_SAMPLES);
  if (crchannels > CONFIG_STM32_ADC_MAX_SAMPLES)
    {
      crchannels = CONFIG_STM32_ADC_MAX_SAMPLES;
    }
