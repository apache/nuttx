/****************************************************************************
 * arch/arm/src/mcx-nxxx/nxxx_irq.c
 *
...
static void nxxx_dumpnvic(const char *msg, int irq)
{
  irqstate_t flags;
...
#else
#  define nxxx_dumpnvic(msg, irq)
#endif

/****************************************************************************
 * Name: nxxx_nmi, nxxx_pendsv, nxxx_pendsv, nxxx_reserved
 *
 * Description:
...
static int nxxx_nmi(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int nxxx_pendsv(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int nxxx_reserved(int irq, void *context, void *arg)
{
  up_irq_save();
...
static inline void nxxx_prioritize_syscall(int priority)
{
  uint32_t regval;
...
static int nxxx_irqinfo(int irq, uintptr_t *regaddr, uint32_t *bit,
                       uintptr_t offset)
{
  unsigned int extint = irq - NXXX_IRQ_EXTINT;

  DEBUGASSERT(irq >= NXXX_IRQ_NMI && irq < NR_IRQS);

  /* Check for external interrupt */

  if (irq >= NXXX_IRQ_EXTINT)
    {
      if (extint < 32)
        {
  ...
  /* Handle processor exceptions.  Only a few can be disabled */
  ...
  else
    {
      *regaddr = NVIC_SYSHCON;
      if (irq == NXXX_IRQ_MEMFAULT)
        {
          *bit = NVIC_SYSHCON_MEMFAULTENA;
        }
      else if (irq == NXXX_IRQ_BUSFAULT)
        {
          *bit = NVIC_SYSHCON_BUSFAULTENA;
        }
      else if (irq == NXXX_IRQ_USAGEFAULT)
        {
          *bit = NVIC_SYSHCON_USGFAULTENA;
        }
      else if (irq == NXXX_IRQ_SYSTICK)
        {
...
void up_irqinitialize(void)
{
  ...
  irq_attach(NXXX_IRQ_SVCALL, arm_svcall, NULL);
  irq_attach(NXXX_IRQ_HARDFAULT, arm_hardfault, NULL);

  /* Set the priority of the SVCall interrupt */

#ifdef CONFIG_ARCH_IRQPRIO
  /* up_prioritize_irq(NXXX_IRQ_PENDSV, NVIC_SYSH_PRIORITY_MIN); */
#endif
  nxxx_prioritize_syscall(NVIC_SYSH_SVCALL_PRIORITY);
  ...
  irq_attach(NXXX_IRQ_MEMFAULT, arm_memfault, NULL);
  up_enable_irq(NXXX_IRQ_MEMFAULT);
#endif

  /* Attach all other processor exceptions (except reset and sys tick) */

#ifdef CONFIG_DEBUG_FEATURES
  irq_attach(NXXX_IRQ_NMI, nxxx_nmi, NULL);
#ifndef CONFIG_ARM_MPU
  irq_attach(NXXX_IRQ_MEMFAULT, arm_memfault, NULL);
#endif
  irq_attach(NXXX_IRQ_BUSFAULT, arm_busfault, NULL);
  irq_attach(NXXX_IRQ_USAGEFAULT, arm_usagefault, NULL);
  irq_attach(NXXX_IRQ_PENDSV, nxxx_pendsv, NULL);
  arm_enable_dbgmonitor();
  irq_attach(NXXX_IRQ_DBGMONITOR, arm_dbgmonitor, NULL);
  irq_attach(NXXX_IRQ_RESERVED, nxxx_reserved, NULL);
#endif

  nxxx_dumpnvic("initial", NR_IRQS);
...
void up_disable_irq(int irq)
{
  ...
  if (nxxx_irqinfo(irq, &regaddr, &bit, NVIC_CLRENA_OFFSET) == 0)
    {
  ...
  else
    {
      ...
      nxxx_gpioirq_disable(irq);
    }
#endif

#if 0 /* Might be useful in early bring-up */
  nxxx_dumpnvic("disable", irq);
#endif
}
...
void up_enable_irq(int irq)
{
  ...
  if (nxxx_irqinfo(irq, &regaddr, &bit, NVIC_ENA_OFFSET) == 0)
    {
  ...
  else
    {
      ...
      nxxx_gpioirq_enable(irq);
    }
#endif

#if 0 /* Might be useful in early bring-up */
  nxxx_dumpnvic("enable", irq);
#endif
}
...
int up_prioritize_irq(int irq, int priority)
{
  ...
  DEBUGASSERT(irq >= NXXX_IRQ_MEMFAULT && irq < NR_IRQS &&
