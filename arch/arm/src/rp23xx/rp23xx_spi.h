/****************************************************************************
 * arch/arm/src/rp23xx/rp23xx_spi.h
 *
...
#ifndef __ARCH_ARM_SRC_RP23XX_RP23XX_SPI_H
#define __ARCH_ARM_SRC_RP23XX_RP23XX_SPI_H
 ...
 *
 * 1. Provide logic in rp23xx_boardinitialize() to configure SPI chip select
 *    pins.
 * 2. Provide rp23xx_spi0/1select() and rp23xx_spi0/1status() functions in
 ...
 *    rp23xx_spi0/1cmddata() functions in your board-specific logic.  These
 ...
 * 5. The handle returned by rp23xx_spibus_initialize() may then be used to
...
/****************************************************************************
 * Name:  rp23xx_spiXselect, rp23xx_spiXstatus, and rp23xx_spiXcmddata
 *
 * Description:
 *   These functions must be provided in your board-specific logic.
 *   The rp23xx_spi0/1select functions will perform chip selection and the
 *   rp23xx_spi0/1status will perform status operations using GPIOs in
 *   the way your board is configured.
 *
 *   If CONFIG_SPI_CMDDATA is defined in the NuttX configuration, then
 *   rp23xx_spi0/1cmddata must also be provided.
...
void  rp23xx_spi0select(struct spi_dev_s *dev,
                        uint32_t devid,
                        bool selected);
uint8_t rp23xx_spi0status(struct spi_dev_s *dev,
                          uint32_t devid);
#ifdef CONFIG_SPI_CMDDATA
int rp23xx_spi0cmddata(struct spi_dev_s *dev,
                       uint32_t devid,
                       bool cmd);
#endif
#endif
...
void  rp23xx_spi1select(struct spi_dev_s *dev,
                        uint32_t devid,
                        bool selected);
uint8_t rp23xx_spi1status(struct spi_dev_s *dev,
                          uint32_t devid);
#ifdef CONFIG_SPI_CMDDATA
int rp23xx_spi1cmddata(struct spi_dev_s *dev,
                       uint32_t devid,
                       bool cmd);
#endif
#endif
...
#endif /* __ASSEMBLY__ */
#endif /* CONFIG_RP23XX_SPI0/1 */
#endif /* __ARCH_ARM_SRC_RP23XX_RP23XX_SPI_H */
