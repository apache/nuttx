...
static void setup_period (struct rp23xx_pwm_lowerhalf_s  * priv);

static void setup_pulse  (struct rp23xx_pwm_lowerhalf_s  * priv);

static void set_enabled  (struct rp23xx_pwm_lowerhalf_s  * priv);

static void clear_enabled(struct rp23xx_pwm_lowerhalf_s  * priv);
...
#endif
{
  struct rp23xx_pwm_lowerhalf_s *data;

  data = calloc(1, sizeof (struct rp23xx_pwm_lowerhalf_s));
...
int pwm_setup(struct pwm_lowerhalf_s  * dev)
{
  ...
  if (priv->pin[0] >= 0)
    {
      rp23xx_gpio_set_function(priv->pin[0], RP23XX_GPIO_FUNC_PWM);
    }

  if (priv->pin[1] >= 0)
    {
      rp23xx_gpio_set_function(priv->pin[1], RP23XX_GPIO_FUNC_PWM);
    }
#else
  if (priv->pin >= 0)
    {
      rp23xx_gpio_set_function(priv->pin, RP23XX_GPIO_FUNC_PWM);
    }
#endif
...
int pwm_shutdown (struct pwm_lowerhalf_s  * dev)
{
  ...
  if (priv->pin[0] >= 0)
    {
      pwminfo("PWM%d setting pin_a %d\n",
              priv->num,
              (priv->flags & RP23XX_PWM_CSR_A_INV) ? 1 : 0);

      rp23xx_gpio_setdir(priv->pin[0], true);
      rp23xx_gpio_put(priv->pin[0],
                      ((priv->flags & RP23XX_PWM_CSR_A_INV) != 0));
      rp23xx_gpio_set_function(priv->pin[0], RP23XX_GPIO_FUNC_SIO);
    }
  ...
  if (priv->pin[1] >= 0)
    {
      pwminfo("PWM%d setting pin_b %d\n",
              priv->num,
              (priv->flags & RP23XX_PWM_CSR_B_INV) ? 1 : 0);

      rp23xx_gpio_setdir(priv->pin[1], true);
      rp23xx_gpio_put(priv->pin[1],
                      ((priv->flags & RP23XX_PWM_CSR_B_INV) != 0));
      rp23xx_gpio_set_function(priv->pin[1], RP23XX_GPIO_FUNC_SIO);
    }
#else
  pwminfo("PWM%d pin %d\n", priv->num, priv->pin);
  ...
  if (priv->pin >= 0)
    {
      rp23xx_gpio_setdir(priv->pin, true);
      rp23xx_gpio_put(priv->pin,
                      ((priv->flags & RP23XX_PWM_CSR_A_INV) != 0));
      rp23xx_gpio_set_function(priv->pin, RP23XX_GPIO_FUNC_SIO);
    }
#endif
...
static inline void set_enabled(struct rp23xx_pwm_lowerhalf_s  * priv)
{
...
static inline void clear_enabled(struct rp23xx_pwm_lowerhalf_s  * priv)
{
  irqstate_t flags = enter_critical_section();
