...
static int rp23xx_cyw_init(gspi_dev_t  *gspi)
{
...
static int rp23xx_cyw_set_isr(gspi_dev_t  *gspi,
                      xcpt_t           thread_isr,
                      void        *thread_isr_arg)
{
  rp23xx_gspi_t *rp_io = (rp23xx_gspi_t *)(gspi->io_dev);
...
/****************************************************************************
 * Name: rp23xx_cyw_interrupt_enable
 *
 * Description:
...
static int rp23xx_cyw_interrupt_enable(gspi_dev_t  *gspi,
                               bool             enable)
{
  rp23xx_gspi_t *rp_io         = (rp23xx_gspi_t *)(gspi->io_dev);
...
static int rp23xx_cyw_deinit(gspi_dev_t *gspi)
{
  rp23xx_gspi_t *rp_io = (rp23xx_gspi_t *)(gspi->io_dev);
...
static int rp23xx_cyw_write(struct gspi_dev_s   *gspi,
                    bool                 increment,
                    ...
                    const uint32_t      *data)
{
  ...
  rp23xx_cyw_interrupt_enable(gspi, false);
  ...
  rp23xx_cyw_interrupt_enable(gspi, true);
...
static int rp23xx_cyw_read(struct gspi_dev_s   *gspi,
                   bool                 increment,
                   enum gspi_cmd_func_e function,
                   uint32_t             address,
                   uint16_t             length,
                             ...
                             uint8_t gpio_intr)
{
  ...
  gspi->init             = rp23xx_cyw_init;
  gspi->deinit           = rp23xx_cyw_deinit;
  gspi->set_isr          = rp23xx_cyw_set_isr;
  gspi->interrupt_enable = rp23xx_cyw_interrupt_enable;
  gspi->write            = rp23xx_cyw_write;
  gspi->read             = rp23xx_cyw_read;
  gspi->io_dev           = rp_io;
