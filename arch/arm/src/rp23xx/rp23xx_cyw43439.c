...
static int my_init(gspi_dev_t  *gspi)
{
  ...
  for (rp_io->pio = 0; rp_io->pio < RP23XX_PIO_NUM; ++rp_io->pio)
    {
      ...
      pio_sm = rp23xx_pio_claim_unused_sm(rp_io->pio, false);

      /* If we did not get one try the next pio block, if any. */

      if (pio_sm < 0) continue;
  ...
  /* Hold the data and clock lines low while nothing is driving them.
   *
   * The data line doubles as the chip's active-high interrupt request, and
   * we service it with a level-high interrupt, so it must read low whenever
   * the cyw43439 has released it.  The RP2350 pads come out of reset with
   * the pull-down enabled, but say so explicitly rather than rely on that.
   */

  rp23xx_gpio_set_pulls(rp_io->gpio_data, false, true);
  rp23xx_gpio_set_pulls(rp_io->gpio_clock, false, true);
                    ...
                    const uint32_t      *data)
{
   ...
   *
   * The RP2350 GPIO_STATUS register dropped the RP2040's OEFROMPERI field,
   * so watch OETOPAD instead.  No override is configured on this pad, so
   * the two carry the same value.
   */
