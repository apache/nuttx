/****************************************************************************
 * arch/arm/src/phy62xx/phy62xx_ble_patch.c
 *
...
void ll_hw_go1(void)
{
  ...
  *(volatile uint32_t *)(LL_HW_BASE + 0x00) = 0x0001;           /* trig */
  ...
  /* 2018-05-23 ZQ
   * fix negative rfPhyFreqOff bug, when in scan_rsq case, ll_hw_go will
   * be excuted before set_channel() so do not change the tx_rx_foff next
   * metal change could modified the set_channel() to deal with the tx_rx_foff
   */
  ...
  if (!g_same_rf_channel_flag)
    {
      if (g_rfPhyFreqOffSet >= 0)
        {
          PHY_REG_WT(0x400300b4, (g_rfPhyFreqOffSet << 16) +
              (g_rfPhyFreqOffSet << 8) + rfChnIdx);
        }
      else
        {
          PHY_REG_WT(0x400300b4, ((255 + g_rfPhyFreqOffSet) << 16) +
              ((255 + g_rfPhyFreqOffSet) << 8) + (rfChnIdx - 1));
        }
    }
  ...
  if (g_rfPhyPktFmt == PKT_FMT_BLE2M)
    {
      subWriteReg(0x40030094, 7, 0, RF_PHY_TPCAL_CALC(g_rfPhyTpCal0_2Mbps,
          g_rfPhyTpCal1_2Mbps, (rfChnIdx - 2) >> 1));
    }
  else
    {
      subWriteReg(0x40030094, 7, 0, RF_PHY_TPCAL_CALC(g_rfPhyTpCal0,
          g_rfPhyTpCal1, (rfChnIdx - 2) >> 1));
    }
  ...
  if ((llModeLast == LL_HW_MODE_TRX) && ((llState == LL_STATE_ADV_UNDIRECTED
      || llState == LL_STATE_ADV_SCAN || llState == LL_STATE_ADV_DIRECTED)
   ...
   *     {
   *         SET_BITS(tx_scanRsp_desc.txheader, LL_DEV_ADDR_TYPE_PUBLIC,
   *              TX_ADD_SHIFT, TX_ADD_MASK);
   *     }
   *     else if(adv_param.ownAddrType == LL_DEV_ADDR_TYPE_RANDOM)
   *     {
   *         SET_BITS(tx_scanRsp_desc.txheader, LL_DEV_ADDR_TYPE_RANDOM,
   *              TX_ADD_SHIFT, TX_ADD_MASK);
   *     }
   */
...
void ll_adptive_adj_next_time1(uint32_t next_time)
{
  ...
  /* read loop timeout counter, system clock may be
   * 16MHz, 32MHz, 64MHz and 48MHz, 96MHz
   */
  ...
  if (connPtr->rx_timeout)
    {
      ...
      slave_conn_event_recv_delay = LL_TIME_DELTA(g_t_llhwgo, ISR_entry_time)
          - 370 + 160; /* 160:timer1 irq->hwgo trigger */
    }
  else
    {
      connPtr->pmCounter.ll_tbd_cnt1 = 0;

      /* slave_conn_event_recv_delay = loop_time - anchor_point +
       * pGlobal_config[SLAVE_CONN_DELAY];
       */

      slave_conn_event_recv_delay = LL_TIME_DELTA(g_t_llhwgo, ISR_entry_time)
          - anchor_point + pGlobal_config[SLAVE_CONN_DELAY];
    }
...
void llConnTerminate1(llConnState_t *connPtr, uint8 reason)
{
  /* ZQ:20210622
   * process chanmp update passed instant(core 4.2 should term link,
   * since core 5.0 just update the )
   * just update chanmap do not trigger ll conn termination
   */

  if (reason == LL_CTRL_PKT_INSTANT_PASSED_PEER_TERM
      && ((uint16)(connPtr->chanMapUpdateEvent - connPtr->currentEvent)
      >= LL_MAX_UPDATE_COUNT_RANGE) && ((!osal_memcmp(connPtr->chanMap,
      connPtr->chanMapUpdate.chanMap, 5))))
    {
 ...
 */
  ...
  set_max_length(0xff);
  ll_hw_set_rx_timeout(scanTime);
  /* maximum scan time, note that actual scan time may exceed
   * the limit if timer expiry when LL engine receiving a report
   */
 ...
 *               a packet was successfully received with MD=0 (i.e. no more
 *               Slave data) after having transmitted a packet with MD=0.
 *               Second, a received packet did not fit in the RX FIFO after
 *               transmitting a packet with MD=0. Third, a packet was received
 *               from the Slave while BLE_L_CONF.ENDC is true or after
 *               Timer 2 Event 2 occurs.
 *
 *               Note: The TASK_ENDOK end cause will also handle the
 *                     TASK_NOSYNC,TASK_RXERR, and TASK_MAXNACK end causes
 *                     as well.
 *   input parameters
 *
 *   @param       None.
...
void llMasterEvt_TaskEndOk1(void)
{
   ...
   * Note: Since Auto-Flush is enabled, numRxFifoFull is incremented instead
   *       of numRxOk when there's no room in the FIFO. When Auto-Flush is
   ...
   *    connPtr->perInfoByChan->numCrcErr[ PHY_GET_DATA_CHAN() ] +=
   *        rfCounters.numRxNotOk;
   *  }
   */
  ...
  if (rfCounters.numRxOk || rfCounters.numRxIgnored ||
            ...
            || connPtr->rx_crcok != 0) /* ever Rx CRC OK packet */
    {
      ...
      connPtr->expirationEvent = connPtr->currentEvent +
  ...
  else /* no data received, or packet received with CRC error */
    {
      ...
      if (rfCounters.numRxNotOk)
        {
          /* clear flag that indicates we received first packet
           * Note: The first packet only really needs to be signalled when a
           *       new connection is formed. However, there's no harm in
           *       resetting it every time in order to simplify the control
           *          logic.
           * Note: True-Low logic is used here to be consistent with nR's
           *          language.
           */
        ...
        {
          /* collect packet error information, TI use HCI ext to get this
           * information. No used by PHY+ now
           */
      ...
      if (connPtr->expirationEvent == connPtr->currentEvent)
        {
          ...
          if (connPtr->firstPacket == 0)
            ...
            {
              /* so terminate immediately with failure to
  ...
  if (llProcessMasterControlProcedures(connPtr) ==
  ...
  else if(connPtr->ctrlDataIsPending == 1)
    {
      ...
      if ((connPtr->ctrlData.header == (pktLenctrl << 8 |
          LL_DATA_PDU_HDR_LLID_CONTROL_PKT)) && (*pBufctrl
  ...
  ll_scheduler(schedule_time - 10 -
      LL_TIME_DELTA(g_ll_conn_ctx.timerExpiryTick, T2));    /* 10us: rough delay from timer expire to timer ISR */
...
uint8_t ll_hw_read_rfifo1(uint8_t *rxPkt, uint16_t *pktLen,
    uint32_t *pktFoot0, uint32_t *pktFoot1)
{
  int rdPtr;
  int wrPtr;
  int rdDepth;
...
uint8 ll_processBasicIRQ_SRX0(uint32_t irq_status)
{
  ...
  if (mode == LL_HW_MODE_SRX
      && (llState == LL_STATE_SCAN || llState == LL_STATE_INIT))
    {
          ...
          if ((irq_status & LIRQ_RD) && (irq_status & LIRQ_COK))       /* bug correct 2018-10-15 */
            {
              ...
              /* cost 21-26us(measure with GPIO),
               * depneds on the length of ADV
               */

              packet_len = ll_hw_read_rfifo1((uint8_t *)
                  (&(g_rx_adv_buf.rxheader)), &pktLen, &pktFoot0, &pktFoot1);
              ...
              if (packet_len != 0
                            ...
                            || (pdu_type == ADV_DIRECT_IND)))
                {
                  ...
                  uint8_t txAdd = (g_rx_adv_buf.rxheader & TX_ADD_MASK) >>
                      TX_ADD_SHIFT;
                  ...
                  if (txAdd == LL_DEV_ADDR_TYPE_RANDOM  &&
                      (g_rx_adv_buf.data[5] & RANDOM_ADDR_HDR)
                      == PRIVATE_RESOLVE_ADDR_HDR)
                    {
                      ...
                      if (g_llRlEnable == TRUE)
                        {
                          rpaListIndex =
                              ll_getRPAListEntry(&g_rx_adv_buf.data[0]);

                          if (rpaListIndex < LL_RESOLVINGLIST_ENTRY_NUM)
                            {
                              peerAddr =
                                &g_llResolvinglist[rpaListIndex].peerAddr[0];

                              /* refer to HCI LE Advertising Report Event,
                               * RPA address type should be
                               * 0x02: Public Identity Address (Corresponds
                              ...
                              addrType =
                                  g_llResolvinglist[rpaListIndex].peerAddrType
                                  + 2;
                  ...
                  else     /* case 2: receive ScanA using device ID, or scan device not using RPA */
                    {
                      ...
                      for (int i = 0; i < LL_RESOLVINGLIST_ENTRY_NUM; i++)
                        {
                          if (g_llResolvinglist[i].peerAddr[0] ==
                              g_rx_adv_buf.data[0]
                              && g_llResolvinglist[i].peerAddr[1] ==
                              g_rx_adv_buf.data[1]
                              && g_llResolvinglist[i].peerAddr[2] ==
                              g_rx_adv_buf.data[2]
                              && g_llResolvinglist[i].peerAddr[3] ==
                              g_rx_adv_buf.data[3]
                              && g_llResolvinglist[i].peerAddr[4] ==
                              g_rx_adv_buf.data[4]
                              && g_llResolvinglist[i].peerAddr[5] ==
                              g_rx_adv_buf.data[5])
                            {
                              /* the device ID in the RPA list */

                              if (g_llResolvinglist[i].privacyMode ==
                                DEVICE_PRIVACY_MODE ||
                   ...
                   * # direct adv only report addr
                   * & addr type match the whitelist
                      ...
                      if (scanInfo.scanType == LL_SCAN_ACTIVE &&
                          (pdu_type == ADV_IND || pdu_type == ADV_SCAN_IND))
                        {
                          ...
                          scanInfo.currentBackoff =
                              (scanInfo.currentBackoff > 0) ?
                              (scanInfo.currentBackoff - 1) : 0;
                              ...
                              if (retAdvFilter)
                                {
                                  ...
                                  T2 = read_current_fine_time();
                                  delay = (T2 > ISR_entry_time) ?
                                      (T2 - ISR_entry_time) : (BASE_TIME_UNITS
                                      - ISR_entry_time + T2);

                                  delay = 118 - delay -
                                     pGlobal_config[LL_ADV_TO_SCAN_REQ_DELAY];
                                  ...
                                  /* 20181012 ZQ: change the txheader
                                   * according to the adtype
                                   */

                                  /* g_tx_adv_buf.txheader |=
                                   *     (((g_rx_adv_buf.rxheader&0x40)<<1)
                                   *     | (scanInfo.ownAddrType <<
                                   *     TX_ADD_SHIFT & TX_ADD_MASK));
                                   */

                                  /* fill scanA, using RPA or device
                                   * ID address
                                   */

                                  /* TODO: move below code before ll_hw_go? */

                                  if (rpaListIndex <
                                      LL_RESOLVINGLIST_ENTRY_NUM &&
                                      !ll_isIrkAllZero(g_llResolvinglist[rpaListIndex].localIrk)
                                      && (scanInfo.ownAddrType ==
                                      LL_DEV_ADDR_TYPE_RPA_PUBLIC
                                      || scanInfo.ownAddrType ==
                                      LL_DEV_ADDR_TYPE_RPA_RANDOM))
                                    {
                                      /* for resolving private address case,
                                       * calculate the scanA with Local IRK
                                       */

                                      ll_CalcRandomAddr(
                                          g_llResolvinglist[rpaListIndex].localIrk,
                                          &g_tx_adv_buf.data[0]);
                                  ...
                                  g_tx_adv_buf.txheader |= (txAdd <<
                                      RX_ADD_SHIFT & RX_ADD_MASK);

                                  /* AdvA, for SCAN REQ, it should identical
                                   * to the ADV_IND/ADV_SCAN_IND
                                   */
                                  ...
                                  g_tx_adv_buf.data[9] =
                                      g_rx_adv_buf.data[3];
                                  g_tx_adv_buf.data[10] =
                                      g_rx_adv_buf.data[4];
                                  g_tx_adv_buf.data[11] =
                                      g_rx_adv_buf.data[5];

								  /* write Tx FIFO */

                                  ll_hw_write_tfifo((uint8 *)
                                      &(g_tx_adv_buf.txheader),
                                      ((g_tx_adv_buf.txheader & 0xff00) >> 8)
                                      + 2);   /* payload length + header length(2) */
          ...
          if (!bSendingScanReq)
            {
              ...
              llScanTime += ((ISR_entry_time > llScanT1) ?
                  (ISR_entry_time - llScanT1) : (BASE_TIME_UNITS - llScanT1
                  + ISR_entry_time));
              ...
              if (llScanTime >= scanInfo.scanWindow * 625)
                {
                  ...
                  if (scanInfo.scanWindow == scanInfo.scanInterval)      /* scanWindow == scanInterval, trigger immediately */
                    ...
                    {
                      /* set_timer4((scanInfo.scanInterval -
                       *     scanInfo.scanWindow) * 625);
                       */

                      ll_schedule_next_event((scanInfo.scanInterval -
                          scanInfo.scanWindow) * 625);
                    }
              ...
              else
                {
                    ...
                    *    *(volatile uint32_t *)(0x40031054),ll_hw_get_anchor()
                    *    ,g_rfifo_rst_cnt,(uint32_t)ISR_entry_time,
      ...
      else if (llState == LL_STATE_INIT)  /* initiator case */
        {
          uint8 bConnecting = FALSE;
          uint8 bMatchAdv = FALSE;
          /* RPA checking OK in previous adv event, and new adv event
          ...
          if ((irq_status & LIRQ_RD) && (irq_status & LIRQ_COK))   /* bug correct 2018-10-15 */
            {
              ...
              packet_len = ll_hw_read_rfifo((uint8_t *)
                  (&(g_rx_adv_buf.rxheader)), &pktLen, &pktFoot0, &pktFoot1);
              ...
              if (packet_len != 0
                ...
                {
                  uint8_t txAdd = (g_rx_adv_buf.rxheader & TX_ADD_MASK)
                      >> TX_ADD_SHIFT;                                  /* adv PDU header, bit 6: TxAdd, 0 - public, 1 - random */

                  uint8_t chSel = (g_rx_adv_buf.rxheader & CHSEL_MASK)
                      >> CHSEL_SHIFT;
                  ...
                  if (txAdd == LL_DEV_ADDR_TYPE_RANDOM  &&
                      ...
                      PRIVATE_RESOLVE_ADDR_HDR))
                    {
                      ...
                      if (g_llRlEnable == TRUE)
                        {
                          /* if the RPA checking is done
                           * in previous scan, compare
                           */
                          ...
                          if (isPeerRpaStore  == TRUE  &&
                            ...
                            {
                                rpaListIndex =
                                    ll_getRPAListEntry(&g_rx_adv_buf.data[0]);    /* spend 30us(48MHz) when the 1st item match */

                              if (rpaListIndex < LL_RESOLVINGLIST_ENTRY_NUM)
                                {
                                  peerAddr = &g_llResolvinglist[rpaListIndex].peerAddr[0];
                                  g_currentPeerAddrType =
                                      g_llResolvinglist[rpaListIndex].peerAddrType + 2;
                    ...
                    {
                      /* case 2: receive InitA using device ID,
                       * or init device not using RPA
                       */
                      ...
                      for (int i = 0; i < LL_RESOLVINGLIST_ENTRY_NUM; i++)
                        {
                          if (g_llResolvinglist[i].peerAddr[0] ==
                              ...
                              g_rx_adv_buf.data[5])
                            {
                              ...
                              if (g_llResolvinglist[i].privacyMode ==
                                  NETWORK_PRIVACY_MODE &&
                                  !ll_isIrkAllZero(
                                  g_llResolvinglist[i].peerIrk))
                                {
                                  bWlRlCheckOk = FALSE;
                                }
                              else
                  ...
                  if (pdu_type == ADV_DIRECT_IND && bWlRlCheckOk == TRUE &&
                      bMatchAdv != TRUE)
                    {
                      /* 20201228,Jie,add RXADD check for direct IND */

                      uint8_t rxAdd = (g_rx_adv_buf.rxheader & RX_ADD_MASK)
                          >> RX_ADD_SHIFT;

                      /* initA is resolvable address case */

                      if (rxAdd == LL_DEV_ADDR_TYPE_RANDOM  &&
                          ((g_rx_adv_buf.data[11] & RANDOM_ADDR_HDR) ==
                          PRIVATE_RESOLVE_ADDR_HDR))
                        {
                          /* should not use RPA case */

                          if (initInfo.ownAddrType !=
                              LL_DEV_ADDR_TYPE_RPA_PUBLIC &&
                              initInfo.ownAddrType !=
                              LL_DEV_ADDR_TYPE_RPA_RANDOM)
                            {
                              bWlRlCheckOk = FALSE;
                            }
                          ...
                          if (rpaListIndex >= LL_RESOLVINGLIST_ENTRY_NUM ||
                              ...
                              || (ll_ResolveRandomAddrs(g_llResolvinglist[rpaListIndex].localIrk,
                              &g_rx_adv_buf.data[6]) != SUCCESS))                            /* resolve failed */
                            {
                              bWlRlCheckOk = FALSE;
                            }
                        }
                  ...
                  /* initiator, 2 types of filter process:
                  ...
                  if (bWlRlCheckOk == TRUE)
                    {
                      ...
                      if ((pGlobal_config[LL_SWITCH] & CONN_CSA2_ALLOW)
                        ...
                        {
                          conn_param[initInfo.connId].channel_selection =
                              LL_CHN_SEL_ALGORITHM_2;

                          SET_BITS(g_tx_adv_buf.txheader,
                              LL_CHN_SEL_ALGORITHM_2, CHSEL_SHIFT,
                              CHSEL_MASK);
                        }
                      else
                        {
                            conn_param[initInfo.connId].channel_selection =
                                LL_CHN_SEL_ALGORITHM_1;

                            SET_BITS(g_tx_adv_buf.txheader,
                                LL_CHN_SEL_ALGORITHM_1, CHSEL_SHIFT,
                                CHSEL_MASK);
                        }

                      /* calculate initA if using RPA list,
                       * otherwise copy the address stored in initInfo
                       */
                      ...
                      if (rpaListIndex < LL_RESOLVINGLIST_ENTRY_NUM &&
						  ...
						  || initInfo.ownAddrType == LL_DEV_ADDR_TYPE_RPA_RANDOM))
                        {
						  ...
						  SET_BITS(g_tx_adv_buf.txheader,
                              LL_DEV_ADDR_TYPE_RANDOM, TX_ADD_SHIFT,
                              TX_ADD_MASK);

                          /* osal_memcpy( &g_currentLocalRpa[0],
                           *     &g_tx_adv_buf.data[0], 6);
                           */

                          g_currentLocalAddrType =
                              LL_DEV_ADDR_TYPE_RPA_RANDOM;
                        }
                      else
                        {
                          if (initInfo.ownAddrType == LL_DEV_ADDR_TYPE_PUBLIC
                              ...
                              LL_DEV_ADDR_TYPE_RPA_PUBLIC)
                            {
                              osal_memcpy((uint8 *)&g_tx_adv_buf.data[0],
                                  &ownPublicAddr[0], 6);
                      ...
                      T2 = read_current_fine_time();
                      delay = (T2 > ISR_entry_time) ? (T2 - ISR_entry_time) :
                          (BASE_TIME_UNITS - ISR_entry_time + T2);

                      if (delay > 118 -
                          pGlobal_config[LL_ADV_TO_CONN_REQ_DELAY] -
                          pGlobal_config[LL_HW_PLL_DELAY])            /* not enough time */
                        {
                          /* not enough time to send conn req,store the RPA */

                          isPeerRpaStore = TRUE;
                          storeRpaListIndex = rpaListIndex;
                          osal_memcpy(&currentPeerRpa[0],
                              &g_rx_adv_buf.data[0], 6);
                      ...
                      else
                        {
						  ...
						  osal_memcpy((uint8 *)&g_tx_adv_buf.data[6],
                              &g_rx_adv_buf.data[0], 6);

						  /* 2020.8.11 Jie:add init req header for RxAdd */

						  SET_BITS(g_tx_adv_buf.txheader, txAdd,
                              RX_ADD_SHIFT, RX_ADD_MASK);
                          ...
                          /* payload length + header length(2) */

                          if (g_currentPeerAddrType >= 0x02)
                            {
                              osal_memcpy(&g_currentPeerRpa[0],
                                  &g_rx_adv_buf.data[0], 6);
						    }

                          if (g_currentLocalAddrType ==
                              LL_DEV_ADDR_TYPE_RPA_RANDOM)
                            {
                              osal_memcpy(&g_currentLocalRpa[0],
                                  &g_tx_adv_buf.data[0], 6);
		  ...
		  if (!bConnecting)           /* if not waiting for scan rsp, schedule next scan */
            {
              if (initInfo.scanMode == LL_SCAN_STOP)
              ...
              else
                {
                  ...
                  llScanTime += ((ISR_entry_time > llScanT1) ?
                    (ISR_entry_time - llScanT1) :
                    (BASE_TIME_UNITS - llScanT1 + ISR_entry_time));
                  ...
                  if (llScanTime >= initInfo.scanWindow * 625)
                    {
                      ...
                      if (initInfo.scanWindow == initInfo.scanInterval)      /* scanWindow == scanInterval, trigger immediately */
                        ...
                        {
                          /* set_timer4((initInfo.scanInterval -
                           * initInfo.scanWindow) * 625);
                           */

                          ll_schedule_next_event((initInfo.scanInterval -
                              initInfo.scanWindow) * 625);
                        }
...
uint8 llSetupStartEncRsp(llConnState_t *connPtr)
{
  ...
  *pBuf = LL_CTRL_START_ENC_RSP;
  ...
  /* control procedure timeout value only needed for
   *  Master after Start Enc Response
   */
	  ...
	  switch (connPtr->ctrlPktInfo.ctrlPkts[0])
        {
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* we have already place packet on TX FIFO,
               * so check if its been ACK'ed
               */

              if (rfCounters.numTxCtrlAck)
                {
                  /* yes, so process the termination
                   * Note: No need to cleanup control packet info
                   *       as we are done.
                   */
                ...
                {
                  /* check if a termination control procedure timeout
                   * has occurred
                   */
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
               ...
               *
               *       In case (b), the control packet stays at the head of
               *       the queue, and there's nothing more to do. Consequently
               *       , return.
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
			  ...
			  /* Note: This does not mean this packet has been
               * ACK'ed or NACK'ed.
               */

              if (rfCounters.numTxCtrl)
                {
                  /*  done with this control packet, so remove from the
                   *  processing queue
                   *  Note: By dequeueing here, it is possible to get another
                   *       control packet at the head of the queue. This is
                   *       techincally not supposed to happen if the spec is
                   *       followed.
                   *  ALT: COULD MAKE MORE BULLET PROOF. SINCE THE REPLACE
                   *      ROUTINE CAN'T BE USED UNTIL THE LTK IS RECEIVED BY
                   *      THE HOST, A DUMMY CONTROL PACKET THAT SITS AT THE
                   *      HEAD UNTIL IT IS REPLACE COULD BE USED INSTEAD.
                   ...
                   * Note: Need to wait for the Host reply to determine if
                   *       the LTK is available or not.
                   */

                  LL_EncLtkReqCback(connPtr->connId,
                      connPtr->encInfo.RAND, connPtr->encInfo.EDIV);
                }
              else /* not done yet */
                {
                  /* check if a update param req control procedure timeout
                   * has occurred
                   * Note: No need to cleanup control packet info
                   *       as we are done.
                   */
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* yes, so check if it has been transmitted yet
               * Note: This only means the packet has been transmitted,
               *       not that it has been ACK'ed or NACK'ed.
               */
              ...
              /* not done until the LL_CTRL_START_ENC_RSP is received, so
               * check it
               * Note: The following code can not be in the previous "if"
               *       statement since it is possible that numTxCtrl could
               *       be true, yet the flag startEncRspRcved isn't. Then on
               *       the next event, numTxCtrl wouldn't be true, and we
               *       would never check the startEncRspRcved flag again.
               *       Since we can't get the LL_START_ENC_RSP until we send
               *       the LL_CTRL_START_ENC_REQ, this isn't an issue.
               */

              if (connPtr->encInfo.startEncRspRcved == TRUE)
                {
                  /* replace control procedure at head of queue to prevent
                   * interleaving
                   */
                ...
                {
                  /* check if a start enc req control procedure timeout has
                   * occurred
                   * Note: No need to cleanup control packet info
                   *       as we are done.
                   */
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
              ...
              if (connPtr->encInfo.SKValid == TRUE)
                {
                  /* so try to begin the last step of
                   * the encryption procedure
                   */

                  if (llSetupStartEncReq(connPtr) == TRUE)
                    {
                      /* ready the flag that indicates
                       * that we've received the response
                       */
                   ...
                   *
                   *    In case (b), the control packet stays at the head of
                   *    the queue, and there's nothing more to do.
                   *    Consequently, return.
                   *    So, in either case, return.
                   */
                  ...
                  if (connPtr->encInfo.LTKValid)
                    {
                       ...
                       * (i.e. SK = AES128(LTK, SKD))
                       */

                      LL_ENC_GenerateSK(connPtr->encInfo.LTK,
                          connPtr->encInfo.SKD, connPtr->encInfo.SK);
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              if (rfCounters.numTxCtrl)
                {
                  /* packet TX'ed, so we are done with the encryption
                   * procedure re-activate slave latency
                   */

                  connPtr->slaveLatency = connPtr->slaveLatencyValue;

				  /* remove control packet from processing queue
                   * and drop through
                   */
                  ...
                  if (connPtr->encInfo.encRestart == TRUE)
                  ...
                  else
                    {
                      ...
                      LL_EncChangeCback(connPtr->connId,
                          LL_ENC_KEY_REQ_ACCEPTED, LL_ENCRYPTION_ON);
                    }

                  /* clear the restart flag in case of another key change
                   * request, and all other encryption flags
                   * Note: But in reality, there isn't a disable encryption
                   *       in BLE, so once encryption is enabled, any call
                   *       to LL_StartEncrypt will result in an encryption
                   *       key change callback.
                   */
                ...
                {
                  /* check if a update param req control procedure timeout has
                   * occurred
                   * Note: No need to cleanup control packet info
                   *       as we are done.
				   */
                  ...
                  if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
                    ...
                    {
                      /* control packet stays at head of queue,
                       * so exit here
                       */
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* not done until the LL_CTRL_PAUSE_ENC_RSP is received,
               * so check it
               */
              ...
              if (connPtr->encInfo.pauseEncRspRcved == TRUE)
                {
                  /* done with this control packet, so remove from the
                   * processing queue and drop through (so the encrypton
                   * response can be processed)
                   * ALT: COULD REPLACE HEAD OF QUEUE WITH DUMMY SO NO OTHER
                   *      CONTROL PROCEDURE CAN INTERLEAVE BEFORE THE ENC_REQ
                   *      IS RECEIVED.
                   */
                ...
                {
                  /* not received yet, so decrement and check control
                   * procedure timeout
                   * check if a start enc req control procedure timeout has
                   * occurred
                   * Note: No need to cleanup control packet info
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
               ...
               * Note: All pending transmissions must also be finished before
               *       this packet is placed in the TX FIFO.
               */

              if (llSetupPauseEncRsp(connPtr) == TRUE)
                {
                  /* clear the flag that indicates an Encryption Request has
                   * been received, which is used by this control procedure
                   * to restart the control procedure timeout
                   */

                  connPtr->encInfo.pauseEncRspRcved = FALSE;

                  /* disable encryption
                   * Note: Not really necessary as no data is supposed
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* yes, so check if it has been transmitted yet
               * Note: This only means the packet has been transmitted, not
               *       that it has been ACK'ed or NACK'ed.
               * Note: The control procedure does not end until the Reject is
               *       ACKed. However, if the ACK is a data packet, it will be
               *       tossed unless data is allowed hereafter. So to avoid
               *       this, only the confirmed transmission of this will be
               *       used to qualify the related flags, but a new procedure
               *       will not be able to begin until this procedure
               *       completes, per the spec.
               */
              ...
              /* we have already place packet on TX FIFO, so check
               * if its been ACK'ed
               */
              ...
              if (rfCounters.numTxCtrlAck)
                {
                  /* done with this control packet, so remove from the
                   * processing queue and drop through
                   */
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
              ...
              connPtr->ctrlPktInfo.ctrlPktActive = llSetupRejectInd(connPtr,
                  connPtr->encInfo.encRejectErrCode);
 ...
 *        # for v4.2, slave may send LL_CTRL_FEATURE_REQ msg.
 *        # to be test later.........  HZF
 *        # check if the control packet procedure is active
 *        if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
 *        {
 ...
 *                connPtr->connId, connPtr->featureSetInfo.featureSet);
 *
 ...
 *          {
 *            # check if a update param req control procedure timeout has
 *            # occurred Note: No need to cleanup control packet info
 *            # as we are done.
 *           if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
 *            {
 ...
 *                  LL_CTRL_PKT_TIMEOUT_TERM, connPtr->connId,
 ...
 *          #       queue, and there's nothing more to do. Consequently,
 *          #       return.
 *          #       So, in either case, return.
 *          return( LL_CTRL_PROC_STATUS_SUCCESS );
 *        }
 ...
 */
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              if (rfCounters.numTxCtrl)
                {
                  /* packet TX'ed, so use this flag on the Slave to indicate
                   * that the feature response procedure has already taken
                   * place on this connection
                   * Note: This is being done to support the HCI extension
                   *       command LL_EXT_SetLocalSupportedFeatures so that
                   *       the user can update the local supported features
                   *       even after a connection is formed. This update will
                   *       be used as long as a feature response feature has
                   *       not been performed by the Master. Once
                  ...
                  /* remove control packet from processing queue and
                   * drop through
                   */
              ...
              else /* not done yet */
                {
                  /* check if a start enc req control procedure timeout has
                   * occurred
                   * Note: No need to cleanup control packet info as we are
                   *       done.
                   */
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              if (connPtr->verExchange.peerInfoValid == TRUE)
                {
                  /* yes, so check if the host has requested
                   * this information
                   */
                  ...
                  if (connPtr->verExchange.hostRequest == TRUE)
                    {
                      ...
                      LL_ReadRemoteVersionInfoCback(LL_STATUS_SUCCESS,
                          connPtr->connId, connPtr->verInfo.verNum,
                          connPtr->verInfo.comId, connPtr->verInfo.subverNum);
                    }
              ...
              else /* no done yet */
                {
				  ...
				  if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
                    {
                      /* we're done waiting, so complete the callback
                       * with error
                       */
                      ...
                      LL_ReadRemoteVersionInfoCback(LL_CTRL_PKT_TIMEOUT_TERM,
                          ...
                          connPtr->verInfo.comId, connPtr->verInfo.subverNum);
                      /* and end it al
                       * Note: No need to cleanup control packet info as
                       *       we are done.
                       */
            ...
            {
              /* since we are in the process of sending the version
               * indication, it is okay to set this flag here even if it
               * is set repeatedly in the of llSetupVersionIndReq failures
               */
              ...
              if (rfCounters.numTxCtrl)
                {
				  ...
				  /* remove control packet from processing queue and drop
                   * through
                   */
              ...
              if (rfCounters.numTxCtrl)
              ...
              else /* not done yet */
                {
                  ...
                  if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
                    {
					  /* we're done waiting, so end it all
                       * Note: No need to cleanup control packet info as we
                       *       are done.
                       */
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              if (rfCounters.numTxCtrl)
              ...
              else /* not done yet */
                {
                  if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
                    {
                      osal_memset(&(connPtr->llCTEModeCtrl), 0,
                          sizeof(connPtr->llCTEModeCtrl));
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
               ...
               */
				...
				if (rfCounters.numTxCtrl)
                {
				  ...
				  /* remove control packet from processing queue and drop
                   * through
                   * 2020-02-12 comment:after send CONN CTE RSP , then clear
                   * txSupp
                   */
          ...
          case LL_CTRL_UNKNOWN_RSP:
               ...
               * Note: Since there are no dependencies for this control packet
               *       we do not have to bother with the active flag.
               */

          if (llSetupUnknownRsp(connPtr) == TRUE)
            {
              /* all we have to do is put this control packet on the TX FIFO,
               * so remove control packet from the processing queue and drop
               * through
               */
  ...
  while (connPtr->ctrlPktInfo.ctrlPktCount > 0)
    {
      ...
      switch (connPtr->ctrlPktInfo.ctrlPkts[0])
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              if (rfCounters.numTxCtrlAck)
                {
                  /* done with this control packet, so remove from
                ...
                {
                  /* check if a termination control procedure timeout has
                   *  occurred
                   * Note: No need to cleanup control packet info
                   *       as we are done.
                   */
        ...
        /* LOG("CONN UPD"); */
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* we have already placed a packet on TX FIFO,
               * so check if its been ACK'ed
               */
			  ...
			  if (rfCounters.numTxCtrlAck)
                {
				  ...
				  /* done with this control packet,
                   * so remove from the processing queue
                   */
                ...
                {
                  /* Core Spec V4.0 now indicates there is no control
                   * procedure timeout. However, it still seems prudent to
                   * monitor for the instant while waiting for the slave's ACK
                   */

                  if (connPtr->nextEvent == connPtr->paramUpdateEvent)
                    {
                      /* this event is the instant, and the control procedure
                       * still has not been ACK'ed, we the instant has passed
                       * Note: No need to cleanup control packet info as we
                       * are done.
                       */

                      llConnTerminate(connPtr,
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              /* we have already placed a packet on TX FIFO, so wait
               * now until we get the slave's LL_START_ENC_REQ
               */
              ...
              if (connPtr->encInfo.startEncReqRcved == TRUE)
                ...
                {
                  /* the slave's Host has failed to provide an LTK, so the
                   * encryption setup has been rejected; end the start
                   * encryption procedure done with this control packet,
                   * so remove from the processing queue
                   */

				  llDequeueCtrlPkt(connPtr);
                  /* disable encryption
                   * Note: Not really necessary as no data is supposed to
                   *       be sent or received.
                   */
                  ...
                  if (connPtr->encInfo.encRejectErrCode ==
                  ...
                  if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
                    {
                      ...
                      if (connPtr->encInfo.encRestart == TRUE)
                      ...
                      else
                        {
                          ...
                          LL_EncChangeCback(connPtr->connId,
                              LL_CTRL_PKT_TIMEOUT_TERM, LL_ENCRYPTION_OFF);
                        }
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
               ...
               * Note: The LL_ENC_RSP will be received first, which will
               *       result in the master calculating its IVm and SKDm,
               *       concatenating it with the slave's IVs and SKDs, and
               *       calculating the SK from the LTK and SKD. After that,
               *       we will receive the LL_START_ENC_REQ from the slave.
               *       So, it is okay to stay in this control procedure until
               *       LL_START_ENC_REQ is received.
               * Note: It is okay to repeatedly set this flag in the event the
               *       setup routine hasn't completed yet (e.g. if the TX FIFO
               *       has not yet become empty).
               */
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
                /* we have already placed a packet on TX FIFO, so wait now
                 * until we get the slave's LL_START_ENC_RSP
                 */
            ...
            {
			  /* so try to put it there; being active depends on a success
               * Note: The llSetupStartEncRsp routine will *not* reset the
               *       control timeout value since the entire encryption
               *       procedure starts with the master sending the
               *       LL_ENC_REQ, and ends when the master receives the
               *       LL_START_ENC_RSP from the slave.
               */
         ...
         /* Encryption Pause Request */
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* we have already placed a packet on TX FIFO, so wait now
               * until we get the slave's LL_PAUSE_ENC_RSP
			   */
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* we have already placed a packet on TX FIFO, so wait now
               * until we get the slave's LL_CTRL_FEATURE_RSP
               */
              ...
              if (rfCounters.numTxCtrl)
                {
                   ...
                   *       the connection feature set is fixed!
                   */
                ...
                {
                    /* check if a start enc req control procedure timeout
                     * has occurred
                     * Note: No need to cleanup control packet info
                     *       as we are done.
                     */
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
			  ...
			  /* connPtr->ctrlPktInfo.ctrlPktActive = llSetupPingReq(connPtr);
			   * llSetupVersionIndReq( connPtr );
               */
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              /* yes, so check if it has been transmitted yet
               * Note: This does not mean this packet has
               * been ACK'ed or NACK'ed.
               */
            ...
            {
              /* so try to put it there; being active depends on a success
               * Note: There is no control procedure timeout associated
		  ...
		  if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
            {
              ...
              if (rfCounters.numTxCtrlAck)
				  ...
				  if (connPtr->phyUpdateInfo.m2sPhy == 0
                      && connPtr->phyUpdateInfo.s2mPhy == 0)
                    {
                      connPtr->phyUpdateInfo.m2sPhy =
                          connPtr->llPhyModeCtrl.local.txPhy;
                      connPtr->phyUpdateInfo.s2mPhy =
                          connPtr->llPhyModeCtrl.local.rxPhy;
					  llPhyModeCtrlUpdateNotify(connPtr, LL_STATUS_SUCCESS);
                    }
                  else
                    {
                ...
                {
                  /* Core Spec V4.0 now indicates there is no control
                   * procedure timeout. However, it still seems prudent
                   * to monitor for the instant while waiting for the
                   * slave's ACK.
                   */
          ...
          if (connPtr->ctrlPktInfo.ctrlPktActive == TRUE)
              ...
              if (rfCounters.numTxCtrl)
              ...
              else /* not done yet */
                {
                  ...
                  if (--connPtr->ctrlPktInfo.ctrlTimeout == 0)
                    {
                      /* we're done waiting, so end it all
                       * Note: No need to cleanup control packet
                       *       info as we are done.
                       */
          ...
          else /* control packet has not been put on the TX FIFO yet */
            {
              if (connPtr->llPhyModeCtrl.isWatingRsp == TRUE)
                {
                  connPtr->ctrlPktInfo.ctrlPktActive =
                      llSetupRejectExtInd(connPtr,
                      LL_STATUS_ERROR_LL_PROCEDURE_COLLISION);
                }
                        ...
                        connPtr->pendingParamUpdate == TRUE)
                {
                    connPtr->ctrlPktInfo.ctrlPktActive =
                        llSetupRejectExtInd(connPtr,
                        LL_STATUS_ERROR_DIFF_TRANSACTION_COLLISION);
                }
              else if (connPtr->llCTEModeCtrl.isWatingRsp == TRUE)
                {
                  ...
                  connPtr->ctrlPktInfo.ctrlPktActive =
                      llSetupRejectExtInd(connPtr,
                      connPtr->llCTEModeCtrl.errorCode);
           ...
           *       control packets are sent by this role. This is a special
           *       case where a terminate indication was received, but we must
           *       as a master wait for our ACK to be sent before terminating.
           */
...
{
  /* first, since this was a failure,
   * clear the number of consecutive successes
   */
...
{
  /* first, since this is a success,
   * clear the number of consecutive failures
   */
...
static void llGenerateNextBackoffCount1(void)
{
  ...
  if (scanInfo.scanBackoffUL == 1)
    ...
    {
      scanInfo.currentBackoff = ((uint16)LL_ENC_GeneratePseudoRandNum()
          % scanInfo.scanBackoffUL) + 1;
    }
...
uint8 ll_processBasicIRQ_ScanTRX0(uint32_t irq_status)
{
  ...
  llScanTime += ((ISR_entry_time > llScanT1) ? (ISR_entry_time - llScanT1)
      : (BASE_TIME_UNITS - llScanT1 + ISR_entry_time));
      ...
      if (packet_len > 0 && pdu_type == ADV_SCAN_RSP)
        {
          ...
          uint8 rpaListIndex;
          uint8 *peerAddr;
          uint8 addrType = (g_rx_adv_buf.rxheader & TX_ADD_MASK)
              >> TX_ADD_SHIFT;
          ...
          /* AdvA of SCAN_RSP should also be checked here. Refer to 4.4.3.2
           * Active Scanning After sending a scan request PDU the Link Layer
           * listens for a scan response PDU from that advertiser. If the
           * scan response PDU was not received from that advertiser,
           * it is considered a failure; otherwise it is considered a success.
           */
          ...
          /* RPA checking. Note that we do not check whether
           * it is the same RPA index
           */
                  ...
                  if (rpaListIndex < LL_RESOLVINGLIST_ENTRY_NUM)
                    {
                      ...
                      addrType = g_llResolvinglist[rpaListIndex].peerAddrType
                          + 2;
                      bCheckOk = TRUE;
                    }
                  else
...
uint8 ll_processBasicIRQ_secondaryAdvTRX0(uint32_t irq_status)
{
   ...
   * bug-case :
   * a device in advertising and receive another device's scan request
   */
  ...
  if (packet_len > 0                                                 /* any better checking rule for rx anything? */
      && (irq_status & LIRQ_COK)
      && pdu_type == ADV_SCAN_REQ)                                   /* && (llState == LL_STATE_ADV_UNDIRECTED || llState == LL_STATE_ADV_SCAN)) */
    {
      /* 1. scan req */
  ...
  else if (pdu_type == ADV_CONN_REQ && (irq_status & LIRQ_COK))     /* &&(llState==LL_STATE_ADV_UNDIRECTED||llState==LL_STATE_ADV_DIRECTED)) */
    {
      uint8_t *peerAddr;
      uint8_t bWlRlCheckOk = TRUE;
      ...
      if (g_rx_adv_buf.data[6] != adv_param.ownAddr[0]
      ...
      else
        {
          ...
          if (txAdd == LL_DEV_ADDR_TYPE_RANDOM &&
                  ...
                  if (rpaListIndex < LL_RESOLVINGLIST_ENTRY_NUM)
                    {
                      ...
                      /* if resolved address success, map the peer address
                       * type to 0x02 or 0x03
                       */

                      g_currentPeerAddrType =
                          g_llResolvinglist[rpaListIndex].peerAddrType + 2;
                      osal_memcpy(&g_currentPeerRpa[0],
                          &g_rx_adv_buf.data[0], 6);   /* save latest peer RPA */
                      bWlRlCheckOk = TRUE;
                    }
                }
            }
            ...
            {
              /* in the RPA list and network privacy mode is selected
               * and non all-0 IRK, check failed
               */
          ...
          /* fixed bug 2018-09-25, LL/CON/ADV/BV-04-C,
           * for direct adv, initA should equal peer Addr
           */

          if (llState == LL_STATE_ADV_DIRECTED)
            {
              /* txAdd != peerInfo.peerAddrType # for (extended) set adv param
               * peer addr type could only be 0x0 or 0x01
               */
  ...
  if (adv_sch_flag)
    {
      ...
      /* adv_param.advNextChan stores the next adv channel, when adv the last
       * adv channel, advNextChan should equal 1st adv channel
       */
...
void LL_IRQHandler1(void)
{
  ...
  if (llTaskState == LL_TASK_EXTENDED_ADV)
  ...
  else
    {
      ...
      if (mode == LL_HW_MODE_SRX && (llState == LL_STATE_SCAN || llState
          == LL_STATE_INIT))
        {
          ret = ll_processBasicIRQ_SRX(irq_status);
        }
      else if ((llSecondaryState == LL_SEC_STATE_ADV || llSecondaryState
          == LL_SEC_STATE_IDLE_PENDING) && (mode == LL_HW_MODE_TRX) &&
      ...
      if (mode == LL_HW_MODE_TRX
        ...
        {
            uint8_t firstAdvChan = (adv_param.advChanMap&LL_ADV_CHAN_37)
                                   != 0 ? 37 :
                                   (adv_param.advChanMap&LL_ADV_CHAN_38)
                                   != 0 ? 38 : 39;
  ...
  /* conn-adv case 2: other ISR, there is pending secondary advertise event,
   * make it happen
   */
  ...
  if (llSecondaryState == LL_SEC_STATE_ADV_PENDING)
    ...
    {
      /* there is pending scan event, make it happen, note that
       * it may stay pending if there is no enough idle time
       */
    ...
    {
      /* there is pending init event, make it happen, note that
       * it may stay pending if there is no enough idle time
       */
 ...
 *              after old connection task end, it will not add new task but
 *              may delete exist task
 *
 *    input parameters
 *
...
void ll_scheduler1(uint32 time)
{
  ...
  /* if timer1 is not running, calculate the time elapse
   * since last timer expiry
   */

  delta = g_ll_conn_ctx.current_timer +
      LL_TIME_DELTA(g_ll_conn_ctx.timerExpiryTick, T1)
      + pGlobal_config[TIMER_ISR_ENTRY_TIME];
  ...
  if (next != LL_INVALID_CONNECTION_ID)
    {
      /* if we want master or slave connection has higher schedule priority,
       * set LL_MASTER_PREEMPHASIS/LL_SLAVE_PREEMPHASIS
       */

      if (g_ll_conn_ctx.scheduleInfo[next].linkRole == LL_ROLE_MASTER)
        {
          min = (time > pGlobal_config[LL_MULTICONN_MASTER_PREEMP])
              ? (time - pGlobal_config[LL_MULTICONN_MASTER_PREEMP]) : 0;
        }
      if (g_ll_conn_ctx.scheduleInfo[next].linkRole == LL_ROLE_SLAVE)
        {
          min = (time > pGlobal_config[LL_MULTICONN_SLAVE_PREEMP])
              ? (time - pGlobal_config[LL_MULTICONN_SLAVE_PREEMP]) : 0;
        }
    }
      ...
      if ((i != g_ll_conn_ctx.currentConn) && conn_param[i].active)
        {
           ...
           *   if there is no enough time for new task, invoke relate
           *       slave/master conn event process function
           *   if (g_ll_conn_ctx.scheduleInfo[i].remainder < delta
           *       + g_ll_conn_ctx.scheduleInfo[i].task_duration)
           */

          if (g_ll_conn_ctx.scheduleInfo[i].remainder < delta + 40)     /* 40 : margin for process delay, unit: us */
            {
              /* no enough time to process the event, regard the event
               * as missed and update the conn context and timer
               */
              ...
              if (g_ll_conn_ctx.scheduleInfo[i].linkRole == LL_ROLE_MASTER)
                {
                  /* temporary update g_ll_conn_ctx.currentConn to current
                   * connection ID because ll_processMissMasterEvt will
                   * invoke function using global variable
                   * g_ll_conn_ctx.currentConn
                   */
                   ...
                   *      uint8 missCE = (( delta -
                   *          g_ll_conn_ctx.scheduleInfo[i].remainder)
                   *          / ( connPtr->curParam.connInterval*625 )) + 1;
              ...
              else if (g_ll_conn_ctx.scheduleInfo[i].linkRole
                  == LL_ROLE_SLAVE)
                {
                  /* temporary update g_ll_conn_ctx.currentConn to current
                   * connection ID because ll_processMissSlaveEvt will
                   * invoke function using global variable
                   * g_ll_conn_ctx.currentConn
                   */
              ...
              switch (g_ll_conn_ctx.scheduleInfo[i].priority)
                {
                  ...
                  case LL_SCH_PRIO_MED:
                      prio_adj = MAX(LL_TASK_MASTER_DURATION,
                          LL_TASK_SLAVE_DURATION) + 2000;
                      break;

                  case LL_SCH_PRIO_HIGH:
                      prio_adj = MAX(LL_TASK_MASTER_DURATION,
                          LL_TASK_SLAVE_DURATION) + 10 + 2000;
                      break;

                  case LL_SCH_PRIO_IMMED:
                      prio_adj = MAX(LL_TASK_MASTER_DURATION,
                          LL_TASK_SLAVE_DURATION) + 20 + 2000;
                      break;
          ...
          if (g_ll_conn_ctx.scheduleInfo[i].remainder < min + prio_adj)
            {
              next = i;
              min = (g_ll_conn_ctx.scheduleInfo[i].remainder > prio_adj)
                  ? (g_ll_conn_ctx.scheduleInfo[i].remainder - prio_adj) : 0;
            }
        }
    }
  ...
  if (g_ll_conn_ctx.scheduleInfo[next].remainder <= delta)          /* TODO: should not go here, if this issue detected, root cause should be invest */
  ...
  else
    {
      ...
      /* logx("-S%d,%d\n",next,g_ll_conn_ctx.scheduleInfo[next].remainder
       * - delta);
       */

      /* update connection context & schedule info */

      g_ll_conn_ctx.current_timer = g_ll_conn_ctx.scheduleInfo[next].remainder
          - delta;
    }
  ...
  if (g_ll_conn_ctx.scheduleInfo[g_ll_conn_ctx.currentConn].linkRole
      == LL_ROLE_SLAVE)
    {
      llState = LL_STATE_CONN_SLAVE;
    }
  else if (g_ll_conn_ctx.scheduleInfo[g_ll_conn_ctx.currentConn].linkRole
      == LL_ROLE_MASTER)
    {
  ...
  g_ll_conn_ctx.scheduleInfo[g_ll_conn_ctx.currentConn].priority
      = LL_SCH_PRIO_LOW;
  ...
  for (k = g_maxConnNum - 1; k >= 0; k--)
    {
      if ((conn_param[k].active) &&
          (g_ll_conn_ctx.scheduleInfo[k].linkRole == LL_ROLE_MASTER))
        {
          break;
        }
    }
  ...
  if (conn_temp == i)
    {
      ...
      uint32 tv_Masters = 0;
      uint32 tv_diff = 0;
      uint32 first_reminder = 0;

      for (i = 0; i < g_maxConnNum; i++)
        {
          if ((conn_param[i].active) &&
              (g_ll_conn_ctx.scheduleInfo[i].linkRole == LL_ROLE_MASTER))
              break;
        }
      ...
      for (i = fist_m + 1; i < jm + 1; i++)
        {
          if ((conn_param[i].active) &&
              (g_ll_conn_ctx.scheduleInfo[i].linkRole == LL_ROLE_MASTER))
            {
              tv_Masters =  first_reminder + g_ll_conn_ctx.per_slot_time
                  * 625 * (i - fist_m);
              ...
              if (tv_Masters > g_ll_conn_ctx.scheduleInfo[i].remainder)
                ...
                {
                  tv_diff = g_ll_conn_ctx.scheduleInfo[i].remainder
                      - tv_Masters;
                }
              /* < 1000 : filter scecondary first create master connection
               * & miss process master event
               */
              ...
              if (tv_diff < 1000)
                {
                  ...
                  else if (g_ll_conn_ctx.scheduleInfo[i].remainder <
                      tv_Masters)
                    {
                      g_ll_conn_ctx.scheduleInfo[i].remainder += tv_diff;
                    }
...
#define   TRACKING_96M_16M_MULTI6_DELTA_LIMIT       (10*6)
  ...
  while (1)
    {
      ...
      *(volatile uint32_t *)0x4000f044 = temp | BIT(16);
      ...
      for (uint8 index = 0; index < 5; index++)
        {
          temp = *(volatile uint32_t *)0x4000f05c;
          *(volatile uint32_t *)0x4000f05c = (temp & 0xfffefe00)
              | 0x0028 | BIT(16);
          WaitRTCCount(3);
          temp1 = (*(volatile uint32_t *)0x4000f064 & 0x1ffff);
          subWriteReg(0x4000f05c, 3, 3, 0);

          if ((g_xtal16M_tmp * 6 >= temp1 ? (g_xtal16M_tmp * 6 - temp1):
              (temp1 - g_xtal16M_tmp * 6)) <
              TRACKING_96M_16M_MULTI6_DELTA_LIMIT)
            {
              /* disable 96M */
 ...
 *    1. boot init function: which should be init when system boot.
 *  note: not include wakeup init function
 *    2. wakeup init function: which should be init when wakeup from
 *    system sleep
...
void wakeup_init1(void)
{
   ...
   * enable rng analog block. RNG analog need > 200us before stable,
   * and it consume few current, so open it at wakeup
   * (volatile uint32_t *) 0x4000f048 |= 1 << 23;
   * =========== config PCRM
   * (volatile uint32_t *) 0x4000f040 = 0x501fb000; # enable xtal out
   * (volatile uint32_t *) 0x4000f044 = 0x01ade8b0;
   * # switch rf,adc to doubler,32M
   * ---by ZQ  2017-10-17
   */
   ...
   * (volatile uint32_t *) 0x4000f044 = 0x01bdf8b0;
   *  #0x01bef830;  # switch rf,adc to doubler, dll_off, dll_ldo on
   * dll will be turn on in rf_ini after xtal settle
   * (volatile uint32_t *) 0x4000f044 = 0x00be0830;
   * #[26:22] 0x02,[21:18]0x0f,[16:12]0x00,[7:4]0x03
   * < 22>:sel_rf_clk_16M;
  ...
  /* each rtc coun1t is about 30.5us
  ...
  if (g_system_clk == SYS_CLK_XTAL_16M)
    ...
    {
      uint32_t tracking_c1;
      uint32_t tracking_c2;
      tracking_c1 = rtc_get_counter();
      check_16MXtal_by_rcTracking();
      check_96MXtal_by_rcTracking();
      tracking_c2 = rtc_get_counter();
      tracking_cnt = (tracking_c2 >= tracking_c1)
		  ? (tracking_c2 - tracking_c1) : (0xffffffff - tracking_c1 + tracking_c2);
      pGlobal_config[WAKEUP_ADVANCE] = 1650 + 30 * tracking_cnt;
    }

  /* ============ config BB Top */

  *(volatile uint32_t *) 0x40030000 = 0x3d068001;

  /* set tx pkt =2 */

  *(volatile uint32_t *) 0x400300bc = 0x834;

  /* [7:0] pll_tm [11:8] rxafe settle */

  *(volatile uint32_t *) 0x400300a4 = 0x140;
  ...
  /* ZQ 20180606, reduce rx timeout for power saving */

  ll_hw_set_rx_timeout(88);

  /* preamble + syncword=40us, sync process = 8us
   * timeout should be larger then 48us,
   * ll_hw_set_rx_timeout(268);     #for ble shoulde be larger than 80+128.
   * if sync, the timeout timer stop.
  ...
  /* [16] 16M [8:4] cnt [3] track_en_rc32k */
  ...
  *(volatile uint32_t *) 0x4000f05c = (temp & 0xfffefe00) | 0x0108;
   ...
   * {
   *     WaitRTCCount(17);
   *     uint32_t  counter_tracking_wakeup = *(volatile uint32_t *)
   *         0x4000f064 & 0x1ffff;
...
{
  /* *((volatile uint32_t *)(0xe000e100)) |= INT_BIT_RTC;
   * # remove, we don't use RTC interrupt
   */
  ...
  g_TIM2_IRQ_to_Sleep_DeltTick = (g_TIM2_IRQ_TIM3_CurrCount >
      (AP_TIM3->CurrentCount)) ? (g_TIM2_IRQ_TIM3_CurrCount
      - (AP_TIM3->CurrentCount)) : 0;

  AP_AON->RTCCC0 = sleep_tick + time;  /* set RTC comparatr0 value */

  /*	*(volatile uint32_t *) 0x4000f024 |= 1 << 20;
   *           # enable comparator0 envent
   *	*(volatile uint32_t *) 0x4000f024 |= 1 << 18;
   *           # counter overflow interrupt
   *	*(volatile uint32_t *) 0x4000f024 |= 1 << 15;
   *           # enable comparator0 inerrupt
   */

  /* *(volatile uint32_t *) 0x4000f024 |= 0x148000;
   *   # combine above 3 statement to save MCU time
   */
...
void wakeupProcess1(void)
{
  ...
  /* g_TIM2_wakeup_delay= (AP_TIM2->CurrentCount) + 12;
   * #12 is used to align the rtc_tick
   */
  ...
  if (pGlobal_config[LL_SWITCH] & RC32_TRACKINK_ALLOW)
    {
      /* sleep_total=((current_RTC_tick - sleep_tick)*counter_tracking) >> 7;
       * # shift 4 for 16MHz -> 1MHz, shift 3 for we count 8 RTC tick
    ...
    {
      /* time = tick * 1000 0000 / f (us). f = 32000Hz for RC,
       * f = 32768Hz for crystal. We also calibrate 32KHz RC to 32768Hz
       * sleep_total = ((current_RTC_tick - sleep_tick) *
       *    TIMER_TO_32K_CRYSTAL) >> 2;
  ...
  /* osal time update, not required. It will be updated when
   * osal_run_system() is called after wakeup
   */
  ...
  if (g_llSleepContext.isTimer4RecoverRequired)
    {
      ...
      if (g_llSleepContext.timer4Remainder > sleep_total + wakeup_time)
        {
          next_time = g_llSleepContext.timer4Remainder - sleep_total -
              wakeup_time;
  ...
  /* ==== measure value, from RTC counter meet comparator
   * 0 -> here : 260us ~ 270us
   */
...
void LL_ENC_Encrypt1(llConnState_t *connPtr, uint8 pktHdr, uint8 pktLen,
    uint8 *pBuf)
{
  AP_PCR->SW_CLK |= BIT(MOD_AES);
  ...
  /* LL_ENC_Encrypt0(connPtr,  pktHdr,  pktLen, pBuf ); */

	{
      ...
      *(volatile uint32_t *)(LL_ENC_BASE + 0x3c) = pByte[0] ;
      pByte++;
      *(volatile uint32_t *)(LL_ENC_BASE + 0x38) = pByte[0] << 24 | pByte[1]
          << 16 | pByte[2] << 8 | pByte[3];
      pByte += 4;
      *(volatile uint32_t *)(LL_ENC_BASE + 0x34) = pByte[0] << 24 | pByte[1]
          << 16 | pByte[2] << 8 | pByte[3];
      pByte += 4;
      *(volatile uint32_t *)(LL_ENC_BASE + 0x30) = pByte[0] << 24 | pByte[1]
          << 16 | pByte[2] << 8 | pByte[3];
      ...
      while (len >= 4)
        {
          *(volatile uint32_t *)(LL_ENC_BASE + 0x0100 + index)
              = pBuf[index + 3] << 24 | pBuf[index + 2] << 16 |
...
uint8 LL_ENC_Decrypt1(llConnState_t *connPtr, uint8 pktHdr, uint8 pktLen,
    uint8 *pBuf)
{
  AP_PCR->SW_CLK |= BIT(MOD_AES);
  ...
  switch (advMode)
    {
      ...
      /* add in A2, simultaneous conn event & scan/adv event */
                ...
                && (pGlobal_config[LL_SWITCH] & SIMUL_CONN_ADV_ALLOW))
        {
          ...
          /* Note: we may need maximum slave number check here. If number of
           *       slave reach ceil, only no-connectable adv is allowed.
           *       The checking could be don't in host
		   */
      ...
      if (g_llRlDeviceNum > 0)
          osal_start_timerEx(LL_TaskID, LL_EVT_RPA_TIMEOUT,
              g_llRlTimeout * 1000);
      ...
      if (llState != LL_STATE_CONN_SLAVE &&
          llState != LL_STATE_CONN_MASTER) /* no conn + adv case */
        {
          ...
          /* if not in connect state, set idle to disable advertise */
      ...
      if (llSecondaryState != LL_SEC_STATE_IDLE) /* conn + adv case */
         ...
         *    # get the 1st adv channel in the adv channel map
         *  if ((llSecondaryState == LL_SEC_STATE_ADV)
         *         && (adv_param.advNextChan != (LL_ADV_CHAN_FIRST + i)))
         *    {      # last adv event is not finished
      ...
      default:  /* we have an invalid value for advertisement mode */
...
llStatus_t LL_PhyUpdate1(uint16 connId)
{
  ...
  phyMode = connPtr->llPhyModeCtrl.req.txPhy &
      connPtr->llPhyModeCtrl.rsp.txPhy;
  phyMode &= connPtr->llPhyModeCtrl.req.rxPhy &
      connPtr->llPhyModeCtrl.rsp.rxPhy;
  ...
  else if ((phyMode&LE_2M_PHY) && (connPtr->llPhyModeCtrl.local.txPhy !=
      LE_2M_PHY))
    {
      connPtr->phyUpdateInfo.m2sPhy = LE_2M_PHY;
      connPtr->phyUpdateInfo.s2mPhy = LE_2M_PHY;
    }
  else if ((phyMode&LE_CODED_PHY) && (connPtr->llPhyModeCtrl.local.txPhy !=
      LE_CODED_PHY))
    {
...
{
  llStatus_t ret;
  ret = LL_SetScanParam0(scanType, scanInterval, scanWindow, ownAddrType,
      scanWlPolicy);
  ...
  switch (scanMode)
    {
      ...
      if (scanInfo.scanMode == LL_SCAN_START)
        {
          ...
          /* 20200804 Jie :if random address isn't defined,can't set
           * ownaddresstype to random
           */
         ...
         * Note: There will always be a valid pointer, so no NULL check
      ...
      /* A2 multiconn, should we consider current LL state to avoid change
       * master/slave configuration now LL slave/master event use same
       * parameter 88
       */
...
void llProcessTxData2(llConnState_t *connPtr, uint8 context)
{
  ...
  while (connPtr->txDataQ.head != NULL)
    {
      ...
      if (llWriteTxData(connPtr, pBuf[1], pBuf[0], &pBuf[2]) ==
          LL_STATUS_SUCCESS)
        {
    ...
    * The Number of Completed Packets event is sent when the number of
    * completed packets is equal to or greater than the user specified limit,
    * which can range from 1 to the LL_MAX_NUM_DATA_BUFFERS (default is one).
    * If the number of completed packets is less than the limit, then this
    * event is only sent if the user indicated that this event to be sent at
    * the end of the connection event.
    * Note: Spec indicates that while the Controller has HCI data packets in
    *       its buffer, it must keep sending the Number Of Completed Packets
    *       event to the Host at least periodically, until it finally reports
    *       that all the pending ACL Data Packets have been transmitted or
    *       flushed.
  ...
  if ((numComplPkts > 0) &&
      ...
      /* || ((context == LL_TX_DATA_CONTEXT_POST_PROCESSING) &&
       * numComplPktsFlush)))
       */
 ...
 *                   4. new data
 *                 in the new RTLP buffer, the data should be in the below
 *                   sequence: 2 --> 3 --> 1 --> 4   (changed)
 *
 *    input parameters
 *
...
uint16 ll_generateTxBuffer1(int txFifo_vacancy, uint16 *pSave_ptr)
{
  ...
  if (connPtr->llMode == LL_HW_RTLP_EMPT
    ...
    {
      ll_hw_write_tfifo((uint8 *)&(connPtr->ll_buf.tx_not_ack_pkt->header),
          ((connPtr->ll_buf.tx_not_ack_pkt->header & 0xff00) >> 8) + 2);
  ...
  /* 1st RTLP event, no porcess 0/1, it should be 0 because we have reset
   * the TFIFO other case, it is 1st not transmit packet/new packet
   */
  ...
  *pSave_ptr = ll_hw_get_tfifo_wrptr();
  ...
  if (connPtr->ll_buf.ntrm_cnt > 0
      && txFifo_vacancy >= connPtr->ll_buf.ntrm_cnt)
    {
      for (i = 0; i < connPtr->ll_buf.ntrm_cnt; i++)
        {
          ll_hw_write_tfifo((uint8 *)
              &(connPtr->ll_buf.tx_ntrm_pkts[i]->header),
              ((connPtr->ll_buf.tx_ntrm_pkts[i]->header & 0xff00) >> 8) + 2);
        }
  ...
  if ((connPtr->ll_buf.tx_not_ack_pkt->valid == 0 ||                                         /* no tx not_ack packet, add on 2017-11-15 */
      ...
      && txFifo_vacancy > connPtr->ll_buf.ntrm_cnt)    /* tricky here:  if the Tx FIFO is full and nothing is sent in last event, then it can't fill new packet(include ctrl pkt) in new event */
    {
      ...
      ll_hw_write_tfifo((uint8 *)&(connPtr->ctrlData.header),
          ((connPtr->ctrlData .header & 0xff00) >> 8) + 2);
      txFifo_vacancy--;
      tx_num++;
  ...
  if (connPtr->ll_buf.ntrm_cnt != 0)
    {
      /* should not be here, new packets should not be sent
       * if there is not-transmit packets
       */
      ...
      for (i = 0; i < new_pkts_num && i < txFifo_vacancy; i++)
        {
          ...
          ll_hw_write_tfifo((uint8 *)
              &(connPtr->ll_buf.tx_conn_desc[idx]->header),
              ((connPtr->ll_buf.tx_conn_desc[idx]->header &
              0xff00) >> 8) + 2);
  ...
  if ((connPtr->llConnCTE.enable) && (connPtr->llCTE_ReqFlag))
    {
      if (connPtr->llConnCTE.CTE_Request_Intv > 0)
        {
          if (connPtr->llConnCTE.CTE_Count_Idx <
              connPtr->llConnCTE.CTE_Request_Intv)
            {
...
llStatus_t LL_SetPhyMode1(uint16 connId,  uint8 allPhy,
    uint8 txPhy, uint8 rxPhy, uint16 phyOptions)
{
  ...
  if (((connPtr->featureSetInfo.featureSet[1] & LL_FEATURE_2M_PHY) !=
      LL_FEATURE_2M_PHY) && ((connPtr->featureSetInfo.featureSet[1] &
      LL_FEATURE_CODED_PHY) != LL_FEATURE_CODED_PHY))
    {
      return(LL_STATUS_ERROR_FEATURE_NOT_SUPPORTED);
    }

  /* check if an updated parameters control procedure
   * is already what's pending
   */
  ...
  uint8 tx_chance = (txPhy ^ connPtr->llPhyModeCtrl.local.txPhy) ^
      connPtr->llPhyModeCtrl.local.txPhy;
  ...
  uint8 rx_chance = (rxPhy ^ connPtr->llPhyModeCtrl.local.rxPhy) ^
      connPtr->llPhyModeCtrl.local.rxPhy;
   ...
   * LL_TS_5.0.3 Table 4.43: PDU payload contents
   * for each case variation for LE 2M PHY
   */
  ...
  switch (opcode)
    {
      ...
      case LL_CTRL_ENC_RSP:
          ...
          pBuf = llMemCopySrc((uint8 *)
              &connPtr->encInfo.SKD[LL_ENC_SKD_S_OFFSET], pBuf,
              LL_ENC_SKD_S_LEN);
          ...
          LL_ENC_ReverseBytes(&connPtr->encInfo.SKD[LL_ENC_SKD_S_OFFSET],
              LL_ENC_SKD_S_LEN);

          /* concatenate the slave's IVs with IVm
           * Note: The IVs MSO is the MSO of the IV.
           * PHY_READ_BYTE((uint8 *)
           *     &connPtr->encInfo.IV[LL_ENC_IV_S_OFFSET], LL_ENC_IV_S_LEN);
           */

          pBuf = llMemCopySrc((uint8 *)
              &connPtr->encInfo.IV[LL_ENC_IV_S_OFFSET], pBuf,
              LL_ENC_IV_S_LEN);

          /* bytes are received LSO..MSO, but need to be maintained as
           * MSO..LSO, per FIPS 197 (AES), so reverse the bytes
           * ALT: POSSIBLE TO MAINTAIN THE IV IN LSO..MSO ORDER SINCE THE
           *      NONCE IS FORMED THAT WAY.
           */

          LL_ENC_ReverseBytes(&connPtr->encInfo.IV[LL_ENC_IV_S_OFFSET],
              LL_ENC_IV_S_LEN);
          ...
          /*      LOG("LTK: %x\r\n", connPtr->encInfo.LTK);
           *      LOG("SKD: %x\r\n", connPtr->encInfo.SKD);
           *      LOG("SK: %x\r\n", connPtr->encInfo.SK[0],
           *      connPtr->encInfo.SK[1], connPtr->encInfo.SK[],
           *      connPtr->encInfo.SK[0], connPtr->encInfo.SK[0],
           *      connPtr->encInfo.SK[0],connPtr->encInfo.SK[0]);
      ...
      case LL_CTRL_VERSION_IND:
          ...
          /* check if the peer's version information has already
           * been obtained
		   */
          ...
          if (connPtr->verExchange.peerInfoValid == TRUE)
          ...
          else /* the peer version info is invalid, so make it valid */
            {
              ...
              pBuf = llMemCopySrc((uint8 *)&connPtr->verInfo.subverNum,
                  pBuf, 2);
              ...
              if (connPtr->verExchange.verInfoSent == FALSE)
                {
                  /* no, so this is a peer's request for our
                   * version information
                   */
      ...
      case LL_CTRL_LENGTH_REQ:
        ...
        if ((connPtr->featureSetInfo.featureSet[0]
        ...
        else
        {
          if (connPtr->llPduLen.isProcessingReq == FALSE)
            {
                pBuf = llMemCopySrc((uint8 *)
                    &(connPtr->llPduLen.remote.MaxRxOctets), pBuf, 2);
                pBuf = llMemCopySrc((uint8 *)
                    &(connPtr->llPduLen.remote.MaxRxTime), pBuf, 2);
                pBuf = llMemCopySrc((uint8 *)
                    &(connPtr->llPduLen.remote.MaxTxOctets), pBuf, 2);
                pBuf = llMemCopySrc((uint8 *)
                    &(connPtr->llPduLen.remote.MaxTxTime), pBuf, 2);
      ...
      case LL_CTRL_PHY_REQ:
          ...
          /* check if the feature response procedure has already been
                  ...
                  if (connPtr->llPhyModeCtrl.isProcessingReq == FALSE)
                    {
                      ...
                      connPtr->llPhyModeCtrl.req.allPhy =
                          connPtr->llPhyModeCtrl.def.allPhy;
                      connPtr->llPhyModeCtrl.rsp.txPhy =
                          connPtr->llPhyModeCtrl.def.txPhy;
                      connPtr->llPhyModeCtrl.rsp.rxPhy =
                          connPtr->llPhyModeCtrl.def.rxPhy;
      ...
      case LL_CTRL_CTE_REQ:
           ...
           * if( connPtr->featureSetInfo.featureSet[LL_CTE_FEATURE_IDX]
           *     & LL_CONN_CTE_RSP)
           */

          if (((connPtr->featureSetInfo.featureSet[LL_CTE_FEATURE_IDX]
              & LL_CONN_CTE_RSP) != LL_CONN_CTE_RSP)
                  ...
                  if (connPtr->llCTEModeCtrl.isProcessingReq == FALSE)
                    {
                      ...
                      if ((connPtr->llConnCTE.enable) &&
                          (connPtr->llRfPhyPktFmt < LL_PHY_CODE))
                        {
                      ...
                      else
                        {
                          if (connPtr->llRfPhyPktFmt >= LL_PHY_CODE)
                            {
                              connPtr->llCTEModeCtrl.errorCode =
                                  LL_STATUS_ERROR_INVALID_LMP_LL_PARAMETER;
                            }
                          else
                            {
                              connPtr->llCTEModeCtrl.errorCode =
                                  LL_STATUS_ERROR_UNSUPPORT_LMP_LL_PARAMETER;
                            }
          ...
          if (connPtr->llCTEModeCtrl.isWatingRsp == TRUE)
            {
              if ((g_pLLcteISample != NULL) && (g_pLLcteQSample != NULL))
                  iqCnt = ll_hw_get_iq_RawSample(g_pLLcteISample,
                      g_pLLcteQSample);
              ...
              if (iqCnt > 0)
                {
                  LL_ConnectionIQReportCback(connPtr->connId,
                                            ...
                                            connPtr->currentChan,
                                            connPtr->lastRssi,
                                            0,  /* before CTE Transmit and sampling , no Antenna change , default 0 */
                                            connPtr->llConnCTE.CTE_Type,
                                            connPtr->llConnCTE.slot_Duration,
                                            0,  /* Packet_Status=0, CRC success,cause only CRC Correctly that can run here */
              ...
              else
                {
                   ...
                   * status = 0x0 : LL_CTE_RSP received successful , but
                   * without a CTE field
				   */
      ...
      case LL_CTRL_UNKNOWN_RSP:
          ...
          if (connPtr->llPhyModeCtrl.isWatingRsp)
            {
              llPhyModeCtrlUpdateNotify(connPtr,
                            ...
                            uint8 *ltk)
{
   ...
   * Note: The RAND will be left in LSO..MSO order as this is assumed to be
   *       the order of the bytes that will be returned to the Host.
   */
   ...
   * Note: The EDIV will be left in LSO..MSO order as this is assumed to be
   *       the order of the bytes that will be returned to the Host.
   */
...
void llSlaveEvt_TaskEndOk1(void)
{
  ...
  if (FALSE == conn_param[g_ll_conn_ctx.currentConn].active)
    {
       ...
       * HZF: if other procedure terminate the link, it should
  ...
  if (rfCounters.numRxOk || rfCounters.numRxIgnored ||        /* we have only "numRxOk" */
      ...
      || connPtr->rx_crcok != 0)     /* ever Rx CRC OK packet */
    {
       ...
       * was not received (see Core spec V4.0, Vol 6, Section 4.5.1), so
       * restore the slave latency value; but note that if this is the
       * beginning of a connection, you can not allow slave latency until
       * the first NESN change has occurred from the master (i.e. until
       * master ACK's slave)
       */
  ...
  else /* either no packets received, or packets received with CRC error */
    {
      ...
      if (connPtr->rx_timeout)     /* no packet was received from the master */
        ...
        {
          /* clear flag that indicates we received first packet
           * Note: The first packet only really needs to be
           *       signalled when a new connection is formed
           *       or a connection's parameters are updated.
           *       However, there's no harm in resetting it
           *       every time in order to simplify the
           *       control logic.
           * Note: True-Low logic is used here to be consistent
           *       with nR's language.
           */

          connPtr->firstPacket = 0;

          /* slave latency may have been disabled because a packet
           * from the master was not received (see Core spec V4.0,
           * Vol 6, Section 4.5.1), so restore the slave latency
           * value; but note that if this is the beginning of a
           * connection, you can not allow slave latency until
           * the first NESN change has occurred from the master
           * (i.e. until master ACK's slave)
           */
      ...
      if (connPtr->expirationEvent == connPtr->currentEvent)
        {
          /* check if either we already got the first packet in a
           * connection or if this isn't the quick LSTO associated
           * with connection establishment
           * Note: The Slave reuses firstPacket when a Update
           *       Parameter control procedure is started, but
           *       in that case, the expiration event will be
           *       well past the event associated with connection
           *       establishement.
           */
    ...
    /* for a new connection, slave latency isn't enabled until the master's
     * NESN bit changes, which is equivalent to receiving a TX ACK; if this
     * is an update parameters, slave latency is also disabled until any
     * first packet arrives, however, to keep things simple for now, we will
     * use the same ACK constraint
	 */
  ...
  if (rfCounters.numTxAck > 0)  /* connPtr->firstPacket == 0) */ /* TODO: test the scenario */
    {
       ...
       *      received from the master. Since this routine is common for
       *      END_OK and RX_TIMEOUT, there's no way to know if the first
       *      Master NESN bit change has taken place. This flag will
       *      indicate it has, so if a master packet is received, the
       *      slave latency value can be restored.
       */
       ...
       * Note: Even when a control procedure is active, but a control
       *       transaction timeout isn't used, we can still skip setting
       *       SL since that kind of control procedure wouldn't have
       *       disabled slave latency to begin with.
  ...
  /* if (connPtr->connected == 0) rx_timeout ==1 )
   * # update by HZF 05-03, if not connected, disable slave latency
   */
  ...
  if (llProcessSlaveControlProcedures(connPtr)
        == LL_CTRL_PROC_STATUS_TERMINATE)
    {
      /* #ifdef MULTI_ROLE */
  ...
  sw_delay = (T2 > ISR_entry_time) ? (T2 - ISR_entry_time)
      : (BASE_TIME_UNITS - ISR_entry_time + T2);

  /* schedule next connection event time
   * <start time> ------> <anchor point>
   * -----> <loop time>(Interrupt trigger)
   * ------> <next conn event time>  note that slaveLatency should be 0,i.e.
   * no latency if there are data to send in slave should not use
   *  conn_param[connId].curParam.connInterval, lastTimeToNextEvt
   *  also cover conn parameter update case
   */

  next_time = connPtr->lastTimeToNextEvt
      * (connPtr->lastSlaveLatency + 1) * 625;

  /* rx -> anchor: 110us
   * next_time = next_time - slave_conn_event_recv_delay
   *    - sw_delay - 110 - calibra_time - conn_param[connId].timerDrift ;
   */
  ...
  int32_t adj_time = slave_conn_event_recv_delay + sw_delay
      + 110 + calibra_time + connPtr->timerDrift;

  /* uint32 aaaa = next_time; */

  next_time = (next_time > adj_time || adj_time < 0)
      ? (next_time - adj_time) : 200;
...
__ATTR_SECTION_XIP__ void init_config(void)
{
  ...
  /* save the app initial_sp  which will be used
   * in wakeupProcess 20180706 by ZQ
   */
  ...
  pGlobal_config[LL_SWITCH] = LL_DEBUG_ALLOW | SLAVE_LATENCY_ALLOW
      | LL_WHITELIST_ALLOW | SIMUL_CONN_ADV_ALLOW | SIMUL_CONN_SCAN_ALLOW; /* RC32_TRACKINK_ALLOW */
   ...
   * t4. sw prepare cal sleep tick initial rf_ini about 300us
   * @16M this part depends on HCLK
  ...
  /* conn_req -> slave connection event calibration time,
   * will advance the receive window
   */

  pGlobal_config[CONN_REQ_TO_SLAVE_DELAY] = 300; /* 192; #500; #192; */

  /* calibration time for 2 connection event, will advance
   * the next conn event receive window SLAVE_CONN_DELAY
   * for sync catch, SLAVE_CONN_DELAY_BEFORE_SYNC for sync not catch
   */

  /* pGlobal_config[SLAVE_CONN_DELAY] = 300;
   * #0; #1500; #0; #3000; #0; ---> update 11-20
   */
  ...
  /* pGlobal_config[LL_HW_RTLP_1ST_TIMEOUT]  =
   *    2000 + pGlobal_config[SLAVE_CONN_DELAY] * 2; #500;
   * pGlobal_config[LL_HW_RTLP_1ST_TIMEOUT]  =
   *    1000 + pGlobal_config[SLAVE_CONN_DELAY]; #500; #eagle
   */
   ...
   * pGlobal_config[LL_TX_PWR_TO_REG_BIAS]   = 0x15;
   * assume when g_rfPhyTxPower = 0x1f, tx power = 10dBm
  ...
  /* TRLP timeout
   * enough for 8Tx + 8Rx : (41 * 8 + 150) * 16 - 150 = 7498us
   */

  pGlobal_config[LL_HW_TRLP_LOOP_TIMEOUT] = 50000;
  ...
  /* reserve time for preparing master conn event,
   * delay should be insert if needn't so long time
   */

  pGlobal_config[LL_MASTER_PROCESS_TARGET] = 200;
  ...
  /* JUMP_FUNCTION(V20_IRQ_HANDLER) =
   *    (uint32_t)&TIM1_IRQHandler1;
   */
  ...
  /* JUMP_FUNCTION(LL_GEN_TRUE_RANDOM) =
   *     (uint32_t)&LL_ENC_GenerateTrueRandNum1;
   */
  ...
  /* JUMP_FUNCTION(LL_PROCESS_SLAVE_CTRL_PROC) =
   *     (uint32_t)&llProcessSlaveControlProcedures1;
   * JUMP_FUNCTION(LL_PROCESS_TX_DATA) = (uint32_t)&llProcessTxData1;
   * JUMP_FUNCTION(OSAL_POWER_CONSERVE) =
   *     (uint32_t)&osal_pwrmgr_powerconserve1;
  ...
  JUMP_FUNCTION(L2CAP_PROCESS_FREGMENT_TX_DATA) =
      (uint32_t)&l2capPocessFragmentTxData;
  ...
  /* JUMP_FUNCTION(LL_SLAVE_EVT_ENDOK) =
   *   (uint32_t)&llSlaveEvt_TaskEndOk1;
   */
...
void ll_patch_master(void)
{
  ...
  /* JUMP_FUNCTION(LL_PROCESS_MASTER_CTRL_PKT) =
   *   (uint32_t)&llProcessMasterControlPacket1;
   */

  JUMP_FUNCTION(LL_CREATE_CONN) = (uint32_t)&LL_CreateConn1;
  JUMP_FUNCTION(LL_START_ENCRYPT) = (uint32_t)&LL_StartEncrypt1;
  JUMP_FUNCTION(LL_ENC_DECRYPT) = (uint32_t)&LL_ENC_Decrypt1;
  JUMP_FUNCTION(LL_PROCESS_MASTER_CTRL_PROC) =
      (uint32_t)&llProcessMasterControlProcedures1;
  JUMP_FUNCTION(LL_PROCESS_SLAVE_CTRL_PROC) =
      (uint32_t)&llProcessSlaveControlProcedures1;
  JUMP_FUNCTION(LL_PROCESSBASICIRQ_SRX) = (uint32_t)&ll_processBasicIRQ_SRX0;
  JUMP_FUNCTION(LL_PROCESSBASICIRQ_SCANTRX) =
      (uint32_t)&ll_processBasicIRQ_ScanTRX0;
 ...
 *    ll_patch_slave();
 *    ll_patch_master();
 *    JUMP_FUNCTION(LL_SCHEDULER) =
 *        (uint32_t)&ll_scheduler1;
 *    JUMP_FUNCTION(LL_PROCESSBASICIRQ_SECADVTRX) =
...
{
  extern void _rom_sec_boot_init(void);
...
hciStatus_t HCI_LE_SetHostChanClassificationCmd(uint8 *chanMap)
{
      ...
      for (uint8_t i = 0; i < g_maxConnNum; i++)
        {
          if (conn_param[i].active)
            {
              llConnState_t *connPtr = &conn_param[i];
              osal_memcpy((uint8_t *)&(connPtr->chanMapUpdate.chanMap[0]),
                  chanMap, LL_NUM_BYTES_FOR_CHAN_MAP);
            }
        }
    }
  ...
  HCI_CommandCompleteEvent(HCI_LE_SET_HOST_CHANNEL_CLASSIFICATION,
 ...
 * {
 *    typedef void (*uart_init_t)(int baud, GPIO_Pin_e tx_pin,
 * GPIO_Pin_e rx_pin,uint32_t cb_addr);
...
void LL_PLUS_GetCurrentPduDle(uint8_t connId,
    ll_pdu_length_ctrl_t *ppdu)
{
...
void rflib_vesion(uint8_t *major, uint8_t *minor,
    uint8_t *revision, char *test_build)
{
  *major = SDK_VER_MAJOR;
  *minor = SDK_VER_MINOR;
 ...
 *
 *           if (current->hdr.len > max_block && (void*)(&current->hdr)
 *   > (void*)(header + OSALMEM_BIGBLK_IDX))
                          ...
                          uint16 maxLength)
{
  ...
  if (((connPtr->ctrlPktInfo.ctrlPktCount > 0) &&
     ...
     * Note: LSTO time is normalized to units of 1.25ms
     * (i.e. 10ms = 8 * 1.25ms).
     */

  if (LL_INVALID_CONN_TIME_PARAM_COMBO(connIntervalMax,
     ...
     * is a multiple/divisor of all other active connection intervals; also
     * make sure that this connection's interval is not less than the allowed
     * maximum connection interval as determined by the maximum number of
     * allowed connections times the number of slots per connection.
     */
  ...
  if (g_ll_conn_ctx.numLLMasterConns > 1) /* if (g_ll_conn_ctx.numLLConns > 0) */
    {
      ...
      if (connInterval >= minCI)
        {
          /* check if this connection's CI is valid
           *  (i.e. a multiple of minCI)
           */
  ...
  else
    {
      ...
      g_ll_conn_ctx.per_slot_time = connPtr->curParam.connInterval
          * 2 / g_maxConnNum;
  ...
  connPtr->paramUpdate.winOffset = 0;
  /* multiconnection, this value could be 0 or x * old conn interval
...
llStatus_t LL_EncLtkReply(uint16 connId, uint8 *key)
{
  uint8         i;
  llStatus_t    status;
  llConnState_t *connPtr;
  ...
  /* asynchronous send msg can not make sure
   * llState = LL_STATE_CONN_SLAVE in multi-role
   */
 ...
 *             that the Long Term Key (LTK) for encryption can not be
 *              provided.
...
llStatus_t LL_EncLtkNegReply(uint16 connId)
{
  ...
  if (connPtr->encInfo.encRestart == TRUE)
  ...
  else /* during a start encryption */
    {
	  ...
	  /* same as LL_ENC_KEY_REQ_REJECTED */
...
hciStatus_t HCI_LE_LtkReqNegReplyCmd(uint16 connHandle)
{
  ...
  rtnParam[1] = LO_UINT16(connHandle);
  rtnParam[2] = HI_UINT16(connHandle);
  HCI_CommandCompleteEvent(HCI_LE_LTK_REQ_NEG_REPLY,
