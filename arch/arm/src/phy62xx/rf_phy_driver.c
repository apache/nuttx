/****************************************************************************
 * arch/arm/src/phy62xx/rf_phy_device.c
 *
...
volatile uint8_t        g_rc32kCalRes       =   0xff;
    ...
    static volatile uint32_t     uart_rx_wIdx = 0;
    static volatile uint32_t     uart_rx_rIdx = 0;
#endif
...
volatile uint16_t g_rfPhy_ct_moniter_word_arry[CT_MONT_BUFF_SIZE] =
    {0
    };
#endif
 ...
 *    input parameters
 *
 *    @param       rfChn:  two point calibration rf channel
 *                         setting(0-80)->2400-2480MHz.
 *    output parameters
 *
 *    @param       None.
 *
...
{
  g_rfPhyTpCal0 = rf_tp_cal(2, 0) + 2;
  g_rfPhyTpCal1 = rf_tp_cal(66, 0) + 2;
  g_rfPhyTpCal0_2Mbps = rf_tp_cal(2, 1) + 2;
  g_rfPhyTpCal1_2Mbps = rf_tp_cal(66, 1) + 2;
}
  ...
  if (g_rfPhyClkSel == RF_PHY_CLK_SEL_16M_XTAL)
    {
      ...
      /* PHY_REG_WT(0x400300d0,0x00000140);
       * # i_pll_ctrl1 # double cp current to compensate for clock;
       * PHY_REG_WT(0x40030104,0x00000140);
       * # i_pll_ctrl5 # double cp current to compensate for clock;
       */
    }
 ...
 *    input parameters
 *
 *    @param       pktMod:0 for Zigbee,
 *                 1 for BLE1M, 2 for BLE2M, 3or4 for BLELR.
 *
 *    output parameters
 *
 *    @param       None.
 ...
 *                fDev     : used to config the tpCal fDelt,
 *                           0 for 0.5M, 1 for 1M
 *
 *
 *    output parameters
 ...
 *    @fn          rf_rxDcoc_cfg
 *
 *    @brief       This function process for rx dc offset calibration and
 *                  canncellation config.
 *    input parameters
 *
 *    @param       rfChn   : rfFreq=2400+rfChn
 *                 bwSet   : used to config rx complex filter bandwitdh.
 *                           1 for 1MHz, other for 2MHz
 *
 *    output parameters
 *
...
void rf_rxDcoc_cfg(uint8_t rfChn, uint8_t bwSet, volatile uint32 *dcCal)
{
  ...
  /* restore the rxTimeOut setting, and set rxto to zero,
   * not to generate rx_done
   */
                      ...
                      if (!(g_rfPhyPktFmt == PKT_FMT_ZIGBEE))
                        {
                           ...
                           *  PHY_REG_WT( 0x400300a4,0x00000140);
                           *  # clr tx_auto
                           *  PHY_REG_WT( 0x400300a4,0x00000150);
                           *  # clr tx_auto
                           *  #wait a while then set tx_bb_en
                           *  int delay=100;while(delay--){};
                           *  PHY_REG_WT( 0x400300a4,0x00000154);
                           *  # clr tx_auto
                           *  }
                           *  else
                           *  {
                           *  ll_hw_trigger();
                           *  #PHY_REG_WT( 0x400300a0,0x0000000e);
                           *  # clr pll_auto override
                           *  #PHY_REG_WT( 0x400300a4,0x00000140);
                           *  # clr tx_auto
                           *  #PHY_REG_WT( 0x400300a4,0x00000154);
                           *  # clr tx_auto
                      ...
                      if (ct_word > 0)
                        {
                          ...
                          int32_t idx = (ct_word) -
                              g_rfPhy_ct_moniter_word_target +
                              (CT_MONT_BUFF_SIZE >> 1);
                              ...
                              for (int i = 0; i < CT_MONT_BUFF_SIZE; i++)
                                {
                                  if (g_rfPhy_ct_moniter_word_arry[i] > 0)
                                      LOG("%d %d\n",
                                          g_rfPhy_ct_moniter_word_target -
                                          (CT_MONT_BUFF_SIZE >> 1) + i ,
                                          g_rfPhy_ct_moniter_word_arry[i]);
                  ...
                  if (ll_hw_get_irq_status() & LIRQ_MD)
                    {
                      if (ll_hw_get_irq_status() & LIRQ_COK)
                      ...
                      else
                        {
                          ...
                          g_dtmPktCount = (g_dtmPktCount == 65535) ? 0 :
                              g_dtmPktCount;
                        }
                      ...
                      if (g_dtmPktCount > 0)
                        {
                          g_dtmRssi = (g_dtmPktCount == 1) ? rssi :
                              ((g_dtmRssi + rssi) >> 1);
                          g_dtmFoff = (g_dtmPktCount == 1) ? foff :
                              ((g_dtmFoff + foff) >> 1);
                          g_dtmCarrSens = (g_dtmPktCount == 1) ? carrSens :
                              ((g_dtmCarrSens + carrSens) >> 1);
                        }
                    }
                }
...
void rf_phy_dtm_cmd_parse(void)
{
  ...
  if (g_dtmCmd == 0 || g_dtmCmd == 3)
      ...
      if (g_dtmCmd == 0)
        {
          if (g_dtmCtrl == 0 && g_dtmPara == 0)
          ...
          else if (g_dtmCtrl == 2 && g_dtmPara == 0x02)
            {
              ...
              g_rfPhyPktFmt = (RF_PHY_DTM_BB_SUPPORT_BLE2M &
                RF_PHY_DTM_BB_SUPPORT_MOD) ? PKT_FMT_BLE2M : PKT_FMT_BLE1M;
            }
          else if (g_dtmCtrl == 2 && g_dtmPara == 0x03)
            {
              g_dtmModeType = RF_PHY_DTM_MODE_SET_PHY_125K;
              g_rfPhyPktFmt = (RF_PHY_DTM_BB_SUPPORT_BLR125K &
                RF_PHY_DTM_BB_SUPPORT_MOD) ? PKT_FMT_BLR125K : PKT_FMT_BLE1M;
            }
          else if (g_dtmCtrl == 2 && g_dtmPara == 0x04)
            {
              g_dtmModeType = RF_PHY_DTM_MODE_SET_PHY_500K;
              g_rfPhyPktFmt = (RF_PHY_DTM_BB_SUPPORT_BLR500K &
                RF_PHY_DTM_BB_SUPPORT_MOD) ? PKT_FMT_BLR500K : PKT_FMT_BLE1M;
            }
          else if (g_dtmCtrl == 2 && g_dtmPara == 0x20)
            {
              g_dtmModeType = RF_PHY_DTM_MODE_SET_PHY_ZB;
              g_rfPhyPktFmt = (RF_PHY_DTM_BB_SUPPORT_ZIGBEE &
                RF_PHY_DTM_BB_SUPPORT_MOD) ? PKT_FMT_ZIGBEE : PKT_FMT_BLE1M;
            }
              ...
              for (uint8_t pin = 0; pin < 23; pin++)
                {
                  ...
                  else if ((pin == P9) || (pin == P10) || (pin == P16) ||
                          (pin == P17))
                    {
                        continue;
                    }
          ...
          else if (g_dtmCtrl == 0x36)
            {
              ...
              g_dtmAccessCode = (g_dtmAccessCode&(0xffffff00)) |
                  g_rfPhyDtmCmd[1];
          ...
          else if (g_dtmCtrl == 0x37)
            {
              ...
              g_dtmAccessCode = (g_dtmAccessCode&(0xffff00ff)) |
                  (g_rfPhyDtmCmd[1] << 8);
          ...
          else if (g_dtmCtrl == 0x38)
            {
              ...
              g_dtmAccessCode = (g_dtmAccessCode&(0xff00ffff)) |
                  (g_rfPhyDtmCmd[1] << 16);
          ...
          else if (g_dtmCtrl == 0x39)
            {
              ...
              g_dtmAccessCode = (g_dtmAccessCode & (0x00ffffff)) |
                  (g_rfPhyDtmCmd[1] << 24);
  ...
  else if (g_dtmCmd == 1 || g_dtmCmd == 2)
    {
      ...
      if (g_dtmPktCount == 3 && (g_rfPhyPktFmt == PKT_FMT_BLR125K ||
          g_rfPhyPktFmt == PKT_FMT_BLR500K))
        {
          g_dtmPKT = 4; /* all ones 'b 11111111 */
        }

      g_dtmModeType = (g_dtmCmd == 1) ?
          RF_PHY_DTM_MODE_RX_PER : RF_PHY_DTM_MODE_TX_BURST;
    }
}
...
void rf_phy_dtm_trigged(void)
{
  ...
  if (g_dtmModeType == RF_PHY_DTM_MODE_RESET)
      ...
      || g_dtmModeType == RF_PHY_DTM_MODE_TX_SINGLE)
    {
      ...
      /* PHY_REG_WT(0x40030040,0x000300f0);
       * # need to extend the preamble length
       */
      ...
      if (g_dtmModeType == RF_PHY_DTM_MODE_TX_BURST)
        {
          ...
          if (!(g_rfPhyPktFmt == PKT_FMT_ZIGBEE))
            {
              #if 0
              /* [15:8] payload Len [7:5] preamble len, [4] tx mode,
               * [3:0] payload type
               * PHY_REG_WT(0x40030040,(0x00030010|(g_dtmLength<<8)|
               * (preambleLen<<5)|(0x0f & g_dtmPKT)));
               */

              if ((g_rfPhyPktFmt == PKT_FMT_BLR125K ||
                  g_rfPhyPktFmt == PKT_FMT_BLR500K) && g_dtmPKT == 0x03)
                {
                  /* for PKT_FMT all ones 11111111
                   * PHY_REG_WT(0x40030040,(0x00030010|((g_dtmLength+2)<<8)|
                   * (preambleLen<<5)|0x04) );
                   */
    ...
    {
      rf_phy_ana_cfg();
      rf_rxDcoc_cfg(2, 1, &g_rfPhyRxDcIQ);        /* set the rfChn as 2402 for BW=1MHz */
      ...
      if ((rfChn & 0x0f) == 0)
      ...
      else
        {
          ...
          if (g_rfPhyFreqOffSet >= 0)
            ...
            {
              PHY_REG_WT(0x400300b4, rfChn - 1 + ((255 + g_rfPhyFreqOffSet)
                  << 16));
            }
        }
      ...
      PHY_REG_WT(0x4003000c, (g_dtmLength + 3) > 255 ? 255 :
          (g_dtmLength < 40 ? 40 : g_dtmLength + 3));
 ...
 *                foff    : estimated freq offset by rx BB ,
 *                          foff-512-->[-512 511]KHz
...
void rf_phy_set_txPower (uint8 txPower)
{
  ...
  PHY_REG_WT(0x400300b8, (PHY_REG_RD(0x400300b8) & 0x0fff) |
      ((txPower & 0x1f) << 12));
}
...
uint8 rc32k_calibration(void)
{
  ...
  *(volatile uint32_t *) 0x4000f05c |= 0x01;                            /* enable RC32K calibration */
  ...
  while (!(*(volatile uint32_t *) 0x4000f068 & 0x200) && delay-- > 0)   /* check RC32K calibration OK flag, normally need >200us */
    {
  ...
  if (delay > 0)
    {
      ...
      *(volatile uint32_t *)0x4000f018 =
          (*(volatile uint32_t *) 0x4000f018 & 0xffffff81) | temp;      /* write the result */
    }
...
void rf_calibrate1(void)
{
   ...
   *  in ble_main, rf_calibration is called after rf_phy_ini,
   *  so comment the rf_phy_ana_cfg
   */
  ...
  rf_rxDcoc_cfg(88, 1, &g_rfPhyRxDcIQ);                       /* set the rfChn as 2488 for BW=1MHz */
...
void rf_phy_dtm_ext_tx_singleTone(uint8_t txPower, uint8_t rfChnIdx,
    uint8_t xtal_cap, int8_t rfFoff, uint32 testTimeUs)
{
  rf_phy_dtm_init();
 ...
 *                rfChnIdx    :   rf channel = 2402+(rfChnIdx<<1)
 *                rfFoff      :   rf freq offset = rfFoff*4KHz
 *                pktType     :   modulaiton data type,
 *                            :   0: prbs9, 1: 1111000: 2 10101010
 *                testTimeUs  :   test loop active time(ms)
 *
 *    output parameters
 *
...
void rf_phy_dtm_ext_tx_modulation(uint8_t txPower, uint8_t rfChnIdx,
    uint8_t xtal_cap, int8_t rfFoff, uint8_t pktType, uint32 testTimeUs)
{
  rf_phy_dtm_init();
 ...
 *                txPktIntv   :   txPkt intv,0 txPkt intv is
...
void rf_phy_dtm_ext_tx_mod_burst(uint8_t txPower, uint8_t rfChnIdx,
    uint8_t xtal_cap, int8_t rfFoff, uint8_t pktType, uint8_t pktLength,
    uint32 txPktNum, uint32 txPktIntv)
{
  rf_phy_dtm_init();
  ...
  while (txPktNum > 0)
    {
      ...
      /* boost vco current[7:4] to 0xf to reduce dcdc spur */

      /* subWriteReg(0x400300d4, 7, 4, 0x0f); */
...
void rf_phy_dtm_ext_rx_demod_burst(uint8_t rfChnIdx, int8_t rfFoff,
    uint8_t xtal_cap, uint8_t pktLength, uint32 rxTimeOut, uint32 rxWindow,
    uint16_t *rxEstFoff, uint8_t *rxEstRssi, uint8_t *rxEstCarrSens,
    uint16_t *rxPktNum)
{
  rf_phy_dtm_init();
