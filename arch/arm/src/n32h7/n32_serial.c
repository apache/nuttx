  ...
  if (priv->rxdma_channel != 0)
    {
      ...
      rxcfg.src_hs_if = n32_getchannel(priv->rxdma);
      rxcfg.dst_hs_if = n32_getchannel(priv->rxdma);
      ...
      if (priv->dev.dmatx.nlength)
        {
          ...
          txcfg.src_hs_if  = n32_getchannel(priv->txdma);
          txcfg.dst_hs_if  = n32_getchannel(priv->txdma);
  ...
  switch (cmd)
    {
      ...
      case TIOCSSWAP:
        {
          uint32_t cr1 = up_serialin(priv, N32_USART_CTRL1_OFFSET);
          uint32_t tmp = cr1 & USART_CTRL1_UEN;
