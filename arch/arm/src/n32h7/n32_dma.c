...
/* All available DMA channels (channels from standard DMA and
...
struct n32_dmach_s
{
  ...
  uint8_t        chan:5;    /* DMA channel/channel channel id */
 ...
 * Note that we keep here standard DMA channels, BDMA channels and MDMA
 * channels.
 */
 ...
 *
 * Description:
 *  Disable standard DMA channel (DMA1/DMA2)
 *
 ****************************************************************************/
...
static void n32_sdma_disable(DMA_CHANNEL dmachan)
{
  ...
  channel = dmachan->chan;

  DEBUGASSERT(channel < 8);

  /* Disable the DMA channel */

  /* Set channel enable bit writable */

  regval  = dmabase_getreg(dmachan, N32_DMA_CHEN_OFFSET);
  regval |= DMA_CHEN_CHWEN(channel);

  /* Clear channel enable bit */

  regval &= ~DMA_CHEN_CH(channel);
...
static int n32_sdma_interrupt(int irq, void *context, void *arg)
{
  ...
  if (irq >= N32_IRQ_DMA1_CHANNEL0 && irq <= N32_IRQ_DMA1_CHANNEL7)
    {
      channel     = irq - N32_IRQ_DMA1_CHANNEL0;
      controller = DMA1;
    }
  else
#endif
  ...
  if (irq >= N32_IRQ_DMA2_CHANNEL0 && irq <= N32_IRQ_DMA2_CHANNEL7)
    {
      channel     = irq - N32_IRQ_DMA2_CHANNEL0;
      controller = DMA2;
    }
  else
#endif
  ...
  if (irq >= N32_IRQ_DMA3_CHANNEL0 && irq <= N32_IRQ_DMA3_CHANNEL7)
    {
      channel     = irq - N32_IRQ_DMA3_CHANNEL0;
      controller = DMA3;
    }
  else
#endif
  ...
  /* Get the channel structure from the channel and controller numbers */

  dmachan = n32_dma_channel_get(channel, controller);

  /* Get the interrupt status for this channel */

  status = (((dmabase_getreg(dmachan, N32_DMA_TCINTSTS_OFFSET)  &
            DMA_INT_CH(channel)) >> channel) << 0) |
           (((dmabase_getreg(dmachan, N32_DMA_BTCINTSTS_OFFSET) &
            DMA_INT_CH(channel)) >> channel) << 1) |
           (((dmabase_getreg(dmachan, N32_DMA_STCINTSTS_OFFSET) &
            DMA_INT_CH(channel)) >> channel) << 2) |
           (((dmabase_getreg(dmachan, N32_DMA_DTCINTSTS_OFFSET) &
            DMA_INT_CH(channel)) >> channel) << 3) |
           (((dmabase_getreg(dmachan, N32_DMA_ERRINTSTS_OFFSET) &
            DMA_INT_CH(channel)) >> channel) << 4);

  /* Clear fetched channel interrupts */
...
size_t n32_getchannel(DMA_HANDLE handle)
{
  DMA_CHANNEL dmachan    = (DMA_CHANNEL)handle;
