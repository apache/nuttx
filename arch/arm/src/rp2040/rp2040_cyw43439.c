...
{
  FAR rp2040_gspi_t *rp_io = (FAR rp2040_gspi_t *)(gspi->private);
...
FAR void *rp2040_cyw_setup(uint8_t gpio_on,
                           uint8_t gpio_select,
                           uint8_t gpio_data,
                           uint8_t gpio_clock,
                           uint8_t gpio_intr)
{
  ...
  ninfo("setup complete. gspi = 0x%08lX\n", (uint32_t) gspi);
...
void rp2040_cyw_remove(FAR void *gspi)
{
