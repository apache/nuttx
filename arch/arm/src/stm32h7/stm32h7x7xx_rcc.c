  ...
  if (timeout > 0)
    {
      ...
      /* Configure PLL1 fractional divider.  Assumes that the fractional latch
       * is initially disabled.  Also assumes that the STM32_PLLCFG_PLL1/2/3CFG
       * values include the corresponding RCC_PLLCFGR_PLL1/2/3FRACEN bit in
       * order to subsequently enable the latch.
       */
