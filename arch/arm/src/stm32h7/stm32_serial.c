...
static int up_setup(struct uart_dev_s *dev)
{
  ...
  struct clk_s *pclk = clk_get_parent(clk);
  struct clk_s *hsi_clk = clk_get("hsi_ker_ck");
  struct clk_s *gpio_clk = clk_get("gpiod");

  clk_enable(hsi_clk);
  clk_set_parent(pclk, hsi_clk);
  clk_enable(clk);
  clk_enable(gpio_clk);
