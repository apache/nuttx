...
/****************************************************************************
 * NuttX side of the RTL8721Dx WiFi bring-up.
 *
 * The WHC host stack talks to the WiFi MAC/PHY (which runs on the NP network
 * processor, km0) over the on-chip AP<->NP IPC.  This file wires that IPC
...
/* KM4 IPC device register base (IPC1_REG_BASE, from SDK hal_platform.h). */

#define IPCKM4_DEV  ((void *)0x41014080)
 ...
 *
 * Description:
 *   NuttX IRQ handler shim for the KM4 IPC interrupt; forwards to the SDK's
 *   IPC_INTHandler, which dispatches to the registered channel callbacks
 *   (WHC WiFi TRX, etc.).
 *
 ****************************************************************************/
...
int rtl8721dx_wifi_initialize(void)
{
  ...
  irq_attach(RTL8721DX_IRQ_IPC_KM4, ameba_ipc_interrupt, IPCKM4_DEV);
