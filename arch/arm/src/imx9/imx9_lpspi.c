...
struct imx9_lpspidev_s
{
  ...
  uint32_t clk_root;          /* SPIn clock root */
#ifdef CONFIG_IMX9_LPSPI_INTERRUPTS
                             ...
                             enum imx9_delay_e type)
{
  ...
  imx9_get_rootclock(GET_CLOCK_ROOT(priv->clk_root), &src_freq);
                                 ...
                                 bool selected)
{
  ...
  /* LPSPI on IMX9 supports PCS 0-3 */
  ...
  if (devid != priv->pcs)
    {
      imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_TCR_OFFSET,
                                LPSPI_TCR_PCS_MASK,
...
void weak_function imx9_lpspi_select(struct spi_dev_s *dev, uint32_t devid,
                                 bool selected)
{
  imx9_lpspi_hw_select(dev, devid, selected);
}

#endif /* CONFIG_IMX9_LPSPI HWPCS */
...
{
  imx9_configure_clock(GET_CLOCK_ROOT(priv->clk_root), true);
}
...
#endif
#ifdef MUX_LPSPI4_CS1
          imx9_iomux_configure(MUX_LPSPI4_CS1);
#endif
#ifdef GPIO_LPSPI4_CS1
          imx9_config_gpio(GPIO_LPSPI4_CS1);
#endif
