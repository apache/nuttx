/****************************************************************************
 * arch/arm/src/imx9/imx9_sar_adc.c
 *
...
#define IMX9_SAR_ADC_MAX_CHANNELS    8
#endif
...
#define IMX9_SAR_ADC_REG(offset)     (IMX9_ADC_BASE + (offset))
...
static int imx9_sar_adc_hw_init(void)
{
  ...
  ret = imx9_configure_clock(ADC_CLK, true);
  if (ret < 0)
    {
      return ret;
    }
  ...
  if (ret < 0)
    {
#ifdef ADC_CLK
      imx9_configure_clock(ADC_CLK, false);
#endif
      return ret;
    }
