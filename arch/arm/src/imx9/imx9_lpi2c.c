...
struct imx9_lpi2c_config_s
{
  ...
  uint32_t clk_root;           /* LPI2C clock root */
      ...
      if (frequency != priv->frequency)
        {
          ...
          imx9_get_rootclock(GET_CLOCK_ROOT(priv->config->clk_root),
                                            &src_freq);
...
static void imx9_lpi2c_clock_enable(struct imx9_lpi2c_priv_s *priv)
{
  imx9_configure_clock(GET_CLOCK_ROOT(priv->config->clk_root), true);
}
...
{
  imx9_configure_clock(GET_CLOCK_ROOT(priv->config->clk_root), false);
}
