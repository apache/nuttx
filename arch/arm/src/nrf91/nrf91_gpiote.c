/****************************************************************************
 * arch/arm/src/nrf91/nrf91_gpiote.c
 *
...
#include "hardware/nrf91_gpiote.h"
...
#  define NRF91_IRQ_GPIOTE  NRF91_IRQ_GPIOTE1
#else
#  define NRF91_IRQ_GPIOTE  NRF91_IRQ_GPIOTE0
#endif
...
struct nrf91_gpiote_callback_s
{
  xcpt_t    callback;
  void     *arg;
  uint32_t  pinset;
...
static struct nrf91_gpiote_callback_s g_gpiote_ch_callbacks[GPIOTE_CHANNELS];

#ifdef CONFIG_NRF91_PER_PIN_INTERRUPTS
/* Callbacks attached to each GPIO pin */

static struct nrf91_gpiote_callback_s
    g_gpiote_pin_callbacks[NRF91_GPIO_NPORTS][NRF91_GPIO_NPINS];
#else
/* Callback for the PORT event */

static struct nrf91_gpiote_callback_s
    g_gpiote_port_callback[NRF91_GPIO_NPORTS];
#endif
...
static inline void nrf91_gpiote_putreg(uint32_t offset, uint32_t value)
{
  putreg32(value, NRF91_GPIOTE_BASE + offset);
}

/****************************************************************************
 * Name: nrf91_gpiote_getreg
 *
 * Description:
...
static inline uint32_t nrf91_gpiote_getreg(uint32_t offset)
{
  return getreg32(NRF91_GPIOTE_BASE + offset);
}

/****************************************************************************
 * Name: nrf91_gpiote_isr
 *
 * Description:
...
static int nrf91_gpiote_isr(int irq, void *context, void *arg)
{
  uint32_t regval = 0;
  int      ret    = OK;
  int      i      = 0;
      ...
      if (g_gpiote_ch_callbacks[i].callback != NULL)
        {
          ...
          regval = nrf91_gpiote_getreg(NRF91_GPIOTE_EVENTS_IN_OFFSET(i));
          if (regval == GPIOTE_EVENT_IN_EVENT)
            {
              ...
              nrf91_gpiote_putreg(NRF91_GPIOTE_EVENTS_IN_OFFSET(i), 0);
            }
        }
    }
  ...
  regval = nrf91_gpiote_getreg(NRF91_GPIOTE_EVENTS_PORT_OFFSET);
  if (regval)
    {
      uint32_t addr = 0;
      ...
      nrf91_gpiote_putreg(NRF91_GPIOTE_EVENTS_PORT_OFFSET, 0);
      ...
      for (i = 0; i < NRF91_GPIO_NPORTS; i++)
        {
          ...
          for (j = 0; j < NRF91_GPIO_NPINS && regval; j++)
            {
...
#ifdef CONFIG_NRF91_PER_PIN_INTERRUPTS
/****************************************************************************
 * Name: nrf91_gpiote_set_pin_event
 *
 * Description:
 ...
 *   mode configuration for the port (see nrf91_gpio_detectmode()). Pin
 *   will be sensitive to high/low according to GPIO_SENSE_LOW/HIGH
 *   (set via nrf91_gpio_config()).
 *
...
void nrf91_gpiote_set_pin_event(uint32_t pinset, xcpt_t func, void *arg)
{
  int        pin    = 0;
  int        port   = 0;
  irqstate_t flags;
...
#else
/****************************************************************************
 * Name: nrf91_gpiote_set_port_event
 *
 * Description:
...
void nrf91_gpiote_set_port_event(uint32_t pinset, xcpt_t func, void *arg)
{
  int        port   = 0;
  irqstate_t flags;
  ...
  if (func)
    {
      ...
      nrf91_gpiote_putreg(NRF91_GPIOTE_INTENSET_OFFSET, GPIOTE_INT_PORT);
    }
  else
    {
#if NRF91_GPIO_NPORTS > 1
      ...
      if (i == NRF91_GPIO_NPORTS)
        {
          nrf91_gpiote_putreg(NRF91_GPIOTE_INTENCLR_OFFSET, GPIOTE_INT_PORT);
        }
#else
      /* Disable the ISR */
...
/****************************************************************************
 * Name: nrf91_gpiote_set_ch_event
 *
 * Description:
...
void nrf91_gpiote_set_ch_event(uint32_t pinset, int channel,
                               ...
                               xcpt_t func, void *arg)
{
  ...
  if (func)
  ...
  else
    {
      ...
      nrf91_gpiote_putreg(NRF91_GPIOTE_INTENCLR_OFFSET,
                          GPIOTE_INT_IN(channel));
...
int nrf91_gpiote_set_event(uint32_t pinset,
                           bool risingedge, bool fallingedge,
                           xcpt_t func, void *arg)
{
  irqstate_t flags;
 ...
 *   nrf91_gpiosetevent with event set to true). Also, once set to TASK mode,
 *   pin control is only possible via tasks on the via nrf91_gpio_write and
...
void nrf91_gpiote_set_task(uint32_t pinset, int channel,
                           bool output_high,
                           enum nrf91_gpiote_outcfg_e outcfg)
{
  uint32_t regval;
  int pin;
#ifdef CONFIG_NRF91_HAVE_PORT1
  ...
  switch (outcfg)
    {
      case NRF91_GPIOTE_SET:
        regval |= GPIOTE_CONFIG_POL_LTH;
        break;
      case NRF91_GPIOTE_CLEAR:
        regval |= GPIOTE_CONFIG_POL_HTL;
        break;
      case NRF91_GPIOTE_TOGGLE:
        regval |= GPIOTE_CONFIG_POL_TG;
        break;
    }
...
int nrf91_gpiote_init(void)
{
  ...
  irq_attach(NRF91_IRQ_GPIOTE, nrf91_gpiote_isr, NULL);
  up_enable_irq(NRF91_IRQ_GPIOTE);
