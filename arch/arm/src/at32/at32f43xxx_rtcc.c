 ...
 *  - extended functions Alarm A and B for AT32F4xx and onwards
 * */
...
#ifndef CONFIG_AT32_PWR
#  error "CONFIG_AT32_PWR must selected to use this driver"
#endif
...
/* Proxy definitions to make the same code work for all the AT32 series *****/

# define AT32_RCC_XXX        AT32_CRM_BPDC
# define RCC_XXX_YYYRST      CRM_BPDC_BPDRST
# define RCC_XXX_RTCEN       CRM_BPDC_ERTCEN
# define RCC_XXX_RTCSEL_MASK CRM_BPDC_ERTCSEL_MASK
# define RCC_XXX_RTCSEL_LSE  CRM_BPDC_ERTCSEL_LEXT
# define RCC_XXX_RTCSEL_LSI  CRM_BPDC_ERTCSEL_LICK
# define RCC_XXX_RTCSEL_HSE  CRM_BPDC_ERTCSEL_HEXT
...
static void rtc_dumpregs(const char *msg)
{
  ...
  rtcinfo("%s:\n", msg);
  rtcinfo("      TR: %08x\n", getreg32(AT32_RTC_TR));
  rtcinfo("      DR: %08x\n", getreg32(AT32_RTC_DR));
  rtcinfo("      CR: %08x\n", getreg32(AT32_RTC_CR));
  rtcinfo("     ISR: %08x\n", getreg32(AT32_RTC_ISR));
  rtcinfo("    PRER: %08x\n", getreg32(AT32_RTC_PRER));
  rtcinfo("    WUTR: %08x\n", getreg32(AT32_RTC_WUTR));
  rtcinfo("  ALRMAR: %08x\n", getreg32(AT32_RTC_ALRMAR));
  rtcinfo("  ALRMBR: %08x\n", getreg32(AT32_RTC_ALRMBR));
  rtcinfo("  SHIFTR: %08x\n", getreg32(AT32_RTC_SHIFTR));
  rtcinfo("    TSTR: %08x\n", getreg32(AT32_RTC_TSTR));
  rtcinfo("    TSDR: %08x\n", getreg32(AT32_RTC_TSDR));
  rtcinfo("   TSSSR: %08x\n", getreg32(AT32_RTC_TSSSR));
  rtcinfo("    CALR: %08x\n", getreg32(AT32_RTC_CALR));
  rtcinfo("   TAFCR: %08x\n", getreg32(AT32_RTC_TAFCR));
  rtcinfo("ALRMASSR: %08x\n", getreg32(AT32_RTC_ALRMASSR));
  rtcinfo("ALRMBSSR: %08x\n", getreg32(AT32_RTC_ALRMBSSR));
  rtcinfo("MAGICREG: %08x\n", getreg32(RTC_MAGIC_REG));

  rtc_state =
    ((getreg32(AT32_EXTI_RTSR) & EXTI_RTC_ALARM) ? 0x1000 : 0) |
    ((getreg32(AT32_EXTI_FTSR) & EXTI_RTC_ALARM) ? 0x0100 : 0) |
    ((getreg32(AT32_EXTI_IMR)  & EXTI_RTC_ALARM) ? 0x0010 : 0) |
    ((getreg32(AT32_EXTI_EMR)  & EXTI_RTC_ALARM) ? 0x0001 : 0);
...
static void rtc_wprunlock(void)
{
  ...
  putreg32(0xca, AT32_RTC_WPR);
  putreg32(0x53, AT32_RTC_WPR);
}
...
static int rtc_synchwait(void)
{
  ...
  regval  = getreg32(AT32_RTC_ISR);
  regval &= ~RTC_ISR_RSF;
  putreg32(regval, AT32_RTC_ISR);
  ...
  if ((regval & RTC_ISR_INITF) == 0)
    {
      ...
      putreg32(RTC_ISR_INIT, AT32_RTC_ISR);
  ...
  if (ret == OK)
    {
      ...
      regval = getreg32(AT32_RTC_CR);
      regval &= ~RTC_CR_FMT;
      putreg32(regval, AT32_RTC_CR);

      /* Configure RTC pre-scaler with the required values */

#ifdef CONFIG_AT32_RTC_HSECLOCK
...
#ifdef CONFIG_RTC_ALARM
static int at32_rtc_alarm_handler(int irq, void *context, void *arg)
{
      ...
      if ((cr & RTC_CR_ALRBIE) != 0)
        {
          ...
          isr  = getreg32(AT32_RTC_ISR) & ~RTC_ISR_ALRBF;
          putreg32(isr, AT32_RTC_ISR);
        }
    }
#endif
...
/****************************************************************************
 * Name: at32_rtchw_set_alrmXr X is a or b
 *
 * Description:
...
static int rtchw_set_alrmar(rtc_alarmreg_t alarmreg)
{
  ...
  modifyreg32(AT32_RTC_CR, (RTC_CR_ALRAE | RTC_CR_ALRAIE), 0);
  ...
  putreg32(alarmreg, AT32_RTC_ALRMAR);
  rtcinfo("  ALRMAR: %08" PRIx32 "\n", getreg32(AT32_RTC_ALRMAR));

  /* Enable RTC alarm */

  modifyreg32(AT32_RTC_CR, 0, (RTC_CR_ALRAE | RTC_CR_ALRAIE));
...
static int rtchw_set_alrmbr(rtc_alarmreg_t alarmreg)
{
  ...
  modifyreg32(AT32_RTC_CR, (RTC_CR_ALRBE | RTC_CR_ALRBIE), 0);
  ...
  putreg32(alarmreg, AT32_RTC_ALRMBR);
  rtcinfo("  ALRMBR: %08x\n", getreg32(AT32_RTC_ALRMBR));

  /* Enable RTC alarm B */

  modifyreg32(AT32_RTC_CR, 0, (RTC_CR_ALRBE | RTC_CR_ALRBIE));
  ...
  if (!g_alarm_enabled)
    {
      ...
      at32_exti_alarm(true, false, true, at32_rtc_alarm_handler, NULL);
...
/****************************************************************************
 * Name: at32_rtc_getalarmdatetime
 *
 * Description:
...
#ifdef CONFIG_RTC_ALARM
static int at32_rtc_getalarmdatetime(rtc_alarmreg_t reg, struct tm *tp)
{
  uint32_t data;
  uint32_t tmp;
  ...
  /* Convert the RTC time to fields in struct tm format.  All of the AT32
...
int up_rtc_initialize(void)
{
   ...
   * unconditionally in at32f40xxx_rcc.c on power up.  This done
  ...
  if (regval != RTC_MAGIC && regval != RTC_MAGIC_TIME_SET)
    {
      ...
      modifyreg32(AT32_RCC_XXX, RCC_XXX_RTCSEL_MASK, RCC_XXX_RTCSEL_HSE);

#elif defined(CONFIG_AT32_RTC_LSICLOCK)
      /* Use the LSI clock as the input to the RTC block */

      rtc_dumpregs("On reset LSI");
      modifyreg32(AT32_RCC_XXX, RCC_XXX_RTCSEL_MASK, RCC_XXX_RTCSEL_LSI);

#elif defined(CONFIG_AT32_RTC_LSECLOCK)
      /* Use the LSE clock as the input to the RTC block */

      rtc_dumpregs("On reset LSE");
      modifyreg32(AT32_RCC_XXX, RCC_XXX_RTCSEL_MASK, RCC_XXX_RTCSEL_LSE);

#endif
      /* Enable the RTC Clock by setting the RTCEN bit in the RCC register */

      modifyreg32(AT32_RCC_XXX, 0, RCC_XXX_RTCEN);
    }
  else /* The RTC is already in use: check if the clock source is changed */
    {
#if defined(CONFIG_AT32_RTC_HSECLOCK) || defined(CONFIG_AT32_RTC_LSICLOCK) || \
    defined(CONFIG_AT32_RTC_LSECLOCK)

      uint32_t clksrc = getreg32(AT32_RCC_XXX);

      rtc_dumpregs("On reset warm");

#if defined(CONFIG_AT32_RTC_HSECLOCK)
...
#endif
        {
          tr_bkp = getreg32(AT32_RTC_TR);
          dr_bkp = getreg32(AT32_RTC_DR);
          ...
          modifyreg32(AT32_RCC_XXX, RCC_XXX_RTCSEL_MASK,
                      RCC_XXX_RTCSEL_HSE);
          ...
          putreg32(tr_bkp, AT32_RTC_TR);
          putreg32(dr_bkp, AT32_RTC_DR);
...
#endif
{
  ...
  do
    {
      dr  = getreg32(AT32_RTC_DR);
      tr  = getreg32(AT32_RTC_TR);
#ifdef CONFIG_AT32_HAVE_RTC_SUBSECONDS
      ssr = getreg32(AT32_RTC_SSR);
      tmp = getreg32(AT32_RTC_TR);
      if (tmp != tr)
        {
          continue;
        }
   ...
   * Month: AT32 is 1-12, struct tm is 0-11.
   * Years: AT32 is 00-99, struct tm is years since 1900.
   * WeekDay: AT32 is 1 = Mon - 7 = Sun
   *
   * Issue:  I am not sure what the AT32 years mean.  Are these the
  ...
  if (nsec)
    {
      ...
      prediv_s   = getreg32(AT32_RTC_PRER) & RTC_PRER_PREDIV_S_MASK;
...
#endif /* CONFIG_AT32_HAVE_RTC_SUBSECONDS */
...
int up_rtc_getdatetime(struct tm *tp)
{
  return at32_rtc_getdatetime_with_subseconds(tp, NULL);
}
#endif
  ...
  if (ret == OK)
    {
      ...
      putreg32(tr, AT32_RTC_TR);
      putreg32(dr, AT32_RTC_DR);
...
int up_rtc_settime(const struct timespec *tp)
{
  ...
  return at32_rtc_setdatetime(&newtime);
}
...
int at32_rtc_setalarm(struct alm_setalarm_s *alminfo)
{
   ...
   * all AT32 fields match the fields of struct tm in this case.  Notice
  ...
  switch (alminfo->ar_id)
    {
      case RTC_ALARMA:
        {
          alarmreg = AT32_RTC_ALRMAR;
          ret = at32_rtc_getalarmdatetime(alarmreg,
