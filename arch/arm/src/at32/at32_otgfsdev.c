...
#include "at32_otgfs.h"

#if defined(CONFIG_USBDEV) && (defined(CONFIG_AT32_OTGFS))
/****************************************************************************
...
#  undef CONFIG_AT32_USBDEV_REGDEBUG
#endif
...
#  define CONFIG_USBDEV_EP0_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP1_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP1_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP2_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP2_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP3_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP3_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP4_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP4_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP5_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP5_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP6_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP6_TXFIFO_SIZE 96
#endif

#ifndef CONFIG_USBDEV_EP7_TXFIFO_SIZE
#  define CONFIG_USBDEV_EP7_TXFIFO_SIZE 96
#endif
    ...
    CONFIG_USBDEV_EP0_TXFIFO_SIZE + CONFIG_USBDEV_EP1_TXFIFO_SIZE + \
     ...
     CONFIG_USBDEV_EP6_TXFIFO_SIZE + CONFIG_USBDEV_EP7_TXFIFO_SIZE ) > 1280
...
#define AT32_RXFIFO_BYTES     ((CONFIG_USBDEV_RXFIFO_SIZE + 3) & ~3)
#define AT32_RXFIFO_WORDS     ((CONFIG_USBDEV_RXFIFO_SIZE + 3) >> 2)

#define AT32_EP0_TXFIFO_BYTES ((CONFIG_USBDEV_EP0_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP0_TXFIFO_WORDS ((CONFIG_USBDEV_EP0_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP0_TXFIFO_WORDS < 16 || AT32_EP0_TXFIFO_WORDS > 256
#  error "CONFIG_USBDEV_EP0_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP1_TXFIFO_BYTES ((CONFIG_USBDEV_EP1_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP1_TXFIFO_WORDS ((CONFIG_USBDEV_EP1_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP1_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP1_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP2_TXFIFO_BYTES ((CONFIG_USBDEV_EP2_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP2_TXFIFO_WORDS ((CONFIG_USBDEV_EP2_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP2_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP2_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP3_TXFIFO_BYTES ((CONFIG_USBDEV_EP3_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP3_TXFIFO_WORDS ((CONFIG_USBDEV_EP3_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP3_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP3_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP4_TXFIFO_BYTES ((CONFIG_USBDEV_EP4_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP4_TXFIFO_WORDS ((CONFIG_USBDEV_EP4_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP4_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP4_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP5_TXFIFO_BYTES ((CONFIG_USBDEV_EP5_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP5_TXFIFO_WORDS ((CONFIG_USBDEV_EP5_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP5_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP5_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP6_TXFIFO_BYTES ((CONFIG_USBDEV_EP6_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP6_TXFIFO_WORDS ((CONFIG_USBDEV_EP6_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP6_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP6_TXFIFO_SIZE is out of range"
#endif

#define AT32_EP7_TXFIFO_BYTES ((CONFIG_USBDEV_EP7_TXFIFO_SIZE + 3) & ~3)
#define AT32_EP7_TXFIFO_WORDS ((CONFIG_USBDEV_EP7_TXFIFO_SIZE + 3) >> 2)

#if AT32_EP7_TXFIFO_WORDS < 16
#  error "CONFIG_USBDEV_EP7_TXFIFO_SIZE is out of range"
#endif

#if defined(CONFIG_AT32_AT32F446) || defined(CONFIG_AT32_AT32F469)
...
#  define OTGFS_GINT_RESERVED ( OTGFS_GINT_RES16 | OTGFS_GINTMSK_EPMISM )
...
#define AT32_TRACEERR_ALLOCFAIL            0x01
#define AT32_TRACEERR_BADCLEARFEATURE      0x02
#define AT32_TRACEERR_BADDEVGETSTATUS      0x03
#define AT32_TRACEERR_BADEPNO              0x04
#define AT32_TRACEERR_BADEPGETSTATUS       0x05
#define AT32_TRACEERR_BADGETCONFIG         0x06
#define AT32_TRACEERR_BADGETSETDESC        0x07
#define AT32_TRACEERR_BADGETSTATUS         0x08
#define AT32_TRACEERR_BADSETADDRESS        0x09
#define AT32_TRACEERR_BADSETCONFIG         0x0a
#define AT32_TRACEERR_BADSETFEATURE        0x0b
#define AT32_TRACEERR_BADTESTMODE          0x0c
#define AT32_TRACEERR_BINDFAILED           0x0d
#define AT32_TRACEERR_DISPATCHSTALL        0x0e
#define AT32_TRACEERR_DRIVER               0x0f
#define AT32_TRACEERR_DRIVERREGISTERED     0x10
#define AT32_TRACEERR_EP0NOSETUP           0x11
#define AT32_TRACEERR_EP0SETUPSTALLED      0x12
#define AT32_TRACEERR_EPINNULLPACKET       0x13
#define AT32_TRACEERR_EPINUNEXPECTED       0x14
#define AT32_TRACEERR_EPOUTNULLPACKET      0x15
#define AT32_TRACEERR_EPOUTUNEXPECTED      0x16
#define AT32_TRACEERR_INVALIDCTRLREQ       0x17
#define AT32_TRACEERR_INVALIDPARMS         0x18
#define AT32_TRACEERR_IRQREGISTRATION      0x19
#define AT32_TRACEERR_NOEP                 0x1a
#define AT32_TRACEERR_NOTCONFIGURED        0x1b
#define AT32_TRACEERR_EPOUTQEMPTY          0x1c
#define AT32_TRACEERR_EPINREQEMPTY         0x1d
#define AT32_TRACEERR_NOOUTSETUP           0x1e
#define AT32_TRACEERR_POLLTIMEOUT          0x1f

/* Trace interrupt codes */

#define AT32_TRACEINTID_USB                1       /* USB Interrupt entry/exit */
#define AT32_TRACEINTID_INTPENDING         2       /* On each pass through the loop */

#define AT32_TRACEINTID_EPOUT              (10 + 0) /* First level interrupt decode */
#define AT32_TRACEINTID_EPIN               (10 + 1)
#define AT32_TRACEINTID_MISMATCH           (10 + 2)
#define AT32_TRACEINTID_WAKEUP             (10 + 3)
#define AT32_TRACEINTID_SUSPEND            (10 + 4)
#define AT32_TRACEINTID_SOF                (10 + 5)
#define AT32_TRACEINTID_RXFIFO             (10 + 6)
#define AT32_TRACEINTID_DEVRESET           (10 + 7)
#define AT32_TRACEINTID_ENUMDNE            (10 + 8)
#define AT32_TRACEINTID_IISOIXFR           (10 + 9)
#define AT32_TRACEINTID_IISOOXFR           (10 + 10)
#define AT32_TRACEINTID_SRQ                (10 + 11)
#define AT32_TRACEINTID_OTG                (10 + 12)

#define AT32_TRACEINTID_EPOUT_XFRC         (40 + 0) /* EPOUT second level decode */
#define AT32_TRACEINTID_EPOUT_EPDISD       (40 + 1)
#define AT32_TRACEINTID_EPOUT_SETUP        (40 + 2)
#define AT32_TRACEINTID_DISPATCH           (40 + 3)

#define AT32_TRACEINTID_GETSTATUS          (50 + 0) /* EPOUT third level decode */
#define AT32_TRACEINTID_EPGETSTATUS        (50 + 1)
#define AT32_TRACEINTID_DEVGETSTATUS       (50 + 2)
#define AT32_TRACEINTID_IFGETSTATUS        (50 + 3)
#define AT32_TRACEINTID_CLEARFEATURE       (50 + 4)
#define AT32_TRACEINTID_SETFEATURE         (50 + 5)
#define AT32_TRACEINTID_SETADDRESS         (50 + 6)
#define AT32_TRACEINTID_GETSETDESC         (50 + 7)
#define AT32_TRACEINTID_GETCONFIG          (50 + 8)
#define AT32_TRACEINTID_SETCONFIG          (50 + 9)
#define AT32_TRACEINTID_GETSETIF           (50 + 10)
#define AT32_TRACEINTID_SYNCHFRAME         (50 + 11)

#define AT32_TRACEINTID_EPIN_XFRC          (70 + 0) /* EPIN second level decode */
#define AT32_TRACEINTID_EPIN_TOC           (70 + 1)
#define AT32_TRACEINTID_EPIN_ITTXFE        (70 + 2)
#define AT32_TRACEINTID_EPIN_EPDISD        (70 + 3)
#define AT32_TRACEINTID_EPIN_TXFE          (70 + 4)

#define AT32_TRACEINTID_EPIN_EMPWAIT       (80 + 0) /* EPIN second level decode */

#define AT32_TRACEINTID_OUTNAK             (90 + 0) /* RXFLVL second level decode */
#define AT32_TRACEINTID_OUTRECVD           (90 + 1)
#define AT32_TRACEINTID_OUTDONE            (90 + 2)
#define AT32_TRACEINTID_SETUPDONE          (90 + 3)
#define AT32_TRACEINTID_SETUPRECVD         (90 + 4)

/* Endpoints ****************************************************************/

/* Number of endpoints */

#define AT32_NENDPOINTS             (8)          /* ep0-3 x 2 for IN and OUT */

/* Odd physical endpoint numbers are IN; even are OUT */

#define AT32_EPPHYIN2LOG(epphy)     ((uint8_t)(epphy)|USB_DIR_IN)
#define AT32_EPPHYOUT2LOG(epphy)    ((uint8_t)(epphy)|USB_DIR_OUT)
...
#define AT32_EP_AVAILABLE           (0x0f)       /* All available endpoints */

/* Maximum packet sizes for full speed endpoints */

#define AT32_MAXPACKET              (64)         /* Max packet size (1-64) */

/* Delays *******************************************************************/

#define AT32_READY_DELAY            400000
#define AT32_FLUSH_DELAY            400000

/* Request queue operations *************************************************/

#define at32_rqempty(ep)            ((ep)->head == NULL)
#define at32_rqpeek(ep)             ((ep)->head)
...
enum at32_ep0state_e
{
  ...
  EP0STATE_SETUP_OUT,      /* OUT SETUP packet received.  Waiting for the DATA
                            ...
                            *   SET:    Set in at32_rxinterrupt() when SETUP OUT
                            *           packet is received.
                            *   TESTED: In at32_ep0out_receive()
                            */
  EP0STATE_SETUP_READY,    /* IN SETUP packet received -OR- OUT SETUP packet and
                            ...
                            *   SET:    (1) at32_ep0out_receive() when the OUT
                            *           SETUP data phase completes, or (2)
                            *           at32_rxinterrupt() when an IN SETUP is
                            *           packet received.
                            *   TESTED: Tested in at32_epout_interrupt() when
                            *           SETUP phase is done to see if the SETUP
                            *           command is ready to be processed.  Also
                            *           tested in at32_ep0out_setup() just to
                            *           double-check that we have a SETUP request
                            *           and any accompanying data.
                            */
  EP0STATE_SETUP_PROCESS,  /* SETUP Packet is being processed by at32_ep0out_setup():
  ...
  EP0STATE_SETUPRESPONSE,  /* Short SETUP response write (without a USB request):
                            ...
                            *           at32_ep0in_setupresponse()
                            *   TESTED: Never
                            */
  EP0STATE_DATA_IN,        /* Waiting for data out stage (with a USB request):
                            *   SET:    In at32_epin_request() when a write
                            *           request is processed on EP0.
                            *   TESTED: In at32_epin() to see if we should
                            *           revert to SETUP processing.
                            */
  EP0STATE_DATA_OUT        /* Waiting for data in phase to complete ( with a
                            * USB request)
                            *   SET:    In at32_epout_request() when a read
                            *           request is processed on EP0.
                            *   TESTED: In at32_epout() to see if we should
                            *           revert to SETUP processing
                            */
};
...
struct at32_req_s
{
  ...
  struct at32_req_s  *flink;          /* Supports a singly linked list */
};
...
struct at32_ep_s
{
  ...
  /* AT32-specific fields */

  struct at32_usbdev_s *dev;          /* Reference to private driver data */
  struct at32_req_s    *head;         /* Request list for this endpoint */
  struct at32_req_s    *tail;
...
struct at32_usbdev_s
{
  /* Common device fields.  This must be the first thing defined in the
   * structure so that it is possible to simply cast from struct usbdev_s
   * to struct at32_usbdev_s.
   */
  ...
  uint8_t                 devstate:4;    /* See enum at32_devstate_e */
  uint8_t                 ep0state:4;    /* See enum at32_ep0state_e */
  ...
  struct at32_ep_s       epin[AT32_NENDPOINTS];
  struct at32_ep_s       epout[AT32_NENDPOINTS];
};
...
#ifdef CONFIG_AT32_USBDEV_REGDEBUG
...
static struct at32_req_s *at32_req_remfirst(
                               struct at32_ep_s *privep);
static bool       at32_req_addlast(struct at32_ep_s *privep,
                    struct at32_req_s *req);

/* Low level data transfers and request operations **************************/

/* Special endpoint 0 data transfer logic */

static void        at32_ep0in_setupresponse(struct at32_usbdev_s *priv,
                     uint8_t *data, uint32_t nbytes);
static inline void at32_ep0in_transmitzlp(struct at32_usbdev_s *priv);
static void        at32_ep0in_activate(void);

static void        at32_ep0out_ctrlsetup(struct at32_usbdev_s *priv);

/* IN request and TxFIFO handling */

static void        at32_txfifo_write(struct at32_ep_s *privep,
                     uint8_t *buf, int nbytes);
static void        at32_epin_transfer(struct at32_ep_s *privep,
                     uint8_t *buf, int nbytes);
static void        at32_epin_request(struct at32_usbdev_s *priv,
                     struct at32_ep_s *privep);

/* OUT request and RxFIFO handling */

static void        at32_rxfifo_read(struct at32_ep_s *privep,
                     uint8_t *dest, uint16_t len);
static void        at32_rxfifo_discard(struct at32_ep_s *privep,
                                        int len);
static void        at32_epout_complete(struct at32_usbdev_s *priv,
                     struct at32_ep_s *privep);
static inline void at32_ep0out_receive(struct at32_ep_s *privep,
                                        int bcnt);
static inline void at32_epout_receive(struct at32_ep_s *privep,
                                       int bcnt);
static void        at32_epout_request(struct at32_usbdev_s *priv,
                     struct at32_ep_s *privep);

/* General request handling */

static void        at32_ep_flush(struct at32_ep_s *privep);
static void        at32_req_complete(struct at32_ep_s *privep,
                     int16_t result);
static void        at32_req_cancel(struct at32_ep_s *privep,
                     int16_t status);

/* Interrupt handling *******************************************************/

static struct      at32_ep_s *at32_ep_findbyaddr(
                               struct at32_usbdev_s *priv,
                               uint16_t eplog);
static int         at32_req_dispatch(struct at32_usbdev_s *priv,
                     const struct usb_ctrlreq_s *ctrl);
static void        at32_usbreset(struct at32_usbdev_s *priv);

/* Second level OUT endpoint interrupt processing */

static inline void at32_ep0out_testmode(struct at32_usbdev_s *priv,
                     uint16_t index);
static inline void at32_ep0out_stdrequest(struct at32_usbdev_s *priv,
                     struct at32_ctrlreq_s *ctrlreq);
static inline void at32_ep0out_setup(struct at32_usbdev_s *priv);
static inline void at32_epout(struct at32_usbdev_s *priv,
                     uint8_t epno);
static inline void at32_epout_interrupt(struct at32_usbdev_s *priv);

/* Second level IN endpoint interrupt processing */

static inline void at32_epin_runtestmode(struct at32_usbdev_s *priv);
static inline void at32_epin(struct at32_usbdev_s *priv, uint8_t epno);
static inline void at32_epin_txfifoempty(struct at32_usbdev_s *priv,
                                          int epno);
static inline void at32_epin_interrupt(struct at32_usbdev_s *priv);

/* Other second level interrupt processing */

static inline void at32_resumeinterrupt(struct at32_usbdev_s *priv);
static inline void at32_suspendinterrupt(struct at32_usbdev_s *priv);
static inline void at32_rxinterrupt(struct at32_usbdev_s *priv);
static inline void at32_enuminterrupt(struct at32_usbdev_s *priv);
#ifdef CONFIG_USBDEV_ISOCHRONOUS
static inline void at32_isocininterrupt(struct at32_usbdev_s *priv);
static inline void at32_isocoutinterrupt(struct at32_usbdev_s *priv);
#endif
#ifdef CONFIG_USBDEV_VBUSSENSING
static inline void at32_sessioninterrupt(struct at32_usbdev_s *priv);
static inline void at32_otginterrupt(struct at32_usbdev_s *priv);
#endif

/* First level interrupt processing */

static int         at32_usbinterrupt(int irq, void *context,
                                      void *arg);
...
static void        at32_enablegonak(struct at32_ep_s *privep);
static void        at32_disablegonak(struct at32_ep_s *privep);

/* Endpoint configuration */

static int         at32_epout_configure(struct at32_ep_s *privep,
                     uint8_t eptype, uint16_t maxpacket);
static int         at32_epin_configure(struct at32_ep_s *privep,
                     uint8_t eptype, uint16_t maxpacket);
static int         at32_ep_configure(struct usbdev_ep_s *ep,
                     const struct usb_epdesc_s *desc, bool last);
static void        at32_ep0_configure(struct at32_usbdev_s *priv);

/* Endpoint disable */

static void        at32_epout_disable(struct at32_ep_s *privep);
static void        at32_epin_disable(struct at32_ep_s *privep);
static int         at32_ep_disable(struct usbdev_ep_s *ep);

/* Endpoint request management */

static struct usbdev_req_s *at32_ep_allocreq(
                                struct usbdev_ep_s *ep);
static void        at32_ep_freereq(struct usbdev_ep_s *ep,
                     struct usbdev_req_s *);

/* Endpoint buffer management */

#ifdef CONFIG_USBDEV_DMA
static void       *at32_ep_allocbuffer(struct usbdev_ep_s *ep,
                                        uint16_t bytes);
static void        at32_ep_freebuffer(struct usbdev_ep_s *ep,
                                       void *buf);
#endif

/* Endpoint request submission */

static int         at32_ep_submit(struct usbdev_ep_s *ep,
                     struct usbdev_req_s *req);

/* Endpoint request cancellation */

static int         at32_ep_cancel(struct usbdev_ep_s *ep,
                     struct usbdev_req_s *req);

/* Stall handling */

static int         at32_epout_setstall(struct at32_ep_s *privep);
static int         at32_epin_setstall(struct at32_ep_s *privep);
static int         at32_ep_setstall(struct at32_ep_s *privep);
static int         at32_ep_clrstall(struct at32_ep_s *privep);
static int         at32_ep_stall(struct usbdev_ep_s *ep, bool resume);
static void        at32_ep0_stall(struct at32_usbdev_s *priv);

/* Endpoint allocation */

static struct usbdev_ep_s *at32_ep_alloc(struct usbdev_s *dev,
                     uint8_t epno, bool in, uint8_t eptype);
static void        at32_ep_free(struct usbdev_s *dev,
                     struct usbdev_ep_s *ep);

/* USB device controller operations *****************************************/

static int         at32_getframe(struct usbdev_s *dev);
static int         at32_wakeup(struct usbdev_s *dev);
static int         at32_selfpowered(struct usbdev_s *dev, bool selfpowered);
static int         at32_pullup(struct usbdev_s *dev, bool enable);
static void        at32_setaddress(struct at32_usbdev_s *priv,
                     uint16_t address);
static int         at32_txfifo_flush(uint32_t txfnum);
static int         at32_rxfifo_flush(void);

/* Initialization ***********************************************************/

static void        at32_swinitialize(struct at32_usbdev_s *priv);
static void        at32_hwinitialize(struct at32_usbdev_s *priv);
...
static struct at32_usbdev_s g_otgfsdev;

static const struct usbdev_epops_s g_epops =
{
  .configure   = at32_ep_configure,
  .disable     = at32_ep_disable,
  .allocreq    = at32_ep_allocreq,
  .freereq     = at32_ep_freereq,
#ifdef CONFIG_USBDEV_DMA
  .allocbuffer = at32_ep_allocbuffer,
  .freebuffer  = at32_ep_freebuffer,
#endif
  .submit      = at32_ep_submit,
  .cancel      = at32_ep_cancel,
  .stall       = at32_ep_stall,
};

static const struct usbdev_ops_s g_devops =
{
  .allocep     = at32_ep_alloc,
  .freeep      = at32_ep_free,
  .getframe    = at32_getframe,
  .wakeup      = at32_wakeup,
  .selfpowered = at32_selfpowered,
  .pullup      = at32_pullup,
};
...
{
  TRACE_STR(AT32_TRACEERR_ALLOCFAIL),
  TRACE_STR(AT32_TRACEERR_BADCLEARFEATURE),
  TRACE_STR(AT32_TRACEERR_BADDEVGETSTATUS),
  TRACE_STR(AT32_TRACEERR_BADEPNO),
  TRACE_STR(AT32_TRACEERR_BADEPGETSTATUS),
  TRACE_STR(AT32_TRACEERR_BADGETCONFIG),
  TRACE_STR(AT32_TRACEERR_BADGETSETDESC),
  TRACE_STR(AT32_TRACEERR_BADGETSTATUS),
  TRACE_STR(AT32_TRACEERR_BADSETADDRESS),
  TRACE_STR(AT32_TRACEERR_BADSETCONFIG),
  TRACE_STR(AT32_TRACEERR_BADSETFEATURE),
  TRACE_STR(AT32_TRACEERR_BADTESTMODE),
  TRACE_STR(AT32_TRACEERR_BINDFAILED),
  TRACE_STR(AT32_TRACEERR_DISPATCHSTALL),
  TRACE_STR(AT32_TRACEERR_DRIVER),
  TRACE_STR(AT32_TRACEERR_DRIVERREGISTERED),
  TRACE_STR(AT32_TRACEERR_EP0NOSETUP),
  TRACE_STR(AT32_TRACEERR_EP0SETUPSTALLED),
  TRACE_STR(AT32_TRACEERR_EPINNULLPACKET),
  TRACE_STR(AT32_TRACEERR_EPINUNEXPECTED),
  TRACE_STR(AT32_TRACEERR_EPOUTNULLPACKET),
  TRACE_STR(AT32_TRACEERR_EPOUTUNEXPECTED),
  TRACE_STR(AT32_TRACEERR_INVALIDCTRLREQ),
  TRACE_STR(AT32_TRACEERR_INVALIDPARMS),
  TRACE_STR(AT32_TRACEERR_IRQREGISTRATION),
  TRACE_STR(AT32_TRACEERR_NOEP),
  TRACE_STR(AT32_TRACEERR_NOTCONFIGURED),
  TRACE_STR(AT32_TRACEERR_EPOUTQEMPTY),
  TRACE_STR(AT32_TRACEERR_EPINREQEMPTY),
  TRACE_STR(AT32_TRACEERR_NOOUTSETUP),
  TRACE_STR(AT32_TRACEERR_POLLTIMEOUT),
  TRACE_STR_END
};
#endif
...
{
  TRACE_STR(AT32_TRACEINTID_USB),
  TRACE_STR(AT32_TRACEINTID_INTPENDING),
  TRACE_STR(AT32_TRACEINTID_EPOUT),
  TRACE_STR(AT32_TRACEINTID_EPIN),
  TRACE_STR(AT32_TRACEINTID_MISMATCH),
  TRACE_STR(AT32_TRACEINTID_WAKEUP),
  TRACE_STR(AT32_TRACEINTID_SUSPEND),
  TRACE_STR(AT32_TRACEINTID_SOF),
  TRACE_STR(AT32_TRACEINTID_RXFIFO),
  TRACE_STR(AT32_TRACEINTID_DEVRESET),
  TRACE_STR(AT32_TRACEINTID_ENUMDNE),
  TRACE_STR(AT32_TRACEINTID_IISOIXFR),
  TRACE_STR(AT32_TRACEINTID_IISOOXFR),
  TRACE_STR(AT32_TRACEINTID_SRQ),
  TRACE_STR(AT32_TRACEINTID_OTG),
  TRACE_STR(AT32_TRACEINTID_EPOUT_XFRC),
  TRACE_STR(AT32_TRACEINTID_EPOUT_EPDISD),
  TRACE_STR(AT32_TRACEINTID_EPOUT_SETUP),
  TRACE_STR(AT32_TRACEINTID_DISPATCH),
  TRACE_STR(AT32_TRACEINTID_GETSTATUS),
  TRACE_STR(AT32_TRACEINTID_EPGETSTATUS),
  TRACE_STR(AT32_TRACEINTID_DEVGETSTATUS),
  TRACE_STR(AT32_TRACEINTID_IFGETSTATUS),
  TRACE_STR(AT32_TRACEINTID_CLEARFEATURE),
  TRACE_STR(AT32_TRACEINTID_SETFEATURE),
  TRACE_STR(AT32_TRACEINTID_SETADDRESS),
  TRACE_STR(AT32_TRACEINTID_GETSETDESC),
  TRACE_STR(AT32_TRACEINTID_GETCONFIG),
  TRACE_STR(AT32_TRACEINTID_SETCONFIG),
  TRACE_STR(AT32_TRACEINTID_GETSETIF),
  TRACE_STR(AT32_TRACEINTID_SYNCHFRAME),
  TRACE_STR(AT32_TRACEINTID_EPIN_XFRC),
  TRACE_STR(AT32_TRACEINTID_EPIN_TOC),
  TRACE_STR(AT32_TRACEINTID_EPIN_ITTXFE),
  TRACE_STR(AT32_TRACEINTID_EPIN_EPDISD),
  TRACE_STR(AT32_TRACEINTID_EPIN_TXFE),
  TRACE_STR(AT32_TRACEINTID_EPIN_EMPWAIT),
  TRACE_STR(AT32_TRACEINTID_OUTNAK),
  TRACE_STR(AT32_TRACEINTID_OUTRECVD),
  TRACE_STR(AT32_TRACEINTID_OUTDONE),
  TRACE_STR(AT32_TRACEINTID_SETUPDONE),
  TRACE_STR(AT32_TRACEINTID_SETUPRECVD),
  TRACE_STR_END
};
#endif
...
static struct at32_req_s *at32_req_remfirst(
                               struct at32_ep_s *privep)
{
  struct at32_req_s *ret = privep->head;
...
{
  at32_epin_transfer(&priv->epin[EP0], buf, nbytes);
  priv->ep0state = EP0STATE_SETUPRESPONSE;
  at32_ep0out_ctrlsetup(priv);
}

/****************************************************************************
 * Name: at32_ep0in_transmitzlp
 *
 * Description:
...
static inline void at32_ep0in_transmitzlp(struct at32_usbdev_s *priv)
{
  at32_ep0in_setupresponse(priv, NULL, 0);
}

/****************************************************************************
 * Name: at32_ep0in_activate
 *
 * Description:
...
static void at32_ep0in_activate(void)
{
  ...
  regval  = at32_getreg(AT32_OTGFS_DIEPCTL0);
...
#  error "Unsupported value of CONFIG_USBDEV_EP0_MAXSIZE"
  ...
  at32_putreg(regval, AT32_OTGFS_DIEPCTL0);

  /* Clear global IN NAK */

  regval  = at32_getreg(AT32_OTGFS_DCTL);
  regval |= OTGFS_DCTL_CGINAK;
  at32_putreg(regval, AT32_OTGFS_DCTL);
}

/****************************************************************************
 * Name: at32_ep0out_ctrlsetup
 *
 * Description:
...
static void at32_ep0out_ctrlsetup(struct at32_usbdev_s *priv)
{
  ...
  at32_putreg(regval, AT32_OTGFS_DOEPTSIZ0);

  /* Then clear NAKing and enable the transfer */

  regval  = at32_getreg(AT32_OTGFS_DOEPCTL0);
  regval |= (OTGFS_DOEPCTL0_CNAK | OTGFS_DOEPCTL0_EPENA);
  at32_putreg(regval, AT32_OTGFS_DOEPCTL0);
}

/****************************************************************************
 * Name: at32_txfifo_write
 *
 * Description:
                               ...
                               uint8_t *buf, int nbytes)
{
  ...
  regaddr = AT32_OTGFS_DFIFO_DEP(privep->epphy);
  ...
  for (i = 0; i < nwords; i++)
    {
      ...
      at32_putreg(regval, regaddr);
    }
}

/****************************************************************************
 * Name: at32_epin_transfer
 *
 * Description:
                                ...
                                uint8_t *buf, int nbytes)
{
  ...
  regval = at32_getreg(AT32_OTGFS_DIEPTSIZ(privep->epphy));
  ...
  at32_putreg(regval, AT32_OTGFS_DIEPTSIZ(privep->epphy));

  /* Read the DIEPCTLx register */

  regval = at32_getreg(AT32_OTGFS_DIEPCTL(privep->epphy));
  ...
  if (privep->eptype == USB_EP_ATTR_XFER_ISOC)
    {
      ...
      uint32_t status = at32_getreg(AT32_OTGFS_DSTS);
  ...
  at32_putreg(regval, AT32_OTGFS_DIEPCTL(privep->epphy));
  ...
  at32_txfifo_write(privep, buf, nbytes);
}

/****************************************************************************
 * Name: at32_epin_request
 *
 * Description:
                               ...
                               struct at32_ep_s *privep)
{
   ...
   * events:
   *
   * 1. From at32_epin as part of the transfer complete interrupt processing
   ...
   * 3. From at32_epin_txfifoempty as part of the TXFE interrupt processing.
   ...
   *
   * 4. From at32_ep_submit when a new write request is received WHILE the
   *    endpoint is not active (privep->active == false).
   */

  /* Check the request from the head of the endpoint request queue */

  privreq = at32_rqpeek(privep);
  if (!privreq)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EPINREQEMPTY), privep->epphy);
      ...
      regaddr = AT32_OTGFS_DIEPCTL(privep->epphy);
      regval  = at32_getreg(regaddr);
  ...
  if (privreq->req.xfrd < privreq->req.len || privep->zlp)
#endif
    {
      ...
      regaddr = AT32_OTGFS_DTXFSTS(privep->epphy);
      ...
      if ((int)(regval & OTGFS_DTXFSTS_MASK) < nwords)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_EMPWAIT),
                   (uint16_t)regval);
          ...
          uint32_t empmsk = at32_getreg(AT32_OTGFS_DIEPEMPMSK);
          empmsk |= OTGFS_DIEPEMPMSK(privep->epphy);
          at32_putreg(empmsk, AT32_OTGFS_DIEPEMPMSK);
      ...
      at32_epin_transfer(privep, buf, nbytes);
  ...
  if (privreq->req.xfrd >= privreq->req.len && !privep->zlp)
    {
      ...
      at32_req_complete(privep, OK);
    }
}

/****************************************************************************
 * Name: at32_rxfifo_read
 *
 * Description:
                              ...
                              uint8_t *dest, uint16_t len)
{
  ...
  regaddr = AT32_OTGFS_DFIFO_DEP(EP0);
...
static void at32_rxfifo_discard(struct at32_ep_s *privep, int len)
{
  if (len > 0)
    {
      uint32_t regaddr;
      ...
      for (i = 0; i < len; i += 4)
        {
          volatile uint32_t data = at32_getreg(regaddr);
          UNUSED(data);
        }
    }
}
                                 ...
                                 struct at32_ep_s *privep)
{
  ...
  if (!privreq)
    {
      ...
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EPOUTQEMPTY), privep->epphy);
      privep->active = false;
      return;
    }
  ...
  at32_epout_request(priv, privep);
}

/****************************************************************************
 * Name: at32_ep0out_receive
 *
 * Description:
                                        ...
                                        int bcnt)
{
  ...
  priv = (struct at32_usbdev_s *)privep->ep.priv;
  ...
  if (priv->ep0state == EP0STATE_SETUP_OUT)
    {
      if (priv->ep0datlen < CONFIG_USBDEV_SETUP_MAXDATASIZE)
        {
          ...
          at32_rxfifo_read(privep, priv->ep0data, readlen);
      ...
      if (bcnt > 0)
        {
          at32_rxfifo_discard(privep,  bcnt);
          priv->ep0datlen += bcnt;
        }
      ...
      if (priv->ep0datlen >= GETUINT16(priv->ctrlreq.len))
        {
          ...
          at32_ep0out_setup(priv);
        }
      else
        {
          /* More data to come, clear NAKSTS */

          uint32_t regval  = at32_getreg(AT32_OTGFS_DOEPCTL0);
  ...
  else
    {
      ...
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_NOOUTSETUP), priv->ep0state);
      at32_rxfifo_discard(privep, bcnt);
      privep->active = false;
    }
}
  ...
  if (!privreq)
    {
      ...
      if (privep->epphy == 0)
        {
          at32_ep0out_receive(privep, bcnt);
        }
      else
        {
          ...
          usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EPOUTQEMPTY),
                   privep->epphy);
  ...
  if (!privep->active)
    {
      ...
      for (; ; )
        {
          ...
          if (!privreq)
            {
              ...
              regaddr = AT32_OTGFS_DOEPCTL(privep->epphy);
              regval  = at32_getreg(regaddr);
          ...
          if (privreq->req.len <= 0)
            {
              usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EPOUTNULLPACKET), 0);
              at32_req_complete(privep, OK);
            }
      ...
      regaddr = AT32_OTGFS_DOEPTSIZ(privep->epphy);
      regval  = at32_getreg(regaddr);
...
static void at32_ep_flush(struct at32_ep_s *privep)
{
  if (privep->isin)
    {
      at32_txfifo_flush(OTGFS_GRSTCTL_TXFNUM_D(privep->epphy));
    }
  else
    {
      at32_rxfifo_flush();
...
static void at32_req_complete(struct at32_ep_s *privep, int16_t result)
{
  struct at32_req_s *privreq;

  /* Remove the request at the head of the request list */

  privreq = at32_req_remfirst(privep);
...
static void at32_req_cancel(struct at32_ep_s *privep, int16_t status)
{
  if (!at32_rqempty(privep))
    {
      at32_ep_flush(privep);
    }

  while (!at32_rqempty(privep))
    {
      usbtrace(TRACE_COMPLETE(privep->epphy),
               (at32_rqpeek(privep))->req.xfrd);
      at32_req_complete(privep, status);
    }
}

/****************************************************************************
 * Name: at32_ep_findbyaddr
 *
 * Description:
...
static struct at32_ep_s *at32_ep_findbyaddr(struct at32_usbdev_s *priv,
                                              uint16_t eplog)
{
  struct at32_ep_s *privep;
  uint8_t epphy = USB_EPNO(eplog);

  if (epphy >= AT32_NENDPOINTS)
    {
      return NULL;
    }
                              ...
                              const struct usb_ctrlreq_s *ctrl)
{
  ...
  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_DISPATCH), 0);
  if (priv->driver)
    {
  ...
  if (ret < 0)
    {
      ...
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_DISPATCHSTALL), 0);
      priv->stalled = true;
    }
...
static void at32_usbreset(struct at32_usbdev_s *priv)
{
  ...
  at32_txfifo_flush(OTGFS_GRSTCTL_TXFNUM_D(EP0));
  ...
  priv->epavail[0] = AT32_EP_AVAILABLE;
  priv->epavail[1] = AT32_EP_AVAILABLE;

  /* Disable all end point interrupts */

  for (i = 0; i < AT32_NENDPOINTS ; i++)
    {
      /* Disable endpoint interrupts */

      at32_putreg(0xff, AT32_OTGFS_DIEPINT(i));
      at32_putreg(0xff, AT32_OTGFS_DOEPINT(i));

      /* Return write requests to the class implementation */

      privep = &priv->epin[i];
      at32_req_cancel(privep, -ESHUTDOWN);
  ...
  switch (testmode)
    {
      ...
      default:
        usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADTESTMODE), testmode);
        priv->dotest   = false;
  ...
  switch (ctrlreq->req)
    {
      case USB_REQ_GETSTATUS:
        {
          ...
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_GETSTATUS), 0);
          if (!priv->addressed ||
               ctrlreq->len != 2 ||
              ...
              switch (ctrlreq->type & USB_REQ_RECIPIENT_MASK)
                {
                  case USB_REQ_RECIPIENT_ENDPOINT:
                    {
                      usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPGETSTATUS),
                               0);
                      privep = at32_ep_findbyaddr(priv, ctrlreq->index);
                      if (!privep)
                        {
                          usbtrace(
                            TRACE_DEVERROR(AT32_TRACEERR_BADEPGETSTATUS), 0);
                          priv->stalled = true;
                        }
                      else
                        {
                          ...
                          priv->ep0data[1] = 0;
                          at32_ep0in_setupresponse(priv, priv->ep0data, 2);
                        }
                    }
                    break;
                  ...
                  case USB_REQ_RECIPIENT_INTERFACE:
                    {
                      usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_IFGETSTATUS),
                               0);
                      priv->ep0data[0] = 0;
                      priv->ep0data[1] = 0;
      ...
      case USB_REQ_CLEARFEATURE:
        {
          ...
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_CLEARFEATURE), 0);
          if (priv->addressed != 0 && ctrlreq->len == 0)
            {
              ...
              if (recipient == USB_REQ_RECIPIENT_ENDPOINT &&
                  ...
                  (privep = at32_ep_findbyaddr(priv,
                                               ctrlreq->index)) != NULL)
                {
                  at32_ep_clrstall(privep);
                  at32_ep0in_transmitzlp(priv);
                }
              ...
              else
                {
                  ...
                  at32_req_dispatch(priv, &priv->ctrlreq);
                }
            }
          else
            {
              usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADCLEARFEATURE), 0);
              priv->stalled = true;
            }
        }
        break;
      ...
      case USB_REQ_SETFEATURE:
        {
          ...
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SETFEATURE), 0);
          if (priv->addressed != 0 && ctrlreq->len == 0)
            {
              uint8_t recipient = ctrlreq->type & USB_REQ_RECIPIENT_MASK;
              if (recipient == USB_REQ_RECIPIENT_ENDPOINT &&
                ...
                {
                  at32_ep0out_testmode(priv, ctrlreq->index);
                }
                ...
                {
                  usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADSETFEATURE), 0);
                  priv->stalled = true;
                }
            }
          else
      ...
      case USB_REQ_SETADDRESS:
        {
          ...
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SETADDRESS),
                   ctrlreq->value);
          if ((ctrlreq->type & USB_REQ_RECIPIENT_MASK) ==
              ...
              priv->devstate != DEVSTATE_CONFIGURED)
            {
              ...
              at32_setaddress(priv, (uint16_t)priv->ctrlreq.value[0]);
              at32_ep0in_transmitzlp(priv);
            }
          else
            {
              usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADSETADDRESS), 0);
              priv->stalled = true;
            }
        }
        break;
      ...
      case USB_REQ_SETDESCRIPTOR:
        ...
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_GETSETDESC), 0);
          if ((ctrlreq->type & USB_REQ_RECIPIENT_MASK) ==
              USB_REQ_RECIPIENT_DEVICE)
            {
              at32_req_dispatch(priv, &priv->ctrlreq);
            ...
            {
              usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADGETSETDESC), 0);
              priv->stalled = true;
            }
        }
        break;
      ...
      case USB_REQ_GETCONFIGURATION:
        ...
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_GETCONFIG), 0);
          if (priv->addressed &&
            ...
            {
              usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADGETCONFIG), 0);
              priv->stalled = true;
            }
        }
        break;
      ...
      case USB_REQ_SETCONFIGURATION:
        ...
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SETCONFIG), 0);
          if (priv->addressed &&
              (ctrlreq->type & USB_REQ_RECIPIENT_MASK) ==
              USB_REQ_RECIPIENT_DEVICE &&
              ctrlreq->index == 0 &&
              ctrlreq->len == 0)
            {
              ...
              int ret = at32_req_dispatch(priv, &priv->ctrlreq);
            ...
            {
              usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADSETCONFIG), 0);
              priv->stalled = true;
            }
        }
        break;
      ...
      case USB_REQ_SETINTERFACE:
        ...
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_GETSETIF), 0);
          at32_req_dispatch(priv, &priv->ctrlreq);
        }
        break;
      ...
      case USB_REQ_SYNCHFRAME:
        ...
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SYNCHFRAME), 0);
        }
        break;

      default:
        {
          usbtrace(TRACE_DEVERROR(AT32_TRACEERR_INVALIDCTRLREQ), 0);
          priv->stalled = true;
        }
        break;
    }
...
static inline void at32_ep0out_setup(struct at32_usbdev_s *priv)
{
  ...
  if (priv->ep0state != EP0STATE_SETUP_READY)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EP0NOSETUP), priv->ep0state);
      return;
    }

  /* Terminate any pending requests */

  at32_req_cancel(&priv->epout[EP0], -EPROTO);
  at32_req_cancel(&priv->epin[EP0],  -EPROTO);
  ...
  if ((ctrlreq.type & USB_REQ_TYPE_MASK) != USB_REQ_TYPE_STANDARD)
  ...
  else
    {
      ...
      at32_ep0out_stdrequest(priv, &ctrlreq);
    }
  ...
  if (priv->stalled)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EP0SETUPSTALLED),
               priv->ep0state);
      at32_ep0_stall(priv);
    }
...
static inline void at32_epout(struct at32_usbdev_s *priv, uint8_t epno)
{
  struct at32_ep_s *privep;
      ...
      if (priv->ep0state == EP0STATE_DATA_OUT)
        {
          ...
          at32_epout_complete(priv, privep);
   ...
   */
    ...
    {
      at32_epout_complete(priv, &priv->epout[epno]);
    }
}

/****************************************************************************
 * Name: at32_epout_interrupt
 *
 * Description:
...
static inline void at32_epout_interrupt(struct at32_usbdev_s *priv)
{
  ...
  regval  = at32_getreg(AT32_OTGFS_DAINT);
  regval &= at32_getreg(AT32_OTGFS_DAINTMSK);
  ...
  if (daint == 0)
    {
      ...
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EPOUTUNEXPECTED),
               (uint16_t)regval);
      ...
      while (daint)
        {
          if ((daint & 1) != 0)
            {
              regval = at32_getreg(AT32_OTGFS_DOEPINT(epno));
              uinfo("DOEPINT(%d) = %08" PRIx32 "\n", epno, regval);
              at32_putreg(0xff, AT32_OTGFS_DOEPINT(epno));
            }
      ...
      if ((daint & 1) != 0)
        {
          ...
          doepint  = at32_getreg(AT32_OTGFS_DOEPINT(epno));
          doepint &= at32_getreg(AT32_OTGFS_DOEPMSK);

          /* Transfer completed interrupt.  This interrupt is triggered when
           * at32_rxinterrupt() removes the last packet data from the
          ...
          if ((doepint & OTGFS_DOEPINT_XFRC) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPOUT_XFRC),
                       (uint16_t)doepint);

              /* Clear the bit in DOEPINTn for this interrupt */

              at32_putreg(OTGFS_DOEPINT_XFRC, AT32_OTGFS_DOEPINT(epno));

              /* Handle the RX transfer data ready event */

              at32_epout(priv, epno);
            }
          ...
          if ((doepint & OTGFS_DOEPINT_EPDISD) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPOUT_EPDISD),
                       (uint16_t)doepint);

              /* Clear the bit in DOEPINTn for this interrupt */

              at32_putreg(OTGFS_DOEPINT_EPDISD, AT32_OTGFS_DOEPINT(epno));
            }
#endif
          ...
          if ((doepint & OTGFS_DOEPINT_SETUP) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPOUT_SETUP),
                       priv->ep0state);
              ...
              at32_putreg(OTGFS_DOEPINT_SETUP, AT32_OTGFS_DOEPINT(epno));
            }
        }
...
static inline void at32_epin_runtestmode(struct at32_usbdev_s *priv)
{
  uint32_t regval = at32_getreg(AT32_OTGFS_DCTL);
...
static inline void at32_epin(struct at32_usbdev_s *priv, uint8_t epno)
{
  struct at32_ep_s *privep = &priv->epin[epno];
...
static inline void at32_epin_interrupt(struct at32_usbdev_s *priv)
{
  ...
  daint  = at32_getreg(AT32_OTGFS_DAINT);
  daint &= at32_getreg(AT32_OTGFS_DAINTMSK);
  ...
  if (daint == 0)
    {
      ...
      daint  = at32_getreg(AT32_OTGFS_DAINT);
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_EPINUNEXPECTED),
               (uint16_t)daint);
          ...
          if ((daint & 1) != 0)
            {
              uinfo("DIEPINT(%d) = %08" PRIx32 "\n",
                    epno, at32_getreg(AT32_OTGFS_DIEPINT(epno)));
              at32_putreg(0xff, AT32_OTGFS_DIEPINT(epno));
            }
      ...
      if ((daint & 1) != 0)
        {
          ...
          mask = at32_getreg(AT32_OTGFS_DIEPMSK);
          ...
          empty = at32_getreg(AT32_OTGFS_DIEPEMPMSK);
          ...
          diepint = at32_getreg(AT32_OTGFS_DIEPINT(epno)) & mask;
          ...
          if ((diepint & OTGFS_DIEPINT_XFRC) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_XFRC),
                       (uint16_t)diepint);
              ...
              at32_putreg(empty, AT32_OTGFS_DIEPEMPMSK);
              at32_putreg(OTGFS_DIEPINT_XFRC, AT32_OTGFS_DIEPINT(epno));
          ...
          if ((diepint & OTGFS_DIEPINT_TOC) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_TOC),
                       (uint16_t)diepint);
              at32_putreg(OTGFS_DIEPINT_TOC, AT32_OTGFS_DIEPINT(epno));
            }
          ...
          if ((diepint & OTGFS_DIEPINT_ITTXFE) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_ITTXFE),
                       (uint16_t)diepint);
              at32_epin_request(priv, &priv->epin[epno]);
              at32_putreg(OTGFS_DIEPINT_ITTXFE, AT32_OTGFS_DIEPINT(epno));
            }
          ...
          if ((diepint & OTGFS_DIEPINT_INEPNE) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_INEPNE),
                       (uint16_t)diepint);
              at32_putreg(OTGFS_DIEPINT_INEPNE, AT32_OTGFS_DIEPINT(epno));
            }
#endif
          ...
          if ((diepint & OTGFS_DIEPINT_EPDISD) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_EPDISD),
                       (uint16_t)diepint);
              at32_putreg(OTGFS_DIEPINT_EPDISD, AT32_OTGFS_DIEPINT(epno));
            }
#endif
          ...
          if ((diepint & OTGFS_DIEPINT_TXFE) != 0)
            {
              usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN_TXFE),
                       (uint16_t)diepint);
              ...
              if ((diepint & OTGFS_DIEPINT_XFRC) == 0)
                {
                  ...
                  at32_epin_txfifoempty(priv, epno);
                }

              /* Clear the pending TxFIFO empty interrupt */

              at32_putreg(OTGFS_DIEPINT_TXFE, AT32_OTGFS_DIEPINT(epno));
            }
        }
...
static inline void at32_resumeinterrupt(struct at32_usbdev_s *priv)
{
  ...
  at32_usbsuspend((struct usbdev_s *)priv, true);
...
static inline void at32_suspendinterrupt(struct at32_usbdev_s *priv)
{
  ...
  at32_usbsuspend((struct usbdev_s *)priv, false);
}

/****************************************************************************
 * Name: at32_rxinterrupt
 *
 * Description:
...
static inline void at32_rxinterrupt(struct at32_usbdev_s *priv)
{
  ...
  while (0 != (at32_getreg(AT32_OTGFS_GINTSTS) & OTGFS_GINT_RXFLVL))
    {
      /* Get the status from the top of the FIFO */

      regval = at32_getreg(AT32_OTGFS_GRXSTSP);
      ...
      /* Workaround for bad values read from the AT32_OTGFS_GRXSTSP register
          ...
          switch (regval & OTGFS_GRXSTSD_PKTSTS_MASK)
            {
              ...
              case OTGFS_GRXSTSD_PKTSTS_OUTNAK:
                {
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_OUTNAK), 0);
                }
                break;

              /* OUT data packet received.
               *
               * PKTSTS = DataOUT, BCNT = size of the received data OUT
               * packet, EPNUM = EPNUM on which the packet was received, DPID
               * = Actual Data PID.
               */

              case OTGFS_GRXSTSD_PKTSTS_OUTRECVD:
                {
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_OUTRECVD), epphy);
                  bcnt = (regval & OTGFS_GRXSTSD_BCNT_MASK) >>
                         OTGFS_GRXSTSD_BCNT_SHIFT;
                  if (bcnt > 0)
                    {
                      at32_epout_receive(privep, bcnt);
                    }
                }
                break;
               ...
               * Num on which the data transfer is complete, DPID = Don't
               * Care.
               */

              case OTGFS_GRXSTSD_PKTSTS_OUTDONE:
                {
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_OUTDONE), epphy);
                }
                break;
              ...
              case OTGFS_GRXSTSD_PKTSTS_SETUPDONE:
                {
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SETUPDONE),
                           epphy);

                  /* Now that the Setup Phase is complete if it was an OUT
                   * enable the endpoint (Doing this here prevents the loss
                   * of the first FIFO word)
                   */
              ...
              case OTGFS_GRXSTSD_PKTSTS_SETUPRECVD:
                {
                  ...
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SETUPRECVD),
                           epphy);

                  /* Read EP0 setup data.  NOTE: If multiple SETUP packets
                   * are received, the last one overwrites the previous setup
                   * packets and only that last SETUP packet will be
                   * processed.
                   */

                  at32_rxfifo_read(&priv->epout[EP0],
...
static inline void at32_enuminterrupt(struct at32_usbdev_s *priv)
{
  ...
  at32_putreg(regval, AT32_OTGFS_GUSBCFG);
}

/****************************************************************************
 * Name: at32_isocininterrupt
 *
 * Description:
  ...
  for (i = 0; i < AT32_NENDPOINTS; i++)
    {
      ...
      regaddr = AT32_OTGFS_DIEPCTL(privep->epphy);
      doepctl = at32_getreg(regaddr);
      dsts    = at32_getreg(AT32_OTGFS_DSTS);
      ...
      at32_req_complete(privep, -EIO);
#warning "Will clear OTGFS_DIEPCTL_USBAEP too"
      at32_epin_disable(privep);
      break;
    }
}
#endif
...
static inline void at32_otginterrupt(struct at32_usbdev_s *priv)
{
  ...
  regval = at32_getreg(AT32_OTGFS_GOTGINT);
...
#warning "Missing logic"
  ...
  at32_putreg(regval, AT32_OTGFS_GOTGINT);
}
#endif

/****************************************************************************
 * Name: at32_usbinterrupt
 *
 * Description:
...
static int at32_usbinterrupt(int irq, void *context, void *arg)
{
  ...
  struct at32_usbdev_s *priv = &g_otgfsdev;
  uint32_t regval;
  uint32_t reserved;

  usbtrace(TRACE_INTENTRY(AT32_TRACEINTID_USB), 0);

  /* Assure that we are in device mode */

  DEBUGASSERT((at32_getreg(AT32_OTGFS_GINTSTS) & OTGFS_GINTSTS_CMOD) ==
  ...
  for (; ; )
    {
      ...
      regval  = at32_getreg(AT32_OTGFS_GINTSTS);
      ...
      at32_putreg(((regval | reserved) & OTGFS_GINT_RC_W1),
                   AT32_OTGFS_GINTSTS);
      ...
      usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_INTPENDING),
               (uint16_t)regval);
      ...
      if ((regval & OTGFS_GINT_OEP) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPOUT),
                   (uint16_t)regval);
          at32_epout_interrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_IEP) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPIN), (uint16_t)regval);
          at32_epin_interrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_MMIS) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_MISMATCH),
                   (uint16_t)regval);
        }
#endif
      ...
      if ((regval & OTGFS_GINT_WKUP) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_WAKEUP),
                   (uint16_t)regval);
          at32_resumeinterrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_USBSUSP) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SUSPEND),
                   (uint16_t)regval);
          at32_suspendinterrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_SOF) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SOF), (uint16_t)regval);
        }
#endif
      ...
      if ((regval & OTGFS_GINT_RXFLVL) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_RXFIFO),
                   (uint16_t)regval);
          at32_rxinterrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_RESETS) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_DEVRESET),
                   (uint16_t)regval);

          /* Perform the device reset */

          at32_usbreset(priv);
          usbtrace(TRACE_INTEXIT(AT32_TRACEINTID_USB), 0);
          return OK;
        }
      ...
      if ((regval & OTGFS_GINT_ENUMDNE) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_ENUMDNE),
                   (uint16_t)regval);
          at32_enuminterrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_IISOIXFR) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_IISOIXFR),
                   (uint16_t)regval);
          at32_isocininterrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_IISOOXFR) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_IISOOXFR),
                   (uint16_t)regval);
          at32_isocoutinterrupt(priv);
        }
#endif
      ...
      if ((regval & OTGFS_GINT_SRQ) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SRQ), (uint16_t)regval);
          at32_sessioninterrupt(priv);
        }
      ...
      if ((regval & OTGFS_GINT_OTG) != 0)
        {
          usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_OTG), (uint16_t)regval);
          at32_otginterrupt(priv);
        }
#endif
    }
...
static void at32_enablegonak(struct at32_ep_s *privep)
{
...
#if 0
  at32_putreg(OTGFS_GINT_GONAKEFF, AT32_OTGFS_GINTSTS);
#endif
  ...
  while ((at32_getreg(AT32_OTGFS_GINTSTS) & OTGFS_GINT_GONAKEFF) == 0);
  at32_putreg(OTGFS_GINT_GONAKEFF, AT32_OTGFS_GINTSTS);
  ...
  while ((at32_getreg(AT32_OTGFS_DCTL) & OTGFS_DCTL_GONSTS) == 0);
#endif
}

/****************************************************************************
 * Name: at32_disablegonak
 *
 * Description:
...
static int at32_ep_configure(struct usbdev_ep_s *ep,
                              ...
                              bool last)
{
  struct at32_ep_s *privep = (struct at32_ep_s *)ep;
  uint16_t maxpacket;
  uint8_t  eptype;
  int ret;
  ...
  if (privep->isin)
    {
      ret = at32_epin_configure(privep, eptype, maxpacket);
    }
  else
    {
      ret = at32_epout_configure(privep, eptype, maxpacket);
    }
...
static void at32_ep0_configure(struct at32_usbdev_s *priv)
{
  ...
  at32_epin_configure(&priv->epin[EP0], USB_EP_ATTR_XFER_CONTROL,
                       CONFIG_USBDEV_EP0_MAXSIZE);
  at32_epout_configure(&priv->epout[EP0], USB_EP_ATTR_XFER_CONTROL,
                        CONFIG_USBDEV_EP0_MAXSIZE);
}
...
#if 0 /* Doesn't happen */
  regaddr = AT32_OTGFS_DOEPINT(privep->epphy);
  while ((at32_getreg(regaddr) & OTGFS_DOEPINT_EPDISD) == 0);
#else
  /* REVISIT: */
...
static void at32_epin_disable(struct at32_ep_s *privep)
{
  ...
  at32_putreg(OTGFS_DIEPINT_INEPNE, AT32_OTGFS_DIEPINT(privep->epphy));
  ...
  regaddr = AT32_OTGFS_DIEPINT(privep->epphy);
  while ((at32_getreg(regaddr) & OTGFS_DIEPINT_INEPNE) == 0);

  /* Clear the INEPNE interrupt indication */

  at32_putreg(OTGFS_DIEPINT_INEPNE, regaddr);
#endif
  ...
  regaddr = AT32_OTGFS_DIEPINT(privep->epphy);
  while ((at32_getreg(regaddr) & OTGFS_DIEPINT_EPDISD) == 0);

  /* Clear the EPDISD interrupt indication */

  at32_putreg(OTGFS_DIEPINT_EPDISD, at32_getreg(regaddr));
...
static int at32_ep_disable(struct usbdev_ep_s *ep)
{
  ...
  if (!ep)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_INVALIDPARMS), 0);
      return -EINVAL;
    }
#endif
...
static struct usbdev_req_s *at32_ep_allocreq(struct usbdev_ep_s *ep)
{
  struct at32_req_s *privreq;
  ...
  usbtrace(TRACE_EPALLOCREQ, ((struct at32_ep_s *)ep)->epphy);

  privreq = (struct at32_req_s *)kmm_malloc(sizeof(struct at32_req_s));
  if (!privreq)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_ALLOCFAIL), 0);
      return NULL;
    }

  memset(privreq, 0, sizeof(struct at32_req_s));
  return &privreq->req;
}

/****************************************************************************
 * Name: at32_ep_freereq
 *
 * Description:
 *   Free an I/O request
 *
...
static void at32_ep_freereq(struct usbdev_ep_s *ep,
                             struct usbdev_req_s *req)
{
  struct at32_req_s *privreq = (struct at32_req_s *)req;
  ...
  usbtrace(TRACE_EPFREEREQ, ((struct at32_ep_s *)ep)->epphy);
  kmm_free(privreq);
}

/****************************************************************************
 * Name: at32_ep_allocbuffer
 *
 * Description:
...
#ifdef CONFIG_USBDEV_DMA
static void *at32_ep_allocbuffer(struct usbdev_ep_s *ep, uint16_t bytes)
{
  usbtrace(TRACE_EPALLOCBUFFER, ((struct at32_ep_s *)ep)->epphy);
...
#ifdef CONFIG_USBDEV_DMA
static void at32_ep_freebuffer(struct usbdev_ep_s *ep, void *buf)
{
  usbtrace(TRACE_EPALLOCBUFFER, ((struct at32_ep_s *)ep)->epphy);
                           ...
                           struct usbdev_req_s *req)
{
  ...
  if (!priv->driver)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_NOTCONFIGURED),
               priv->usbdev.speed);
      return -ESHUTDOWN;
    }
#endif
  ...
  if (privep->stalled)
  ...
  else
    {
      ...
      if (at32_req_addlast(privep, privreq) && !privep->active)
        {
...
{
#if 1
  /* This implementation follows the requirements from the AT32 F4 reference
   * manual.
   */
  ...
  return OK;
#else
  /* This implementation follows the Artery code example. */
...
static int at32_ep_setstall(struct at32_ep_s *privep)
{
  ...
  if (privep->isin == 1)
    {
      return at32_epin_setstall(privep);
    }
  else
    {
      return at32_epout_setstall(privep);
    }
}

/****************************************************************************
 * Name: at32_ep_clrstall
 *
 * Description:
...
static int at32_ep_stall(struct usbdev_ep_s *ep, bool resume)
{
  ...
  if (resume)
    {
      ret = at32_ep_clrstall(privep);
    }
  else
    {
      ret = at32_ep_setstall(privep);
    }
...
static void at32_ep0_stall(struct at32_usbdev_s *priv)
{
  at32_epin_setstall(&priv->epin[EP0]);
  at32_epout_setstall(&priv->epout[EP0]);
  priv->stalled = true;
  at32_ep0out_ctrlsetup(priv);
}
...
static struct usbdev_ep_s *at32_ep_alloc(struct usbdev_s *dev,
                                          ...
                                          uint8_t eptype)
{
  struct at32_usbdev_s *priv = (struct at32_usbdev_s *)dev;
  uint8_t epavail;
  irqstate_t flags;
  int epphy;
  int epno = 0;
  ...
  if (epphy > 0)
    {
      ...
      if (epphy >= AT32_NENDPOINTS)
        {
          usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADEPNO), (uint16_t)epphy);
          return NULL;
        }
  ...
  if (epavail)
    {
      ...
      for (epno = 1; epno < AT32_NENDPOINTS; epno++)
        {
          uint8_t bit = 1 << epno;
          if ((epavail & bit) != 0)
            {
  ...
  usbtrace(TRACE_DEVERROR(AT32_TRACEERR_NOEP), (uint16_t)eplog);
  leave_critical_section(flags);
  return NULL;
}
...
static int at32_getframe(struct usbdev_s *dev)
{
  ...
  regval = at32_getreg(AT32_OTGFS_DSTS);
...
static void at32_setaddress(struct at32_usbdev_s *priv, uint16_t address)
{
  uint32_t regval;

  /* Set the device address in the DCFG register */

  regval = at32_getreg(AT32_OTGFS_DCFG);
  regval &= ~OTGFS_DCFG_DAD_MASK;
  regval |= ((uint32_t)address << OTGFS_DCFG_DAD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DCFG);
...
static int at32_txfifo_flush(uint32_t txfnum)
{
  ...
  at32_putreg(regval, AT32_OTGFS_GRSTCTL);
...
static int at32_rxfifo_flush(void)
{
  ...
  at32_putreg(OTGFS_GRSTCTL_RXFFLSH, AT32_OTGFS_GRSTCTL);
...
static void at32_swinitialize(struct at32_usbdev_s *priv)
{
  ...
  memset(priv, 0, sizeof(struct at32_usbdev_s));
  ...
  for (i = 0; i < AT32_NENDPOINTS; i++)
    {
      ...
      privep->epphy    = i;
      privep->ep.eplog = AT32_EPPHYIN2LOG(i);
  ...
  for (i = 0; i < AT32_NENDPOINTS; i++)
    {
      ...
      privep->epphy    = i;
      privep->ep.eplog = AT32_EPPHYOUT2LOG(i);
...
static void at32_hwinitialize(struct at32_usbdev_s *priv)
{
  ...
  at32_putreg(OTGFS_GRSTCTL_CSRST, AT32_OTGFS_GRSTCTL);
  for (timeout = 0; timeout < AT32_READY_DELAY; timeout++)
    {
      regval = at32_getreg(AT32_OTGFS_GRSTCTL);
...
# endif
  ...
  at32_putreg(regval, AT32_OTGFS_GCCFG);
  up_mdelay(20);
  ...
  regval  = at32_getreg(AT32_OTGFS_PCGCCTL);
  regval &= ~(1 << 0);
  at32_putreg(0, AT32_OTGFS_PCGCCTL);
  ...
  at32_putreg(AT32_RXFIFO_WORDS, AT32_OTGFS_GRXFSIZ);

  /* EP0 TX */

  address = AT32_RXFIFO_WORDS;
  regval  = (address << OTGFS_DIEPTXF0_TX0FD_SHIFT) |
            (AT32_EP0_TXFIFO_WORDS << OTGFS_DIEPTXF0_TX0FSA_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF0);

  /* EP1 TX */

  address += AT32_EP0_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP1_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF1);

  /* EP2 TX */

  address += AT32_EP1_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP2_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF2);

  /* EP3 TX */

  address += AT32_EP2_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP3_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF3);

  /* EP4 TX */

  address += AT32_EP3_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP4_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF4);

  /* EP5 TX */

  address += AT32_EP4_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP5_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF5);

  /* EP6 TX */

  address += AT32_EP5_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP6_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF6);

  /* EP7 TX */

  address += AT32_EP6_TXFIFO_WORDS;
  regval   = (address << OTGFS_DIEPTXF_INEPTXSA_SHIFT) |
             (AT32_EP7_TXFIFO_WORDS << OTGFS_DIEPTXF_INEPTXFD_SHIFT);
  at32_putreg(regval, AT32_OTGFS_DIEPTXF7);

  /* Flush the FIFOs */

  at32_txfifo_flush(OTGFS_GRSTCTL_TXFNUM_DALL);
  at32_rxfifo_flush();

  /* Clear all pending Device Interrupts */

  at32_putreg(0, AT32_OTGFS_DIEPMSK);
  at32_putreg(0, AT32_OTGFS_DOEPMSK);
  at32_putreg(0, AT32_OTGFS_DIEPEMPMSK);
  at32_putreg(0xffffffff, AT32_OTGFS_DAINT);
  at32_putreg(0, AT32_OTGFS_DAINTMSK);

  /* Configure all IN endpoints */

  for (i = 0; i < AT32_NENDPOINTS; i++)
    {
      regval = at32_getreg(AT32_OTGFS_DIEPCTL(i));
      ...
      at32_putreg(regval, AT32_OTGFS_DIEPCTL(i));
      at32_putreg(0, AT32_OTGFS_DIEPTSIZ(i));
      at32_putreg(0xff, AT32_OTGFS_DIEPINT(i));
    }
  ...
  for (i = 0; i < AT32_NENDPOINTS; i++)
    {
      regval = at32_getreg(AT32_OTGFS_DOEPCTL(i));
      ...
      at32_putreg(regval, AT32_OTGFS_DOEPCTL(i));
      at32_putreg(0, AT32_OTGFS_DOEPTSIZ(i));
      at32_putreg(0xff, AT32_OTGFS_DOEPINT(i));
    }

  /* Disable all interrupts. */

  at32_putreg(0, AT32_OTGFS_GINTMSK);

  /* Clear any pending USB_OTG Interrupts */

  at32_putreg(0xffffffff, AT32_OTGFS_GOTGINT);

  /* Clear any pending interrupts */

  regval = at32_getreg(AT32_OTGFS_GINTSTS);
  regval &=  OTGFS_GINT_RESERVED;
  at32_putreg(regval | OTGFS_GINT_RC_W1, AT32_OTGFS_GINTSTS);
  ...
  at32_putreg(regval, AT32_OTGFS_GINTMSK);
  ...
  at32_putreg(OTGFS_GAHBCFG_GINTMSK | OTGFS_GAHBCFG_TXFELVL,
               AT32_OTGFS_GAHBCFG);
}
...
void arm_usbinitialize(void)
{
  ...
  /* Uninitialize the hardware so th`at we know that we are starting from a
   * known state.
   */
  ...
  ret = irq_attach(AT32_IRQ_OTGFS, at32_usbinterrupt, NULL);
  if (ret < 0)
    {
  ...
  at32_hwinitialize(priv);

  /* Disconnect device */

  at32_pullup(&priv->usbdev, false);
...
void arm_usbuninitialize(void)
{
  ...
  if (priv->driver)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_DRIVERREGISTERED), 0);
  ...
  up_disable_irq(AT32_IRQ_OTGFS);
  irq_detach(AT32_IRQ_OTGFS);
...
int usbdev_register(struct usbdevclass_driver_s *driver)
{
  ...
  if (priv->driver)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_DRIVER), 0);
      return -EBUSY;
    }
#endif
  ...
  if (ret)
    {
      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BINDFAILED), (uint16_t)-ret);
      priv->driver = NULL;
    }
  else
    {
...
#endif /* CONFIG_USBDEV && CONFIG_AT32_OTGFSDEV */
