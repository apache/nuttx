        ...
        else
          {
            switch (ctrlreq->type & USB_REQ_RECIPIENT_MASK)
              ...
              case USB_REQ_RECIPIENT_ENDPOINT:
                {
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_EPGETSTATUS), 0);
                  privep = at32_ep_findbyaddr(priv, ctrlreq->index);
                  if (!privep)
                    {
                      usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADEPGETSTATUS),
                               0);
                      priv->stalled = true;
                    }
                  else
              ...
              case USB_REQ_RECIPIENT_INTERFACE:
                {
                  usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_IFGETSTATUS), 0);
                  priv->ep0data[0] = 0;
                  priv->ep0data[1] = 0;
              ...
              default:
                {
                  usbtrace(TRACE_DEVERROR(AT32_TRACEERR_BADGETSTATUS), 0);
                  priv->stalled = true;
                }
                break;
              }
        ...
        if (priv->addressed != 0 && ctrlreq->len == 0)
          {
            ...
            if (recipient == USB_REQ_RECIPIENT_ENDPOINT &&
                ...
                (privep = at32_ep_findbyaddr(priv, ctrlreq->index)) != NULL)
              {
      ...
      if (epphy < AT32_NENDPOINTS)
        {
          ...
          switch (regval & OTGFS_GRXSTSD_PKTSTS_MASK)
            ...
            case OTGFS_GRXSTSD_PKTSTS_SETUPDONE:
              {
                usbtrace(TRACE_INTDECODE(AT32_TRACEINTID_SETUPDONE), epphy);
