...
/* The AT32 ADC lower-half driver functionality overview:
 *   - one lower-half driver for all AT32 ADC IP cores,
...
/* AT32 ADC "lower-half" support must be enabled */

#ifdef CONFIG_AT32_ADC

/* This implementation is for the AT32 ADC IP version 1 and 2 */

#if !defined(HAVE_IP_ADC_V1) && !defined(HAVE_IP_ADC_V2)
#  error "AT32 ADC IP version not specified"
#endif
 ...
 */
    ...
    (AT32_CFGR_PLLSRC != 0 || AT32_SYSCLK_SW != RCC_CFGR_SW_HSI)
...
#  define ADC_SMPR_DEFAULT    ADC_SMPR_92p5
...
/* ADC resolution. Not supported for basic AT32 ADC IPv1 */

#ifndef CONFIG_AT32_HAVE_IP_ADC_V1_BASIC
...
#ifdef CONFIG_AT32_HAVE_IP_ADC_V1_BASIC
#  undef HAVE_ADC_CMN_REGS
#else
#  define HAVE_ADC_CMN_REGS
#endif
#if defined(HAVE_ADC_CMN_REGS) && AT32_NADC > 1
...
#ifdef CONFIG_AT32_HAVE_IP_ADC_V1
#  define ADC_HAVE_SCAN 1
#  ifndef CONFIG_AT32_ADC1_SCAN
#    define CONFIG_AT32_ADC1_SCAN 0
#  endif
#  ifndef CONFIG_AT32_ADC2_SCAN
#    define CONFIG_AT32_ADC2_SCAN 0
#  endif
#  ifndef CONFIG_AT32_ADC3_SCAN
#    define CONFIG_AT32_ADC3_SCAN 0
#  endif
#else
#  undef ADC_HAVE_SCAN
#endif
...
#if !defined(CONFIG_AT32_ADC_NOIRQ) || defined(ADC_HAVE_DMA)
#  define ADC_HAVE_CB
#else
#  undef ADC_HAVE_CB
#endif
...
struct at32_dev_s
{
  ...
  const struct at32_adc_ops_s *llops; /* Low-level ADC ops */
...
#  endif
  ...
  uint16_t r_dmabuffer[CONFIG_AT32_ADC_MAX_SAMPLES];
#endif

  /* List of selected ADC channels to sample */

  uint8_t  r_chanlist[CONFIG_AT32_ADC_MAX_SAMPLES];
...
static void at32_modifyreg32(unsigned int addr, uint32_t clrbits,
                              uint32_t setbits);
#endif
static uint32_t adc_getreg(struct at32_dev_s *priv, int offset);
static void adc_putreg(struct at32_dev_s *priv, int offset,
                       uint32_t value);
static void adc_modifyreg(struct at32_dev_s *priv, int offset,
                          uint32_t clrbits, uint32_t setbits);
#ifdef HAVE_ADC_CMN_REGS
static uint32_t adccmn_base_get(struct at32_dev_s *priv);
static void adccmn_modifyreg(struct at32_dev_s *priv, uint32_t offset,
                             uint32_t clrbits, uint32_t setbits);
static uint32_t adccmn_getreg(struct at32_dev_s *priv, uint32_t offset);
#endif
#ifdef ADC_HAVE_TIMER
static uint16_t tim_getreg(struct at32_dev_s *priv, int offset);
static void tim_putreg(struct at32_dev_s *priv, int offset,
                       uint16_t value);
static void tim_modifyreg(struct at32_dev_s *priv, int offset,
                          uint16_t clrbits, uint16_t setbits);
static void tim_dumpregs(struct at32_dev_s *priv, const char *msg);
#endif

static void adc_rccreset(struct at32_dev_s *priv, bool reset);

/* ADC Interrupt Handler */

#ifndef CONFIG_AT32_ADC_NOIRQ
static int  adc_interrupt(struct adc_dev_s *dev);
#  if defined(AT32_IRQ_ADC1) && defined(CONFIG_AT32_ADC1)
static int  adc1_interrupt(int irq, void *context, void *arg);
#  endif
#  if defined(AT32_IRQ_ADC12) && (defined(CONFIG_AT32_ADC1) || \
                                 defined(CONFIG_AT32_ADC2))
static int  adc12_interrupt(int irq, void *context, void *arg);
#  endif
#  if (defined(AT32_IRQ_ADC3) && defined(CONFIG_AT32_ADC3))
static int  adc3_interrupt(int irq, void *context, void *arg);
#  endif

#  if defined(AT32_IRQ_ADC)
...
static void adc_enable(struct at32_dev_s *priv, bool enable);

static uint32_t adc_sqrbits(struct at32_dev_s *priv, int first,
                            int last, int offset);
...
static int  adc_ioc_change_sleep_between_opers(struct adc_dev_s *dev,
                                               int cmd, bool arg);
static void adc_power_down_idle(struct at32_dev_s *priv,
                                bool pdi_high);
static void adc_power_down_delay(struct at32_dev_s *priv,
                                 bool pdd_high);
#endif
...
#ifdef ADC_HAVE_TIMER
static void adc_timstart(struct at32_dev_s *priv, bool enable);
static int  adc_timinit(struct at32_dev_s *priv);
#endif

#if defined(ADC_HAVE_DMA) && !defined(CONFIG_AT32_ADC_NOIRQ)
static void adc_dmaconvcallback(DMA_HANDLE handle, uint8_t isr,
                                void *arg);
#endif

static void adc_reg_startconv(struct at32_dev_s *priv, bool enable);
#ifdef ADC_HAVE_INJECTED
static void adc_inj_startconv(struct at32_dev_s *priv, bool enable);
static int adc_inj_set_ch(struct adc_dev_s *dev, uint8_t ch);
#endif

#ifdef ADC_HAVE_EXTCFG
static int adc_extcfg_set(struct at32_dev_s *priv, uint32_t extcfg);
#endif
#ifdef ADC_HAVE_JEXTCFG
static int adc_jextcfg_set(struct at32_dev_s *priv, uint32_t jextcfg);
#endif

static void adc_dumpregs(struct at32_dev_s *priv);

#ifdef CONFIG_AT32_ADC_LL_OPS
static int adc_llops_setup(struct at32_adc_dev_s *dev);
static void adc_llops_shutdown(struct at32_adc_dev_s *dev);
static void adc_intack(struct at32_adc_dev_s *dev, uint32_t source);
static void adc_inten(struct at32_adc_dev_s *dev, uint32_t source);
static void adc_intdis(struct at32_adc_dev_s *dev, uint32_t source);
static uint32_t adc_intget(struct at32_adc_dev_s *dev);
static uint32_t adc_regget(struct at32_adc_dev_s *dev);
static void adc_llops_reg_startconv(struct at32_adc_dev_s *dev,
                                    bool enable);
static int adc_offset_set(struct at32_adc_dev_s *dev, uint8_t ch,
                          uint8_t i, uint16_t offset);
#  ifdef ADC_HAVE_EXTCFG
static void adc_llops_extcfg_set(struct at32_adc_dev_s *dev,
                                 uint32_t extcfg);
#  endif
#  ifdef ADC_HAVE_JEXTCFG
static void adc_llops_jextcfg_set(struct at32_adc_dev_s *dev,
                                  uint32_t jextcfg);
#  endif
#  ifdef ADC_HAVE_DMA
static int adc_regbufregister(struct at32_adc_dev_s *dev,
                              uint16_t *buffer, uint8_t len);
#  endif
#  ifdef ADC_HAVE_INJECTED
static uint32_t adc_injget(struct at32_adc_dev_s *dev, uint8_t chan);
static void adc_llops_inj_startconv(struct at32_adc_dev_s *dev,
                                    bool enable);
#  endif
#  ifdef CONFIG_AT32_ADC_CHANGE_SAMPLETIME
static void adc_sampletime_set(struct at32_adc_dev_s *dev,
                               struct adc_sample_time_s *time_samples);
static void adc_sampletime_write(struct at32_adc_dev_s *dev);
#  endif
static void adc_llops_dumpregs(struct at32_adc_dev_s *dev);
static int  adc_llops_multicfg(struct at32_adc_dev_s *dev, uint8_t mode);
static void adc_llops_enable(struct at32_adc_dev_s *dev, bool enable);
#endif
...
#ifdef CONFIG_AT32_ADC_LL_OPS
static const struct at32_adc_ops_s g_adc_llops =
{
...
static struct at32_dev_s g_adcpriv1 =
{
#ifdef CONFIG_AT32_ADC_LL_OPS
  .llops       = &g_adc_llops,
#endif
#ifndef CONFIG_AT32_ADC_NOIRQ
#  if defined(AT32_IRQ_ADC1)
  .irq         = AT32_IRQ_ADC1,
  .isr         = adc1_interrupt,
#  elif defined(AT32_IRQ_ADC12)
  .irq         = AT32_IRQ_ADC12,
  .isr         = adc12_interrupt,
#  elif defined(AT32_IRQ_ADC)
  .irq         = AT32_IRQ_ADC,
  .isr         = adc123_interrupt,
#  else
#    error "No AT32_IRQ_ADC1 AT32_IRQ_ADC12 or AT32_IRQ_ADC  defined for CONFIG_AT32_ADC1"
#  endif
#endif /* CONFIG_AT32_ADC_NOIRQ */
#ifdef HAVE_ADC_CMN_DATA
  .cmn         = &ADC1CMN_DATA,
  ...
  .intf        = 1,
  .initialized = 0,
  .anioc_trg   = CONFIG_AT32_ADC1_ANIOC_TRIGGER,
#ifdef HAVE_ADC_RESOLUTION
  .resolution  = CONFIG_AT32_ADC1_RESOLUTION,
#endif
  .base        = AT32_ADC1_BASE,
  ...
  .trigger     = CONFIG_AT32_ADC1_TIMTRIG,
  .tbase       = ADC1_TIMER_BASE,
  .pclck       = ADC1_TIMER_PCLK_FREQUENCY,
  .freq        = CONFIG_AT32_ADC1_SAMPLE_FREQUENCY,
#endif
#ifdef ADC1_HAVE_DMA
  .dmachan     = DMAMUX_ADC1,
#  ifdef ADC_HAVE_DMACFG
  .dmacfg      = CONFIG_AT32_ADC1_DMA_CFG,
#  endif
  .hasdma      = true,
#endif
#ifdef ADC_HAVE_SCAN
  .scan        = CONFIG_AT32_ADC1_SCAN,
#endif
};
...
static struct at32_dev_s g_adcpriv2 =
{
#ifdef CONFIG_AT32_ADC_LL_OPS
  .llops       = &g_adc_llops,
#endif
#ifndef CONFIG_AT32_ADC_NOIRQ
#  if defined(AT32_IRQ_ADC12)
  .irq         = AT32_IRQ_ADC12,
  .isr         = adc12_interrupt,
#  elif defined(AT32_IRQ_ADC)
  .irq         = AT32_IRQ_ADC,
  .isr         = adc123_interrupt,
#  else
#    error "No AT32_IRQ_ADC12 or AT32_IRQ_ADC  defined for CONFIG_AT32_ADC2"
#  endif
#endif /* CONFIG_AT32_ADC_NOIRQ */
#ifdef HAVE_ADC_CMN_DATA
  .cmn         = &ADC2CMN_DATA,
  ...
  .intf        = 2,
  .initialized = 0,
  .anioc_trg   = CONFIG_AT32_ADC2_ANIOC_TRIGGER,
#ifdef HAVE_ADC_RESOLUTION
  .resolution  = CONFIG_AT32_ADC2_RESOLUTION,
#endif
  .base        = AT32_ADC2_BASE,
  ...
  .trigger     = CONFIG_AT32_ADC2_TIMTRIG,
  .tbase       = ADC2_TIMER_BASE,
  .pclck       = ADC2_TIMER_PCLK_FREQUENCY,
  .freq        = CONFIG_AT32_ADC2_SAMPLE_FREQUENCY,
#endif
#ifdef ADC2_HAVE_DMA
  .dmachan     = DMAMUX_ADC2,
#  ifdef ADC_HAVE_DMACFG
  .dmacfg      = CONFIG_AT32_ADC2_DMA_CFG,
#  endif
  .hasdma      = true,
#endif
#ifdef ADC_HAVE_SCAN
  .scan        = CONFIG_AT32_ADC2_SCAN,
#endif
};
...
static struct at32_dev_s g_adcpriv3 =
{
#ifdef CONFIG_AT32_ADC_LL_OPS
  .llops       = &g_adc_llops,
#endif
#ifndef CONFIG_AT32_ADC_NOIRQ
#  if defined(AT32_IRQ_ADC3)
  .irq         = AT32_IRQ_ADC3,
  .isr         = adc3_interrupt,
#  elif defined(AT32_IRQ_ADC)
  .irq         = AT32_IRQ_ADC,
  .isr         = adc123_interrupt,
#  else
#    error "No AT32_IRQ_ADC3 or AT32_IRQ_ADC  defined for CONFIG_AT32_ADC3"
#  endif
#endif /* CONFIG_AT32_ADC_NOIRQ */
#ifdef HAVE_ADC_CMN_DATA
  .cmn         = &ADC3CMN_DATA,
  ...
  .intf        = 3,
  .initialized = 0,
  .anioc_trg   = CONFIG_AT32_ADC3_ANIOC_TRIGGER,
#ifdef HAVE_ADC_RESOLUTION
  .resolution  = CONFIG_AT32_ADC3_RESOLUTION,
#endif
  .base        = AT32_ADC3_BASE,
  ...
  .trigger     = CONFIG_AT32_ADC3_TIMTRIG,
  .tbase       = ADC3_TIMER_BASE,
  .pclck       = ADC3_TIMER_PCLK_FREQUENCY,
  .freq        = CONFIG_AT32_ADC3_SAMPLE_FREQUENCY,
#endif
#ifdef ADC3_HAVE_DMA
  .dmachan     = DMAMUX_ADC3,
#  ifdef ADC_HAVE_DMACFG
  .dmacfg      = CONFIG_AT32_ADC3_DMA_CFG,
#  endif
  .hasdma      = true,
#endif
#ifdef ADC_HAVE_SCAN
  .scan        = CONFIG_AT32_ADC3_SCAN,
#endif
};
...
static uint32_t adccmn_base_get(struct at32_dev_s *priv)
{
  ...
  if (priv->base == AT32_ADC1_BASE || priv->base == AT32_ADC2_BASE)
    {
      base = AT32_ADC12CMN_BASE;
    }
#  if defined(CONFIG_AT32_ADC3)
  else
    {
      base = AT32_ADC3CMN_BASE;
    }
#  endif

#elif defined(HAVE_IP_ADC_V1)
  base = AT32_ADCCMN_BASE;
  UNUSED(priv);
#endif
                             ...
                             uint32_t clrbits, uint32_t setbits)
{
  ...
  at32_modifyreg32(offset + base, clrbits, setbits);
}
...
static void tim_dumpregs(struct at32_dev_s *priv, const char *msg)
{
  ...
  ainfo("  CR1: %04x CR2:  %04x SMCR:  %04x DIER:  %04x\n",
        tim_getreg(priv, AT32_GTIM_CR1_OFFSET),
        tim_getreg(priv, AT32_GTIM_CR2_OFFSET),
        tim_getreg(priv, AT32_GTIM_SMCR_OFFSET),
        tim_getreg(priv, AT32_GTIM_DIER_OFFSET));
  ainfo("   SR: %04x EGR:  0000 CCMR1: %04x CCMR2: %04x\n",
        tim_getreg(priv, AT32_GTIM_SR_OFFSET),
        tim_getreg(priv, AT32_GTIM_CCMR1_OFFSET),
        tim_getreg(priv, AT32_GTIM_CCMR2_OFFSET));
  ainfo(" CCER: %04x CNT:  %04x PSC:   %04x ARR:   %04x\n",
        tim_getreg(priv, AT32_GTIM_CCER_OFFSET),
        tim_getreg(priv, AT32_GTIM_CNT_OFFSET),
        tim_getreg(priv, AT32_GTIM_PSC_OFFSET),
        tim_getreg(priv, AT32_GTIM_ARR_OFFSET));
  ainfo(" CCR1: %04x CCR2: %04x CCR3:  %04x CCR4:  %04x\n",
        tim_getreg(priv, AT32_GTIM_CCR1_OFFSET),
        tim_getreg(priv, AT32_GTIM_CCR2_OFFSET),
        tim_getreg(priv, AT32_GTIM_CCR3_OFFSET),
        tim_getreg(priv, AT32_GTIM_CCR4_OFFSET));
#if AT32_NATIM > 0
  if (priv->tbase == AT32_TMR1_BASE
#  ifdef AT32_TMR8_BASE
      || priv->tbase == AT32_TMR8_BASE
#  endif
    )
    {
      ainfo("  RCR: %04x BDTR: %04x DCR:   %04x DMAR:  %04x\n",
            tim_getreg(priv, AT32_ATIM_RCR_OFFSET),
            tim_getreg(priv, AT32_ATIM_BDTR_OFFSET),
            tim_getreg(priv, AT32_ATIM_DCR_OFFSET),
            tim_getreg(priv, AT32_ATIM_DMAR_OFFSET));
    }
  else
#endif
    {
      ainfo("  DCR: %04x DMAR: %04x\n",
            tim_getreg(priv, AT32_GTIM_DCR_OFFSET),
            tim_getreg(priv, AT32_GTIM_DMAR_OFFSET));
    }
}
#endif
  ...
  if (enable)
    {
      ...
      tim_modifyreg(priv, AT32_GTIM_CR1_OFFSET, 0, GTIM_CR1_CEN);
    }
  else
    {
      /* Disable the counter */

      tim_modifyreg(priv, AT32_GTIM_CR1_OFFSET, GTIM_CR1_CEN, 0);
    }
}
#endif
...
static int adc_timinit(struct at32_dev_s *priv)
{
  ...
  tim_modifyreg(priv, AT32_GTIM_CR1_OFFSET, clrbits, setbits);

  /* Set the reload and prescaler values */

  tim_putreg(priv, AT32_GTIM_PSC_OFFSET, prescaler - 1);
  tim_putreg(priv, AT32_GTIM_ARR_OFFSET, reload);
...
#  endif
    ...
    {
      tim_putreg(priv, AT32_ATIM_RCR_OFFSET, 0);
      tim_putreg(priv, AT32_ATIM_BDTR_OFFSET, ATIM_BDTR_MOE); /* Check me */
    }
#endif

  /* TIMx event generation: Bit 0 UG: Update generation */

  tim_putreg(priv, AT32_GTIM_EGR_OFFSET, GTIM_EGR_UG);
      ...
      case 0: /* TimerX CC1 event */
        {
          ...
          tim_putreg(priv, AT32_GTIM_CCR1_OFFSET, (uint16_t)(reload >> 1));
        }
        break;
      ...
      case 1: /* TimerX CC2 event */
        {
          ...
          tim_putreg(priv, AT32_GTIM_CCR2_OFFSET, (uint16_t)(reload >> 1));
        }
        break;
      ...
      case 2: /* TimerX CC3 event */
        {
          ...
          tim_putreg(priv, AT32_GTIM_CCR3_OFFSET, (uint16_t)(reload >> 1));
        }
        break;
      ...
      case 3: /* TimerX CC4 event */
        {
          ...
          tim_putreg(priv, AT32_GTIM_CCR4_OFFSET, (uint16_t)(reload >> 1));
        }
        break;
  ...
  ccer = tim_getreg(priv, AT32_GTIM_CCER_OFFSET);
  ccer &= ~ccenable;
  tim_putreg(priv, AT32_GTIM_CCER_OFFSET, ccer);

  /* Fetch the CR2, CCMR1, and CCMR2 register (already have ccer) */

  cr2   = tim_getreg(priv, AT32_GTIM_CR2_OFFSET);
  ccmr1 = tim_getreg(priv, AT32_GTIM_CCMR1_OFFSET);
  ccmr2 = tim_getreg(priv, AT32_GTIM_CCMR2_OFFSET);
...
#  endif
  ...
  /* For only these timers can be used: 2-4, 6, 7, 9,
   * 10. Reset the output compare and output compare N IDLE State
   */

  if (priv->tbase >= AT32_TMR2_BASE && priv->tbase <= AT32_TMR4_BASE)
    {
      /* Reset output N polarity level, output N state, output compare state,
       * output compare N idle state.
       */
  ...
  tim_putreg(priv, AT32_GTIM_CR2_OFFSET, cr2);
  tim_putreg(priv, AT32_GTIM_CCMR1_OFFSET, ccmr1);
  tim_putreg(priv, AT32_GTIM_CCMR2_OFFSET, ccmr2);
  tim_putreg(priv, AT32_GTIM_CCER_OFFSET, ccer);
  tim_putreg(priv, AT32_GTIM_EGR_OFFSET, egr);

  /* Set the ARR Preload Bit */

  tim_modifyreg(priv, AT32_GTIM_CR1_OFFSET, 0, GTIM_CR1_ARPE);
  ...
  if (enable)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_CR_OFFSET, 0, ADC_CR_ADSTART);
    }
  else
    {
      regval = adc_getreg(priv, AT32_ADC_CR_OFFSET);
      ...
      if ((regval & ADC_CR_ADSTART) != 0)
        {
          ...
          adc_putreg(priv, AT32_ADC_CR_OFFSET, regval | ADC_CR_ADSTP);

          /* Wait for the conversion to stop */

          while ((adc_getreg(priv, AT32_ADC_CR_OFFSET) &
                  ADC_CR_ADSTP) != 0);
        }
    }
}
  ...
  if (enable)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, 0, ADC_CR2_SWSTART);
    }
  else
    {
      /* Stop the conversion */

      adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, ADC_CR2_SWSTART, 0);
    }
}
#else  /* ADV IPv1 BASIC */
static void adc_reg_startconv(struct at32_dev_s *priv, bool enable)
  ...
  if (enable)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_CR_OFFSET, 0, ADC_CR_JADSTART);
    }
  else
    {
      regval = adc_getreg(priv, AT32_ADC_CR_OFFSET);
      ...
      if ((regval & ADC_CR_JADSTART) != 0)
        {
          ...
          adc_putreg(priv, AT32_ADC_CR_OFFSET, regval | ADC_CR_JADSTP);
  ...
  if (enable)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, 0, ADC_CR2_JSWSTART);
    }
  else
    {
      /* Stop the conversion */

      adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, ADC_CR2_JSWSTART, 0);
    }
}
#endif
...
static void adc_rccreset(struct at32_dev_s *priv, bool reset)
{
   ...
   * For the basic AT32 ADC IPv1, there is an individual bit to reset
  ...
  if (reset)
    {
      ...
      modifyreg32(AT32_RCC_RSTR, 0, adcbit);
    }
  else
    {
      /* Release ADC from reset state */

      modifyreg32(AT32_RCC_RSTR, adcbit, 0);
    }
}
#elif defined(HAVE_IP_ADC_V1)
static void adc_rccreset(struct at32_dev_s *priv, bool reset)
{
  ...
  /* Pick the appropriate bit in the RCC reset register.
   * For the AT32 ADC IPv1, there is one common reset for all ADCs.
   */
...
static void adc_rccreset(struct at32_dev_s *priv, bool reset)
{
  ...
  /* Pick the appropriate bit in the RCC reset register.
   * For the AT32 ADC IPv2, there is an individual bit to reset each
   * ADC block.
   */
  ...
  if (enable)
    {
      ...
      adc_putreg(priv, AT32_ADC_CR_OFFSET, regval | ADC_CR_ADEN);

      /* Wait for the ADC to be ready */

      while ((adc_getreg(priv, AT32_ADC_ISR_OFFSET) & ADC_INT_ARDY) == 0);
    }
  else if ((regval & ADC_CR_ADEN) != 0 && (regval & ADC_CR_ADDIS) == 0)
    {
      ...
      adc_putreg(priv, AT32_ADC_CR_OFFSET, regval | ADC_CR_ADDIS);

      /* Wait for the ADC to be disabled */

      while ((adc_getreg(priv, AT32_ADC_CR_OFFSET) & ADC_CR_ADEN) != 0);
    }
}
#else  /* HAVE_IP_ADC_V1 */
static void adc_enable(struct at32_dev_s *priv, bool enable)
{
#ifdef ADC_SR_ADONS
  bool enabled = (adc_getreg(priv, AT32_ADC_SR_OFFSET) & ADC_SR_ADONS) != 0;
#else
  bool enabled = false;
#endif
  ...
  if (!enabled && enable)
    {
      adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, 0, ADC_CR2_ADON);
    }
  else if (enabled && !enable)
    {
      adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, ADC_CR2_ADON, 0);
    }
}
#endif
                                ...
                                void *arg)
{
  ...
  struct at32_dev_s *priv = (struct at32_dev_s *)dev->ad_priv;
  int i;
  ...
  adc_modifyreg(priv, AT32_ADC_DMAREG_OFFSET, ADC_DMAREG_DMA, 0);
  adc_modifyreg(priv, AT32_ADC_DMAREG_OFFSET, 0, ADC_DMAREG_DMA);
}
#endif
...
#if defined(HAVE_IP_ADC_V2)
static void adc_watchdog_cfg(struct at32_dev_s *priv)
{
  uint32_t clrbits = 0;
  uint32_t setbits = 0;
  ...
  adc_putreg(priv, AT32_ADC_TR1_OFFSET, 0x0fff0000);
  ...
  adc_modifyreg(priv, AT32_ADC_CFGR1_OFFSET, clrbits, setbits);
}
#else
static void adc_watchdog_cfg(struct at32_dev_s *priv)
{
  ...
  adc_putreg(priv, AT32_ADC_HTR_OFFSET, 0x00000fff);

  /* Initialize the watchdog low threshold register */

  adc_putreg(priv, AT32_ADC_LTR_OFFSET, 0x00000000);
  ...
  adc_modifyreg(priv, AT32_ADC_CR1_OFFSET, clrbits, setbits);
}
#endif
...
#if defined(HAVE_IP_ADC_V2)
static void adc_calibrate(struct at32_dev_s *priv)
{
#if 0 /* Doesn't work */
  /* Calibrate the ADC */

  adc_modifyreg(priv, AT32_ADC_CR_OFFSET, ADC_CR_ADCALDIF, AD_CR_ADCAL);

  /* Wait for the calibration to complete */

  while ((adc_getreg(priv, AT32_ADC_CR_OFFSET) & ADC_CR_ADCAL) != 0);
...
static void adc_calibrate(struct at32_dev_s *priv)
{
  ...
  adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, 0, ADC_CR2_RSTCAL);

  /* Wait for the calibration register reset to complete */

  while ((adc_getreg(priv, AT32_ADC_CR2_OFFSET) & ADC_CR2_RSTCAL) != 0);

  /* Start ADC auto-calibration procedure  */

  adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, 0, ADC_CR2_CAL);

  /* Wait for the calibration procedure to complete */

  while ((adc_getreg(priv, AT32_ADC_CR2_OFFSET) & ADC_CR2_CAL) != 0);
...
static void adc_mode_cfg(struct at32_dev_s *priv)
{
  uint32_t clrbits = 0;
  uint32_t setbits = 0;
...
static void adc_mode_cfg(struct at32_dev_s *priv)
{
  ...
  adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, clrbits, setbits);
}
#endif
...
#if defined(HAVE_IP_ADC_V2)
static void adc_voltreg_cfg(struct at32_dev_s *priv)
{
  /* Set ADC voltage regulator to intermediate state */

  adc_modifyreg(priv, AT32_ADC_CR_OFFSET, ADC_CR_ADVREGEN_MASK,
...
#ifdef CONFIG_AT32_ADC_CHANGE_SAMPLETIME
  adc_sampletime_write((struct at32_adc_dev_s *)dev->ad_priv);
#else
  struct at32_dev_s *priv = (struct at32_dev_s *)dev->ad_priv;

  adc_putreg(priv, AT32_ADC_SMPR1_OFFSET, ADC_SMPR1_DEFAULT);
  adc_putreg(priv, AT32_ADC_SMPR2_OFFSET, ADC_SMPR2_DEFAULT);
#  ifdef AT32_ADC_SMPR3_OFFSET
  adc_putreg(priv, AT32_ADC_SMPR3_OFFSET, ADC_SMPR3_DEFAULT);
#  endif
#  ifdef AT32_ADC_SMPR0_OFFSET
  adc_putreg(priv, AT32_ADC_SMPR0_OFFSET, ADC_SMPR0_DEFAULT);
#  endif
#endif
}
...
#if defined(HAVE_IP_ADC_V2)
static void adc_common_cfg(struct at32_dev_s *priv)
{
  uint32_t clrbits = 0;
  uint32_t setbits = 0;
  ...
  adccmn_modifyreg(priv, AT32_ADC_CCR_OFFSET, clrbits, setbits);
}
#elif defined(HAVE_IP_ADC_V1) && !defined(HAVE_BASIC_ADC)
static void adc_common_cfg(struct at32_dev_s *priv)
{
...
#ifdef HAVE_IP_ADC_V2
static void adc_dma_cfg(struct at32_dev_s *priv)
{
  uint32_t clrbits = 0;
  uint32_t setbits = 0;
  ...
  if (priv->dma != NULL)
    {
      ...
      at32_dmafree(priv->dma);
    }

  priv->dma = at32_dmachannel(priv->dmachan);

#ifndef CONFIG_AT32_ADC_NOIRQ
  /* Start DMA only if standard ADC interrupts used */

  at32_dmasetup(priv->dma,
                 priv->base + AT32_ADC_DR_OFFSET,
                 (uint32_t)priv->r_dmabuffer,
                 priv->rnchannels,
                 ADC_DMA_CONTROL_WORD);

  at32_dmastart(priv->dma, adc_dmaconvcallback, dev, false);
#endif
}
#endif /* ADC_HAVE_DMA */
...
#ifndef CONFIG_AT32_ADC_NO_STARTUP_CONV
  ...
  if (enable)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, 0, regval);
    }
  else
    {
      /* Disable all ADC interrupts */

      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, ADC_IER_ALLINTS, 0);
    }
}
...
#ifdef HAVE_BASIC_ADC
#  if defined(CONFIG_AT32_ADC1)
  /* TSVREF bit is only available in the AT32_ADC1_CR2 register. */

  if (priv->intf == 1)
    {
      if (enable)
        {
          adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, 0, ADC_CR2_TSVREFE);
        }
      else
        {
          adc_modifyreg(priv, AT32_ADC_CR2_OFFSET, ADC_CR2_TSVREFE, 0);
        }
    }

  ainfo("AT32_ADC_CR2 value: 0x%08" PRIx32 "\n",
        adc_getreg(priv, AT32_ADC_CR2_OFFSET));
#  endif /* CONFIG_AT32_ADC1 */
#else    /* !HAVE_BASIC_ADC */
  if (enable)
    {
      adccmn_modifyreg(priv, AT32_ADC_CCR_OFFSET, 0, ADC_CCR_TSVREFE);
    }
  else
    {
      adccmn_modifyreg(priv, AT32_ADC_CCR_OFFSET, ADC_CCR_TSVREFE, 0);
    }

  ainfo("AT32_ADC_CCR value: 0x%08" PRIx32 "\n",
...
#if defined(HAVE_IP_ADC_V1)
  adc_modifyreg(priv, AT32_ADC_CR1_OFFSET, ADC_CR1_RES_MASK,
                res << ADC_CR1_RES_SHIFT);
#elif defined(HAVE_IP_ADC_V2)
  adc_modifyreg(priv, AT32_ADC_CFGR1_OFFSET, ADC_CFGR1_RES_MASK,
...
static int adc_extcfg_set(struct at32_dev_s *priv, uint32_t extcfg)
{
   ...
   *
   * - The position with of the EXTSEL field varies from one AT32 MCU
   *   to another.
   * - The width of the EXTSEL field varies from one AT32 MCU to another.
   */
  ...
  if (exten > 0)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_EXTREG_OFFSET, clrbits, setbits);
    }
...
static int adc_jextcfg_set(struct at32_dev_s *priv, uint32_t jextcfg)
{
   ...
   *
   * - The position with of the JEXTSEL field varies from one AT32 MCU
   *   to another.
   * - The width of the JEXTSEL field varies from one AT32 MCU to another.
   */
  ...
  if (jexten > 0)
    {
      ...
      adc_modifyreg(priv, AT32_ADC_JEXTREG_OFFSET, clrbits, setbits);
    }
...
static void adc_dumpregs(struct at32_dev_s *priv)
{
  ...
  ainfo("ISR:  0x%08" PRIx32 " IER:  0x%08" PRIx32
        ...
        adc_getreg(priv, AT32_ADC_ISR_OFFSET),
        adc_getreg(priv, AT32_ADC_IER_OFFSET),
        adc_getreg(priv, AT32_ADC_CR_OFFSET),
        adc_getreg(priv, AT32_ADC_CFGR1_OFFSET));
#else
  ainfo("SR:   0x%08" PRIx32 " CR1:  0x%08" PRIx32
        " CR2:  0x%08" PRIx32 "\n",
        adc_getreg(priv, AT32_ADC_SR_OFFSET),
        adc_getreg(priv, AT32_ADC_CR1_OFFSET),
        adc_getreg(priv, AT32_ADC_CR2_OFFSET));
#endif

  ainfo("SQR1: 0x%08" PRIx32 " SQR2: 0x%08" PRIx32
        " SQR3: 0x%08" PRIx32 "\n",
        adc_getreg(priv, AT32_ADC_SQR1_OFFSET),
        adc_getreg(priv, AT32_ADC_SQR2_OFFSET),
        adc_getreg(priv, AT32_ADC_SQR3_OFFSET));

  ainfo("SMPR1: 0x%08" PRIx32 " SMPR2: 0x%08" PRIx32 "\n",
        adc_getreg(priv, AT32_ADC_SMPR1_OFFSET),
        adc_getreg(priv, AT32_ADC_SMPR2_OFFSET));

#if defined(AT32_ADC_SQR4_OFFSET)
  ainfo("SQR4: 0x%08" PRIx32 "\n",
        adc_getreg(priv, AT32_ADC_SQR4_OFFSET));
#endif

#if defined(AT32_ADC_SQR5_OFFSET)
  ainfo("SQR5: 0x%08" PRIx32 "\n",
        adc_getreg(priv, AT32_ADC_SQR5_OFFSET));
#endif

#ifdef ADC_HAVE_INJECTED
  ainfo("JSQR: 0x%08" PRIx32 "\n", adc_getreg(priv, AT32_ADC_JSQR_OFFSET));
#endif
...
static void adc_enable_vbat_channel(struct adc_dev_s *dev, bool enable)
{
  ...
  if (enable)
    {
      adccmn_modifyreg(priv, AT32_ADC_CCR_OFFSET, 0, ADC_CCR_VBATEN);
    }
  else
    {
      adccmn_modifyreg(priv, AT32_ADC_CCR_OFFSET, ADC_CCR_VBATEN, 0);
    }
...
static void adc_ioc_enable_awd_int(struct at32_dev_s *priv, bool enable)
{
  if (enable)
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, 0, ADC_IER_AWD);
    }
  else
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, ADC_IER_AWD, 0);
    }
}
...
static void adc_ioc_enable_eoc_int(struct at32_dev_s *priv, bool enable)
{
  if (enable)
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, 0, ADC_IER_EOC);
    }
  else
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, ADC_IER_EOC, 0);
    }
}
...
static void adc_ioc_enable_jeoc_int(struct at32_dev_s *priv,
                                    bool enable)
{
  if (enable)
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, 0, ADC_IER_JEOC);
    }
  else
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, ADC_IER_JEOC, 0);
    }
}
...
static void adc_ioc_enable_ovr_int(struct at32_dev_s *priv, bool enable)
{
  if (enable)
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, 0, ADC_IER_OVR);
    }
  else
    {
      adc_modifyreg(priv, AT32_ADC_IER_OFFSET, ADC_IER_OVR, 0);
    }
}
  ...
  if (enable)
    {
      ...
      at32_modifyreg32(AT32_RCC_CR, 0, RCC_CR_HSION);
      while ((getreg32(AT32_RCC_CR) & RCC_CR_HSIRDY) == 0);
    }
  else
    {
      /* Disable the HSI */

      at32_modifyreg32(AT32_RCC_CR, RCC_CR_HSION, 0);
    }
}
#endif
...
#ifdef AT32_ADC_SQR5_OFFSET
  bits = adc_sqrbits(priv, ADC_SQR5_FIRST, ADC_SQR5_LAST,
                     ADC_SQR5_SQ_OFFSET);
  adc_modifyreg(priv, AT32_ADC_SQR5_OFFSET, ~ADC_SQR5_RESERVED, bits);
#endif

#ifdef AT32_ADC_SQR4_OFFSET
  bits = adc_sqrbits(priv, ADC_SQR4_FIRST, ADC_SQR4_LAST,
                     ADC_SQR4_SQ_OFFSET);
  adc_modifyreg(priv, AT32_ADC_SQR4_OFFSET, ~ADC_SQR4_RESERVED, bits);
#endif

  bits = adc_sqrbits(priv, ADC_SQR3_FIRST, ADC_SQR3_LAST,
                     ADC_SQR3_SQ_OFFSET);
  adc_modifyreg(priv, AT32_ADC_SQR3_OFFSET, ~ADC_SQR3_RESERVED, bits);

  bits = adc_sqrbits(priv, ADC_SQR2_FIRST, ADC_SQR2_LAST,
                     ADC_SQR2_SQ_OFFSET);
  adc_modifyreg(priv, AT32_ADC_SQR2_OFFSET, ~ADC_SQR2_RESERVED, bits);

  bits = ((uint32_t)priv->rnchannels - 1) << ADC_SQR1_L_SHIFT;
  bits |= adc_sqrbits(priv, ADC_SQR1_FIRST,
                      ADC_SQR1_LAST, ADC_SQR1_SQ_OFFSET);
  adc_modifyreg(priv, AT32_ADC_SQR1_OFFSET, ~ADC_SQR1_RESERVED, bits);
...
static int adc_inj_set_ch(struct adc_dev_s *dev, uint8_t ch)
{
  ...
  adc_modifyreg(priv, AT32_ADC_JSQR_OFFSET, clrbits, setbits);
...
static int adc_interrupt(struct adc_dev_s *dev)
{
  ...
  regval  = adc_getreg(priv, AT32_ADC_ISR_OFFSET);
  ...
  if ((regval & ADC_ISR_EOC) != 0)
    {
      ...
      data = adc_getreg(priv, AT32_ADC_DR_OFFSET) & ADC_DR_RDATA_MASK;
  ...
  adc_putreg(priv, AT32_ADC_ISR_OFFSET, pending);
 ...
 *
 * Description:
 *   ADC interrupt handler for the AT32 L15XX family.
 *
 * Input Parameters:
 ...
 *
 * Description:
 *   ADC1/2 interrupt handler for the AT32 F1/F3 families.
 *
 * Input Parameters:
 *
 * Returned Value:
...
#if defined(AT32_IRQ_ADC12) && \
    (defined(CONFIG_AT32_ADC1) || defined(CONFIG_AT32_ADC2))
static int adc12_interrupt(int irq, void *context, void *arg)
{
#ifdef CONFIG_AT32_ADC1
  adc_interrupt(&g_adcdev1);
 ...
 *
 * Description:
 *   ADC3 interrupt handler for the AT32 F1 family.
 *
 * Input Parameters:
 *
 * Returned Value:
 ...
 *
 * Description:
 *   ADC1/2/3 interrupt handler for the AT32 F2/F4 families.
 *
 * Input Parameters:
 *
 * Returned Value:
...
static void adc_intack(struct at32_adc_dev_s *dev, uint32_t source)
{
  ...
  adc_putreg(priv, AT32_ADC_ISR_OFFSET, (source & ADC_ISR_ALLINTS));
#else
  /* Cleared by writing 0 to it */

  adc_modifyreg(priv, AT32_ADC_ISR_OFFSET, (source & ADC_ISR_ALLINTS), 0);
#endif
}
...
static void adc_inten(struct at32_adc_dev_s *dev, uint32_t source)
{
  ...
  adc_modifyreg(priv, AT32_ADC_IER_OFFSET, 0, (source & ADC_IER_ALLINTS));
}
...
static void adc_intdis(struct at32_adc_dev_s *dev, uint32_t source)
{
  ...
  adc_modifyreg(priv, AT32_ADC_IER_OFFSET, (source & ADC_IER_ALLINTS), 0);
}
...
static uint32_t adc_regget(struct at32_adc_dev_s *dev)
{
  ...
  return adc_getreg(priv, AT32_ADC_DR_OFFSET) & ADC_DR_RDATA_MASK;
}
                              ...
                              uint16_t *buffer, uint8_t len)
{
  ...
  at32_dmastart(priv->dma, NULL, dev, false);
...
static uint32_t adc_injget(struct at32_adc_dev_s *dev, uint8_t chan)
{
  ...
  regval = adc_getreg(priv, AT32_ADC_JDR1_OFFSET + 4 * (chan)) &
           ADC_JDR_JDATA_MASK;
  ...
  for (i = 0, shift = 0; i < priv->adc_channels; i++)
    {
      ...
      switch (i)
        {
#if defined(AT32_ADC_SMPR0_OFFSET) && defined(AT32_ADC_SMPR3_OFFSET)
          case 9:
            {
              adc_putreg(priv, AT32_ADC_SMPR3_OFFSET, value);
              shift = 0;
              value = 0;
              break;
            }
          ...
          case 19:
            {
              adc_putreg(priv, AT32_ADC_SMPR2_OFFSET, value);
              shift = 0;
              value = 0;
              break;
            }
          ...
          case 29:
            {
              adc_putreg(priv, AT32_ADC_SMPR1_OFFSET, value);
              shift = 0;
              value = 0;
              break;
            }
          ...
          case (ADC_CHANNELS_NUMBER - 1):
            {
              adc_putreg(priv, AT32_ADC_SMPR0_OFFSET, value);
              shift = 0;
              value = 0;
              break;
            }

#elif defined(AT32_ADC_SMPR1_OFFSET) && defined(AT32_ADC_SMPR2_OFFSET)
          case (ADC_CHANNELS_NUMBER - 1):
            {
              adc_putreg(priv, AT32_ADC_SMPR2_OFFSET, value);
              shift = 0;
...
void adc_sampletime_set(struct at32_adc_dev_s *dev,
                        struct adc_sample_time_s *time_samples)
{
  struct at32_dev_s *priv = (struct at32_dev_s *)dev;
  uint8_t ch_index;
 ...
 *   with CONFIG_AT32_ADCx_INJECTED_CHAN definitions
 *
 ...
 *
 *   where:
 *      y = CONFIG_AT32_ADCx_INJECTED_CHAN, and y > 0
 *
 *   If CONFIG_AT32_ADCx_INJECTED_CHAN = 0, then all channels from chanlist
      ...
      case 1:
        {
          ainfo("ADC1 selected\n");
          dev = &g_adcdev1;
          cj_channels = CONFIG_AT32_ADC1_INJECTED_CHAN;
          cr_channels = channels - cj_channels;
#  ifdef ADC_HAVE_INJECTED
          if (cj_channels > 0)
            {
      ...
      case 2:
        {
          ainfo("ADC2 selected\n");
          dev = &g_adcdev2;
          cj_channels = CONFIG_AT32_ADC2_INJECTED_CHAN;
          cr_channels = channels - cj_channels;
#  ifdef ADC_HAVE_INJECTED
          if (cj_channels > 0)
            {
...
#endif /* CONFIG_AT32_ADC2 */
#ifdef CONFIG_AT32_ADC3
      case 3:
        {
          ainfo("ADC3 selected\n");
          dev = &g_adcdev3;
          cj_channels = CONFIG_AT32_ADC3_INJECTED_CHAN;
          cr_channels = channels - cj_channels;
#  ifdef ADC_HAVE_INJECTED
          if (cj_channels > 0)
            {
...
#  endif
  ...
  priv = (struct at32_dev_s *)dev->ad_priv;

  /* Configure regular channels */

  DEBUGASSERT(cr_channels <= CONFIG_AT32_ADC_MAX_SAMPLES);
  if (cr_channels > CONFIG_AT32_ADC_MAX_SAMPLES)
    {
      cr_channels = CONFIG_AT32_ADC_MAX_SAMPLES;
    }
