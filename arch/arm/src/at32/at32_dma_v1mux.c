...
#ifndef CONFIG_AT32_DMAMUX
#  error "Configuration error, CONFIG_AT32_DMAMUX not defined!"
#endif
...
typedef const struct at32_dmamux_s *DMA_MUX;

/* This structure describes one DMA controller */

struct at32_dma_s
{
  uint8_t       first;           /* Offset in at32_dmach_s array */
...
typedef struct at32_dmach_s *DMA_CHANNEL;

/* DMA operations */

struct at32_dma_ops_s
{
  ...
  void (*dma_sample)(DMA_HANDLE handle, struct at32_dmaregs_s *regs);
...
#if defined(CONFIG_AT32_DMA1) || defined(CONFIG_AT32_DMA2)
static void at32_dma12_disable(DMA_CHANNEL dmachan);
static int at32_dma12_interrupt(int irq, void *context, void *arg);
static void at32_dma12_setup(DMA_HANDLE handle, uint32_t paddr,
                              uint32_t maddr, size_t ntransfers,
                              uint32_t ccr);
static void at32_dma12_start(DMA_HANDLE handle, dma_callback_t callback,
                              void *arg, bool half);
static size_t at32_dma12_residual(DMA_HANDLE handle);
#ifdef CONFIG_DEBUG_DMA_INFO
static void at32_dma12_sample(DMA_HANDLE handle,
                               struct at32_dmaregs_s *regs);
static void at32_dma12_dump(DMA_HANDLE handle,
                             const struct at32_dmaregs_s *regs,
                             const char *msg);
#endif
#endif
...
static void at32_dmamux_sample(DMA_MUX dmamux, uint8_t chan,
                                struct at32_dmaregs_s *regs);
static void at32_dmamux_dump(DMA_MUX dmamux, uint8_t channel,
                              const struct at32_dmaregs_s *regs);
#endif
static DMA_CHANNEL at32_dma_channel_get(uint8_t channel,
                                         uint8_t controller);
static void at32_gdma_limits_get(uint8_t controller, uint8_t *first,
                                  uint8_t *last);
...
static const struct at32_dma_ops_s g_dma_ops[DMA_CONTROLLERS] =
{
#ifdef CONFIG_AT32_DMA1
  /* 0 - DMA1 */

    {
      .dma_disable   = at32_dma12_disable,
      .dma_interrupt = at32_dma12_interrupt,
      .dma_setup     = at32_dma12_setup,
      .dma_start     = at32_dma12_start,
      .dma_residual  = at32_dma12_residual,
#ifdef CONFIG_DEBUG_DMA_INFO
      .dma_sample    = at32_dma12_sample,
      .dma_dump      = at32_dma12_dump,
#endif
    },
#else
    {
...
static const struct at32_dmamux_s g_dmamux[DMAMUX_NUM] =
{
  /* 1 - DMA1 */
  ...
  /* 2 - DMA2 */

    {
      .id      = 2,
      .nchan   = DMAMUX_NCHANNELS,
      .base    = AT32_DMAMUX2_BASE
    }
};

/* This array describes the state of each controller */

static const struct at32_dma_s g_dma[DMA_NCHANNELS] =
{
  /* 0 - DMA1 */
  ...
  /* 1 - DMA2 */

    {
      ...
      .first  = DMA2_FIRST,
      .nchan  = DMA2_NCHAN,
      .dmamux = &g_dmamux[DMAMUX2], /* DMAMUX1 channels 7-13 */
      .dmamux_offset = 7
    }
};
...
static struct at32_dmach_s g_dmach[DMA_NCHANNELS] =
{
#ifdef CONFIG_AT32_DMA1
  /* DMA1 */

    {
      .ctrl     = DMA1,
      .chan     = 0,
      .irq      = AT32_IRQ_DMA1CH1,
      .shift    = DMA_CHAN_SHIFT(0),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(0),
    },

    {
      .ctrl     = DMA1,
      .chan     = 1,
      .irq      = AT32_IRQ_DMA1CH2,
      .shift    = DMA_CHAN_SHIFT(1),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(1),
    },

    {
      .ctrl     = DMA1,
      .chan     = 2,
      .irq      = AT32_IRQ_DMA1CH3,
      .shift    = DMA_CHAN_SHIFT(2),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(2),
    },

    {
      .ctrl     = DMA1,
      .chan     = 3,
      .irq      = AT32_IRQ_DMA1CH4,
      .shift    = DMA_CHAN_SHIFT(3),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(3),
    },

    {
      .ctrl     = DMA1,
      .chan     = 4,
      .irq      = AT32_IRQ_DMA1CH5,
      .shift    = DMA_CHAN_SHIFT(4),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(4),
    },

    {
      .ctrl     = DMA1,
      .chan     = 5,
      .irq      = AT32_IRQ_DMA1CH6,
      .shift    = DMA_CHAN_SHIFT(5),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(5),
    },

    {
      .ctrl     = DMA1,
      .chan     = 6,
      .irq      = AT32_IRQ_DMA1CH7,
      .shift    = DMA_CHAN_SHIFT(6),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(6),
    },
...
#  if DMA1_NCHAN > 7
    {
      .ctrl     = DMA1,
      .chan     = 7,
      .irq      = AT32_IRQ_DMA1CH8,
      .shift    = DMA_CHAN_SHIFT(7),
      .base     = AT32_DMA1_BASE + AT32_DMACHAN_OFFSET(7),
    },
#  endif
#endif
  ...
  /* DMA2 */

    {
      .ctrl     = DMA2,
      .chan     = 0,
      .irq      = AT32_IRQ_DMA2CH1,
      .shift    = DMA_CHAN_SHIFT(0),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(0),
    },

    {
      .ctrl     = DMA2,
      .chan     = 1,
      .irq      = AT32_IRQ_DMA2CH2,
      .shift    = DMA_CHAN_SHIFT(1),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(1),
    },

    {
      .ctrl     = DMA2,
      .chan     = 2,
      .irq      = AT32_IRQ_DMA2CH3,
      .shift    = DMA_CHAN_SHIFT(2),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(2),
    },

    {
      .ctrl     = DMA2,
      .chan     = 3,
      .irq      = AT32_IRQ_DMA2CH4,
      .shift    = DMA_CHAN_SHIFT(3),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(3),
    },

    {
      .ctrl     = DMA2,
      .chan     = 4,
      .irq      = AT32_IRQ_DMA2CH5,
      .shift    = DMA_CHAN_SHIFT(4),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(4),
    },

    {
      .ctrl     = DMA2,
      .chan     = 5,
      .irq      = AT32_IRQ_DMA2CH6,
      .shift    = DMA_CHAN_SHIFT(5),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(5),
    },
...
#  if DMA2_NCHAN > 6
    {
      .ctrl     = DMA2,
      .chan     = 6,
      .irq      = AT32_IRQ_DMA2CH7,
      .shift    = DMA_CHAN_SHIFT(6),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(6),
    },
#  endif
...
#  if DMA2_NCHAN > 7
    {
      .ctrl     = DMA2,
      .chan     = 7,
      .irq      = AT32_IRQ_DMA2CH8,
      .shift    = DMA_CHAN_SHIFT(7),
      .base     = AT32_DMA2_BASE + AT32_DMACHAN_OFFSET(7),
    },
#  endif
#endif
};
                                         ...
                                         uint8_t controller)
{
  ...
  at32_gdma_limits_get(controller, &first, &nchan);
...
static void at32_dma12_disable(DMA_CHANNEL dmachan)
{
  ...
  regval = dmachan_getreg(dmachan, AT32_DMACHAN_CCR_OFFSET);
  regval &= ~DMA_CCR_ALLINTS;

  /* Disable the DMA channel */

  regval &= ~DMA_CCR_EN;
  dmachan_putreg(dmachan, AT32_DMACHAN_CCR_OFFSET, regval);

  /* Clear pending channel interrupts */

  dmabase_putreg(dmachan, AT32_DMA_IFCR_OFFSET,
                 DMA_ISR_CHAN_MASK(dmachan->chan));
}

/****************************************************************************
 * Name: at32_dma12_interrupt
 *
 * Description:
...
static int at32_dma12_interrupt(int irq, void *context, void *arg)
{
...
#ifdef CONFIG_AT32_DMA1
  else if (irq >= AT32_IRQ_DMA1CH1 && irq <= AT32_IRQ_DMA1CH5)
    {
      channel = irq - AT32_IRQ_DMA1CH1;
      controller = DMA1;
    }
  else if (irq >= AT32_IRQ_DMA1CH6 && irq <= AT32_IRQ_DMA1CH7)
    {
      channel = irq - AT32_IRQ_DMA1CH6 + (6 - 1);
      controller = DMA1;
    }
#endif
#ifdef CONFIG_AT32_DMA2
  else if (irq >= AT32_IRQ_DMA2CH1 && irq <= AT32_IRQ_DMA2CH7)
    {
      channel = irq - AT32_IRQ_DMA2CH1;
      controller = DMA2;
    }
#endif
  else
  ...
  dmachan = at32_dma_channel_get(channel, controller);

  /* Get the interrupt status (for this channel only) */

  isr = dmabase_getreg(dmachan, AT32_DMA_ISR_OFFSET) &
        DMA_ISR_CHAN_MASK(dmachan->chan);
  ...
  dmabase_putreg(dmachan, AT32_DMA_IFCR_OFFSET, isr);

  return OK;
}

/****************************************************************************
 * Name: at32_dma12_setup
 *
 * Description:
                              ...
                              uint32_t ccr)
{
  ...
  dmachan_putreg(dmachan, AT32_DMACHAN_CPAR_OFFSET, paddr);

  /* Set the memory address in the DMA_CMARx register. The data will be
   * written to or read from this memory after the peripheral event.
   */

  dmachan_putreg(dmachan, AT32_DMACHAN_CMAR_OFFSET, maddr);
  ...
  dmachan_putreg(dmachan, AT32_DMACHAN_CNDTR_OFFSET, ntransfers);
                              ...
                              void *arg, bool half)
{
  ...
  ccr  = dmachan_getreg(dmachan, AT32_DMACHAN_CCR_OFFSET);
  ccr |= DMA_CCR_EN;
  ...
  dmachan_putreg(dmachan, AT32_DMACHAN_CCR_OFFSET, ccr);
}

/****************************************************************************
 * Name: at32_dma12_residual
 ****************************************************************************/
...
static size_t at32_dma12_residual(DMA_HANDLE handle)
{
  ...
  return dmachan_getreg(dmachan, AT32_DMACHAN_CNDTR_OFFSET);
}

/****************************************************************************
 * Name: at32_dma12_sample
 ****************************************************************************/

#ifdef CONFIG_DEBUG_DMA_INFO
void at32_dma12_sample(DMA_HANDLE handle, struct at32_dmaregs_s *regs)
{
  DMA_CHANNEL dmachan = (DMA_CHANNEL)handle;
  irqstate_t flags;
  ...
  regs->isr = dmabase_getreg(dmachan, AT32_DMA_ISR_OFFSET);
  regs->ccr = dmachan_getreg(dmachan, AT32_DMACHAN_CCR_OFFSET);
  regs->cndtr = dmachan_getreg(dmachan, AT32_DMACHAN_CNDTR_OFFSET);
  regs->cpar = dmachan_getreg(dmachan, AT32_DMACHAN_CPAR_OFFSET);
  regs->cmar = dmachan_getreg(dmachan, AT32_DMACHAN_CMAR_OFFSET);

#if 0
  at32_dmamux_sample(g_dma[dmachan->ctrl].dmamux,
                             ...
                             const char *msg)
{
  ...
  dmainfo("DMA%d Registers: %s\n",
          ...
          msg);
  dmainfo("    ISR[%08x]: %08x\n",
          dmabase + AT32_DMA_ISR_OFFSET,
          regs->isr);
  dmainfo("    CCR[%08x]: %08x\n",
          dmachan->base + AT32_DMACHAN_CCR_OFFSET,
          regs->ccr);
  dmainfo("  CNDTR[%08x]: %08x\n",
          dmachan->base + AT32_DMACHAN_CNDTR_OFFSET,
          regs->cndtr);
  dmainfo("   CPAR[%08x]: %08x\n",
          dmachan->base + AT32_DMACHAN_CPAR_OFFSET,
          regs->cpar);
  dmainfo("   CMAR[%08x]: %08x\n",
          dmachan->base + AT32_DMACHAN_CMAR_OFFSET,
          regs->cmar);

  at32_dmamux_dump(g_dma[dmachan->ctrl].dmamux,
                      dmachan->chan + g_dma[dmachan->ctrl].dmamux_offset,
                      regs);
}
#endif

#endif /* CONFIG_AT32_DMA1 || CONFIG_AT32_DMA2 */

/****************************************************************************
 * Name: at32_dmamux_sample
 ****************************************************************************/
...
{
  #if 0
  regs->dmamux.ccr = dmamux_getreg(dmamux, AT32_DMAMUX_CXCR_OFFSET(chan));
  regs->dmamux.csr = dmamux_getreg(dmamux, AT32_DMAMUX_CSR_OFFSET);
  regs->dmamux.rg0cr = dmamux_getreg(dmamux, AT32_DMAMUX_RG0CR_OFFSET);
  regs->dmamux.rg1cr = dmamux_getreg(dmamux, AT32_DMAMUX_RG1CR_OFFSET);
  regs->dmamux.rg2cr = dmamux_getreg(dmamux, AT32_DMAMUX_RG2CR_OFFSET);
  regs->dmamux.rg3cr = dmamux_getreg(dmamux, AT32_DMAMUX_RG3CR_OFFSET);
  regs->dmamux.rgsr = dmamux_getreg(dmamux, AT32_DMAMUX_RGSR_OFFSET);
  #endif
}
#endif

/****************************************************************************
 * Name: at32_dmamux_dump
 ****************************************************************************/
                              ...
                              const struct at32_dmaregs_s *regs)
{
  ...
  dmainfo("    CCR[%08x]: %08x\n",
          ...
          regs->dmamux.ccr);
  dmainfo("    CSR[%08x]: %08x\n",
          dmamux->base + AT32_DMAMUX_CSR_OFFSET, regs->dmamux.csr);
  dmainfo("  RG0CR[%08x]: %08x\n",
          dmamux->base + AT32_DMAMUX_RG0CR_OFFSET, regs->dmamux.rg0cr);
  dmainfo("  RG1CR[%08x]: %08x\n",
          dmamux->base + AT32_DMAMUX_RG1CR_OFFSET, regs->dmamux.rg1cr);
  dmainfo("  RG2CR[%08x]: %08x\n",
          dmamux->base + AT32_DMAMUX_RG2CR_OFFSET, regs->dmamux.rg2cr);
  dmainfo("  RG3CR[%08x]: %08x\n",
          dmamux->base + AT32_DMAMUX_RG3CR_OFFSET, regs->dmamux.rg3cr);
  dmainfo("   RGSR[%08x]: %08x\n",
          dmamux->base + AT32_DMAMUX_RGSR_OFFSET, regs->dmamux.rgsr);
#endif
};
#endif
  ...
  for (channel = 0; channel < DMA_NCHANNELS; channel++)
    {
      ...
      dmamux_putreg(g_dma[controller].dmamux, 0, 1);
    }
}
 ...
 *
 * Input Parameters:
 *   dmamap - Identifies the stream/channel resource. For the AT32+, this
 *     is a bit-encoded value as provided by the DMAMAP_* definitions
 *     in hardware/at32g4xxxx_dmamux.h
 *
 * Returned Value:
...
DMA_HANDLE at32_dmachannel(unsigned int dmamap)
{
  DMA_CHANNEL dmachan;
  uint8_t dmamux_req;
  irqstate_t flags;
                      ...
                      bool half)
{
  ...
  dmamux_putreg(dmamux, AT32_DMAMUX_CXCR_OFFSET(dmamux_chan), regval);
 ...
 *   reset and at32_dmasetup() must be called before at32_dmastart()
 *   can be called again
 *
 * Assumptions:
 *   - DMA handle allocated by at32_dmachannel()
...
void at32_dmastop(DMA_HANDLE handle)
{
  ...
  dmamux_putreg(dmamux, AT32_DMAMUX_CXCR_OFFSET(dmamux_chan), 0);
}
...
bool at32_dmacapable(uint32_t maddr, uint32_t count, uint32_t ccr)
{
  ...
  if ((maddr & AT32_REGION_MASK) != (mend & AT32_REGION_MASK))
    {
      return false;
    }

  switch (maddr & AT32_REGION_MASK)
    {
      case AT32_PERIPH_BASE:
      case AT32_FMC_PC_CARD:
      case AT32_FSMC_BANK1:
      case AT32_FMC_QSPI2:
      case AT32_FSMC_BANK3:
      case AT32_FMC_QSPI_BASE:
      case AT32_SRAM_BASE:
      case AT32_CODE_BASE:
      case AT32_FMX_SDRAM:
