/****************************************************************************
 * arch/arm/src/stm32u0/stm32u0_rcc.c
 *
...
static inline void rcc_reset(void)
{
  ...
  /* Make sure that MSI is on and ready.  MSI is the default system clock
   * source after reset.
   */
  ...
  while ((getreg32(STM32_RCC_CR) & RCC_CR_MSIRDY) == 0);

  /* Reset CFGR register (MSI as the system clock) */
  ...
  putreg32(RCC_CR_MSION, STM32_RCC_CR);

  /* Reset PLLCFGR register */

  putreg32(0, STM32_RCC_PLLCFG);

  /* DBG clock enable */

  putreg32(RCC_DBGCFGR_DBGEN, STM32_RCC_DBGCFGR);
}
...
static inline void rcc_enableapb1(void)
{
  ...
  /* LPUART 1 clock enable */

  regval |= RCC_APB1ENR_LPUART1EN;
#endif
...
static void stm32_stdclockconfig(void)
{
  ...
  /* Enable the source clock for the PLL (via HSE, HSI or MSI), HSE, HSI
   * or MSI.
   */
...
#elif (STM32_SYSCLK_SW == RCC_CFGR_SW_MSI) || \
      ((STM32_SYSCLK_SW == RCC_CFGR_SW_PLL) && STM32_PLLCFG_PLLSRC == RCC_PLLCFG_PLLSRC_MSI)

  /* The PLL is using the MSI, or the MSI is the system clock.  In either
   * case, we need to enable MSI clocking with the board provided range.
   * The MSI range can be modified only when MSI is off or when MSI is
   * ready - both conditions are satisfied here since rcc_reset() left us
   * running on a ready MSI.
   */

  regval  = getreg32(STM32_RCC_CR);
#ifdef STM32_MSI_RANGE
  regval &= ~RCC_CR_MSIRANGE_MASK;
  regval |= STM32_MSI_RANGE | RCC_CR_MSIRGSEL;
#endif
  regval |= RCC_CR_MSION;
  putreg32(regval, STM32_RCC_CR);

  /* Wait until the MSI clock is ready.  Since this is an internal clock,
   * no timeout is expected
   */
