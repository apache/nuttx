...
typedef int (*foreach_qtd_t)(struct lpc43_qtd_s *qtd, uint32_t **bp,
                             void *arg);
...
static int lpc43_qtd_foreach(struct lpc43_qh_s *qh, foreach_qtd_t handler,
         void *arg);
static int lpc43_qtd_discard(struct lpc43_qtd_s *qtd, uint32_t **bp,
         void *arg);
...
static int lpc43_qtd_addbpl(struct lpc43_qtd_s *qtd, const void *buffer,
         size_t buflen);
static struct lpc43_qtd_s *lpc43_qtd_setupphase(
...
static int lpc43_qtd_ioccheck(struct lpc43_qtd_s *qtd, uint32_t **bp,
                              void *arg);
static int lpc43_qh_ioccheck(struct lpc43_qh_s *qh, uint32_t **bp,
                             void *arg);
#ifdef CONFIG_USBHOST_ASYNCH
static int lpc43_qtd_cancel(struct lpc43_qtd_s *qtd, uint32_t **bp,
                            void *arg);
...
static void lpc43_ehci_bottomhalf(void *arg);
static int lpc43_ehci_interrupt(int irq, void *context, void *arg);

/* USB Host Controller Operations *******************************************/

static int lpc43_wait(struct usbhost_connection_s *conn,
         struct usbhost_hubport_s **hport);
static int lpc43_rh_enumerate(struct usbhost_connection_s *conn,
         struct usbhost_hubport_s *hport);
static int lpc43_enumerate(struct usbhost_connection_s *conn,
         struct usbhost_hubport_s *hport);

static int lpc43_ep0configure(struct usbhost_driver_s *drvr,
         usbhost_ep_t ep0, uint8_t funcaddr, uint8_t speed,
         uint16_t maxpacketsize);
static int lpc43_epalloc(struct usbhost_driver_s *drvr,
                         const struct usbhost_epdesc_s *epdesc,
                         usbhost_ep_t *ep);
static int lpc43_epfree(struct usbhost_driver_s *drvr, usbhost_ep_t ep);
static int lpc43_alloc(struct usbhost_driver_s *drvr,
         uint8_t **buffer, size_t *maxlen);
static int lpc43_free(struct usbhost_driver_s *drvr,
         uint8_t *buffer);
static int lpc43_ioalloc(struct usbhost_driver_s *drvr,
         uint8_t **buffer, size_t buflen);
static int lpc43_iofree(struct usbhost_driver_s *drvr,
         uint8_t *buffer);
static int lpc43_ctrlin(struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
         const struct usb_ctrlreq_s *req, uint8_t *buffer);
static int lpc43_ctrlout(struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
         const struct usb_ctrlreq_s *req, const uint8_t *buffer);
static ssize_t lpc43_transfer(struct usbhost_driver_s *drvr,
         usbhost_ep_t ep, uint8_t *buffer, size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static int lpc43_asynch(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
         uint8_t *buffer, size_t buflen, usbhost_asynch_t callback,
         void *arg);
#endif
static int lpc43_cancel(struct usbhost_driver_s *drvr, usbhost_ep_t ep);
#ifdef CONFIG_USBHOST_HUB
static int lpc43_connect(struct usbhost_driver_s *drvr,
         struct usbhost_hubport_s *hport, bool connected);
#endif
static void lpc43_disconnect(struct usbhost_driver_s *drvr,
                             struct usbhost_hubport_s *hport);
...
static const struct lpc43_ehci_trace_s g_trace1[TRACE1_NSTRINGS] =
{
  ...
  TRENTRY(EHCI_TRACE1_QTDFOREACH_FAILED, TR_FMT1,
          "EHCI ERROR: lpc43_qtd_foreach failed: %d\n"),
  ...
  TRENTRY(EHCI_TRACE1_ADDBPL_FAILED, TR_FMT1,
          "EHCI ERROR: lpc43_qtd_addbpl failed: %d\n"),
  ...
  TRENTRY(EHCI_TRACE1_RESET_FAILED, TR_FMT1,
          "EHCI ERROR: lpc43_reset failed: %d\n"),
  ...
  while (qh)
    {
      ...
      else if (lpc43_virtramaddr(physaddr & QH_HLP_MASK) ==
 ...
 *
 * Description:
 *   This is a lpc43_qtd_foreach callback.  It simply unlinks the QTD,
 ...
 *
 * Description:
 *   This is a lpc43_qtd_foreach call-out function.  It dumps the context of
 *   one qTD
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   This is a lpc43_qh_foreach callout function.  It dumps a QH structure
                                          ...
                                          struct lpc43_epinfo_s *epinfo)
{
  ...
  regval = ((uint32_t)epinfo->devaddr << QH_EPCHAR_DEVADDR_SHIFT) |
           ...
           ((uint32_t)lpc43_ehci_speed(epinfo->speed) <<
           QH_EPCHAR_EPS_SHIFT) |
           QH_EPCHAR_DTC |
...
static struct lpc43_qtd_s *
  lpc43_qtd_setupphase(struct lpc43_epinfo_s *epinfo,
 ...
 *
 * Description:
 *   This function is a lpc43_qtd_foreach() callback function.  It services
 ...
 *
 * Description:
 *   This function is a lpc43_qh_foreach() callback function.  It services
 ...
 *
 * Description:
 *   This function is a lpc43_qtd_foreach() callback function.  It removes
...
static inline void lpc43_ioc_bottomhalf(void)
{
  ...
  qh = (struct lpc43_qh_s *)
       lpc43_virtramaddr(lpc43_swap32(*bp) & QH_HLP_MASK);
                         ...
                         usbhost_ep_t *ep)
{
  ...
  epinfo = (struct lpc43_epinfo_s *)
    kmm_zalloc(sizeof(struct lpc43_epinfo_s));
  if (!epinfo)
    {
...
static int lpc43_free(struct usbhost_driver_s *drvr, uint8_t *buffer)
{
                         ...
                         uint8_t **buffer, size_t buflen)
{
  ...
  /* The only special requirements for I/O buffers are they might need to be
   * user accessible (depending on how the class driver implements its
   * buffering).
   */
...
{
  /* lpc43_ctrlin can handle both directions.  We just need to work around
...
struct usbhost_connection_s *lpc43_ehci_initialize(int controller)
{
  ...
  g_asynchead.hw.epchar        = lpc43_swap32(QH_EPCHAR_H |
                                              QH_EPCHAR_EPS_FULL);
