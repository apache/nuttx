...
/* The Flexcom SPI "port" number used as an input to sam_flexcom_spibus_initialize encodes
 * information about the Flexcom SPI controller (0/1/2/3/4) and the SPI chip select
 * (0-1)
 */
...
struct flexcom_spi_dev_s;  /* Forward reference */
...
/****************************************************************************
 * Name:  sam_flexcom_spi[0|1|2|3|4]select, sam_flexcom_spi[0|1|2|3|4]status, and sam_flexcom_spi[0|1|2|3|4]cmddata
 *
 * Description:
 *   These external functions must be provided by board-specific logic.  They
 *   include:
 *
 *   o sam_flexcom_spi[0|1|2|3|4]select is a functions to manage the board-specific chip
 *     selects
 *   o sam_flexcom_spi[0|1|2|3|4]status and sam_flexcom_spi[0|1|2|3|4]cmddata:  Implementations of the
 ...
 *     sam_spibus_initialize()) are provided by common SAM2/3/4 logic.
 *
 *  To use this common SPI logic on your board:
 *
 *   1. Provide logic in sam_boardinitialize() to configure FLEXCOM SPI chip select
 *      pins.
 *   2. Provide sam_flexcom_spi[0|1|2|3|4]select() and sam_flexcom_spi[0|1|2|3|4]status() functions in
 ...
 *      configured.
 *   2. If CONFIG_FLEXCOMx_SPI_CMDDATA is defined in the NuttX configuration, provide
 *      sam_flexcom_spi[0|1|2|3|4]cmddata() functions in your board-specific logic.  This
 *      function will perform cmd/data selection operations using PIOs in
 *      the way your board is configured.
 *   3. Add a call to sam_flexcom_spibus_initialize() in your low level application
 *      initialization logic
 *   4. The handle returned by sam_flexcom_spibus_initialize() may then be used to
 *      bind the FLEXCOM SPI driver to higher level logic (e.g., calling
...
#ifdef CONFIG_SAMA5_FLEXCOM0_SPI
int sam_flexcom0_spicmddata(FAR struct spi_dev_s *dev, uint32_t devid, bool cmd);
#endif
#ifdef CONFIG_SAMA5_FLEXCOM1_SPI
int sam_flexcom1_spicmddata(FAR struct spi_dev_s *dev, uint32_t devid, bool cmd);
#endif
#ifdef CONFIG_SAMA5_FLEXCOM2_SPI
int sam_flexcom2_spicmddata(FAR struct spi_dev_s *dev, uint32_t devid, bool cmd);
#endif
#ifdef CONFIG_SAMA5_FLEXCOM3_SPI
int sam_flexcom3_spicmddata(FAR struct spi_dev_s *dev, uint32_t devid, bool cmd);
#endif
#ifdef CONFIG_SAMA5_FLEXCOM4_SPI
int sam_flexcom4_spicmddata(FAR struct spi_dev_s *dev, uint32_t devid, bool cmd);
#endif
#endif
