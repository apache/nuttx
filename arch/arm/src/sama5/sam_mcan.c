...
#if defined(CONFIG_CAN) && (defined(CONFIG_SAMA5_MCAN0) || defined (CONFIG_SAMA5_MCAN1))
...
#else /* if defined(CONFIG_SAMA5_MCAN_CLKSRC_MAIN */
...
/*do not believe this is needed for armv7A */
#if 0
#  ifndef CONFIG_ARMV7A_DCACHE_WRITETHROUGH
#    warning !!! This driver will not work without CONFIG_ARMV7A_DCACHE_WRITETHROUGH=y!!!
#  endif
#endif
#else
#  define MCAN_ALIGN        (0)
...
/* errata sheet states TSEG1 must not be programmed as 0 */
...
/* revisit - number of dedicated + number of FIFO muct be <= 32. Not checked */
...
#warning g_mcan0_msgram declaration needs alternative if no caching enabled
...
{
  .rxpinset         = PIO_MCAN0_RX,
  .txpinset         = PIO_MCAN0_TX,
...
#warning g_mcan1_msgram declaration needs alternative if no caching enabled
...
{
  .rxpinset         = PIO_MCAN1_RX,
  .txpinset         = PIO_MCAN1_TX,
...
static void mcan_dumpregs(FAR struct sam_mcan_s *priv, FAR const char *msg)
{
  ...
  caninfo ("UTMI ref clk=%d\n",getreg32(SAM_SFR_VBASE + SAM_SFR_UTMICKTRIM_OFFSET) & 0x03);
...
static int mcan_ioctl(FAR struct can_dev_s *dev, int cmd, unsigned long arg)
{
  ...
  /* Handle the command */
  /* Note: the registers to read/write depend on whether this is CAN FD or not */
  /* needs a revisit */
...
static int mcan_hw_initialize(struct sam_mcan_s *priv)
{
  ...
  caninfo("MCAN%d\n", config->port);
  caninfo("mcan%d_msgram is at %lx\n",config->port, &config->msgram);
...
FAR struct can_dev_s *sam_mcan_initialize(int port)
{
  ...
  regval = PMC_PCR_GCKEN | PMC_PCR_CMD | PMC_PCR_EN | PMC_PCR_GCKDIV(CONFIG_SAMA5_MCAN_CLKSRC_PRESCALER - 1) | SAMA5_MCAN_CLKSRC;
  ...
  if (port == MCAN0)
    {
      ...
      regval  = getreg32(SAM_SFR_VBASE + SAM_SFR_CAN_OFFSET) & SAM_MCAN0_SFR_MASK;
  ...
  if (port == MCAN1)
    {
      ...
      regval  = getreg32(SAM_SFR_VBASE + SAM_SFR_CAN_OFFSET)  & SAM_MCAN1_SFR_MASK;
      regval |= SAM_MCAN_SFR_GET_MSB(config->msgram.stdfilters) << SAM_MCAN_SFR_SHIFT;
