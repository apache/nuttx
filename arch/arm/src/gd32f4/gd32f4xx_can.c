/****************************************************************************
 * arch/arm/src/gd32f4/gd32f4xx_can.c
 *
...
#include "gd32f4xx_gpio.h"
    ...
    (defined(CONFIG_GD32F4_CAN0) || defined(CONFIG_GD32F4_CAN1))
...
#define CAN_BIT_QUANTA (CONFIG_GD32F4_CAN_TSEG1 + CONFIG_GD32F4_CAN_TSEG2 + 1)

#ifndef CONFIG_DEBUG_CAN_INFO
#  undef CONFIG_GD32F4_CAN_REGDEBUG
#endif

/* CAN error interrupts */

#ifdef CONFIG_CAN_ERRORS
#  define GD32_CAN_ERRINT (CAN_IER_LECIE | CAN_IER_ERRIE |             \
...
static uint32_t gd32can_getreg(struct gd32_can_s *priv,
                                int offset);
static uint32_t gd32can_getfreg(struct gd32_can_s *priv,
                                 int offset);
static void gd32can_putreg(struct gd32_can_s *priv, int offset,
                            uint32_t value);
static void gd32can_putfreg(struct gd32_can_s *priv, int offset,
                             uint32_t value);
#ifdef CONFIG_GD32F4_CAN_REGDEBUG
static void gd32can_dumpctrlregs(struct gd32_can_s *priv,
                                  const char *msg);
static void gd32can_dumpmbregs(struct gd32_can_s *priv,
                                const char *msg);
static void gd32can_dumpfiltregs(struct gd32_can_s *priv,
                                  const char *msg);
#else
...
static int  gd32can_addextfilter(struct gd32_can_s *priv,
                                  struct canioc_extfilter_s *arg);
static int  gd32can_delextfilter(struct gd32_can_s *priv,
                                  int arg);
#endif
static int  gd32can_addstdfilter(struct gd32_can_s *priv,
                                  struct canioc_stdfilter_s *arg);
static int  gd32can_delstdfilter(struct gd32_can_s *priv,
                                  int arg);

/* CAN driver methods */

static void gd32can_reset(struct can_dev_s *dev);
static int  gd32can_setup(struct can_dev_s *dev);
static void gd32can_shutdown(struct can_dev_s *dev);
static void gd32can_rxint(struct can_dev_s *dev, bool enable);
static void gd32can_txint(struct can_dev_s *dev, bool enable);
static int  gd32can_ioctl(struct can_dev_s *dev, int cmd,
                           unsigned long arg);
static int  gd32can_remoterequest(struct can_dev_s *dev,
                                   uint16_t id);
static int  gd32can_send(struct can_dev_s *dev,
                          struct can_msg_s *msg);
static bool gd32can_txready(struct can_dev_s *dev);
static bool gd32can_txempty(struct can_dev_s *dev);

#ifdef CONFIG_CAN_ERRORS
static void gd32can_errint(struct can_dev_s *dev, bool enable);
#endif

/* CAN interrupt handling */

static int  gd32can_rxinterrupt(struct can_dev_s *dev, int rxmb);
...
static int  gd32can_bittiming(struct gd32_can_s *priv);
static int  gd32can_cellinit(struct gd32_can_s *priv);
static int  gd32can_filterinit(struct gd32_can_s *priv);

/* TX mailbox status */

static bool gd32can_txmb0empty(uint32_t tsr_regval);
static bool gd32can_txmb1empty(uint32_t tsr_regval);
static bool gd32can_txmb2empty(uint32_t tsr_regval);
...
static struct gd32_can_s g_can0priv =
{
  ...
  .cantx            = GD32_IRQ_CAN0_TX,
#ifdef CONFIG_CAN_ERRORS
  .cansce           = GD32_IRQ_CAN0_EWMC,
#endif
  .filter           = 0,
  .base             = GD32_CAN0_BASE,
  .fbase            = GD32_CAN0_BASE,
  .baud             = CONFIG_GD32F4_CAN0_BAUD,
};
...
static struct gd32_can_s g_can1priv =
{
  ...
  .cantx            = GD32_IRQ_CAN1_TX,
#ifdef CONFIG_CAN_ERRORS
  .cansce           = GD32_IRQ_CAN1_EWMC,
#endif
  .filter           = CAN_NFILTERS / 2,
  .base             = GD32_CAN1_BASE,
  .fbase            = GD32_CAN0_BASE,
  .baud             = CONFIG_GD32F4_CAN1_BAUD,
};
...
#ifdef CONFIG_GD32F4_CAN_REGDEBUG
static uint32_t gd32can_vgetreg(uint32_t addr)
{
...
static uint32_t gd32can_getreg(struct gd32_can_s *priv, int offset)
{
  return gd32can_vgetreg(priv->base + offset);
}

static uint32_t gd32can_getfreg(struct gd32_can_s *priv, int offset)
{
  return gd32can_vgetreg(priv->fbase + offset);
}
...
#ifdef CONFIG_GD32F4_CAN_REGDEBUG
static void gd32can_vputreg(uint32_t addr, uint32_t value)
{
...
static void gd32can_putreg(struct gd32_can_s *priv, int offset,
                            uint32_t value)
{
  gd32can_vputreg(priv->base + offset, value);
}

static void gd32can_putfreg(struct gd32_can_s *priv, int offset,
                             uint32_t value)
{
  gd32can_vputreg(priv->fbase + offset, value);
}
                                  ...
                                  const char *msg)
{
  ...
  caninfo("  MCR: %08" PRIx32 "   MSR: %08" PRIx32 "   TSR: %08" PRIx32 "\n",
          getreg32(priv->base + GD32_CAN_MCR_OFFSET),
          getreg32(priv->base + GD32_CAN_MSR_OFFSET),
          getreg32(priv->base + GD32_CAN_TSR_OFFSET));

  caninfo(" RF0R: %08" PRIx32 "  RF1R: %08" PRIx32 "\n",
          getreg32(priv->base + GD32_CAN_RF0R_OFFSET),
          getreg32(priv->base + GD32_CAN_RF1R_OFFSET));

  caninfo("  IER: %08" PRIx32 "   ESR: %08" PRIx32 "   BTR: %08" PRIx32 "\n",
          getreg32(priv->base + GD32_CAN_IER_OFFSET),
          getreg32(priv->base + GD32_CAN_ESR_OFFSET),
          getreg32(priv->base + GD32_CAN_BTR_OFFSET));
}
#endif

/****************************************************************************
 * Name: gd32can_dumpmbregs
 *
 * Description:
                                ...
                                const char *msg)
{
  ...
  caninfo(" TI0R: %08" PRIx32 " TDT0R: %08" PRIx32 " TDL0R: %08"
          ...
          getreg32(priv->base + GD32_CAN_TI0R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDT0R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDL0R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDH0R_OFFSET));

  caninfo(" TI1R: %08" PRIx32 " TDT1R: %08" PRIx32 " TDL1R: %08"
          PRIx32 " TDH1R: %08" PRIx32 "\n",
          getreg32(priv->base + GD32_CAN_TI1R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDT1R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDL1R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDH1R_OFFSET));

  caninfo(" TI2R: %08" PRIx32 " TDT2R: %08" PRIx32 " TDL2R: %08"
          PRIx32 " TDH2R: %08" PRIx32 "\n",
          getreg32(priv->base + GD32_CAN_TI2R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDT2R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDL2R_OFFSET),
          getreg32(priv->base + GD32_CAN_TDH2R_OFFSET));

  caninfo(" RI0R: %08" PRIx32 " RDT0R: %08" PRIx32 " RDL0R: %08"
          PRIx32 " RDH0R: %08" PRIx32 "\n",
          getreg32(priv->base + GD32_CAN_RI0R_OFFSET),
          getreg32(priv->base + GD32_CAN_RDT0R_OFFSET),
          getreg32(priv->base + GD32_CAN_RDL0R_OFFSET),
          getreg32(priv->base + GD32_CAN_RDH0R_OFFSET));
  ...
  caninfo(" RI1R: %08" PRIx32 " RDT1R: %08" PRIx32 " RDL1R: %08"
          ...
          getreg32(priv->base + GD32_CAN_RDL1R_OFFSET),
          getreg32(priv->base + GD32_CAN_RDH1R_OFFSET));
}
#endif

/****************************************************************************
 * Name: gd32can_dumpfiltregs
 *
 * Description:
                                  ...
                                  const char *msg)
{
  ...
  caninfo(" FMR: %08" PRIx32 "   FM1R: %08" PRIx32 "  FS1R: %08"
          ...
          getreg32(priv->base + GD32_CAN_FMR_OFFSET),
          getreg32(priv->base + GD32_CAN_FM1R_OFFSET),
          getreg32(priv->base + GD32_CAN_FS1R_OFFSET),
          getreg32(priv->base + GD32_CAN_FFA1R_OFFSET),
          getreg32(priv->base + GD32_CAN_FA1R_OFFSET));

  for (i = 0; i < CAN_NFILTERS; i++)
    {
      caninfo(" F%dR1: %08" PRIx32 " F%dR2: %08" PRIx32 "\n",
              i, getreg32(priv->base + GD32_CAN_FIR_OFFSET(i, 1)),
              i, getreg32(priv->base + GD32_CAN_FIR_OFFSET(i, 2)));
    }
}
#endif
...
static int gd32can_setup(struct can_dev_s *dev)
{
  ...
  gd32can_dumpctrlregs(priv, "After cell initialization");
  gd32can_dumpmbregs(priv, NULL);
  ...
  gd32can_dumpfiltregs(priv, "After filter initialization");
...
#ifdef CONFIG_CAN_ERRORS
  ret = irq_attach(priv->cansce, gd32can_sceinterrupt, dev);
  if (ret < 0)
    {
...
static int gd32can_remoterequest(struct can_dev_s *dev, uint16_t id)
{
  ...
  if (regval & CAN_MSR_ERRI)
    {
      ...
      regval = gd32can_getreg(priv, GD32_CAN_ESR_OFFSET);
      ...
      gd32can_putreg(priv, GD32_CAN_TSR_OFFSET, regval);

      /* Clear ERRI flag */

      gd32can_putreg(priv, GD32_CAN_MSR_OFFSET, CAN_MSR_ERRI);
    }
...
static int gd32can_bittiming(struct gd32_can_s *priv)
{
  ...
  caninfo("CAN%" PRIu8 " PCLK1: %lu baud: %" PRIu32 "\n",
          priv->port, (unsigned long) GD32_PCLK1_FREQUENCY, priv->baud);
  ...
  /* Otherwise, nquanta is CAN_BIT_QUANTA, ts1 is CONFIG_GD32F4_CAN_TSEG1,
   * ts2 is CONFIG_GD32F4_CAN_TSEG2 and we calculate brp to achieve
   * CAN_BIT_QUANTA quanta in the bit time
   */

  else
    {
      ts1 = CONFIG_GD32F4_CAN_TSEG1;
      ts2 = CONFIG_GD32F4_CAN_TSEG2;
      brp = (tmp + (CAN_BIT_QUANTA / 2)) / CAN_BIT_QUANTA;
      DEBUGASSERT(brp >= 1 && brp <= CAN_BTR_BRP_MAX);
    }
 ...
 *   between the two CAN modules:  CAN0 uses only filter 0 (but reserves
 *   0 through CAN_NFILTERS/2-1); CAN1 uses only filter CAN_NFILTERS/2
...
static int gd32can_filterinit(struct gd32_can_s *priv)
{
  ...
  /* Assign half the filters to CAN0, half to CAN1 (28 banks on GD32F4). */

  regval  = gd32can_getfreg(priv, GD32_CAN_FMR_OFFSET);
  regval &= ~CAN_FMR_CAN1SB_MASK;
  regval |= (CAN_NFILTERS / 2) << CAN_FMR_CAN1SB_SHIFT;
  gd32can_putfreg(priv, GD32_CAN_FMR_OFFSET, regval);
...
#ifdef CONFIG_CAN_EXTID
static int gd32can_delextfilter(struct gd32_can_s *priv, int arg)
{
  return -ENOTTY;
}
#endif
...
static int gd32can_delstdfilter(struct gd32_can_s *priv, int arg)
{
  return -ENOTTY;
}

/****************************************************************************
 * Name: gd32can_txmb0empty
 *
 * Input Parameters:
...
struct can_dev_s *gd32_caninitialize(int port)
{
  ...
  /* NOTE: Peripheral clocking for CAN is enabled here.  gd32_clockconfig()
  ...
  if (port == 0)
    {
      ...
      /* Enable CAN0 clock (filters live in CAN0 block). */

      modifyreg32(GD32_RCU_APB1EN, 0, RCU_APB1EN_CAN0EN);

      /* Configure CAN0 pins.  Ambiguous pinmap entries must be
       * disambiguated in the board.h file.
       */
  ...
  if (port == 1)
    {
      ...
      /* CAN1 shares filter registers with CAN0; enable both clocks. */

      modifyreg32(GD32_RCU_APB1EN, 0, RCU_APB1EN_CAN0EN);
      modifyreg32(GD32_RCU_APB1EN, 0, RCU_APB1EN_CAN1EN);

      /* Configure CAN1 pins.  Ambiguous pinmap entries must be
       * disambiguated in the board.h file.
       */

      gd32_gpio_config(GPIO_CAN1_RX);
      gd32_gpio_config(GPIO_CAN1_TX);
    }
  else
#endif
    {
...
#endif /* CONFIG_CAN && (CONFIG_GD32F4_CAN0 || CONFIG_GD32F4_CAN1) */
