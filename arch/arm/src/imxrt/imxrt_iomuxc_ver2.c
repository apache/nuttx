  ...
  if (padctl <= IMXRT_PADCTL_GPIO_EMC_B1_41)
  ...
  else if (padctl <= IMXRT_PADCTL_GPIO_SD_B1_05)
    {
      ...
      switch (ioset & IOMUX_PULL_MASK)
        {
          case IOMUX_PULL_UP:
            regval |= PADCTL_SD_B1_PULL_UP;
            break;

          case IOMUX_PULL_DOWN:
            regval |= PADCTL_SD_B1_PULL_DOWN;
            break;

          case IOMUX_PULL_NONE:
            regval |= PADCTL_SD_B1_PULL_NONE;
            break;
  ...
  else if (padctl <= IMXRT_PADCTL_GPIO_DISP_B1_11)
    {
      ...
      switch (ioset & IOMUX_PULL_MASK)
        {
          case IOMUX_PULL_UP:
            regval |= PADCTL_DISP_B1_PULL_UP;
            break;

          case IOMUX_PULL_DOWN:
            regval |= PADCTL_DISP_B1_PULL_DOWN;
            break;

          case IOMUX_PULL_NONE:
            regval |= PADCTL_DISP_B1_PULL_NONE;
            break;
