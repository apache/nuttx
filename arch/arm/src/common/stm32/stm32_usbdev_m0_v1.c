/****************************************************************************
 * arch/arm/src/common/stm32/stm32_usbdev_m0_v1.c
 *
...
/* Ported from the STM32 F1 implementation.  References:
 ...
 * CAN/USB memory.
 * We will use the first STM32_NENDPOINTS*4 words for the buffer table.
...
#define STM32_TRACEINTID_INTERRUPT          0x0014
#define STM32_TRACEINTID_IFGETSTATUS        0x0015
#define STM32_TRACEINTID_LPCTR              0x0016
#define STM32_TRACEINTID_NOSTDREQ           0x0017
#define STM32_TRACEINTID_RESET              0x0018
#define STM32_TRACEINTID_SETCONFIG          0x0019
#define STM32_TRACEINTID_SETFEATURE         0x001a
#define STM32_TRACEINTID_SUSP               0x001b
#define STM32_TRACEINTID_SYNCHFRAME         0x001c
...
static inline void stm32_seteptxcount(uint8_t epno,
                                      uint16_t count);
static inline void stm32_seteptxaddr(uint8_t epno,
                                     uint16_t addr);
...
const struct trace_msg_t g_usb_trace_strings_intdecode[] =
{
  ...
  TRACE_STR(STM32_TRACEINTID_INTERRUPT),
...
static void stm32_seteptxstatus(uint8_t epno, uint16_t state)
{
  ...
  /* The exclusive OR will set STAT_TX bits to 1 if there value is
   * different from the bits requested in 'state'
   */
...
static void stm32_seteprxstatus(uint8_t epno, uint16_t state)
{
  ...
  /* The exclusive OR will set STAT_RX bits to 1 if there value is
   * different from the bits requested in 'state'
   */
                          ...
                          const uint8_t *buf, uint32_t nbytes)
{
  ...
  /* Indicate that there is data in the TX packet memory.
   * This will be cleared when the next data out interrupt is received.
   */
...
#warning "REVISIT: If the EP supports double buffering, then we can do better"
  ...
  /* Either
   * (1) we are committed to sending the null packet
              ...
              switch (priv->ctrl.type & USB_REQ_RECIPIENT_MASK)
                {
                      ...
                      if (epno >= STM32_NENDPOINTS)
                        {
                          usbtrace(TRACE_DEVERROR(
                                   STM32_TRACEERR_BADEPGETSTATUS), epno);
                          priv->ep0state = EP0STATE_STALLED;
                        }
                      else
                        {
                  ...
                  case USB_REQ_RECIPIENT_INTERFACE:
                    {
                      usbtrace(TRACE_INTDECODE(
                               STM32_TRACEINTID_IFGETSTATUS), 0);
                      response.w = 0;
                      nbytes     = 2; /* Response size: 2 bytes */
                    }
                    break;
          ...
          if ((priv->ctrl.type & USB_REQ_RECIPIENT_MASK) !=
          ...
          else
            {
              ...
              if (epno < STM32_NENDPOINTS && index.b[MSB] == 0 &&
                ...
                {
                  usbtrace(TRACE_DEVERROR(
                           STM32_TRACEERR_BADCLEARFEATURE), 0);
                  priv->ep0state = EP0STATE_STALLED;
                }
            }
        }
...
static int stm32_usb_interrupt(int irq, void *context, void *arg)
{
  ...
  usbtrace(TRACE_INTENTRY(STM32_TRACEINTID_INTERRUPT), istr);
  ...
  if ((istr & USB_ISTR_WKUP & priv->imask) != 0)
    {
       ...
       * The cause of the resume is indicated in the FNR register
       */
               ...
               uint16_t setbits, uint16_t clrbits)
{
  ...
  /* Then make the interrupt mask bits in the CNTR register match the
   * shadow register (Hmmm... who is shadowing whom?)
   */
...
static void stm32_initresume(struct stm32_usbdev_s *priv)
{
   ...
   * a self-powered device.  Also, low power mode is automatically cleared
   * by hardware when a WKUP interrupt event occurs).
   */
                         ...
                         struct usbdev_req_s *req)
{
  ...
  /* Handle OUT (host-to-device) requests */
  ...
  else
    {
      ...
      /* This there a incoming data pending the availability of a
       * request?
       */
...
static struct usbdev_ep_s *stm32_allocep(struct usbdev_s *dev,
                                         uint8_t epno,
...
void arm_usbinitialize(void)
{
  ...
  if (irq_attach(STM32_IRQ_USB, stm32_usb_interrupt, priv) != 0)
    {
      usbtrace(TRACE_DEVERROR(STM32_TRACEERR_IRQREGISTRATION),
               (uint16_t)STM32_IRQ_USB);
      goto errout;
    }
...
int usbdev_register(struct usbdevclass_driver_s *driver)
{
  ...
  if (ret)
    {
      usbtrace(TRACE_DEVERROR(STM32_TRACEERR_BINDFAILED),
              (uint16_t) - ret);
    }
  else
    {
      ...
      up_enable_irq(STM32_IRQ_USB);

      /* Enable pull-up to connect the device.  The host should enumerate
       * us some time after this
       */
