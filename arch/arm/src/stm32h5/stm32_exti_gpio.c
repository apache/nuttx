                         ...
                         bool event, xcpt_t func, void *arg)
{
  ...
  /* Set the mux for this EXTI to the correct bank */

  /* Which EXTICR# does this port land on? (0-3) */

  uint32_t exticrn = pin / 4;

  /* Which EXTI## field does this port land on? (0-3) */

  uint32_t shift = (pin % 4) * 8;

  /* Shift our clear and set bits into place */

  uint32_t extin_clrbits = 0b11111111 << shift;
  uint32_t extin_setbits;

  if (func)
    {
      extin_setbits = port << shift;
    }
  else
    {
      extin_setbits = 0;
  ...
  /* Set or clear the MUX port according to the given port */

  uint32_t reg = STM32_EXTI_BASE + STM32_EXTI_EXTICR1_OFFSET
                 + (0x04 * exticrn);
  modifyreg32(reg, extin_clrbits, extin_setbits);
