 ...
 * For SDMMC1, IDMA cannot access SRAM123. Refer to ST AN5200.
...
#  define STM32H5_SRAM_START  STM32_SRAM1_BASE
#  define STM32H5_SRAM_END    (STM32_SRAM3_BASE + STM32_SRAM3_SIZE)
#endif
...
#  define STM32_RCC_D1CCIPR_SDMMCSEL  RCC_CCIPR4_SDMMC1SEL
#endif

#if STM32_RCC_D1CCIPR_SDMMCSEL  == RCC_CCIPR4_SDMMC1SEL
...
struct stm32_dev_s
{
  ...
  uint32_t           nblocks;         /* Current block count (for DTMODE selection) */
  uint32_t           receivecnt;      /* Real count to receive */
#if !defined(CONFIG_STM32_SDMMC_IDMA)
  struct work_s      cbfifo;          /* Monitor for Lame FIFO */
#endif
  uint8_t            rxfifo[FIFO_SIZE_IN_BYTES] /* To offload with IDMA and support un-aligned buffers */
  ...
  /* REVISIT: hack for now to avoid sorting out the DMA's reach */

  uint8_t *sdmmc_rxbuffer;
#endif
};
                             ...
                             uint32_t dlen, bool receive)
{
  ...
  /* Select DTMODE:
   * - CMD53 byte mode: nblocks=1, blocksize = actual byte count < 128
   *   -> use DTMODE_SDIO (SDIO multibyte), matching bare-metal
   *   TRANSFER_MODE_SDIO
   * - CMD53 block mode: blocksize = registered block size (128 for CC33xx)
   *   -> use DTMODE_BLOCK for per-block CRC checking
   * Threshold 128 = CC33xx registered Fn2 block size. Byte mode transfers
   * always have blocksize < 128 because the CMD53 byte count is < 128.
   */

#if defined(HAVE_SDMMC_SDIO_MODE)
  if (priv->sdiomode && priv->nblocks == 1 && priv->blocksize < 128)
    {
      dctrl |= STM32_SDMMC_DCTRL_DTMODE_SDIO;   /* CMD53 byte/multibyte mode */
    }
  else
#endif
    {
...
static void stm32_datadisable(struct stm32_dev_s *priv)
{
  ...
  /* Clear all error/completion flags first; DPSM may reject new
   * configuration if stale flags are set.
   */

  sdmmc_putreg32(priv, STM32_SDMMC_ICR_ALLFLAGS,
  ...
  /* Wait for DPSMACT to clear (1 µs per iteration, up to 10 ms).
   * DTIMER/DLEN/DCTRL can only be written when DPSMACT=0; without this
   * wait, stm32_dataconfig silently ignores those writes and every
   * subsequent transfer is misconfigured (TXFIFOF=1, DATAEND never fires).
   */

  int timeout = 10000;

  while ((sdmmc_getreg32(priv, STM32_SDMMC_STA_OFFSET) &
          STM32_SDMMC_STA_DPSMACT) && --timeout > 0)
    {
      up_udelay(1);
    }
  ...
  if (timeout == 0)
    {
      mcerr("datadisable: DPSMACT stuck after 10ms! STA=0x%08" PRIx32
            " DTIMER=0x%08" PRIx32 "\n",
            sdmmc_getreg32(priv, STM32_SDMMC_STA_OFFSET),
            sdmmc_getreg32(priv, STM32_SDMMC_DTIMER_OFFSET));
    }
}
...
static void stm32_recvdma(struct stm32_dev_s *priv)
{
  ...
  if (priv->unaligned_rx)
  ...
  else
    {
       ...
       * stm32_dmarecvsetup(), but the cache can speculatively prefetch
       * into this (cacheable) buffer between that point and DMA
       * completion, leaving stale lines that shadow the data just
       * written by the IDMA. Invalidate again now that the transfer is
       * complete, before the buffer is consumed, so the CPU reads the
       * freshly received data instead of a previously cached sector.
       * The buffer and length are cache-line aligned here (that is why
       * this aligned path was taken), so no adjacent memory is affected.
       */
...
static void stm32_reset(struct sdio_dev_s *dev)
{
  ...
  if (priv->base == STM32_SDMMC1_BASE)
    {
      regaddress = STM32_RCC_AHB4RSTR;
      restval    = RCC_AHB4RSTR_SDMMC1RST;
    }
#endif
  ...
  if (priv->base == STM32_SDMMC2_BASE)
    {
      regaddress = STM32_RCC_AHB4RSTR;
      restval    = RCC_AHB4RSTR_SDMMC2RST;
    }
#endif
                              ...
                              const uint8_t *buffer, size_t buflen)
{
  ...
  if (priv->base == STM32_SDMMC1_BASE)
    {
      /* STM32H5: SDMMC1 IDMA can access all SRAM (SRAM1/2/3).
       * Just verify the buffer falls within the valid SRAM range.
       */

      if ((uintptr_t)buffer < STM32H5_SRAM_START ||
          (uintptr_t)buffer + buflen > STM32H5_SRAM_END)
        {
                              ...
                              uint8_t *buffer, size_t buflen)
{
  ...
  /* If the previous transfer left the DPSM active, force hardware and
   * software state cleanup before starting a new transfer.
   */

  uint32_t sta = sdmmc_getreg32(priv, STM32_SDMMC_STA_OFFSET);

  if (sta & STM32_SDMMC_STA_DPSMACT)
    {
      mcwarn("WARNING: DPSM still active (STA=0x%08" PRIx32
             "), forcing cleanup\n", sta);

      /* Step 1: Disable IDMA (must precede DCTRL clear) */

      sdmmc_putreg32(priv, 0, STM32_SDMMC_IDMACTRLR_OFFSET);

      /* Step 2: Stop DPSM by clearing DCTRL */

      sdmmc_putreg32(priv, 0, STM32_SDMMC_DCTRL_OFFSET);

      /* Step 3: Clear DLEN */

      sdmmc_putreg32(priv, 0, STM32_SDMMC_DLEN_OFFSET);

      /* Step 4: Clear all interrupt flags */

      sdmmc_putreg32(priv, STM32_SDMMC_ICR_ALLFLAGS,
                     STM32_SDMMC_ICR_OFFSET);

      /* Step 5: Wait for DPSMACT to deassert */
      ...
      if (timeout == 0)
        {
          mcerr("ERROR: DPSMACT won't clear, FIFO reset\n");
          sdmmc_putreg32(priv, STM32_SDMMC_DCTRL_FIFORST,
                         STM32_SDMMC_DCTRL_OFFSET);
          up_udelay(10);
          sdmmc_putreg32(priv, 0, STM32_SDMMC_DCTRL_OFFSET);
          sdmmc_putreg32(priv, STM32_SDMMC_ICR_ALLFLAGS,
      ...
      /* Step 6: Reset driver internal state */
  ...
  if (slotno == 0)
    {
      ...
      priv->sdmmc_rxbuffer = (uint8_t *)kmm_memalign(ARMV8M_DCACHE_LINESIZE,
                                                      SDMMC_MAX_BLOCK_SIZE);
