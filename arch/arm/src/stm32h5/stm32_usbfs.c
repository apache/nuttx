/****************************************************************************
 * arch/arm/src/stm32h5/stm32_usbfs.c
 *
 ...
 * bidirectional endpoints. We'll define STM32_NENDPOINTS to be 8,
 * because that is how many endpoint register sets there are. With a 2048
 * byte USB RAM, there is room for a 64-byte buffer table + 16 64-byte
 * buffers.
 */
...
/* Packet sizes. We use a 64 byte max packet size for all endpoint types
 * Max packet size for USB 2.0 Full-Speed is 64 bytes. With 64-byte buffer
 * sizes, there is space in the USB RAM for 16 64-byte buffers.
 */
...
/* Buffer descriptor table. The buffer table is positioned at the
 * beginning of the 2048-byte USB memory.  We will use the first
 * STM32_NENDPOINTS*2 words for the buffer table. That is exactly 64 bytes,
 * leaving 1,984 bytes for endpoint buffers.
 */

#define STM32_BTABLE_ADDRESS  (0x00)   /* Start at the beginning of USB/CAN RAM */
#define STM32_DESC_SIZE       (8)      /* Each descriptor is 2*4=8 bytes in size */
 ...
 * With 64-byte buffers and a 2048 byte USB RAM, we can support 8
...
#define STM32_NBUFFERS        16
#define STM32_BUFFER_BIT(bn)  (1 << (bn))
#define STM32_BUFFER_ALLSET   0xffff
...
#define STM32_TRACEINTID_CTR                0x0016
#define STM32_TRACEINTID_INTERRUPT          0x0017
...
struct stm32_usbdev_s
{
  ...
  uint16_t               bufavail;      /* Bitset of available buffers */
...
static void   stm32_transfer(struct stm32_usbdev_s *priv);
...
const struct trace_msg_t g_usb_trace_strings_intdecode[] =
{
  ...
  TRACE_STR(STM32_TRACEINTID_CTR),
...
static inline void stm32_seteptxcount(uint8_t epno, uint16_t count)
{
  ...
  txbd = (*epaddr & 0x0000ffff) | (count << 16);
  *epaddr = txbd;
}
...
static inline uint16_t stm32_geteptxaddr(uint8_t epno)
{
  uint32_t eptxaddr;
  volatile uint32_t *txaddr = (uint32_t *)STM32_USB_TX(epno);
  eptxaddr = ((uint32_t)*txaddr) & 0x0000ffff;
  return (uint16_t) eptxaddr;
}
...
static void stm32_seteprxcount(uint8_t epno, uint16_t count)
{
  ...
  rxbd = (*epaddr & 0x0000ffff) | rxcount;
  *epaddr = rxbd;
}
...
{
  volatile uint32_t *epaddr = (uint32_t *)STM32_USB_RX(epno);
  return (((*epaddr) & USB_COUNT_RX_MASK) >> 16);
}
...
static inline uint16_t stm32_geteprxaddr(uint8_t epno)
{
  uint32_t eprxaddr;
  volatile uint32_t *rxaddr = (uint32_t *)STM32_USB_RX(epno);
  eprxaddr = ((uint32_t)*rxaddr) & 0x0000ffff;
  return (uint16_t) eprxaddr;
}
                            ...
                            uint16_t pma, uint16_t nbytes)
{
  ...
  dest = (uint32_t *)(STM32_USB_FS_RAM_BASE + (uint32_t)pma);
  for (i = nwords; i != 0; i--)
    {
...
stm32_copyfrompma(uint8_t *buffer, uint16_t pma, uint16_t nbytes)
{
  ...
  src = (uint32_t *)(STM32_USB_FS_RAM_BASE + (uint32_t)pma);
  for (i = nwords; i != 0; i--)
    {
...
static int stm32_interrupt(int irq, void *context, void *arg)
{
  ...
  uint32_t istr = stm32_getreg(STM32_USB_ISTR);
  ...
  if ((istr & USB_ISTR_CTR & priv->imask) != 0)
    {
      ...
      usbtrace(TRACE_INTDECODE(STM32_TRACEINTID_CTR), istr);
      stm32_transfer(priv);
    }

exit_interrupt:
  usbtrace(TRACE_INTEXIT(STM32_TRACEINTID_INTERRUPT),
  ...
  for (bufndx = 2; bufndx < STM32_NBUFFERS; bufndx++)
    {
      ...
      if ((priv->bufavail & bit) != 0)
        {
          /* Yes.. Mark the buffer no longer available */
...
void stm32_usbsuspend(struct usbdev_s *dev, bool resume)
{
  ...
  uinfo("resume: %d\n", resume);
  regval = getreg32(STM32_USB_CNTR);
  if (resume)
    {
      regval &= ~(USB_CNTR_FSUSP);
    }
  ...
  putreg32(regval, STM32_USB_CNTR);
}
...
void arm_usbinitialize(void)
{
  ...
  /* Check if VDDUSB is Valid */

  putreg32(PWR_USBSCR_USB33DEN, STM32_PWR_USBSCR);
  while (!(getreg32(STM32_PWR_VMSR) & PWR_VMSR_USB33RDY))
    {
    }
  ...
  if (irq_attach(STM32_IRQ_USB_FS, stm32_interrupt, NULL) != 0)
    {
      usbtrace(TRACE_DEVERROR(STM32_TRACEERR_IRQREGISTRATION),
               (uint16_t)STM32_IRQ_USB_FS);
      goto errout;
    }
...
void arm_usbuninitialize(void)
{
  ...
  up_disable_irq(STM32_IRQ_USB_FS);
  irq_detach(STM32_IRQ_USB_FS);
