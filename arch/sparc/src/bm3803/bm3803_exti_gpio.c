...
int bm3803_gpioset_irq(uint32_t pinset, bool enable, bool trig, bool edge, xcpt_t func, void *arg)
{
  ...
  if (func)
    ...
    {
      /* Only disable IRQ if shared handler does not have any active callbacks. */
  ...
  BM3803_REG.PioDir = BM3803_REG.PioDir & ~(1<<(pin+4));

  /* Configure rising/falling edges */
  modifyreg32((uint32_t)&BM3803_REG.PioIrq, le, trig ? le : 0);
  modifyreg32((uint32_t)&BM3803_REG.PioIrq, pl, edge ? pl : 0);
  modifyreg32((uint32_t)&BM3803_REG.PioIrq, 0x1F<<(pin*8), isel);
  modifyreg32((uint32_t)&BM3803_REG.PioIrq, en, enable ? en : 0);
  return OK;
}
