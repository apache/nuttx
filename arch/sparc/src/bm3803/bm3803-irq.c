...
void up_irqinitialize(void)
{
  ...
  for (irq = 1; irq < NR_IRQS; irq++)
     ...
     */
      ...
      if ( (( irq >= 0x00 ) && ( irq <= 0x11 )) || (( irq >= 0x20 ) && ( irq <= 0xFF )) )
        {
          continue;
        }
  ...
  /* Interrupts are enabled by setting the te bit in the psr status register */
...
void up_disable_irq(int irq)
{
  ...
  DEBUGASSERT(irq >= BM3803_IRQ_FIRST_INTERRUPT && irq <= BM3803_IRQ_LAST_INTERRUPT);
...
{
  int bitno;
  /* Enable the interrupt by setting the associated bit in the Interrupt_Mask Register */
      ...
      if (irq <= BM3803_IRQ_LAST_INTERRUPT)
        {
         /* Get the set of unmasked, pending interrupts.   */
           ...
           regval1 = BM3803_REG.IntMask;
	   regval2 = BM3803_REG.IntPend;
  ...
  if (irq >= BM3803_IRQ_FIRST_INTERRUPT)
    {
	   ...
	   * will clear the corresponding bit(s) in the interrupt pending register
	   */
	  bitno = irq - BM3803_IRQ_FIRST_INTERRUPT + 1;
          BM3803_REG.IntClear |= (1 << bitno);
        }
      else
        {
          /* Value out of range.. just ignore */
 ...
 * Interrupt mask and priority register. Level 1 has higher priority than level 0.
  ...
  if (irq >= BM3803_IRQ_FIRST_INTERRUPT)
    {
	  ...
	  bitno = irq - BM3803_IRQ_FIRST_INTERRUPT + 1;
      	  ...
      	  BM3803_REG.IntMask &= (~(1 << shift));
	  BM3803_REG.IntMask |= (priority<<shift);
        }
      else
        {
          /* Value out of range.. just ignore */
