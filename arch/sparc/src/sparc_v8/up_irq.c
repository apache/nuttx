/****************************************************************************
 * arch/sparc/src/sparc_v8/up_irq.c
 *
...
irqstate_t up_irq_save(void)
{
  ...
  //sparc_get_psr( status );	   /* Get psr status */

  ret     = sparc_disable_interrupts();               /* Save the status */
//  BM3803_REG.IntMask = 0;
...
void up_irq_restore(irqstate_t irqstate)
{
 // register irqstate_t status;
  ...
  //sparc_get_psr( status );	    /* Get psr status */
  //status &= (~SPARC_PSR_ET_MASK);   /* Clear et bits */
  //irqstate &= SPARC_PSR_ET_MASK;    /* Retain et only */
  //status   |= irqstate; 	    /* Set new et bits */
  //sparc_set_psr( status );	    /* Restore interrupt state */
  //BM3803_REG.IntMask = irqstate;
