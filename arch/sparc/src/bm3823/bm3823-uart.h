...
#define Uart1_set_baudrate(baudrate)	    (EXTER_REG.UartScaler1 = (uint32_t)((((BOARD_PERIPH_CLOCK*10)/(baudrate * 8))-5)/10))

#define Uart1_parity_config(uart_parity)     ( uart_parity    ==  ODD                                                      \
                                            ? (EXTER_REG.UartCtrl1   =   ((EXTER_REG.UartCtrl1 |  MSK_UART_PAR) | MSK_UART_ENABLE_PAR)) \
                                            : ( uart_parity  ==  EVEN                                                     \
                                              ? (EXTER_REG.UartCtrl1 =   ((EXTER_REG.UartCtrl1 & ~MSK_UART_PAR) | MSK_UART_ENABLE_PAR)) \
                                              : (EXTER_REG.UartCtrl1 &= ~MSK_UART_ENABLE_PAR )                                   \
                                              )                                                                           \
                                            )

#define Uart1_flow_ctrl_config(uart_flow)    ( uart_flow    ==  ON                     \
                                            ? (EXTER_REG.UartCtrl1 |=  MSK_UART_ENABLE_FLOW) \
                                            : (EXTER_REG.UartCtrl1 &= ~MSK_UART_ENABLE_FLOW) \
                                            )

#define Uart1_loopback_config(uart_loopb)    ( uart_loopb   ==  ON                     \
                                            ? (EXTER_REG.UartCtrl1 |=  MSK_UART_LOOPBACK)    \
                                            : (EXTER_REG.UartCtrl1 &= ~MSK_UART_LOOPBACK)    \
                                            )

#define Uart1_enable()                       (EXTER_REG.UartCtrl1 |=  (MSK_UART_ENABLE_RX | MSK_UART_ENABLE_TX))
#define Uart1_disable()                      (EXTER_REG.UartCtrl1 &= ~(MSK_UART_ENABLE_RX | MSK_UART_ENABLE_TX))

#define Uart1_tx_ready()                     ((EXTER_REG.UartStatus1 & MSK_UART_TXh_READY)  == MSK_UART_TXh_READY )
#define Uart1_rx_ready()                     ((EXTER_REG.UartStatus1 & MSK_UART_DATA_READY) == MSK_UART_DATA_READY)

#define Uart1_send_byte(ch)  		     (EXTER_REG.UartData1 = ch)

/***************************************************************************************************************************************/

#define Uart2_set_baudrate(baudrate)	    (EXTER_REG.UartScaler2 = (((BOARD_PERIPH_CLOCK*10)/(baudrate*8))-5)/10)

#define Uart2_parity_config(uart_parity)     ( uart_parity    ==  ODD                                                      \
                                            ? (EXTER_REG.UartCtrl2   =   ((EXTER_REG.UartCtrl2 |  MSK_UART_PAR) | MSK_UART_ENABLE_PAR)) \
                                            : ( uart_parity  ==  EVEN                                                     \
                                              ? (EXTER_REG.UartCtrl2 =   ((EXTER_REG.UartCtrl2 & ~MSK_UART_PAR) | MSK_UART_ENABLE_PAR)) \
                                              : (EXTER_REG.UartCtrl2 &= ~MSK_UART_ENABLE_PAR )                                   \
                                              )                                                                           \
                                            )

#define Uart2_flow_ctrl_config(uart_flow)    ( uart_flow    ==  ON                     \
                                            ? (EXTER_REG.UartCtrl2 |=  MSK_UART_ENABLE_FLOW) \
                                            : (EXTER_REG.UartCtrl2 &= ~MSK_UART_ENABLE_FLOW) \
                                            )

#define Uart2_loopback_config(uart_loopb)    ( uart_loopb   ==  ON                     \
                                            ? (EXTER_REG.UartCtrl2 |=  MSK_UART_LOOPBACK)    \
                                            : (EXTER_REG.UartCtrl2 &= ~MSK_UART_LOOPBACK)    \
                                            )

#define Uart2_enable()                       (EXTER_REG.UartCtrl2 |=  (MSK_UART_ENABLE_RX | MSK_UART_ENABLE_TX))
#define Uart2_disable()                      (EXTER_REG.UartCtrl2 &= ~(MSK_UART_ENABLE_RX | MSK_UART_ENABLE_TX))

#define Uart2_tx_ready()                     ((EXTER_REG.UartStatus2 & MSK_UART_TXh_READY)  == MSK_UART_TXh_READY )
#define Uart2_rx_ready()                     ((EXTER_REG.UartStatus2 & MSK_UART_DATA_READY) == MSK_UART_DATA_READY)

#define Uart2_send_byte(ch)  		     (EXTER_REG.UartData2 = ch)

/***************************************************************************************************************************************/

#define Uart3_set_baudrate(baudrate)	    (BM3823_REG.UartScaler3 = (((BOARD_PERIPH_CLOCK*10)/(baudrate*8))-5)/10)

#define Uart3_parity_config(uart_parity)     ( uart_parity    ==  ODD                                                      \
                                            ? (BM3823_REG.UartCtrl3   =   ((BM3823_REG.UartCtrl3 |  MSK_UART_PAR) | MSK_UART_ENABLE_PAR)) \
                                            : ( uart_parity  ==  EVEN                                                     \
                                              ? (BM3823_REG.UartCtrl3 =   ((BM3823_REG.UartCtrl3 & ~MSK_UART_PAR) | MSK_UART_ENABLE_PAR)) \
                                              : (BM3823_REG.UartCtrl3 &= ~MSK_UART_ENABLE_PAR )                                   \
                                              )                                                                           \
                                            )

#define Uart3_flow_ctrl_config(uart_flow)    ( uart_flow    ==  ON                     \
                                            ? (BM3823_REG.UartCtrl3 |=  MSK_UART_ENABLE_FLOW) \
                                            : (BM3823_REG.UartCtrl3 &= ~MSK_UART_ENABLE_FLOW) \
                                            )

#define Uart3_loopback_config(uart_loopb)    ( uart_loopb   ==  ON                     \
                                            ? (BM3823_REG.UartCtrl3 |=  MSK_UART_LOOPBACK)    \
                                            : (BM3823_REG.UartCtrl3 &= ~MSK_UART_LOOPBACK)    \
                                            )

#define Uart3_enable()                       (BM3823_REG.UartCtrl3 |=  (MSK_UART_ENABLE_RX | MSK_UART_ENABLE_TX))
#define Uart3_disable()                      (BM3823_REG.UartCtrl3 &= ~(MSK_UART_ENABLE_RX | MSK_UART_ENABLE_TX))

#define Uart3_tx_ready()                     ((BM3823_REG.UartStatus3 & MSK_UART_TXh_READY)  == MSK_UART_TXh_READY )
#define Uart3_rx_ready()                     ((BM3823_REG.UartStatus3 & MSK_UART_DATA_READY) == MSK_UART_DATA_READY)

#define Uart3_send_byte(ch)  		     (BM3823_REG.UartData3 = ch)
/************************************************************************************
 * Public Types
 ************************************************************************************/
