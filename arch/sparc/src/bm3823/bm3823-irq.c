...
void up_disable_irq(int irq)
{
  ...
  DEBUGASSERT(irq >= BM3823_IRQ_FIRST_INTERRUPT && irq <= BM3823_IRQ_LAST_INTERRUPT);
      ...
      if (irq <= BM3823_IRQ_LAST_INTERRUPT)
        {
         /* Get the set of unmasked, pending interrupts.   */
           ...
           regval1 = INTER_REG.IntMask;
	   regval2 = INTER_REG.IntPendForce;
  ...
  if (irq >= BM3823_IRQ_FIRST_INTERRUPT)
    {
	  ...
	  bitno = irq - BM3823_IRQ_FIRST_INTERRUPT - 1;
      	  ...
      	  INTER_REG.IntMask &= (~(1 << shift));
	  INTER_REG.IntMask |= (priority<<shift);
        }
      else
        {
          /* Value out of range.. just ignore */
