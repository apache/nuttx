...
static int up_receive(struct uart_dev_s *dev, uint32_t *status)
{
  ...
  return  (int)(up_serialin(priv, BM3823_UART_RXREG_OFFSET) & UART_RXREG_MASK);
}
...
static bool up_rxavailable(struct uart_dev_s *dev)
{
  ...
  return (up_serialin(priv, BM3823_UART_STATREG_OFFSET) & MSK_UART_DATA_READY) != 0;
}
...
static bool up_txready(struct uart_dev_s *dev)
{
  ...
  return (up_serialin(priv, BM3823_UART_STATREG_OFFSET) & MSK_UART_TXh_READY) != 0;
}
...
static bool up_txempty(struct uart_dev_s *dev)
{
  ...
  return (up_serialin(priv, BM3823_UART_STATREG_OFFSET) & MSK_UART_TXs_READY) != 0;
}
