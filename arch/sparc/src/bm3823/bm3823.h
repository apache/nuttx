...
#define BM3823_REG_TIMER_CONTROL_EN    0x00000001  /* 1 = enable counting */
                                              /* 0 = hold scalar and counter */
#define BM3823_REG_TIMER_CONTROL_RL    0x00000002  /* 1 = reload at 0 */
                                              /* 0 = stop at 0 */
#define BM3823_REG_TIMER_CONTROL_LD    0x00000004  /* 1 = load counter */
                                              /* 0 = no function */
...
typedef struct {
	volatile unsigned int MemCfg1;		 //Memory Configuration Register 1
	volatile unsigned int MemCfg2;		 //Memory Configuration Register 2
	volatile unsigned int MemCfg3;		 //Memory Configuration Register 3
	...
	volatile unsigned int WriteProt1;	 //Write protection 1

	volatile unsigned int WriteProt2;	 //Write protection 2/* 0x20 */
	...
	volatile unsigned int UartData1;   //UA1 data
	volatile unsigned int UartStatus1; //UA1 status register
	volatile unsigned int UartCtrl1;   //UA1 control register
	volatile unsigned int UartScaler1; //UA1 scaler register

	volatile unsigned int UartData2;   //UA2 data
	volatile unsigned int UartStatus2; //UA2 status register
	volatile unsigned int UartCtrl2;   //UA2 control register
	volatile unsigned int UartScaler2; //UA2 scaler register
...
typedef struct {
	...
	volatile unsigned int MemStatus;	/* 0x10 */
	volatile unsigned int CacheCtrl;	//Cache control register
	...
	volatile unsigned int MemCfg1;		 //Interior Memory Configuration Register 1
	volatile unsigned int Dummy9;
	volatile unsigned int Dummy10;
	volatile unsigned int Dummy11;
	...
	volatile unsigned int IntMask;     	//Interrupt Mask and Priority Register register
	volatile unsigned int IntPendForce;	//Interrupt Pending and IntForce Register register
	volatile unsigned int IntSubPrio;		//Interrupt Clear Register register
	volatile unsigned int IntClear;
	...
	volatile unsigned int IntMask2;     	//Interrupt Mask and Priority Register register
	volatile unsigned int IntPend2;		//Interrupt Pending Register register
	volatile unsigned int IntClear2;	//Interrupt Clear Register register
	volatile unsigned int Dummy36;
...
#define BM3823_Disable_interrupt( _source, _previous ) \
  do { \
    ...
    _level = sparc_disable_interrupts(); \
      ...
      INTER_REG.Interrupt_Mask = _previous & ~_mask; \
    sparc_enable_interrupts( _level ); \
    (_previous) &= _mask; \
  } while (0)
