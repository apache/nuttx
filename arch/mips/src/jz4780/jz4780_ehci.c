...
#include "jz4780_ehci.h"

#define jhzuinfo uinfo
...
#ifndef CONFIG_JZ4780_EHCI_NQHS
#  define CONFIG_JZ4780_EHCI_NQHS (JZ_EHCI_NRHPORT + 1)
#endif
...
#ifndef CONFIG_JZ4780_EHCI_NQTDS
#  define CONFIG_JZ4780_EHCI_NQTDS (JZ_EHCI_NRHPORT + 3)
#endif

/* Buffers must be aligned to the cache line size */

#define DCACHE_LINEMASK (JZ4780_DCACHE_LINESIZE - 1)

/* Configurable size of a request/descriptor buffers */

#ifndef CONFIG_JZ4780_EHCI_BUFSIZE
#  define CONFIG_JZ4780_EHCI_BUFSIZE 128
#endif

#define JZ4780_EHCI_BUFSIZE \
  ((CONFIG_JZ4780_EHCI_BUFSIZE + DCACHE_LINEMASK) & ~DCACHE_LINEMASK)

/* Debug options */

#ifndef CONFIG_DEBUG_USB_INFO
#  undef CONFIG_JZ4780_EHCI_REGDEBUG
#endif
...
#if defined(CONFIG_JZ4780_UDPHS) && !defined(CONFIG_JZ4780_USB_DRP)
#  undef CONFIG_JZ4780_UHPHS_RHPORT1
#endif
...
struct jz_qh_s
{
  ...
  struct jz_epinfo_s *epinfo;  /* Endpoint used for the transfer */
...
struct jz_list_s
{
  struct jz_list_s *flink;    /* Link to next entry in the list */
                              /* Variable length entry data follows */
};

/* List traversal callout functions */

typedef int (*foreach_qh_t)(struct jz_qh_s *qh, uint32_t **bp, void *arg);
typedef int (*foreach_qtd_t)(struct jz_qtd_s *qtd, uint32_t **bp,
                             void *arg);
...
struct jz_rhport_s
{
  ...
  struct jz_epinfo_s ep0;      /* EP0 endpoint info */
...
struct jz_ehci_s
{
  ...
  struct jz_epinfo_s ep0;      /* Endpoint 0 */
  struct jz_list_s *qhfree;    /* List of free Queue Head (QH) structures */
  struct jz_list_s *qtdfree;   /* List of free qTDs */
  ...
  struct jz_rhport_s rhport[JZ_EHCI_NRHPORT];
};
...
static uint16_t jz_read16(const uint8_t *addr);
static uint32_t jz_read32(const uint8_t *addr);
#if 0 /* Not used */
static void jz_write16(uint16_t memval, uint8_t *addr);
static void jz_write32(uint32_t memval, uint8_t *addr);
#endif

#ifdef CONFIG_ENDIAN_BIG
static uint16_t jz_swap16(uint16_t value);
static uint32_t jz_swap32(uint32_t value);
#else
#  define jz_swap16(value) (value)
#  define jz_swap32(value) (value)
#endif

#ifdef CONFIG_JZ4780_EHCI_REGDEBUG
static void jz_printreg(volatile uint32_t *regaddr, uint32_t regval,
         bool iswrite);
static void jz_checkreg(volatile uint32_t *regaddr, uint32_t regval,
         bool iswrite);
static uint32_t jz_getreg(volatile uint32_t *regaddr);
static void jz_putreg(uint32_t regval, volatile uint32_t *regaddr);
#else
static inline uint32_t jz_getreg(volatile uint32_t *regaddr);
static inline void jz_putreg(uint32_t regval, volatile uint32_t *regaddr);
#endif
...
static struct jz_qh_s *jz_qh_alloc(void);
static void jz_qh_free(struct jz_qh_s *qh);
static struct jz_qtd_s *jz_qtd_alloc(void);
static void jz_qtd_free(struct jz_qtd_s *qtd);

/* List Management **********************************************************/

static int jz_qh_foreach(struct jz_qh_s *qh, uint32_t **bp,
         foreach_qh_t handler, void *arg);
static int jz_qtd_foreach(struct jz_qh_s *qh, foreach_qtd_t handler,
         void *arg);
static int jz_qtd_discard(struct jz_qtd_s *qtd, uint32_t **bp, void *arg);
static int jz_qh_discard(struct jz_qh_s *qh);

/* Cache Operations *********************************************************/

#if 0 /* Not used */
static int jz_qtd_invalidate(struct jz_qtd_s *qtd, uint32_t **bp,
         void *arg);
static int jz_qh_invalidate(struct jz_qh_s *qh);
#endif
static int jz_qtd_flush(struct jz_qtd_s *qtd, uint32_t **bp, void *arg);
static int jz_qh_flush(struct jz_qh_s *qh);

/* Endpoint Transfer Handling ***********************************************/

#ifdef CONFIG_JZ4780_EHCI_REGDEBUG
static void jz_qtd_print(struct jz_qtd_s *qtd);
static void jz_qh_print(struct jz_qh_s *qh);
static int jz_qtd_dump(struct jz_qtd_s *qtd, uint32_t **bp, void *arg);
static int jz_qh_dump(struct jz_qh_s *qh, uint32_t **bp, void *arg);
#else
#  define jz_qtd_print(qtd)
#  define jz_qh_print(qh)
#  define jz_qtd_dump(qtd, bp, arg)
#  define jz_qh_dump(qh, bp, arg)
#endif

static inline uint8_t jz_ehci_speed(uint8_t usbspeed);
static int jz_ioc_setup(struct jz_rhport_s *rhport,
         struct jz_epinfo_s *epinfo);
static int jz_ioc_wait(struct jz_epinfo_s *epinfo);
static void jz_qh_enqueue(struct jz_qh_s *qhead, struct jz_qh_s *qh);
static struct jz_qh_s *jz_qh_create(struct jz_rhport_s *rhport,
         struct jz_epinfo_s *epinfo);
static int jz_qtd_addbpl(struct jz_qtd_s *qtd, const void *buffer,
         size_t buflen);
static struct jz_qtd_s *jz_qtd_setupphase(struct jz_epinfo_s *epinfo,
         const struct usb_ctrlreq_s *req);
static struct jz_qtd_s *jz_qtd_dataphase(struct jz_epinfo_s *epinfo,
         void *buffer, int buflen, uint32_t tokenbits);
static struct jz_qtd_s *jz_qtd_statusphase(uint32_t tokenbits);
static int jz_async_setup(struct jz_rhport_s *rhport,
         struct jz_epinfo_s *epinfo, const struct usb_ctrlreq_s *req,
         uint8_t *buffer, size_t buflen);
#ifndef CONFIG_USBHOST_INT_DISABLE
static int jz_intr_setup(struct jz_rhport_s *rhport,
         struct jz_epinfo_s *epinfo, uint8_t *buffer, size_t buflen);
#endif
static ssize_t jz_transfer_wait(struct jz_epinfo_s *epinfo);
#ifdef CONFIG_USBHOST_ASYNCH
static inline int jz_ioc_async_setup(struct jz_rhport_s *rhport,
         struct jz_epinfo_s *epinfo, usbhost_asynch_t callback,
         void *arg);
static void jz_asynch_completion(struct jz_epinfo_s *epinfo);
#endif

/* Interrupt Handling *******************************************************/

static int jz_qtd_ioccheck(struct jz_qtd_s *qtd, uint32_t **bp, void *arg);
static int jz_qh_ioccheck(struct jz_qh_s *qh, uint32_t **bp, void *arg);
static int jz_qtd_cancel(struct jz_qtd_s *qtd, uint32_t **bp, void *arg);
static int jz_qh_cancel(struct jz_qh_s *qh, uint32_t **bp, void *arg);
static inline void jz_ioc_bottomhalf(void);
static inline void jz_portsc_bottomhalf(void);
static inline void jz_syserr_bottomhalf(void);
static inline void jz_async_advance_bottomhalf(void);
static void jz_ehci_bottomhalf(void *arg);
static int jz_ehci_tophalf(int irq, void *context, void *arg);
...
static int jz_ioalloc(struct usbhost_driver_s *drvr,
         uint8_t **buffer, size_t buflen);
static int jz_iofree(struct usbhost_driver_s *drvr,
         uint8_t *buffer);
...
static ssize_t jz_transfer(struct usbhost_driver_s *drvr,
...
static struct jz_ehci_s g_ehci =
{
...
static struct jz_qh_s g_asynchead aligned_data(32);

#ifndef CONFIG_USBHOST_INT_DISABLE
/* The head of the periodic queue */

static struct jz_qh_s g_intrhead   aligned_data(32);

/* The frame list */

#ifdef CONFIG_JZ4780_EHCI_PREALLOCATE
...
static struct jz_qh_s g_qhpool[CONFIG_JZ4780_EHCI_NQHS]
                       aligned_data(32);

/* Queue Element Transfer Descriptor (qTD) pool */

static struct jz_qtd_s g_qtdpool[CONFIG_JZ4780_EHCI_NQTDS]
                        aligned_data(32);
...
static struct jz_qh_s *g_qhpool;

/* Queue Element Transfer Descriptor (qTD) pool */

static struct jz_qtd_s *g_qtdpool;
...
static inline uint32_t jz_read32(const uint8_t *addr)
{
#ifdef CONFIG_ENDIAN_BIG
  return (uint32_t)jz_read16(&addr[0]) << 16 |
         (uint32_t)jz_read16(&addr[2]);
#else
  return (uint32_t)jz_read16(&addr[2]) << 16 |
         (uint32_t)jz_read16(&addr[0]);
#endif
}
...
{
#ifdef CONFIG_ENDIAN_BIG
  jz_write16(memval >> 16, &addr[0]);
  jz_write16(memval & 0xffff, &addr[2]);
#else
  jz_write16(memval & 0xffff, &addr[0]);
  jz_write16(memval >> 16, &addr[2]);
#endif
}
#endif
...
static uint32_t jz_swap32(uint32_t value)
{
  return (uint32_t)jz_swap16((uint16_t)((value >> 16) & 0xffff)) |
         (uint32_t)jz_swap16((uint16_t)(value & 0xffff)) << 16;
}
#endif
 ...
 *
 * Description:
 *   Print the contents of a JZ4780 EHCI register
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   Check if it is time to output debug information for accesses to a JZ4780
 *   EHCI register
 *
 ****************************************************************************/
                         ...
                         bool iswrite)
{
  ...
  if (regaddr == prevaddr && regval == preval && prevwrite == iswrite)
  ...
  else
    {
      ...
      jz_printreg(regaddr, regval, iswrite);
    }
}
#endif
...
static uint32_t jz_getreg(volatile uint32_t *regaddr)
{
  ...
  jz_checkreg(regaddr, regval, false);
  return regval;
}
#else
static inline uint32_t jz_getreg(volatile uint32_t *regaddr)
...
static void jz_putreg(uint32_t regval, volatile uint32_t *regaddr)
{
  ...
  jz_checkreg(regaddr, regval, true);
...
static struct jz_qh_s *jz_qh_alloc(void)
{
  ...
  qh = (struct jz_qh_s *)g_ehci.qhfree;
  if (qh)
    {
      g_ehci.qhfree = ((struct jz_list_s *)qh)->flink;
      memset(qh, 0, sizeof(struct jz_qh_s));
    }
...
static void jz_qh_free(struct jz_qh_s *qh)
{
  struct jz_list_s *entry = (struct jz_list_s *)qh;
...
static struct jz_qtd_s *jz_qtd_alloc(void)
{
  ...
  qtd = (struct jz_qtd_s *)g_ehci.qtdfree;
  if (qtd)
    {
      g_ehci.qtdfree = ((struct jz_list_s *)qtd)->flink;
      memset(qtd, 0, sizeof(struct jz_qtd_s));
    }
...
static void jz_qtd_free(struct jz_qtd_s *qtd)
{
  struct jz_list_s *entry = (struct jz_list_s *)qtd;
  ...
  while (qh)
    {
      ...
      else if (jz_virtramaddr(physaddr & QH_HLP_MASK) ==
       ...
       */
        ...
        {
          physaddr = jz_swap32(qh->hw.hlp) & QH_HLP_MASK;
          next     = (struct jz_qh_s *)jz_virtramaddr(physaddr);
        }
                           ...
                           void *arg)
{
  ...
  physaddr = jz_swap32(*bp);   /* Physical address of first qTD in CPU order */
  ...
  qtd  = (struct jz_qtd_s *)jz_virtramaddr(physaddr);
  next = NULL;
  ...
  while (qtd)
    {
      ...
      if ((jz_swap32(qtd->hw.nqp) & QTD_NQP_T) != 0)
        {
          /* Set the next pointer to NULL.  This will terminate the loop. */
        ...
        {
          physaddr = jz_swap32(qtd->hw.nqp) & QTD_NQP_NTEP_MASK;
          next     = (struct jz_qtd_s *)jz_virtramaddr(physaddr);
        }
 ...
 *
 * Description:
 *   This is a jz_qtd_foreach callback.  It simply unlinks the QTD, updates
...
static int jz_qh_discard(struct jz_qh_s *qh)
{
  ...
  ret = jz_qtd_foreach(qh, jz_qtd_discard, NULL);
  if (ret < 0)
    {
...
/****************************************************************************
 * Name: jz_qtd_invalidate
 *
 * Description:
 *   This is a callback from jz_qtd_foreach.  It simply invalidates D-cache
...
static int jz_qh_invalidate(struct jz_qh_s *qh)
{
  ...
  return jz_qtd_foreach(qh, jz_qtd_invalidate, NULL);
}
#endif
 ...
 *
 * Description:
 *   This is a callback from jz_qtd_foreach.  It simply flushes D-cache for
...
static int jz_qh_flush(struct jz_qh_s *qh)
{
  ...
  return jz_qtd_foreach(qh, jz_qtd_flush, NULL);
}
 ...
 *
 * Description:
 *   This is a jz_qtd_foreach callout function.  It dumps the context of one
 *   qTD
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   This is a jz_qh_foreach callout function.  It dumps a QH structure and
...
{
  jz_qh_print(qh);
  return jz_qtd_foreach(qh, jz_qtd_dump, NULL);
}
#endif
...
static void jz_qh_enqueue(struct jz_qh_s *qhead, struct jz_qh_s *qh)
{
  ...
  physaddr = (uintptr_t)jz_physramaddr((uintptr_t)qh);
  qhead->hw.hlp = jz_swap32(physaddr | QH_HLP_TYP_QH);
                                      ...
                                      struct jz_epinfo_s *epinfo)
{
  ...
  regval = ((uint32_t)epinfo->devaddr << QH_EPCHAR_DEVADDR_SHIFT) |
           ...
           ((uint32_t)jz_ehci_speed(epinfo->speed) << QH_EPCHAR_EPS_SHIFT) |
           QH_EPCHAR_DTC |
...
#warning REVISIT
  ...
  qh->hw.epcaps = jz_swap32(regval);

  /* Mark this as the end of this list.  This will be overwritten if/when the
   * next qTD is added to the queue.
   */

  qh->hw.hlp         = jz_swap32(QH_HLP_T);
  qh->hw.overlay.nqp = jz_swap32(QH_NQP_T);
  qh->hw.overlay.alt = jz_swap32(QH_AQP_T);
  return qh;
}
                          ...
                          size_t buflen)
{
  ...
  physaddr = (uint32_t)jz_physramaddr((uintptr_t)buffer);
  ...
  for (ndx = 0; ndx < 5; ndx++)
    {
      ...
      qtd->hw.bpl[ndx] = jz_swap32(physaddr);
                                            ...
                                            const struct usb_ctrlreq_s *req)
{
  ...
  qtd->hw.nqp = jz_swap32(QTD_NQP_T);
  qtd->hw.alt = jz_swap32(QTD_AQP_T);
  ...
  qtd->hw.token = jz_swap32(regval);

  /* Add the buffer data */

  ret = jz_qtd_addbpl(qtd, req, USB_SIZEOF_CTRLREQ);
  if (ret < 0)
    {
                                           ...
                                           uint32_t tokenbits)
{
  ...
  ret = jz_qtd_addbpl(qtd, buffer, buflen);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_ADDBPL_FAILED, -ret);
      jz_qtd_free(qtd);
...
#else
  jhzuinfo("RHport%d EP%d: buffer=%p, buflen=%d, req=%p\n",
  ...
  if (req != NULL)
    {
      ...
      qtd = jz_qtd_setupphase(epinfo, req);
      if (qtd == NULL)
        {
      ...
      physaddr = jz_physramaddr((uintptr_t)qtd);
      *flink = jz_swap32(physaddr);
  ...
  if (buffer != NULL && buflen > 0)
    {
      ...
      qtd = jz_qtd_dataphase(epinfo, buffer, buflen, tokenbits);
      if (qtd == NULL)
        {
  ...
  if (req != NULL)
    {
      ...
      qtd = jz_qtd_statusphase(tokenbits);
      if (qtd == NULL)
        {
      ...
      *flink = jz_swap32(physaddr);
      ...
      if (alt)
        {
          *alt = jz_swap32(physaddr);
        }
    }

  /* Add the new QH to the head of the asynchronous queue list */

  jz_qh_enqueue(&g_asynchead, qh);
  return OK;
                          ...
                          uint8_t *buffer, size_t buflen)
{
...
#else
  jhzuinfo("RHport%d EP%d: buffer=%p, buflen=%d\n",
  ...
  physaddr = jz_physramaddr((uintptr_t)qtd);
  qh->hw.overlay.nqp = jz_swap32(physaddr);

  /* Request to disable the periodic schedule */

  regval  = jz_getreg(&HCOR->usbcmd);
  regval &= ~EHCI_USBCMD_PSEN;
  jz_putreg(regval, &HCOR->usbcmd);

  /* Wait for Periodic Schedule Status (PSS) bit in USBSTS to clear,
   * confirming DMA engine has stopped.
   */

  while ((jz_getreg(&HCOR->usbsts) & EHCI_USBSTS_PSS) != 0)
    {
    }

  /* Add the new QH to the head of the interrupt transfer list */

  jz_qh_enqueue(&g_intrhead, qh);
...
#warning REVISIT
  ...
  /* Did jz_ioc_wait() or nxrmutex_lock report an error? */
...
/****************************************************************************
 * Name: jz_qtd_ioccheck
 *
 * Description:
 *   This function is a jz_qtd_foreach() callback function.  It services
...
static int jz_qtd_ioccheck(struct jz_qtd_s *qtd, uint32_t **bp, void *arg)
{
  struct jz_epinfo_s *epinfo = (struct jz_epinfo_s *)arg;
  DEBUGASSERT(qtd && epinfo);

  /* Make sure we reload the QTD from memory */
  ...
  **bp = qtd->hw.nqp;
  ...
  epinfo->xfrd -= (jz_swap32(qtd->hw.token) & QTD_TOKEN_NBYTES_MASK) >>
 ...
 *
 * Description:
 *   This function is a jz_qh_foreach() callback function.  It services one
...
static int jz_qh_ioccheck(struct jz_qh_s *qh, uint32_t **bp, void *arg)
{
  ...
  token = jz_swap32(qh->hw.overlay.token);
  ...
  ret = jz_qtd_foreach(qh, jz_qtd_ioccheck, (void *)qh->epinfo);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
  ...
  if ((jz_swap32(qh->fqp) & QTD_NQP_T) != 0)
    {
 ...
 *
 * Description:
 *   This function is a jz_qtd_foreach() callback function.  It removes each
 ...
 *
 * Description:
 *   This function is a jz_qh_foreach() callback function.  It cancels
...
static int jz_qh_cancel(struct jz_qh_s *qh, uint32_t **bp, void *arg)
{
  ...
  regval = jz_getreg(&HCOR->usbcmd);
  jz_putreg(regval & ~(EHCI_USBCMD_ASEN | EHCI_USBCMD_PSEN),
               &HCOR->usbcmd);
  ...
  **bp = qh->hw.hlp;
  ...
  ret = jz_qtd_foreach(qh, jz_qtd_cancel, NULL);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
...
static inline void jz_ioc_bottomhalf(void)
{
  ...
  qh = (struct jz_qh_s *)jz_virtramaddr(jz_swap32(*bp) & QH_HLP_MASK);
  ...
  if (qh && qh != &g_asynchead)
    {
      ...
      ret = jz_qh_foreach(qh, &bp, jz_qh_ioccheck, NULL);
      if (ret < 0)
        {
...
static inline void jz_portsc_bottomhalf(void)
{
  ...
  for (rhpndx = 0; rhpndx < JZ_EHCI_NRHPORT; rhpndx++)
    {
      rhport = &g_ehci.rhport[rhpndx];
      portsc = jz_getreg(&HCOR->portsc[rhpndx]);
      ...
      jz_putreg(portsc, &HCOR->portsc[rhpndx]);
    }
}

/****************************************************************************
 * Name: jz_syserr_bottomhalf
 *
 * Description:
...
/****************************************************************************
 * Name: jz_async_advance_bottomhalf
 *
 * Description:
...
static void jz_ehci_bottomhalf(void *arg)
{
  ...
  jz_putreg(EHCI_HANDLED_INTS, &HCOR->usbintr);
}

/****************************************************************************
 * Name: jz_ehci_tophalf
 *
 * Description:
...
static int jz_ehci_tophalf(int irq, void *context, void *arg)
{
  ...
  usbsts = jz_getreg(&HCOR->usbsts);
  regval = jz_getreg(&HCOR->usbintr);
  ...
  if (pending != 0)
    {
      ...
      DEBUGVERIFY(work_queue(HPWORK, &g_ehci.work, jz_ehci_bottomhalf,
                            (void *)pending, 0));
    }
                            ...
                            struct usbhost_hubport_s *hport)
{
  ...
  DEBUGASSERT(rhpndx >= 0 && rhpndx < JZ_EHCI_NRHPORT);
  rhport = &g_ehci.rhport[rhpndx];
  ...
  regval = jz_getreg(&HCOR->portsc[rhpndx]);
  if ((regval & EHCI_PORTSC_LSTATUS_MASK) == EHCI_PORTSC_LSTATUS_KSTATE)
    {
      ...
      regval  = getreg32(JZ_SFR_OHCIICR);
      regval |= SFR_OHCIICR_RES(rhpndx);
      putreg32(regval, JZ_SFR_OHCIICR);
#endif
  ...
  DEBUGASSERT((jz_getreg(&HCOR->usbsts) & EHCI_USBSTS_HALTED) == 0);
  ...
  regval  = jz_getreg(regaddr);
  regval &= ~EHCI_PORTSC_PE;
  regval |= EHCI_PORTSC_RESET;
  jz_putreg(regval, regaddr);
  ...
  while ((jz_getreg(regaddr) & EHCI_PORTSC_RESET) != 0);
  ...
  if (ret < 0)
    {
       ...
       * will cause jz_wait() to return and we will try the connection
       * again.
       */
...
{
  struct jz_epinfo_s *epinfo = (struct jz_epinfo_s *)ep0;
  int ret;
                       ...
                       usbhost_ep_t *ep)
{
...
#else
  jhzuinfo("EP%d DIR=%s FA=%08x TYPE=%d Interval=%d MaxPacket=%d\n",
  ...
  epinfo = kmm_zalloc(sizeof(struct jz_epinfo_s));
  if (!epinfo)
    {
   ...
   * inside of struct jz_epinfo_s and just memcpy here.
   */
...
static int jz_epfree(struct usbhost_driver_s *drvr, usbhost_ep_t ep)
{
  struct jz_epinfo_s *epinfo = (struct jz_epinfo_s *)ep;
                     ...
                     uint8_t **buffer, size_t *maxlen)
{
  ...
  *buffer = (uint8_t *)
    kmm_memalign(JZ4780_DCACHE_LINESIZE, JZ4780_EHCI_BUFSIZE);
  if (*buffer)
    {
      *maxlen = JZ4780_EHCI_BUFSIZE;
      ret = OK;
    }
                       ...
                       uint8_t **buffer, size_t buflen)
{
  ...
  *buffer = kumm_memalign(JZ4780_DCACHE_LINESIZE, buflen);
                      ...
                      uint8_t *buffer)
{
  ...
  struct jz_epinfo_s *ep0info = (struct jz_epinfo_s *)ep0;
  uint16_t len;
  ssize_t nbytes;
  int ret;
...
#else
  jhzuinfo("RHPort%d type: %02x req: %02x value: %02x%02x index: %02x%02x "
        "len: %04x\n",
  ...
  ret = jz_ioc_setup(rhport, ep0info);
  if (ret != OK)
    {
  ...
  ret = jz_async_setup(rhport, ep0info, req, buffer, len);
  if (ret < 0)
    {
      uerr("ERROR: jz_async_setup failed: %d\n", ret);
      goto errout_with_iocwait;
    }

  /* And wait for the transfer to complete */

  nbytes = jz_transfer_wait(ep0info);
  nxrmutex_unlock(&g_ehci.lock);
...
{
  /* jz_ctrlin can handle both directions.  We just need to work around the
   * differences in the function signatures.
   */

  return jz_ctrlin(drvr, ep0, req, (uint8_t *)buffer);
}
                            ...
                            size_t buflen)
{
  ...
  ret = jz_ioc_setup(rhport, epinfo);
  if (ret != OK)
    {
      usbhost_trace1(EHCI_TRACE1_DEVDISCONNECTED, -ret);
      goto errout_with_lock;
  ...
  switch (epinfo->xfrtype)
    {
      case USB_EP_ATTR_XFER_BULK:
        ret = jz_async_setup(rhport, epinfo, NULL, buffer, buflen);
        break;

#ifndef CONFIG_USBHOST_INT_DISABLE
      case USB_EP_ATTR_XFER_INT:
        ret = jz_intr_setup(rhport, epinfo, buffer, buflen);
        break;
#endif
...
# warning "Isochronous endpoint support not emplemented"
  ...
  nbytes = jz_transfer_wait(epinfo);
  nxrmutex_unlock(&g_ehci.lock);
  return nbytes;
                      ...
                      usbhost_asynch_t callback, void *arg)
{
  ...
  ret = jz_ioc_async_setup(rhport, epinfo, callback, arg);
  if (ret != OK)
    {
      usbhost_trace1(EHCI_TRACE1_DEVDISCONNECTED, -ret);
      goto errout_with_lock;
      ...
      case USB_EP_ATTR_XFER_BULK:
        {
          ...
          bp = (uint32_t *)&g_asynchead.hw.hlp;
          qh = (struct jz_qh_s *)
               jz_virtramaddr(jz_swap32(*bp) & QH_HLP_MASK);
...
# warning "Isochronous endpoint support not emplemented"
   ...
   *     list.  In this case, jz_hq_foreach will return zero
   * 2a) The transfer is not active and still pending.  It was removed from
   *     the list and jz_hq_foreach will return one.
   * 2b) The is active but not yet complete.  This is currently handled the
   *     same as 2a).  REVISIT: This needs to be fixed.
   * 3)  Some bad happened and jz_hq_foreach returned an error code < 0.
   */

  ret = jz_qh_foreach(qh, &bp, jz_qh_cancel, epinfo);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
...
struct usbhost_connection_s *jz_ehci_initialize(int controller)
{
  ...
  DEBUGASSERT((sizeof(struct jz_qh_s) & 0x1f) == 0);
  DEBUGASSERT((sizeof(struct jz_qtd_s) & 0x1f) == 0);
  ...
  for (i = 0; i < JZ_EHCI_NRHPORT; i++)
    {
      struct jz_rhport_s *rhport = &g_ehci.rhport[i];
...
#ifndef CONFIG_JZ4780_EHCI_PREALLOCATE
  /* Allocate a pool of free Queue Head (QH) structures */

  g_qhpool = (struct jz_qh_s *)
    kmm_memalign(32, CONFIG_JZ4780_EHCI_NQHS * sizeof(struct jz_qh_s));
  if (!g_qhpool)
    {
  ...
  for (i = 0; i < CONFIG_JZ4780_EHCI_NQHS; i++)
    {
      /* Put the QH structure in a free list */

      jz_qh_free(&g_qhpool[i]);
    }

#ifndef CONFIG_JZ4780_EHCI_PREALLOCATE
  /* Allocate a pool of free  Transfer Descriptor (qTD) structures */

  g_qtdpool = (struct jz_qtd_s *)
    kmm_memalign(32, CONFIG_JZ4780_EHCI_NQTDS * sizeof(struct jz_qtd_s));
  if (!g_qtdpool)
    {
...
#if !defined(CONFIG_JZ4780_EHCI_PREALLOCATE) && !defined(CONFIG_USBHOST_INT_DISABLE)
  ...
  for (i = 0; i < CONFIG_JZ4780_EHCI_NQTDS; i++)
    {
      /* Put the TD in a free list */

      jz_qtd_free(&g_qtdpool[i]);
    }
  ...
  modifyreg32(JZ_UHPEHCI_VSECTION + 0xb0, 0, 1 << 6);
  ...
  memset(&g_asynchead, 0, sizeof(struct jz_qh_s));
  physaddr                     = jz_physramaddr((uintptr_t)&g_asynchead);
  g_asynchead.hw.hlp           = jz_swap32(physaddr | QH_HLP_TYP_QH);
  g_asynchead.hw.epchar        = jz_swap32(QH_EPCHAR_H |
                                            QH_EPCHAR_EPS_FULL);
  g_asynchead.hw.overlay.nqp   = jz_swap32(QH_NQP_T);
  g_asynchead.hw.overlay.alt   = jz_swap32(QH_NQP_T);
  g_asynchead.hw.overlay.token = jz_swap32(QH_TOKEN_HALTED);
  g_asynchead.fqp              = jz_swap32(QTD_NQP_T);
  ...
  jz_putreg(jz_swap32(physaddr), &HCOR->asynclistaddr);
  ...
  memset(&g_intrhead, 0, sizeof(struct jz_qh_s));
  g_intrhead.hw.hlp           = jz_swap32(QH_HLP_T);
  g_intrhead.hw.overlay.nqp   = jz_swap32(QH_NQP_T);
  g_intrhead.hw.overlay.alt   = jz_swap32(QH_NQP_T);
  g_intrhead.hw.overlay.token = jz_swap32(QH_TOKEN_HALTED);
  g_intrhead.hw.epcaps        = jz_swap32(QH_EPCAPS_SSMASK(1));

  /* Attach the periodic QH to Period Frame List */

  physaddr = jz_physramaddr((uintptr_t)&g_intrhead);

  for (i = 0; i < FRAME_LIST_SIZE; i++)
    {
      g_framelist[i] = jz_swap32(physaddr) | PFL_TYP_QH;
    }
  ...
  physaddr = jz_physramaddr((uintptr_t)g_framelist);

  jz_putreg(jz_swap32(physaddr), &HCOR->periodiclistbase);
#endif
...
#  endif
  ...
  regval  = jz_getreg(&HCOR->configflag);
  regval |= EHCI_CONFIGFLAG;
  jz_putreg(regval, &HCOR->configflag);
  ...
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_RUN_FAILED, jz_getreg(&HCOR->usbsts));
      return NULL;
    }

  /* Interrupt Configuration ************************************************/

  ret = irq_attach(JZ4780_IRQ_EHCI, jz_ehci_tophalf, NULL);

  if (ret != 0)
    {
      usbhost_trace1(EHCI_TRACE1_IRQATTACH_FAILED, JZ4780_IRQ_EHCI);
      return NULL;
    }
  ...
  for (i = 0; i < JZ_EHCI_NRHPORT; i++)
    {
      g_ehci.rhport[i].connected =
        ((jz_getreg(&HCOR->portsc[i]) & EHCI_PORTSC_CCS) != 0);
    }

  /* Enable interrupts at the interrupt controller */

  up_enable_irq(JZ4780_IRQ_EHCI); /* enable USB interrupt */
  usbhost_vtrace1(EHCI_VTRACE1_INIITIALIZED, 0);

  return &g_ehciconn;
}

#endif /* CONFIG_JZ4780_EHCI */
