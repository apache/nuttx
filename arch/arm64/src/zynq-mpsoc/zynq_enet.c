...
#ifndef CONFIG_ZYNQ_GMAC_NRXBUFFERS
#  define CONFIG_ZYNQ_GMAC_NRXBUFFERS  16
#endif

/* Number of buffer for TX */

#ifndef CONFIG_ZYNQ_GMAC_NTXBUFFERS
#  define CONFIG_ZYNQ_GMAC_NTXBUFFERS  8
#endif

#ifndef CONFIG_ZYNQ_GMAC_PHYADDR
#  error "CONFIG_ZYNQ_GMAC_PHYADDR must be defined in the NuttX configuration"
#endif
...
#  undef CONFIG_ZYNQ_GMAC_REGDEBUG
#endif

#ifdef CONFIG_NET_DUMPPACKET
#  define zynq_dumppacket(m,a,n) lib_dumpbuffer(m,a,n)
#else
#  define zynq_dumppacket(m,a,n)
#endif
...
#define ZYNQ_TXTIMEOUT   (60*CLK_TCK)
...
#if defined(CONFIG_ZYNQ_GMAC_REGDEBUG) && defined(CONFIG_DEBUG_FEATURES)
static bool zynq_checkreg(struct zynq_gmac_s *priv, bool wr,
                         uint32_t regval, uintptr_t address);
static uint32_t zynq_getreg(struct zynq_gmac_s *priv, uintptr_t addr);
static void zynq_putreg(struct zynq_gmac_s *priv, uintptr_t addr,
                       uint32_t val);
#else
#  define zynq_getreg(priv,addr)     getreg32(priv->base + addr)
#  define zynq_putreg(priv,addr,val) putreg32(val,priv->base + addr)
#endif

/* Buffer management */

static uint16_t zynq_txinuse(struct zynq_gmac_s *priv);
static uint16_t zynq_txfree(struct zynq_gmac_s *priv);
static int  zynq_buffer_initialize(struct zynq_gmac_s *priv);
static void zynq_buffer_free(struct zynq_gmac_s *priv);

/* Common TX logic */

static int  zynq_transmit(struct zynq_gmac_s *priv);
static int  zynq_txpoll(struct net_driver_s *dev);
static void zynq_dopoll(struct zynq_gmac_s *priv);

/* Interrupt handling */

static int  zynq_recvframe(struct zynq_gmac_s *priv);
static void zynq_receive(struct zynq_gmac_s *priv);
static void zynq_txdone(struct zynq_gmac_s *priv);

static void zynq_interrupt_work(void *arg);
static int  zynq_gmac_interrupt(int irq, void *context, void *arg);

/* Watchdog timer expirations */

static void zynq_txtimeout_work(void *arg);
static void zynq_txtimeout_expiry(wdparm_t arg);

/* NuttX callback functions */

static int  zynq_ifup(struct net_driver_s *dev);
static int  zynq_ifdown(struct net_driver_s *dev);

static void zynq_txavail_work(void *arg);
static int  zynq_txavail(struct net_driver_s *dev);

#if defined(CONFIG_NET_MCASTGROUP) || defined(CONFIG_NET_ICMPv6)
static unsigned int zynq_hashindx(const uint8_t *mac);
static int  zynq_addmac(struct net_driver_s *dev, const uint8_t *mac);
#endif
#ifdef CONFIG_NET_MCASTGROUP
static int  zynq_rmmac(struct net_driver_s *dev, const uint8_t *mac);
#endif

#ifdef CONFIG_NETDEV_IOCTL
static int  zynq_ioctl(struct net_driver_s *dev, int cmd, unsigned long arg);
#endif

/* PHY Initialization */

#if defined(CONFIG_DEBUG_NET) && defined(CONFIG_DEBUG_INFO)
static void zynq_phydump(struct zynq_gmac_s *priv);
#else
#  define zynq_phydump(priv)
#endif

#if defined(CONFIG_NETDEV_PHY_IOCTL) && defined(CONFIG_ARCH_PHY_INTERRUPT)
static int  zynq_phyintenable(struct zynq_gmac_s *priv);
#endif
static void zynq_enablemdio(struct zynq_gmac_s *priv);
static void zynq_disablemdio(struct zynq_gmac_s *priv);
static int  zynq_phywait(struct zynq_gmac_s *priv);
static int  zynq_phyreset(struct zynq_gmac_s *priv);
static int  zynq_phyfind(struct zynq_gmac_s *priv, uint8_t *phyaddr);
#ifdef CONFIG_ZYNQ_GMAC_AUTONEG
static int  zynq_autonegotiate(struct zynq_gmac_s *priv);
#else
static void zynq_linkspeed(struct zynq_gmac_s *priv);
#endif
static void zynq_mdcclock(struct zynq_gmac_s *priv);
static int  zynq_phyinit(struct zynq_gmac_s *priv);

/* GMAC Initialization */

static void zynq_txreset(struct zynq_gmac_s *priv);
static void zynq_rxreset(struct zynq_gmac_s *priv);
static void zynq_gmac_reset(struct zynq_gmac_s *priv);
static void zynq_macaddress(struct zynq_gmac_s *priv);
static int  zynq_gmac_configure(struct zynq_gmac_s *priv);
...
#ifdef CONFIG_ZYNQ_ENET1
static uint8_t g_enet1_pktbuf[MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE];
#endif
#ifdef CONFIG_ZYNQ_ENET2
static uint8_t g_enet2_pktbuf[MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE];
#endif
#ifdef CONFIG_ZYNQ_ENET3
static uint8_t g_enet3_pktbuf[MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE];
#endif
#ifdef CONFIG_ZYNQ_ENET4
static uint8_t g_enet4_pktbuf[MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE];
#endif

#ifdef CONFIG_ZYNQ_GMAC_PREALLOCATE
/* Preallocated data */

/* TX descriptors list */

#ifdef CONFIG_ZYNQ_ENET1
static struct gmac_txdesc_s g_enet1_txdesc[CONFIG_ZYNQ_GMAC_NTXBUFFERS + 1]
              aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET2
static struct gmac_txdesc_s g_enet2_txdesc[CONFIG_ZYNQ_GMAC_NTXBUFFERS + 1]
              aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET3
static struct gmac_txdesc_s g_enet3_txdesc[CONFIG_ZYNQ_GMAC_NTXBUFFERS + 1]
              aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET4
static struct gmac_txdesc_s g_enet4_txdesc[CONFIG_ZYNQ_GMAC_NTXBUFFERS + 1]
              aligned_data(8);
#endif
...
#ifdef CONFIG_ZYNQ_ENET1
static struct gmac_rxdesc_s g_enet1_rxdesc[CONFIG_ZYNQ_GMAC_NRXBUFFERS + 1]
              aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET2
static struct gmac_rxdesc_s g_enet2_rxdesc[CONFIG_ZYNQ_GMAC_NRXBUFFERS + 1]
              aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET3
static struct gmac_rxdesc_s g_enet3_rxdesc[CONFIG_ZYNQ_GMAC_NRXBUFFERS + 1]
              aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET4
static struct gmac_rxdesc_s g_enet4_rxdesc[CONFIG_ZYNQ_GMAC_NRXBUFFERS + 1]
              aligned_data(8);
#endif
...
#ifdef CONFIG_ZYNQ_ENET1
static uint8_t g_enet1_txbuffer
               [CONFIG_ZYNQ_GMAC_NTXBUFFERS *GMAC_TX_UNITSIZE]
               aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET2
static uint8_t g_enet2_txbuffer
               [CONFIG_ZYNQ_GMAC_NTXBUFFERS * GMAC_TX_UNITSIZE]
               aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET3
static uint8_t g_enet3_txbuffer
               [CONFIG_ZYNQ_GMAC_NTXBUFFERS * GMAC_TX_UNITSIZE]
               aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET4
static uint8_t g_enet4_txbuffer
               [CONFIG_ZYNQ_GMAC_NTXBUFFERS * GMAC_TX_UNITSIZE]
               aligned_data(8);
#endif
...
#ifdef CONFIG_ZYNQ_ENET1
static uint8_t g_enet1_rxbuffer
               [CONFIG_ZYNQ_GMAC_NRXBUFFERS * GMAC_RX_UNITSIZE]
               aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET2
static uint8_t g_enet2_rxbuffer
               [CONFIG_ZYNQ_GMAC_NRXBUFFERS * GMAC_RX_UNITSIZE]
               aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET3
static uint8_t g_enet3_rxbuffer
               [CONFIG_ZYNQ_GMAC_NRXBUFFERS * GMAC_RX_UNITSIZE]
               aligned_data(8);
#endif
#ifdef CONFIG_ZYNQ_ENET4
static uint8_t g_enet4_rxbuffer
               [CONFIG_ZYNQ_GMAC_NRXBUFFERS * GMAC_RX_UNITSIZE]
               aligned_data(8);
#endif
...
static struct zynq_gmac_s g_gmac[] =
{
#ifdef CONFIG_ZYNQ_ENET1
  {
    .dev =
      {
        .d_buf     = g_enet1_pktbuf,   /* Single packet buffer */
        .d_ifup    = zynq_ifup,        /* I/F up (new IP address) callback */
        .d_ifdown  = zynq_ifdown,      /* I/F down callback */
        .d_txavail = zynq_txavail,     /* New TX data callback */
#ifdef CONFIG_NET_MCASTGROUP
        .d_addmac  = zynq_addmac,      /* Add multicast MAC address */
        .d_rmmac   = zynq_rmmac,       /* Remove multicast MAC address */
#endif
#ifdef CONFIG_NETDEV_IOCTL
        .d_ioctl   = zynq_ioctl,       /* Support PHY ioctl() calls */
#endif
      },
    .base         = ZYNQ_MPSOC_GEM0_ADDR,
    .num          = 0,
    .ifup         = true,
    .irq          = ZYNQ_MPSOC_IRQ_GEM0,
#ifdef CONFIG_ZYNQ_GMAC_PREALLOCATE
    .txdesc       = g_enet1_txdesc,
    .rxdesc       = g_enet1_rxdesc,
    .txbuffer     = g_enet1_txbuffer,
    .rxbuffer     = g_enet1_rxbuffer,
#endif
  },
#endif
...
#ifdef CONFIG_ZYNQ_ENET2
  {
    .dev =
      {
        .d_buf     = g_enet2_pktbuf,   /* Single packet buffer */
        .d_ifup    = zynq_ifup,        /* I/F up (new IP address) callback */
        .d_ifdown  = zynq_ifdown,      /* I/F down callback */
        .d_txavail = zynq_txavail,     /* New TX data callback */
#ifdef CONFIG_NET_MCASTGROUP
    ...
    .base         = ZYNQ_MPSOC_GEM1_ADDR,
    .num          = 1,
    .ifup         = true,
    .irq          = ZYNQ_MPSOC_IRQ_GEM1,
#ifdef CONFIG_ZYNQ_GMAC_PREALLOCATE
    .txdesc       = g_enet2_txdesc,
    .rxdesc       = g_enet2_rxdesc,
    .txbuffer     = g_enet2_txbuffer,
    .rxbuffer     = g_enet2_rxbuffer,
#endif
  },
#endif
...
#ifdef CONFIG_ZYNQ_ENET3
  {
    .dev =
      {
        .d_buf     = g_enet3_pktbuf,   /* Single packet buffer */
        .d_ifup    = zynq_ifup,        /* I/F up (new IP address) callback */
        .d_ifdown  = zynq_ifdown,      /* I/F down callback */
        .d_txavail = zynq_txavail,     /* New TX data callback */
#ifdef CONFIG_NET_MCASTGROUP
    ...
    .base         = ZYNQ_MPSOC_GEM2_ADDR,
    .num          = 2,
    .ifup         = true,
    .irq          = ZYNQ_MPSOC_IRQ_GEM2,
#ifdef CONFIG_ZYNQ_GMAC_PREALLOCATE
    .txdesc       = g_enet3_txdesc,
    .rxdesc       = g_enet3_rxdesc,
    .txbuffer     = g_enet3_txbuffer,
    .rxbuffer     = g_enet3_rxbuffer,
#endif
  },
#endif
...
#ifdef CONFIG_ZYNQ_ENET4
  {
    .dev =
      {
        .d_buf     = g_enet4_pktbuf,   /* Single packet buffer */
        .d_ifup    = zynq_ifup,        /* I/F up (new IP address) callback */
        .d_ifdown  = zynq_ifdown,      /* I/F down callback */
        .d_txavail = zynq_txavail,     /* New TX data callback */
#ifdef CONFIG_NET_MCASTGROUP
    ...
    .base         = ZYNQ_MPSOC_GEM3_ADDR,
    .num          = 3,
    .ifup         = true,
    .irq          = ZYNQ_MPSOC_IRQ_GEM3,
#ifdef CONFIG_ZYNQ_GMAC_PREALLOCATE
    .txdesc       = g_enet4_txdesc,
    .rxdesc       = g_enet4_rxdesc,
    .txbuffer     = g_enet4_txbuffer,
    .rxbuffer     = g_enet4_rxbuffer,
#endif
  },
#endif
};
 ...
 *   false: This is the zynqe as the preceding register access.
 *
 ****************************************************************************/

#ifdef CONFIG_ZYNQ_GMAC_REGDEBUG
static bool zynq_checkreg(struct zynq_gmac_s *priv, bool wr, uint32_t regval,
                         uintptr_t address)
{
...
#ifdef CONFIG_ZYNQ_GMAC_REGDEBUG
static uint32_t zynq_getreg(struct zynq_gmac_s *priv, uintptr_t address)
{
  uint32_t regval = getreg32(priv->base + address);

  if (zynq_checkreg(priv, false, regval, address))
    {
      ninfo("%08x->%08x\n", address, regval);
    }
...
#ifdef CONFIG_ZYNQ_GMAC_REGDEBUG
static void zynq_putreg(struct zynq_gmac_s *priv,
                       uintptr_t address, uint32_t regval)
{
  if (zynq_checkreg(priv, true, regval, address))
    {
      ninfo("%08x<-%08x\n", address, regval);
    }

  putreg32(regval, priv->base + address);
}
#endif

/****************************************************************************
 * Function: zynq_txinuse
 *
 * Description:
...
static uint16_t zynq_txinuse(struct zynq_gmac_s *priv)
{
  ...
  if ((uint32_t)priv->txtail > txhead32)
    {
      txhead32 += CONFIG_ZYNQ_GMAC_NTXBUFFERS;
    }
...
static uint16_t zynq_txfree(struct zynq_gmac_s *priv)
{
  ...
  return (CONFIG_ZYNQ_GMAC_NTXBUFFERS - 1) - zynq_txinuse(priv);
}

/****************************************************************************
 * Function: zynq_buffer_initialize
 *
 * Description:
...
static int zynq_buffer_initialize(struct zynq_gmac_s *priv)
{
#ifndef CONFIG_ZYNQ_GMAC_PREALLOCATE
  /* Not use pre-allocated buffers */

  size_t allocsize;

  /* Allocate buffers */

  allocsize = (CONFIG_ZYNQ_GMAC_NTXBUFFERS + 1) *
  ...
  allocsize = (CONFIG_ZYNQ_GMAC_NRXBUFFERS + 1) *
  ...
  allocsize = CONFIG_ZYNQ_GMAC_NTXBUFFERS * GMAC_TX_UNITSIZE;
  ...
  allocsize = CONFIG_ZYNQ_GMAC_NRXBUFFERS * GMAC_RX_UNITSIZE;
...
static int zynq_transmit(struct zynq_gmac_s *priv)
{
  ...
  zynq_dumppacket("Transmit packet", dev->d_buf, dev->d_len);
  ...
  if (dev->d_len > 0)
    {
      ...
      dest = (uintptr_t)txdesc->addr;
      memcpy((void *)dest, dev->d_buf, dev->d_len);
      up_clean_dcache(txdesc->addr, txdesc->addr + dev->d_len);
    }
  ...
  if (priv->txhead == CONFIG_ZYNQ_GMAC_NTXBUFFERS - 1)
    {
  ...
  if (++priv->txhead >= CONFIG_ZYNQ_GMAC_NTXBUFFERS)
    {
      priv->txhead = 0;
    }
  ...
  regval  = zynq_getreg(priv, ZYNQ_GMAC_NCR);
  regval |= GMAC_NCR_TSTART;
  zynq_putreg(priv, ZYNQ_GMAC_NCR, regval);

  /* Setup the TX timeout watchdog (perhaps restarting the timer) */

  wd_start(&priv->txtimeout, ZYNQ_TXTIMEOUT,
           zynq_txtimeout_expiry, (wdparm_t)priv);
...
static int zynq_txpoll(struct net_driver_s *dev)
{
  struct zynq_gmac_s *priv = (struct zynq_gmac_s *)dev->d_private;
...
/****************************************************************************
 * Function: zynq_dopoll
 *
 * Description:
 *   The function is called in order to perform an out-of-sequence TX poll.
 *   This is done:
 *
 *   1. After completion of a transmission (zynq_txdone),
 *   2. When new TX data is available (zynq_txavail), and
 *   3. After a TX timeout to restart the sending process
 *      (zynq_txtimeout_expiry).
 *
 * Input Parameters:
 *   priv - Reference to the driver state structure
 *
  ...
  if (zynq_txfree(priv) > 0)
    {
      ...
      devif_poll(dev, zynq_txpoll);
    }
}

/****************************************************************************
 * Function: zynq_recvframe
 *
 * Description:
  ...
  while ((rxdesc->addr & GMACRXD_ADDR_OWNER) != 0)
    {
          ...
          while (rxndx != priv->rxndx)
            {
              ...
              if (++priv->rxndx >= CONFIG_ZYNQ_GMAC_NRXBUFFERS)
                {
                  priv->rxndx = 0;
                }
            }
      ...
      if (++rxndx >= CONFIG_ZYNQ_GMAC_NRXBUFFERS)
        {
          rxndx = 0;
        }
      ...
      if (isframe)
        {
          ...
          src = (uintptr_t)(rxdesc->addr & GMACRXD_ADDR_MASK);

          up_invalidate_dcache(src, src + copylen);

          /* And do the copy */

          memcpy(dest, (uintptr_t *)src, copylen);
          dest   += copylen;
          pktlen += copylen;
...
static void zynq_receive(struct zynq_gmac_s *priv)
{
  ...
  /* Loop while while zynq_recvframe() successfully retrieves valid
   * GMAC frames.
   */

  while (zynq_recvframe(priv) == OK)
    {
      zynq_dumppacket("Received packet", dev->d_buf, dev->d_len);
  ...
  while (priv->txhead != priv->txtail)
    {
      ...
      /* Is this TX descriptor done transmitting? (ZYNQD36 datasheet,
       * p. 934)
      ...
      if ((txdesc->status & GMACTXD_STA_USED) == 0)
        {
          ...
          if (priv->txtail == 0 &&
              (uintptr_t)txdesc !=
              zynq_getreg(priv, ZYNQ_GMAC_TBQB))
            {
      ...
      if (++priv->txtail >= CONFIG_ZYNQ_GMAC_NTXBUFFERS)
        {
       ...
       * TX descriptors (see comments in zynq_transmit()).
       */

      zynq_putreg(priv, ZYNQ_GMAC_IER, GMAC_INT_RCOMP);
    }
...
static void zynq_interrupt_work(void *arg)
{
  struct zynq_gmac_s *priv = (struct zynq_gmac_s *)arg;
  uint32_t isr;
  uint32_t rsr;
  uint32_t tsr;
  uint32_t imr;
  ...
  net_lock();
  isr = zynq_getreg(priv, ZYNQ_GMAC_ISR);
  rsr = zynq_getreg(priv, ZYNQ_GMAC_RSR);
  tsr = zynq_getreg(priv, ZYNQ_GMAC_TSR);
  imr = zynq_getreg(priv, ZYNQ_GMAC_IMR);
  ...
  if ((pending & GMAC_INT_TCOMP) != 0 || (tsr & GMAC_TSR_TXCOMP) != 0)
    {
      ...
      if ((tsr & GMAC_TSR_RLE) != 0)
        {
          ...
          clrbits = GMAC_TSR_RLE | zynq_txinuse(priv);
          zynq_txreset(priv);
      ...
      zynq_putreg(priv, ZYNQ_GMAC_TSR, clrbits);
  ...
  if ((pending & GMAC_INT_RCOMP) != 0 || (rsr & GMAC_RSR_REC) != 0)
    {
      ...
      zynq_putreg(priv, ZYNQ_GMAC_RSR, clrbits);
  ...
  /* EMAC Errata section 41.3.1 */

  if (isr & GMAC_INT_RXUBR)
    {
      regval = zynq_getreg(priv, ZYNQ_GMAC_NCR) & (~GMAC_NCR_RXEN);
      zynq_putreg(priv, ZYNQ_GMAC_NCR, regval);
      zynq_putreg(priv, ZYNQ_GMAC_NCR, (regval | GMAC_NCR_RXEN));
    }

  /* clear int status */

  zynq_putreg(priv, ZYNQ_GMAC_ISR, isr);
...
static int zynq_gmac_interrupt(int irq, void *context, void *arg)
{
  ...
  work_queue(ETHWORK, &priv->irqwork, zynq_interrupt_work, priv, 0);
  return OK;
}

/****************************************************************************
 * Function: zynq_txtimeout_work
 *
 * Description:
...
/****************************************************************************
 * Function: zynq_txtimeout_expiry
 *
 * Description:
...
static void zynq_txtimeout_expiry(wdparm_t arg)
{
  ...
  work_queue(ETHWORK, &priv->irqwork, zynq_txtimeout_work, priv, 0);
}
...
static int zynq_ifup(struct net_driver_s *dev)
{
  ...
  zynq_gmac_configure(priv);

  /* Set the MAC address (should have been configured while we were down) */

  zynq_macaddress(priv);

  /* Initialize for PHY access */

  ret = zynq_phyinit(priv);
  if (ret < 0)
    {
      nerr("ERROR: zynq_phyinit failed: %d\n", ret);
      return ret;
    }
  ...
  ret = zynq_autonegotiate(priv);
  if (ret < 0)
    {
      nerr("ERROR: zynq_autonegotiate failed: %d\n", ret);
      return ret;
    }
#else
...
static int zynq_ifdown(struct net_driver_s *dev)
{
   ...
   * a known configuration that will guarantee the zynq_ifup() always
   * successfully brings the interface back up.
   */

  zynq_gmac_reset(priv);
  ...
  if (work_available(&priv->pollwork))
    {
      ...
      work_queue(ETHWORK, &priv->pollwork, zynq_txavail_work, priv, 0);
    }
...
static int zynq_addmac(struct net_driver_s *dev, const uint8_t *mac)
{
  ...
  ndx = zynq_hashindx(mac);

  /* Add the multicast address to the hardware multicast hash table */

  if (ndx >= 32)
    {
      regaddr = ZYNQ_GMAC_HRT;    /* Hash Register Top [63:32] Register */
      bit     = 1 << (ndx - 32);  /* Bit 0-31 */
    }
  else
    {
      regaddr = ZYNQ_GMAC_HRB;    /* Hash Register Bottom [31:0] Register */
      bit     = 1 << ndx;         /* Bit 0-31 */
    }

  regval = zynq_getreg(priv, regaddr);
  regval |= bit;
  zynq_putreg(priv, regaddr, regval);
  ...
  regval  = zynq_getreg(priv, ZYNQ_GMAC_NCFGR);
...
static int zynq_rmmac(struct net_driver_s *dev, const uint8_t *mac)
{
  ...
  if (ndx >= 32)
    {
      regaddr1 = ZYNQ_GMAC_HRT;    /* Hash Register Top [63:32] Register */
      regaddr2 = ZYNQ_GMAC_HRB;    /* Hash Register Bottom [31:0] Register */
      bit      = 1 << (ndx - 32);  /* Bit 0-31 */
    }
  else
    {
      regaddr1 = ZYNQ_GMAC_HRB;    /* Hash Register Bottom [31:0] Register */
      regaddr2 = ZYNQ_GMAC_HRT;    /* Hash Register Top [63:32] Register */
      bit      = 1 << ndx;         /* Bit 0-31 */
    }

  regval  = zynq_getreg(priv, regaddr1);
  regval &= ~bit;
  zynq_putreg(priv, regaddr1, regval);
  ...
  if (regval == 0 && zynq_getreg(priv, regaddr2) == 0)
    {
      ...
      case SIOCGMIIREG: /* Get register from MII PHY */
        {
          ...
          zynq_enablemdio(priv);

          /* Read from the requested register */

          ret = zynq_phyread(priv, req->phy_id, req->reg_num, &req->val_out);

          /* Disable the management port */

          zynq_disablemdio(priv);
        }
        break;
      ...
      case SIOCSMIIREG: /* Set register in MII PHY */
        {
          ...
          ret = zynq_phywrite(priv, req->phy_id, req->reg_num, req->val_in);
...
static void zynq_phydump(struct zynq_gmac_s *priv)
{
  ...
  zynq_phyread(priv, phy->phyaddr, GMII_MCR, &phyval);
  ninfo("       MCR: %04x\n", phyval);
  zynq_phyread(priv, phy->phyaddr, GMII_MSR, &phyval);
  ninfo("       MSR: %04x\n", phyval);
  zynq_phyread(priv, phy->phyaddr, GMII_ADVERTISE, &phyval);
  ninfo(" ADVERTISE: %04x\n", phyval);
  zynq_phyread(priv, phy->phyaddr, GMII_LPA, &phyval);
  ninfo("       LPR: %04x\n", phyval);
  zynq_phyread(priv, phy->phyaddr, GMII_1000BTCR, &phyval);
  ninfo("  1000BTCR: %04x\n", phyval);
  zynq_phyread(priv, phy->phyaddr, GMII_1000BTSR, &phyval);
  ninfo("  1000BTSR: %04x\n", phyval);
  zynq_phyread(priv, phy->phyaddr, GMII_ESTATUS, &phyval);
  ninfo("   ESTATUS: %04x\n", phyval);
  ...
  for (retries = 0; retries < PHY_RETRY_MAX; retries++)
    {
      ...
      if ((zynq_getreg(priv, ZYNQ_GMAC_NSR) & GMAC_NSR_IDLE) != 0)
        {
          return OK;
        }
    }
...
static int zynq_phyreset(struct zynq_gmac_s *priv)
{
  ...
  ret = zynq_phywrite(priv, phy->phyaddr, GMII_MCR, GMII_MCR_RESET);
  if (ret < 0)
    {
      nerr("ERROR: zynq_phywrite failed: %d\n", ret);
    }
  ...
  for (timeout = 0; timeout < 10; timeout++)
    {
      mcr = GMII_MCR_RESET;
      int result = zynq_phyread(priv, phy->phyaddr, GMII_MCR, &mcr);
      if (result < 0)
        {
...
static int zynq_phyfind(struct zynq_gmac_s *priv, uint8_t *phyaddr)
{
  ...
  zynq_phyread(priv, candidate, GMII_PHYID1, &phyval);
  phyid = phyval << 16;
  zynq_phyread(priv, candidate, GMII_PHYID2, &phyval);
  phyid |= phyval;
  if (phyid == phy->uid)
    {
      *phyaddr = candidate;
  ...
  /* The current address does not work... try another */
    ...
    {
      nerr("ERROR: zynq_phyread failed for PHY address %02x: %d\n",
           candidate, ret);
          ...
          if (phyid == phy->uid)
            {
              *phyaddr = candidate;
              ninfo("  Vendor Model Number:   %04x\n", (phyval &
                    GMII_PHYID2_MODEL_MASK) >> GMII_PHYID2_MODEL_SHIFT);
              ninfo("  Model Revision Number: %04x\n", (phyval &
                    GMII_PHYID2_REV_MASK) >> GMII_PHYID2_REV_SHIFT);
              ret = OK;
              break;
            }
        }
                 ...
                 uint8_t regaddr, uint16_t *phyval)
{
  ...
  ret = zynq_phywait(priv);
  if (ret < 0)
    {
      nerr("ERROR: zynq_phywait failed: %d\n", ret);
      return ret;
    }
  ...
  zynq_putreg(priv, ZYNQ_GMAC_MAN, regval);
  ...
  *phyval = (uint16_t)(zynq_getreg(priv, ZYNQ_GMAC_MAN) &
                       GMAC_MAN_DATA_MASK);
  return OK;
}
...
static int zynq_autonegotiate(struct zynq_gmac_s *priv)
{
  ...
  ret = zynq_phywrite(priv, phy->phyaddr, GMII_ADVERTISE, advertise);
  if (ret < 0)
    {
  ...
  ret = zynq_phyread(priv, phy->phyaddr, GMII_1000BTCR, &btcr);
  if (ret < 0)
    {
  ...
  ret = zynq_phywrite(priv, phy->phyaddr, GMII_1000BTCR, btcr);
  if (ret < 0)
    {
  ...
  ret  = zynq_phyread(priv, phy->phyaddr, GMII_MCR, &phyval);
  if (ret < 0)
    {
  ...
  ret = zynq_phywrite(priv, phy->phyaddr, GMII_MCR, phyval);
  if (ret < 0)
    {
  ...
  for (; ; )
    {
      ret  = zynq_phyread(priv, phy->phyaddr, GMII_MSR, &phyval);
      if (ret < 0)
        {
  ...
  for (; ; )
    {
      ret  = zynq_phyread(priv, phy->phyaddr, GMII_1000BTSR, &btsr);
      if (ret < 0)
        {
      ...
      if ((btsr & GMII_1000BTSR_LP1000BASETFULL) != 0 &&
          (btcr & GMII_1000BTCR_1000BASETHALF) != 0)
        {
          ...
          mpsoc_clk_rate_set(gem0_ref + priv->num, ZYNQ_GEM_FREQUENCY_1000);
          break;
        }
      ...
      ret  = zynq_phyread(priv, phy->phyaddr, GMII_LPA, &lpa);
      if (ret < 0)
        {
      ...
      if ((advertise & GMII_ADVERTISE_100BASETXFULL) != 0 &&
          (lpa & GMII_LPA_100BASETXFULL) != 0)
        {
          ...
          mpsoc_clk_rate_set(gem0_ref + priv->num, ZYNQ_GEM_FREQUENCY_100);
          break;
        }
               ...
               (lpa & GMII_LPA_10BASETXFULL) != 0)
        {
          ...
          mpsoc_clk_rate_set(gem0_ref + priv->num, ZYNQ_GEM_FREQUENCY_10);
          break;
        }
  ...
  ncr = zynq_getreg(priv, ZYNQ_GMAC_NCR);
  zynq_putreg(priv, ZYNQ_GMAC_NCR, ncr & ~(GMAC_NCR_RXEN | GMAC_NCR_TXEN));
  ...
  regval |= linkmode;
  zynq_putreg(priv, ZYNQ_GMAC_NCFGR, regval);
  zynq_putreg(priv, ZYNQ_GMAC_NCR, ncr);

  /* Enable RGMII enable */

  regval  = zynq_getreg(priv, ZYNQ_GMAC_UR);
  regval |= GMAC_UR_RGMII;
  zynq_putreg(priv, ZYNQ_GMAC_UR, regval);
...
#ifndef CONFIG_ZYNQ_GMAC_AUTONEG
statoc void zynq_linkspeed(struct zynq_gmac_s *priv)
{
  uint32_t regval;
  uint32_t ncr;
...
#ifdef ZYNQ_GMAC_ETHFD
  regval |= GMAC_NCFGR_FD;
#endif

#if defined(ZYNQ_GMAC_ETH100MBPS)
  regval |= GMAC_NCFGR_SPD;
#elif defined(ZYNQ_GMAC_ETH1000MBPS) */
  regval |= GMAC_NCFGR_GBE;
#endif

  zynq_puttreg(priv, ZYNQ_GMAC_NCFGR, regval);
  zynq_putreg(priv, ZYNQ_GMAC_NCR, ncr);
}
#endif

/****************************************************************************
 * Function: zynq_mdcclock
 *
 * Description:
...
static void zynq_mdcclock(struct zynq_gmac_s *priv)
{
  ...
  ncfgr  = zynq_getreg(priv, ZYNQ_GMAC_NCFGR);
  ncfgr &= ~GMAC_NCFGR_CLK_MASK;

  mck = mpsoc_clk_rate_get(gem0_ref + priv->num);
  ...
  zynq_putreg(priv, ZYNQ_GMAC_NCFGR, ncfgr);
...
static int zynq_phyinit(struct zynq_gmac_s *priv)
{
  ...
  phy->phyaddr = CONFIG_ZYNQ_GMAC_PHYADDR;
  ret = zynq_phyfind(priv, &phy->phyaddr);
  if (ret < 0)
    {
      nerr("ERROR: zynq_phyfind failed: %d\n", ret);
      return ret;
    }
...
static void zynq_txreset(struct zynq_gmac_s *priv)
{
  ...
  for (ndx = 0; ndx < CONFIG_ZYNQ_GMAC_NTXBUFFERS; ndx++)
    {
      ...
      txdesc[ndx].addr   = (uint32_t)bufaddr;
      txdesc[ndx].status = GMACTXD_STA_USED;
    }

  /* Mark the final descriptor in the list */

  txdesc[CONFIG_ZYNQ_GMAC_NTXBUFFERS - 1].status = GMACTXD_STA_USED |
                                                   GMACTXD_STA_WRAP;

  txdesc[ndx].status = GMACTXD_STA_USED |
                       GMACTXD_STA_WRAP | GMACTXD_STA_LAST;
  txdesc[ndx].addr   = 0;

  /* Flush the entire TX descriptor table to RAM */

  up_clean_dcache((uintptr_t)txdesc,
                 (uintptr_t)txdesc + (CONFIG_ZYNQ_GMAC_NTXBUFFERS + 1) *
                 sizeof(struct gmac_txdesc_s));

  /* Set the Transmit Buffer Queue Base Register */

  zynq_putreg(priv, ZYNQ_GMAC_TBQB, (uintptr_t)txdesc);
  zynq_putreg(priv, ZYNQ_GMAC_TBQBAPQ0, (uintptr_t)txdesc +
              CONFIG_ZYNQ_GMAC_NTXBUFFERS * sizeof(struct gmac_txdesc_s));
}

/****************************************************************************
 * Function: zynq_rxreset
 *
 * Description:
...
static void zynq_rxreset(struct zynq_gmac_s *priv)
{
  ...
  priv->rxndx = 0;
  for (ndx = 0; ndx < CONFIG_ZYNQ_GMAC_NRXBUFFERS; ndx++)
    {
      ...
      rxdesc[ndx].addr   = (uint32_t)bufaddr;
      rxdesc[ndx].status = 0;
    }

  /* Mark the final descriptor in the list */

  rxdesc[CONFIG_ZYNQ_GMAC_NRXBUFFERS - 1].addr |= GMACRXD_ADDR_WRAP;

  rxdesc[ndx].status = 0;
  rxdesc[ndx].addr   = GMACRXD_ADDR_WRAP | GMACRXD_ADDR_OWNER;

  /* Flush the entire RX descriptor table to RAM */

  up_clean_dcache((uintptr_t)rxdesc,
                  (uintptr_t)rxdesc + (CONFIG_ZYNQ_GMAC_NRXBUFFERS + 1)*
                  sizeof(struct gmac_rxdesc_s));

  /* Set the Receive Buffer Queue Base Register */

  zynq_putreg(priv, ZYNQ_GMAC_RBQB, (uintptr_t)rxdesc);
  zynq_putreg(priv, ZYNQ_GMAC_RBQBAPQ0, (uintptr_t)rxdesc +
              CONFIG_ZYNQ_GMAC_NRXBUFFERS * sizeof(struct gmac_rxdesc_s));
}

/****************************************************************************
 * Function: zynq_gmac_reset
 *
 * Description:
...
static void zynq_gmac_reset(struct zynq_gmac_s *priv)
{
  ...
  zynq_putreg(priv, ZYNQ_GMAC_IDR, GMAC_INT_ALL);
  ...
  zynq_putreg(priv, ZYNQ_GMAC_NCR, GMAC_NCR_CLRSTAT);
#endif
}

/****************************************************************************
 * Function: zynq_macaddress
 *
 * Description:
...
static void zynq_macaddress(struct zynq_gmac_s *priv)
{
  ...
  zynq_putreg(priv, ZYNQ_GMAC_SAB1, regval);

  regval = (uint32_t)dev->d_mac.ether.ether_addr_octet[4] |
           (uint32_t)dev->d_mac.ether.ether_addr_octet[5] << 8;
  zynq_putreg(priv, ZYNQ_GMAC_SAT1, regval);
}

/****************************************************************************
 * Function: zynq_gmac_configure
 *
 * Description:
 *  Configure the GMAC interface for normal operation.
 *
...
static int zynq_gmac_configure(struct zynq_gmac_s *priv)
{
  ...
  zynq_putreg(priv, ZYNQ_GMAC_RSR, regval);
  ...
  regval = GMAC_TSR_UBR | GMAC_TSR_COL | GMAC_TSR_RLE | GMAC_TSR_TXGO |
           ...
           GMAC_TSR_LCO;
  zynq_putreg(priv, ZYNQ_GMAC_TSR, regval);

  /* Clear any pending interrupts */

  regval = zynq_getreg(priv, ZYNQ_GMAC_ISR);
  zynq_putreg(priv, ZYNQ_GMAC_ISR, regval);
   ...
   *   NBC        : Depends on CONFIG_ZYNQ_GMAC_NBC
  ...
  regval |= GMAC_DCFGR_TXCOEN;
#endif

  zynq_putreg(priv, ZYNQ_GMAC_DCFGR, regval);
  ...
  regval = GMAC_INT_MFS | GMAC_INT_RCOMP | GMAC_INT_RXUBR |
           ...
           GMAC_INT_PDRSFT;
  zynq_putreg(priv, ZYNQ_GMAC_IER, regval);
  return OK;
}
 ...
 *
 * Input Parameters:
 *   intf - In the case where there are multiple GMACs, this value
...
int zynq_gmac_initialize(int intf)
{
  ...
  priv->dev.d_private = &g_gmac[intf];

  /* Allocate buffers */

  ret = zynq_buffer_initialize(priv);
  if (ret < 0)
    {
      nerr("ERROR: zynq_buffer_initialize failed: %d\n", ret);
      return ret;
    }
  ...
  ret = irq_attach(priv->irq, zynq_gmac_interrupt, priv);
  if (ret < 0)
    {
  ...
  ret = zynq_phy_boardinitialize(priv);
  if (ret < 0)
    {
...
{
#if defined(CONFIG_ZYNQ_ENET)
  int i;

  /* Initialize all IFs */

  for (i = 0; i < nitems(g_gmac); i++)
    {
      zynq_gmac_initialize(i);
    }
#endif /* CONFIG_ZYNQ_ENET */
}
