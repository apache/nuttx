                        ...
                        uint32_t message_id, uint32_t len, uint32_t *header)
{
  ...
  msg = (msg_status_t *)imx9_smt_get_hdraddr(channel);

  /* Valid channel and buffer? */

  if (msg != NULL)
    {
      static atomic_t s_token;
      atomic_set(&s_token , 0);
  ...
  if (status == OK)
    {
      ...
      typedef struct
      {
       ...
       msg_rpower5_t *msgtx = (msg_rpower5_t *)msg;
  ...
  if (status == OK)
    {
      ...
      typedef struct
      {
       ...
       msg_rpower4_t *msgtx = (msg_rpower4_t *)msg;
