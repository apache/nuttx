...
static void imx95_pcie_linkcap_update(struct dw_pcie_s *pci, uint32_t new);
...
static int imx95_pcie_start_link(struct dw_pcie_s *pci)
{
  ...
  imx95_pcie_linkcap_update(pci, PCI_EXP_LNKCAP_SLS_2_5GB);
  ...
  if (pci->max_link_speed > 1)
    {
      ...
      imx95_pcie_linkcap_update(pci, pci->max_link_speed);
...
err_reset_phy:
  ...
  pciinfo("PHY DEBUG_R0=0x%08x DEBUG_R1=0x%08x\n",
          dw_pcie_readl_dbi(pci, PCIE_PORT_DEBUG0),
          dw_pcie_readl_dbi(pci, PCIE_PORT_DEBUG1));
  imx95_pcie_linkcap_update(pci, link_cap);
