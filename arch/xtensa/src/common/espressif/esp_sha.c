                              ...
                              const unsigned char *buffer)
{
  ...
  sha_ll_fill_text_block((const void *)buffer, ctx->block_size / 32);

  if (ctx->first_block)
    {
      sha_ll_start_block(ctx->mode);
    }
  else
    {
      sha_ll_continue_block(ctx->mode);
    }
  ...
  if (ctx->final_block)
    {
      sha_ll_load(ctx->mode);
      while (sha_ll_busy())
        {
        }
    }

  sha_ll_read_digest(ctx->mode, ctx->state, ctx->output_size / 32);
