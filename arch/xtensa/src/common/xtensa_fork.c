 ...
 *   effort" -- a vfork() child is not entitled to them.  The frame chain is
 *   different in kind:  without it the child cannot execute one return
 *   instruction, entitled or not.
 *
 * Input Parameters:
 ...
 *   they have to move.
 *
 *   A fork() child stays on the parent's own stack addresses, inside its
 *   duplicated address environment.  It has nothing to copy.
 *
 *   A vfork() child gets a stack of its own.  That needs the copy, and on
 *   this architecture the copy needs xtensa_fork_rebase() to go with it.
 *
 ...
 *   build is every caller:  vfork() is a plain function call into the same
 *   image.
 *
 ...
 *   and its first return goes wherever the debris points.
 *
