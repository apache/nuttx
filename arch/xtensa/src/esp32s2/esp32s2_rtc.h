...
/* Number of cycles to wait from the 32KHz XTAL oscillator to
...
/* Indicates that 32KHz oscillator gets input from external oscillator
...
enum esp32s2_rtc_cpu_freq_src_e
{
 ...
 * an extra enum member for the external 32KHz oscillator. For convenience,
...
enum esp32s2_slow_clk_sel_e
{
  ...
  SLOW_CLK_90K = RTC_SLOW_FREQ_RTC,
  ...
  /* External 32KHz oscillator connected to 32K_XP pin */
...
struct esp32s2_cpu_freq_config_s
{
  /* The clock from which CPU clock is derived */

  enum esp32s2_rtc_cpu_freq_src_e source;
...
uint32_t esp32s2_rtc_clk_slow_freq_get_hz(void);
...
uint32_t esp32s2_rtc_clk_fast_freq_get_hz(void);
...
uint64_t esp32s2_rtc_time_us_to_slowclk(uint64_t time_in_us,
                                        uint32_t period);
...
uint64_t esp32s2_rtc_time_slowclk_to_us(uint64_t rtc_cycles,
                                        uint32_t period);
...
uint32_t esp32s2_clk_slowclk_cal_get(void);

/****************************************************************************
 * Name: esp32s2_rtc_bbpll_disable
 *
 * Description:
 *   Disable BBPLL.
 *
...
void esp32s2_rtc_bbpll_disable(void);
...
void esp32s2_rtc_sleep_set_wakeup_time(uint64_t t);
...
void esp32s2_rtc_cpu_freq_set_xtal(void);
...
void esp32s2_rtc_set_boot_time(uint64_t time_us);
...
uint64_t esp32s2_rtc_get_boot_time(void);
...
void esp32s2_rtc_clk_cpu_freq_set_config(
...
void esp32s2_rtc_clk_cpu_freq_get_config(
