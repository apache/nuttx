...
struct esp32s2_lowerhalf_s
{
  ...
  struct esp32s2_cbinfo_s cbinfo[RTC_ALARM_LAST];
#endif
};
...
static void rtc_lh_alarm_callback(void *arg, unsigned int alarmid)
{
  struct esp32s2_lowerhalf_s *lower;
  struct esp32s2_cbinfo_s *cbinfo;
  rtc_alarm_callback_t cb;
  void *priv;

  DEBUGASSERT((RTC_ALARM0 <= alarmid) && (alarmid < RTC_ALARM_LAST));

  lower        = (struct esp32s2_lowerhalf_s *)arg;
                          ...
                          const struct rtc_time *rtctime)
{
  ...
  /* Now set the time (with an accuracy of seconds) */
 ...
 *
 * Returned Value:
 *   Returns true if RTC date-time has been previously set.
 *
 ****************************************************************************/

static bool rtc_lh_havesettime(struct rtc_lowerhalf_s *lower)
{
  if (esp32s2_rtc_get_boot_time() == 0)
    {
      return false;
    }
...
static int rtc_lh_setalarm(struct rtc_lowerhalf_s *lower,
                           const struct lower_setalarm_s *alarminfo)
{
  struct esp32s2_lowerhalf_s *priv;
  struct esp32s2_cbinfo_s *cbinfo;
  ...
  priv = (struct esp32s2_lowerhalf_s *)lower;
...
/****************************************************************************
 * Name: esp32s2_rtc_lowerhalf
 *
 * Description:
 *   Instantiate the RTC lower half driver for the ESP32S2.
 *
 * Input Parameters:
 *   None
 *
...
struct rtc_lowerhalf_s *esp32s2_rtc_lowerhalf(void)
{
...
int esp32s2_rtc_driverinit(void)
{
  ...
  /* Instantiate the ESP32S2 lower-half RTC driver */

  lower = esp32s2_rtc_lowerhalf();
  if (lower == NULL)
    {
      return ret;
    }
