...
static void espnow_txheadadd(FAR struct espnow_driver_s *priv,
                             FAR struct iob_s *iob);
static FAR struct iob_s *espnow_txtailget(FAR struct espnow_driver_s *priv);
static void espnow_rxtailadd(FAR struct espnow_driver_s *priv,
                             FAR struct iob_s *iob);
static FAR struct iob_s *espnow_rxtailget(FAR struct espnow_driver_s *priv);
...
static FAR struct iob_s *espnow_rxtailget(FAR struct espnow_driver_s *priv)
{
  ...
  /* Remove the IOB from the tx queue */
...
static void espnow_transmit(FAR struct espnow_driver_s *priv)
{
  ...
  while ((iob = espnow_txtailget(priv)))
    {
      ret = espnow_send(iob);
...
static void espnow_work(void *arg)
{
  ...
  while ((iob = espnow_rxtailget(priv)))
    {
                             ...
                             FAR struct radiodev_properties_s *properties)
{
  ...
  properties->sp_hubnode.nv_addr[0] = CONFIG_ESP32_ESPNOW_PKTRADIO_HUB_NODE;
#endif
...
int pktradio_espnow(void)
{
  ...
  radio               = &priv->radio;
  dev                 = &radio->r_dev;
  dev->d_ifup         = espnow_ifup;          /* I/F up (new IP address) callback */
