              ...
              switch (ctrlreq->type & USB_REQ_RECIPIENT_MASK)
                {
                  case USB_REQ_RECIPIENT_ENDPOINT:
                    {
                      usbtrace(
                        TRACE_INTDECODE(ESP32S3_TRACEINTID_EPGETSTATUS), 0);
                      ...
                      if (!privep)
                      ...
                      else
                        {
                          ...
                          priv->ep0data[1] = 0;
                          esp32s3_ep0in_setupresponse(priv,
                                                      priv->ep0data, 2);
                        }
                    }
                    break;
                  ...
                  case USB_REQ_RECIPIENT_INTERFACE:
                    {
                      usbtrace(
                        TRACE_INTDECODE(ESP32S3_TRACEINTID_IFGETSTATUS), 0);
                      priv->ep0data[0] = 0;
                      priv->ep0data[1] = 0;
                  ...
                  default:
                    {
                      usbtrace(TRACE_DEVERROR(ESP32S3_TRACEERR_BADGETSTATUS),
                               0);
                      priv->stalled = true;
                    }
                    break;
          ...
          if (priv->addressed != 0 && ctrlreq->len == 0)
            {
              ...
              if (recipient == USB_REQ_RECIPIENT_ENDPOINT &&
                ...
                {
                  usbtrace(TRACE_DEVERROR(ESP32S3_TRACEERR_BADSETFEATURE),
                           0);
                  priv->stalled = true;
                }
            }
          ...
          case OTG_GRXSTSD_PKTSTS_SETUPRECVD:
            {
              ...
              usbtrace(TRACE_INTDECODE(ESP32S3_TRACEINTID_SETUPRECVD),
                       epphy);
