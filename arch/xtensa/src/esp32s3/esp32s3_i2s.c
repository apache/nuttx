...
struct esp32s3_i2s_config_s
{
  ...
  int32_t  dma_channel;       /* I2S DMA used */
  ...
  bool is_mclk_rx;            /* Is MCLK source from RX module? */
...
static const struct esp32s3_i2s_config_s esp32s3_i2s0_config =
{
  ...
  .periph           = ESP32S3_PERIPH_I2S0,
  .irq              = ESP32S3_IRQ_I2S0,
  ...
  .bit_shift        = true,
  .mono_en          = false,
  .ws_width         = CONFIG_ESP32S3_I2S0_DATA_BIT_WIDTH,
};
...
static const struct esp32s3_i2s_config_s esp32s3_i2s1_config =
{
  ...
  .periph           = ESP32S3_PERIPH_I2S1,
  .irq              = ESP32S3_IRQ_I2S1,
  ...
  .bit_shift        = true,
  .mono_en          = false,
  .ws_width         = CONFIG_ESP32S3_I2S1_DATA_BIT_WIDTH,
};
...
#ifdef I2S_HAVE_TX
static int i2s_txdma_start(struct esp32s3_i2s_s *priv)
{
  struct esp32s3_buffer_s *bfcontainer;
                                     ...
                                     struct esp32s3_buffer_s *bfcontainer)
{
  ...
  /* FIXME: DMA already registered by rxdma() */
                           ...
                           struct esp32s3_buffer_s *bfcontainer)
{
  ...
  /* Request a GDMA channel for I2S peripheral */

  priv->config->dma_channel = esp32s3_dma_request(i2s_dma_dev, 1, 1, true);
  if (priv->config->dma_channel < 0)
    {
...
{
  uint32_t tx_en_bit = 0;
  uint32_t rx_en_bit = 0;
  ...
  if (priv->config->dout_pin != I2S_GPIO_UNUSED)
    {
      ...
      esp32s3_configgpio(priv->config->dout_pin, OUTPUT_FUNCTION_3);
  ...
  if (priv->config->din_pin != I2S_GPIO_UNUSED)
    {
      esp32s3_configgpio(priv->config->din_pin, INPUT_FUNCTION_3);
  ...
  if (priv->config->role == I2S_ROLE_SLAVE)
      ...
      if (priv->config->tx_en && !priv->config->rx_en)
        {
          ...
          esp32s3_configgpio(priv->config->ws_pin, INPUT_FUNCTION_3);
          esp32s3_gpio_matrix_in(priv->config->ws_pin,
                               priv->config->ws_out_insig, 0);

          esp32s3_gpiowrite(priv->config->bclk_pin, 1);
          esp32s3_configgpio(priv->config->bclk_pin, INPUT_FUNCTION_3);
  ...
  else
    {
      ...
      if (priv->config->mclk_pin != I2S_GPIO_UNUSED)
        {
          bool is_i2s0 = priv->config->port == ESP32S3_I2S0 ? true : false;
      ...
      if (priv->config->rx_en && !priv->config->tx_en)
        {
          ...
          esp32s3_gpiowrite(priv->config->ws_pin, 1);
          esp32s3_configgpio(priv->config->ws_pin, OUTPUT_FUNCTION_3);
          esp32s3_gpio_matrix_out(priv->config->ws_pin,
                                priv->config->ws_in_outsig, 0, 0);

          esp32s3_gpiowrite(priv->config->bclk_pin, 1);
          esp32s3_configgpio(priv->config->bclk_pin, OUTPUT_FUNCTION_3);
  ...
  if (priv->config->tx_en)
    {
      ...
      modifyreg32(I2S_TX_CONF_CHAN_REG(priv->config->port),
                  I2S_TX_CHAN_MOD_M,
      ...
      switch (priv->config->audio_std_mode)
        {
          case I2S_TDM_PHILIPS:
            tx_en_bit = I2S_TX_TDM_EN;
            rx_en_bit = I2S_RX_TDM_EN;
            break;
          case I2S_TDM_MSB:
            tx_en_bit = I2S_TX_TDM_EN;
            rx_en_bit = I2S_RX_TDM_EN;
          ...
          case I2S_TDM_PCM:
            ...
            break;
          case I2S_PDM:
            tx_en_bit = I2S_TX_TDM_EN | I2S_TX_PDM_EN;
            rx_en_bit = I2S_RX_TDM_EN | I2S_RX_PDM_EN;
            break;
        };
      ...
      /* Configure TX WS signal width. Set to 1 to enable transmitter in PCM
       * standard mode.
       */
      ...
      /* FIXME: verify it I2S_TX_WS_IDLE_POL change the channels order or if
       * both the left channel and right channel transmit the left channel
       * data
       */
      ...
      /* FIXME: We don't have FIFO register on S3 */
  ...
  if (priv->config->rx_en)
    {
      ...
      /* FIXME: there is not RX CHAN MODE, only I2S_TX_CHAN_MOD */
      ...
      if (priv->config->bit_shift)
        {
          modifyreg32(I2S_RX_CONF1_REG(priv->config->port),
                      0, I2S_RX_WS_IDLE_POL);
        }
      else
        {
          modifyreg32(I2S_RX_CONF1_REG(priv->config->port),
                      I2S_RX_WS_IDLE_POL, 0);
        }

      /* Configure RX WS signal width. Set to to enable receiver in PCM
       * standard mode. FIXME: Who to set PCM mode?
       */
      ...
      /* FIXME: We don't have FIFO force on S3 */
...
static uint32_t i2s_set_clock(struct esp32s3_i2s_s *priv)
{
  ...
  modifyreg32(I2S_TX_CLKM_CONF_REG(priv->config->port), I2S_CLK_EN, 0);
  ...
  if (priv->config->tx_en)
    {
      ...
      modifyreg32(I2S_RX_CONF_REG(priv->config->port), 0, I2S_TX_FIFO_RESET);
      modifyreg32(I2S_RX_CONF_REG(priv->config->port), I2S_TX_FIFO_RESET, 0);
  ...
  if (priv->config->rx_en)
    {
      ...
      modifyreg32(DMA_IN_LINK_CH0_REG, DMA_INLINK_START_CH0,
                  DMA_INLINK_STOP_CH0);

      /* Disable DMA interrupt */

      modifyreg32(I2S_INT_ENA_REG(priv->config->port),
                  DMA_IN_SUC_EOF_CH0_INT_ENA, 0);
...
static int i2s_interrupt(int irq, void *context, void *arg)
{
  ...
  if (priv->config->tx_en)
    {
      if (status & I2S_TX_DONE_INT_ST)
        {
          cur = (struct esp32s3_dmadesc_s *)
                getreg32(DMA_OUT_EOF_DES_ADDR_CH0);
  ...
  if (priv->config->rx_en)
    {
      if (status & I2S_RX_DONE_INT_ST)
        {
          cur = (struct esp32s3_dmadesc_s *)
                getreg32(DMA_IN_SUC_EOF_DES_ADDR_CH1);
