...
EXTRA_LIBPATHS += -L $(ARCH_SRCDIR)$(DELIM)chip$(DELIM)$(ESP_HAL_3RDPARTY_REPO)$(DELIM)components$(DELIM)xtensa$(DELIM)$(CHIP_SERIES)
...
ARCHSCRIPT += $(ARCH_SRCDIR)$(DELIM)chip$(DELIM)$(ESP_HAL_3RDPARTY_REPO)$(DELIM)components$(DELIM)esp_hal_wdt$(DELIM)$(CHIP_SERIES)$(DELIM)rom.wdt.ld
...
CHIP_CSRCS += chip$(DELIM)$(ESP_HAL_3RDPARTY_REPO)$(DELIM)components$(DELIM)esp_hal_pmu$(DELIM)$(CHIP_SERIES)$(DELIM)rtc_cntl_hal.c
...
CHIP_CSRCS += chip$(DELIM)$(ESP_HAL_3RDPARTY_REPO)$(DELIM)components$(DELIM)esp_hal_dma$(DELIM)gdma_hal_ahb_v1.c
...
# esp-hal-3rdparty needs these ESP-IDF console macros; only
# Bootloader.mk defined them before, and only for UART consoles.

ifeq ($(CONFIG_UART0_SERIAL_CONSOLE),y)
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_UART_NUM=0
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_ROM_SERIAL_PORT_NUM=0
else ifeq ($(CONFIG_UART1_SERIAL_CONSOLE),y)
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_UART_NUM=1
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_ROM_SERIAL_PORT_NUM=1
else
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_UART_NUM=-1
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_ROM_SERIAL_PORT_NUM=-1
endif
...
ifeq ($(CONFIG_ESP32S3_USBSERIAL),y)
  CFLAGS += ${DEFINE_PREFIX}CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=1
endif
