                                  ...
                                  int intr_prio)
{
    ...
    esp32s3_setup_irq(intr_num, intr_source, intr_prio, ESP32S3_CPUINT_LEVEL);
}
...
static void interrupt_handler_set_wrapper(int n, void *fn, void *arg)
{
  ...
  ret = irq_attach(n + XTENSA_IRQ_FIRSTPERIPH, esp_int_adpt_cb, adapter);
  DEBUGASSERT(ret == OK);
...
static int IRAM_ATTR semphr_give_from_isr_wrapper(void *semphr, void *hptw)
{
  ...
  return esp_errno_trans(semphr_give_wrapper(&bt_sem->sem));
}
  ...
  if (g_lp_cntl.lpclk_sel == BTDM_LPCLK_SEL_XTAL)
    {
      ...
      set_div_ret = btdm_lpclk_set_div(esp32s3_rtc_clk_xtal_freq_get() * 2);
