...
Async listener            Callback executed on the low priority work queue
...
* **Direct publishing from interrupt handlers**: the Zephyr original
  allows ``zbus_chan_pub()`` from ISRs with ``K_NO_WAIT``.  This port is
  a userspace library and its primitives (semaphores, message queues,
  lazy initialization) are not ISR-safe.  The port provides a deferred
  alternative instead: ``ZBUS_ISR_PUBLISHER_DEFINE(name, chan, type)``
  plus ``zbus_isr_pub()`` (``CONFIG_ZBUS_ISR_PUBLISHER``), callable from
  interrupt handlers, which snapshots the message and performs the
  publication from a work queue.  Semantics are last-value-wins under
  interrupt bursts; the cost is one work item plus two message-sized
  static buffers per publisher.
...
* Initialization is lazy (``pthread_once`` on the first API call)
  ...
  needed.
* Async listeners run on the low priority work queue (``LPWORK``)
  instead of the Zephyr system work queue, and there is no per-listener
...
* FLAT build (the library uses the kernel ``file_mq_*`` and work queue
  interfaces directly).
...
* ``CONFIG_ZBUS_ASYNC_LISTENER`` -- async listeners (requires
  ``CONFIG_SCHED_LPWORK``).
* ``CONFIG_ZBUS_ISR_PUBLISHER`` -- deferred publisher callable from
  interrupt handlers (requires a work queue).
