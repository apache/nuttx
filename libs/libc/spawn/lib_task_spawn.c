 ...
 *
 * Description:
 *   This function marshals task_spawn() parameters and invokes the
 *   nx_task_spawn() system call.
 *
 *   task_spawn() and posix_spawn() are NuttX OS interfaces.  In PROTECTED
 *   and KERNEL build modes, they can be reached from applications only via
               ...
               FAR char * const argv[], FAR char * const envp[])
{
  ...
  ret = nxtask_spawn(name, entry, file_actions, attr, argv, envp);
  if (ret < 0)
    {
      return -ret;
    }
