...
static void libelf_elfsize(FAR struct mod_loadinfo_s *loadinfo, bool alloc)
{
   ...
   * Its sections need no more than a natural word, so ask for that.
   */
 ...
 *   filesystem can move the blocks of a file, thus it gives an address only
 *   together with a pin that holds the blocks in place, and it expects the
 *   pin back; XIPFS is the one in tree.  A filesystem with a fixed layout
 *   has nothing to hold and answers FIOC_XIPBASE with an address only; ROMFS
 *   and TMPFS are those.  Ask for the pin first, because a filesystem that
 *   needs a pin is not safe without one.  Do not ask at all if this build
 *   cannot hold a pin, or the pin stays for ever.
 *
 *   The loader does not use mmap() here, although both filesystems supply
 *   it.  mmap() records the mapping against the group of the task that
 *   called the loader, but the release occurs when the task of the module
 *   exits, which is a different group.  The pin would thus outlive the
 *   module, and the extent would never move again.
 *
 * Returned Value:
...
static int libelf_pinhold(FAR struct mod_loadinfo_s *loadinfo)
{
  ...
  /* The descriptor the pin was taken through belongs to whichever task
   * called the loader, and the pin has to be given back when the module is
   * unloaded -- which for an executed module happens on the spawned task,
   * in another group entirely.  Take a reference to the file instead, which
   * belongs to no group.
   */
 ...
 *
 * Description:
 *   Give back an XIP pin and the file it was held through.  A compacting
 *   filesystem cannot reclaim the extent until every instance executing
 *   from it has let go, so this is not merely tidiness.
 *
 ****************************************************************************/
      ...
      if (loadinfo->fdpic)
        {
          ...
          if (loadinfo->xipbase != 0)
            ...
            {
              /* The filesystem cannot show its media, thus copy the text to
               * RAM.  The instances no longer share the text, but the module
               * still runs.
               */
...
#  endif
        ...
        {
          /* Everything else keeps the relative position of text and data,
           * because its data references are at a fixed offset from the
           * code that makes them.  One allocation, data behind text.
           */
