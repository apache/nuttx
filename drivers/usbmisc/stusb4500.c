...
#define BUFFER_SIZE 128
          ...
          int offset = snprintf(buffer, BUFFER_SIZE, "reg:%02X, value:%02X", reg,
                                rxbuf[0]);

          for (int rxbytes = 1; rxbytes < len && offset < BUFFER_SIZE; rxbytes++)
            {
...
static int stusb4500_ioctl(FAR struct file* filep, int cmd, unsigned long arg)
{
  ...
  switch (cmd)
    ...
    case USBCIOC_READ_PD_STATUS:
    {
      ...
      /* Read PD status */
      stusb4500_getreg(priv, STUSB4500_REG_RDO_REG_STATUS_0, &status->bytes[0], 4);
      break;
    }
    ...
    case USBCIOC_READ_PWR:
    {
      ...
      stusb4500_getreg(priv, STUSB4500_REG_DPM_SNK_PDO2_0, &pdo->bytes[0], 4);
      break;
    }
    default:
    {
