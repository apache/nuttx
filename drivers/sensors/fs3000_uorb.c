...
/* This is struct is just for "aesthetics", not used */

struct fs3000_sensor_s
{
...
struct fs3000_dev_s
{
  struct fs3000_sensor_s dev;     /* Sensor private data */
                            ...
                            FAR float *airvelocity)
{
  ...
  *airvelocity = (float) (0.00219 * value) - 1.17;
#else
  *airvelocity = (float) (0.00275 * value) - 1.1;
#endif
...
static int fs3000_thread(int argc, char** argv)
{
  ...
  while (true)
    {
      ret = fs3000_get_data(priv, (uint8_t *) &data[0]);
      if (ret < 0)
        {
      ...
      ret = fs3000_get_velocity(priv, (uint8_t *)data, &airvelocity);
      if (ret < 0)
        {
          goto thread_sleep;
        }
 ...
 *
 * Description:
 *   Register the FS3000 character device
...
int fs3000_register(int devno, FAR struct i2c_master_s *i2c)
{
