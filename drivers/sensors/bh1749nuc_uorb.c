  ...
  if (lower->type == SENSOR_TYPE_RGB)
    {
      ...
      rgb_data.r = sensor_data_usat(
        sensor_data_muli(priv->dev->scale_r, tmp));
      tmp = bh1749nuc_read16(dev, BH1749NUC_GREEN_DATA_LSB);
      rgb_data.g = sensor_data_usat(
        sensor_data_muli(priv->dev->scale_g, tmp));
      tmp = bh1749nuc_read16(dev, BH1749NUC_BLUE_DATA_LSB);
      rgb_data.b = sensor_data_usat(
        sensor_data_muli(priv->dev->scale_b, tmp));
  ...
  else if (lower->type == SENSOR_TYPE_IR)
    {
      ...
      ir_data.ir = sensor_data_usat(
        sensor_data_muli(priv->dev->scale_ir, tmp));
      ...
      if (ir->enabled)
        {
          ...
          ir_data.ir = sensor_data_usat(
            sensor_data_muli(dev->scale_ir, tmp));
