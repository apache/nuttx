                                                  ...
                                                  thresholds)
{
  ...
  if (thresholds.upper > apds9922_als_res_table[res_index])
    {
      snerr(\
       "ALS upper threshold out of range: %" PRIu32 ", max: %" PRIu32 "\n",
        thresholds.upper, apds9922_als_res_table[res_index]);
      return -EINVAL;
    }

  if (thresholds.lower > apds9922_als_res_table[res_index])
    {
      snerr(\
       "AKS lower threshold out of range: %" PRIu32 ", max: %" PRIu32 "\n",
        thresholds.lower, apds9922_als_res_table[res_index]);
      return -EINVAL;
    }
                              ...
                              FAR struct adps9922_ps_thresh thresholds)
{
  ...
  if (thresholds.upper > apds9922_ps_res_table[res_index])
    {
      snerr("ERROR: ps upper threshold out of range: %d, max: %" PRIu32 "\n",
             thresholds.upper, apds9922_ps_res_table[res_index]);
      return -EINVAL;
    }

  if  (thresholds.lower > apds9922_ps_res_table[res_index])
    {
      snerr("ERROR: ps lower threshold out of range: %d, max: %" PRIu32 "\n",
             thresholds.lower, apds9922_ps_res_table[res_index]);
      return -EINVAL;
    }
                                ...
                                uint16_t lev)
{
  ...
  if (lev > apds9922_ps_res_table[res_index])
    {
      return -EINVAL;
    }
