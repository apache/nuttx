                            ...
                            FAR struct motor_params_s *param)
{
  ...
  if (param->pattern.strength >= 0 && param->pattern.strength <= 3)
    {
      ret = drv2624_i2c_write_bits(priv, DRV2624_R0X0D,
                                   DRV2624_R0X0D_DIG_MEM_GAIN_MASK,
                                   param->pattern.strength);
    }

  if (param->force >= 0 && param->force <= DRV2624_R0X0E_DEFAULT_VALUE)
    {
      ret = drv2624_i2c_writereg(priv, DRV2624_R0X0E_RTP_INPUT,
                                 param->force);
    }
