...
int block_cache_prime(struct block_cache_dev_s * dev, blkcnt_t aligned_start)
{
  ...
  if (cache_index >= 0)
    {
      bcinfo("prime: %d, origin %lu", cache_index, aligned_start);
                              ...
                              blkcnt_t startblock, unsigned int nblocks)
{
  ...
  bcinfo("startblock: %lu, nblocks: %u\n", startblock, nblocks);
  ...
  while (remaining > 0)
    {
      ...
      bcinfo("use %ld blocks from offset %lu\n", read_count, offset);
  ...
  if (dev)
    {
      ...
      if (ret < 0)
        {
          ferr("ERROR: node geometry failed: %d\n", ret);
          kmm_free(dev);
          return ret;
        }
