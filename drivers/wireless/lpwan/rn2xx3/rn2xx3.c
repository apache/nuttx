...
struct txlvl_s
{
  ...
  float power; /* Transmit power in dBm */
};
...
static int8_t rn2xx3_txpwrlevel(FAR float *dbm,
 ...
 *    level.
 *
 * Return: `nan` if level doesn't exist.
 *
 ****************************************************************************/

static float rn2xx3_txpwrdbm(int8_t lvl, FAR const struct txlvl_s *arr,
                             uint8_t len)
{
...
static int rn2xx3_gettxpwr(FAR struct rn2xx3_dev_s *priv, FAR float *txpwr)
{
  ssize_t length;
  char response[20];
  int8_t txlevel;
