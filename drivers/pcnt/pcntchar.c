...
{
  /* The lower half Pulse Counter (PCNT) driver */
 ...
 *
 * Description:
 *   Prepare the pulse counter (pcnt) peripheral for use. This method just
 *   calls the lower half `open` routine if it exists.
 *
 * Input Parameters:
 ...
 *
 * Description:
 *   Close the pulse counter (pcnt) peripheral after use. This method just
 *   calls the lower half `close` routine if it exists.
 *
 * Input Parameters:
 *   filep - Pointer system file data
 *
 ...
 *
 * Description:
 *   This function reads data from the pulse counter (pcnt) device into
 *   the provided buffer.
 ...
 *
 * Input Parameters:
 *   pcnt - An instance of the lower half pulse counter (pcnt) driver
 *
 * Returned Value:
...
int pcntchar_register(FAR struct pcnt_dev_s *pcnt)
{
  ...
  /* Allocate a pulse counter (pcnt) character device structure */

  priv = kmm_zalloc(dev_size);
  if (priv)
    {
      /* Initialize the pulse counter (pcnt) character device structure */
