/****************************************************************************
 * drivers/pci/pcie_designware_ep.c
 *
                                       ...
                                       uint16_t interrupt_num)
{
  ...
  msg_data = (funcno << PCIE_MSIX_DOORBELL_PF_SHIFT) |
             (interrupt_num - 1);
...
int dw_pcie_ep_raise_msix_irq(FAR struct dw_pcie_ep_s *ep, uint8_t funcno,
                              uint16_t interrupt_num)
{
  FAR struct pci_epf_msix_tbl *msix_tbl;
  FAR struct dw_pcie_ep_func_s *ep_func;
  FAR struct pci_epc_ctrl_s *epc = ep->epc;
  uint32_t aligned_offset;
  uint32_t tbl_offset;
 ...
 *
 * Description:
 *   This function is used to Initialize DWC EP specific registers
 *
 * Input Parameters:
 *   ep               - DWC EP of the endpoint controller
 *
...
int dw_pcie_ep_init_registers(FAR struct dw_pcie_ep_s *ep)
{
  FAR struct dw_pcie_s *pci = to_dw_pcie_from_ep(ep);
  FAR struct dw_pcie_ep_func_s *ep_func;
  FAR struct pci_epc_ctrl_s *epc = ep->epc;
