...
void clock_resynchronize(FAR struct timespec *rtc_diff)
{
  ...
  if (curr_ts.tv_sec > rtc_time.tv_sec ||
  ...
  else
    {
      ...
      atomic64_fetch_add(&g_system_ticks, diff_ticks);
#else
      atomic_fetch_add(&g_system_ticks, diff_ticks);
#endif
    }
...
#ifdef CONFIG_SYSTEM_TIME64
      atomic64_fetch_add(&g_system_ticks, 1);
#else
      atomic_fetch_add(&g_system_ticks, 1);
#endif
}
#endif
