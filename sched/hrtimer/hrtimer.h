...
/* Spinlock to protect access to the high-resolution timer red-black tree */

extern spinlock_t g_hrtspinlock;

/* Red-Black tree containing all active high-resolution timers */

extern struct hrtimer_tree_s g_activetree;
...
static inline_function
int hrtimer_cmp(FAR const hrtimer_node_t *a,
                FAR const hrtimer_node_t *b)
{
  ...
  return clock_compare(btimer->expired,
                       atimer->expired);
}
...
RB_PROTOTYPE(hrtimer_tree_s, hrtimer_node_s, entry, hrtimer_cmp);
