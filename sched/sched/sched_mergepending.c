 ...
 * Returned Value:
 *   true if the head of the ready-to-run task list has changed indicating
 *     a context switch is needed on this CPU.
 *
 * Assumptions:
 * - The caller has established a critical section before calling this
 *   function.
...
{
  FAR struct tcb_s *rtcb;
  FAR struct tcb_s *ptcb = (FAR struct tcb_s *)dq_peek(list_readytorun());
  FAR struct tcb_s *next;
  bool ret = false;
  int cpu;
  int minprio;
  int cpumask = ALL_CPUS;
  ...
  cpu = nxsched_select_cpu(cpumask);
  if (cpu >= CONFIG_SMP_NCPUS)
    {
      /* There are no available CPUs. */
  ...
  while (ptcb && ptcb->sched_priority >= minprio)
    {
      ...
      if (!CPU_ISSET(cpu, &ptcb->affinity))
        {
          cpu = nxsched_select_cpu(ptcb->affinity & cpumask);
          if (cpu >= CONFIG_SMP_NCPUS)
            {
      ...
      if (ptcb->sched_priority >= rtcb->sched_priority)
        {
          ret |= nxsched_deliver_task(cpu, curr_cpu);

          /* Mask the cpu away for the next round. */
      ...
      if (cpu >= CONFIG_SMP_NCPUS)
        {
          /* There are no available CPUs left. */
