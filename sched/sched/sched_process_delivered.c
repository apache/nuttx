...
void nxsched_process_delivered(int cpu)
{
  ...
  enum task_deliver_e priority = g_delivertasks[cpu];
  ...
  g_delivertasks[cpu] = NOT_ASSIGNED;

  if (!nxsched_switch_running(cpu, priority == SWITCH_EQUAL))
    {
      ...
      int target_cpu = nxsched_select_cpu(tcb->affinity);

      if (target_cpu < CONFIG_SMP_NCPUS && target_cpu != cpu &&
          tcb->sched_priority > current_task(target_cpu)->sched_priority)
        {
