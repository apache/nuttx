  ...
  if (mutex)
    {
      ...
      if (mholder == NXSEM_MRESET)
        {
          /* Mutex was reset and the freed task is posting */
      ...
      /* If this is called with NXSEM_MRESET | NXSEM_MBLOCKED_BIT, the call
       * is done by nxsem_reset.
       * Otherwise someone is trying to free a mutex which is held by another
       * thread - is not allowed
       */

      if (mholder != (NXSEM_MRESET | NXSEM_MBLOCKS_BIT) &&
          (mholder & (~NXSEM_MBLOCKS_BIT)) != nxsched_gettid())
        {
          /* Release the blocking bit and return with -EINVAL */
