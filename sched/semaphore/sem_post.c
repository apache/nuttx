  ...
  if (mutex)
    {
      ...
      do
        {
          mholder = atomic_read(NXMUTEX_HOLDER(sem));
        }
      while (!atomic_try_cmpxchg(NXMUTEX_HOLDER(sem), &mholder,
                                 mholder | NXMUTEX_BLOCKS_BIT));

      /* If the mutex is reset, just return with OK. if this task is not
       * the holder, return -EINVAL
       */

      if (mholder == NXMUTEX_RESET ||
          this_task()->pid != (mholder & (~NXMUTEX_BLOCKS_BIT)))
        {
          atomic_store(NXMUTEX_HOLDER(sem), mholder);
          leave_critical_section(flags);
          return mholder == NXMUTEX_RESET ? OK : -EINVAL;
        }
